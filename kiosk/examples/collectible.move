...
module kiosk::collectible {
    ...
    use mgo::display::{Self, Display};
    use mgo::borrow::{Self, Referent, Borrow};
    ...
    public fun claim_ticket<OTW: drop, T: store>(otw: OTW, max_supply: Option<u32>, ctx: &mut TxContext) {
        assert!(mgo::types::is_one_time_witness(&otw), ENotOneTimeWitness);
    ...
    // ): vector<Collectible<T>> {
        ...
        while (len > 0) {
            ...
            mgo::transfer::transfer(obj, sender(ctx));
            // ));
