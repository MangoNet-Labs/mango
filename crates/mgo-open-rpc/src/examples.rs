...
use mgo_json_rpc_types::{
    ...
    Page, ProtocolConfigResponse, RPCTransactionRequestParams, Stake, StakeStatus, MgoCoinMetadata,
    MgoCommittee, MgoData, MgoEvent, MgoExecutionStatus, MgoGetPastObjectRequest,
    MgoLoadedChildObject, MgoLoadedChildObjectsResponse, MgoMoveAbility, MgoMoveAbilitySet,
    MgoMoveNormalizedFunction, MgoMoveNormalizedModule, MgoMoveNormalizedStruct,
    MgoMoveNormalizedType, MgoMoveVisibility, MgoObjectData, MgoObjectDataFilter,
    MgoObjectDataOptions, MgoObjectRef, MgoObjectResponse, MgoObjectResponseQuery, MgoParsedData,
    MgoPastObjectResponse, MgoTransactionBlock, MgoTransactionBlockData,
    MgoTransactionBlockEffects, MgoTransactionBlockEffectsV1, MgoTransactionBlockEvents,
    MgoTransactionBlockResponse, MgoTransactionBlockResponseOptions,
    MgoTransactionBlockResponseQuery, TransactionBlockBytes, TransactionBlocksPage,
    TransactionFilter, TransferObjectParams,
};
use mgo_json_rpc_types::{MgoTypeTag, ValidatorApy, ValidatorApys};
use mgo_open_rpc::ExamplePairing;
use mgo_protocol_config::Chain;
use mgo_protocol_config::ProtocolConfig;
use mgo_types::balance::Supply;
use mgo_types::base_types::random_object_ref;
use mgo_types::base_types::{
    MoveObjectType, ObjectDigest, ObjectID, ObjectType, SequenceNumber, MgoAddress,
    TransactionDigest,
};
use mgo_types::committee::Committee;
use mgo_types::crypto::{get_key_pair_from_rng, AccountKeyPair, AggregateAuthoritySignature};
...
use mgo_types::object::MoveObject;
...
use mgo_types::transaction::ObjectArg;
use mgo_types::transaction::TEST_ONLY_GAS_UNIT_FOR_TRANSFER;
use mgo_types::transaction::{CallArg, TransactionData};
use mgo_types::utils::to_sender_signed_transaction;
use mgo_types::{parse_mgo_struct_tag, MGO_FRAMEWORK_PACKAGE_ID};
...
impl RpcExampleProvider {
    ...
    pub fn examples(&mut self) -> BTreeMap<String, Vec<ExamplePairing>> {
        [
            ...
            self.mgo_get_committee_info(),
            self.mgo_get_reference_gas_price(),
            self.mgox_get_all_balances(),
            self.mgox_get_all_coins(),
            self.mgox_get_balance(),
            self.mgox_get_coin_metadata(),
            self.mgo_get_latest_checkpoint_sequence_number(),
            self.mgox_get_coins(),
            self.mgox_get_total_supply(),
            self.mgox_get_dynamic_fields(),
            self.mgox_get_dynamic_field_object(),
            self.mgox_get_owned_objects(),
            self.mgo_get_loaded_child_objects(),
            self.mgo_get_move_function_arg_types(),
            self.mgo_get_normalized_move_function(),
            self.mgo_get_normalized_move_module(),
            self.mgo_get_normalized_move_modules_by_package(),
            self.mgo_get_normalized_move_struct(),
            self.multi_get_objects_example(),
            self.multi_get_transaction_blocks(),
            self.mgox_get_validators_apy(),
            self.mgox_get_dynamic_fields(),
            self.mgox_get_dynamic_field_object(),
            self.mgox_get_owned_objects(),
            self.mgox_query_events(),
            self.mgox_get_latest_mgo_system_state(),
            self.get_protocol_config(),
            self.mgo_get_chain_identifier(),
            self.mgox_get_stakes(),
            self.mgox_get_stakes_by_ids(),
            self.mgox_resolve_name_service_address(),
            self.mgox_resolve_name_service_names(),
            self.mgo_try_multi_get_past_objects(),
        ]
        .into_iter()
    ...
    fn batch_transaction_examples(&mut self) -> Examples {
        let signer = MgoAddress::from(ObjectID::new(self.rng.gen()));
        let recipient = MgoAddress::from(ObjectID::new(self.rng.gen()));
        ...
        let tx_params = vec![
            RPCTransactionRequestParams::MoveCallRequestParams(MoveCallParams {
                package_object_id: MGO_FRAMEWORK_PACKAGE_ID,
                module: "pay".to_string(),
                function: "split".to_string(),
                type_arguments: vec![MgoTypeTag::new("0x2::mgo::MGO".to_string())],
                arguments: vec![
                    MgoJsonValue::new(json!(coin_ref.0)).unwrap(),
                    MgoJsonValue::new(json!(random_amount)).unwrap(),
                ],
            }),
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    (
                        "options",
                        json!(MgoTransactionBlockResponseOptions::full_content()),
                    ),
                    (
                        "request_type",
    ...
    fn dev_inspect_transaction_block(&mut self) -> Examples {
        ...
        let dev_inspect_results = DevInspectResults {
            ...
            events: MgoTransactionBlockEvents { data: vec![] },
            results: None,
            error: None,
            raw_txn_data: vec![],
            raw_effects: vec![],
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ("sender_address", json!(MgoAddress::from(ObjectID::new(self.rng.gen())))),
                    ("tx_bytes", json!(tx_bytes.tx_bytes)),
                    ("gas_price", json!(1000)),
                    ("epoch", json!(8888)),
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    ("options", json!(MgoObjectDataOptions::full_content())),
                ],
                json!(objects),
            )],
        )
    ...
    fn get_object_responses(&mut self, object_count: usize) -> Vec<MgoObjectResponse> {
        (0..object_count)
            .map(|_| {
                let object_id = ObjectID::new(self.rng.gen());
                let coin = GasCoin::new(object_id, 100000000);

                MgoObjectResponse::new_with_data(MgoObjectData {
                    content: Some(
                        MgoParsedData::try_from_object(
                            coin.to_object(SequenceNumber::from_u64(1)),
                            GasCoin::layout(),
                        )
                        .unwrap(),
                    ),
                    owner: Some(Owner::AddressOwner(MgoAddress::from(ObjectID::new(
                        self.rng.gen(),
                    )))),
    ...
    fn get_past_object_example(&mut self) -> Examples {
        ...
        let result = MgoPastObjectResponse::VersionFound(MgoObjectData {
            content: Some(
                MgoParsedData::try_from_object(
                    coin.to_object(SequenceNumber::from_u64(1)),
                    GasCoin::layout(),
    ...
    fn get_owned_objects(&mut self) -> Examples {
        let owner = MgoAddress::from(ObjectID::new(self.rng.gen()));
        let result = (0..4)
            .map(|_| MgoObjectData {
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    (
                        "query",
                        json!(MgoObjectResponseQuery {
                            filter: Some(MgoObjectDataFilter::StructType(
                                StructTag::from_str("0x2::coin::Coin<0x2::mgo::MGO>").unwrap()
                            )),
                            options: Some(
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    (
                        "options",
                        json!(MgoTransactionBlockResponseOptions::new()
                            .with_input()
                            .with_effects()
                            .with_events()),
                    ),
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    (
                        "query",
                        json!(MgoTransactionBlockResponseQuery {
    ...
    fn get_transfer_data_response(
        ...
        let object_change = ObjectChange::Transferred {
            ...
            object_type: parse_mgo_struct_tag("0x2::example::Object").unwrap(),
        ...
        let result = MgoTransactionBlockResponse {
            digest: *tx_digest,
            effects: Some(MgoTransactionBlockEffects::V1(
                MgoTransactionBlockEffectsV1 {
                    status: MgoExecutionStatus::Success,
                    executed_epoch: 0,
                    ...
                    gas_object: OwnedObjectRef {
                        ...
                        reference: MgoObjectRef::from(gas_ref),
                    },
            ...
            timestamp_ms: None,
            transaction: Some(MgoTransactionBlock {
                data: MgoTransactionBlockData::try_from(data1, &&mut NoOpsModuleResolver).unwrap(),
    ...
    fn get_events(&mut self) -> Examples {
        ...
        let event = MgoEvent {
            ...
            sender: MgoAddress::from(ObjectID::new(self.rng.gen())),
            type_: parse_mgo_struct_tag("0x9::test::TestEvent").unwrap(),
    ...
    fn mgo_get_reference_gas_price(&mut self) -> Examples {
        let result = 1000;
        Examples::new(
    ...
    fn mgox_get_all_balances(&mut self) -> Examples {
        let address = MgoAddress::from(ObjectID::new(self.rng.gen()));
    ...
    fn mgox_get_all_coins(&mut self) -> Examples {
        let limit = 3;
        let owner = MgoAddress::from(ObjectID::new(self.rng.gen()));
    ...
    fn mgox_get_balance(&mut self) -> Examples {
        let owner = MgoAddress::from(ObjectID::new(self.rng.gen()));
    ...
    fn mgox_get_coin_metadata(&mut self) -> Examples {
        let result = MgoCoinMetadata {
            decimals: 9,
            name: "Usdc".to_string(),
    ...
    fn mgo_get_latest_checkpoint_sequence_number(&mut self) -> Examples {
        let result = "507021";
        Examples::new(
    ...
    fn mgox_get_coins(&mut self) -> Examples {
        let coin_type = "0x2::mgo::MGO".to_string();
        let owner = MgoAddress::from(ObjectID::new(self.rng.gen()));
        let coins = (0..3)
            .map(|_| Coin {
                coin_type: coin_type.clone(),
        ...
        Examples::new(
            "mgox_getCoins",
            vec![ExamplePairing::new(
                "Gets all MGO coins owned by the address provided. Return a paginated list of `limit` results per page. Similar to `mgox_getAllCoins`, but provides a way to filter by coin type.",
                vec![
                    ("owner", json!(owner)),
    ...
    fn mgox_get_total_supply(&mut self) -> Examples {
    ...
    fn mgo_get_loaded_child_objects(&mut self) -> Examples {
        ...
        let seqs = (0..6)
            .map(|x| {
                ...
                MgoLoadedChildObject::new(ObjectID::new(self.rng.gen()), sequence)
            })
            .collect::<Vec<_>>();
        let result = {
            MgoLoadedChildObjectsResponse {
                loaded_child_objects: seqs,
            }
        };

        Examples::new(
            "mgo_getLoadedChildObjects",
            vec![ExamplePairing::new(
    ...
    fn mgo_get_move_function_arg_types(&mut self) -> Examples {
        let result = vec![
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    ("module", json!("mgofrens".to_string())),
    ...
    fn mgo_get_normalized_move_function(&mut self) -> Examples {
        let ability_set = MgoMoveAbilitySet {
            abilities: vec![MgoMoveAbility::Store, MgoMoveAbility::Key],
        };

        let result = MgoMoveNormalizedFunction {
            is_entry: false,
            type_parameters: vec![ability_set],
            parameters: vec![MgoMoveNormalizedType::U64],
            visibility: MgoMoveVisibility::Public,
            return_: vec![MgoMoveNormalizedType::U64],
        };
    ...
    fn mgo_get_normalized_move_module(&mut self) -> Examples {
        let result = MgoMoveNormalizedModule {
    ...
    fn mgo_get_normalized_move_modules_by_package(&mut self) -> Examples {
        let result = MgoMoveNormalizedModule {
            address: ObjectID::new(self.rng.gen()).to_string(),
            exposed_functions: BTreeMap::new(),
            file_format_version: 6,
    ...
    fn mgo_get_normalized_move_struct(&mut self) -> Examples {
        let abilities = MgoMoveAbilitySet {
            abilities: vec![MgoMoveAbility::Store, MgoMoveAbility::Key],
        };
        let fields = vec![].into_iter().collect::<Vec<_>>();
        let type_parameters = vec![].into_iter().collect::<Vec<_>>();
        let result = MgoMoveNormalizedStruct {
            abilities,
            fields,
            type_parameters,
        };
    ...
    fn mgox_get_validators_apy(&mut self) -> Examples {
        let result = vec![
            ValidatorApy {
                address: MgoAddress::from(ObjectID::new(self.rng.gen())),
                apy: 0.06,
            },
            ValidatorApy {
                address: MgoAddress::from(ObjectID::new(self.rng.gen())),
    ...
    fn mgox_get_dynamic_fields(&mut self) -> Examples {
        let object_id = ObjectID::new(self.rng.gen());
        ...
        Examples::new("mgox_getDynamicFields",
        vec![ExamplePairing::new(
    ...
    fn mgox_get_dynamic_field_object(&mut self) -> Examples {
        ...
        let struct_tag = parse_mgo_struct_tag("0x9::test::TestField").unwrap();
        let resp = MgoObjectResponse::new_with_data(MgoObjectData {
            content: Some(
                MgoParsedData::try_from_object(
                    unsafe {
            ...
            type_: Some(ObjectType::Struct(MoveObjectType::from(
                parse_mgo_struct_tag("0x9::test::TestField").unwrap(),
            ))),
            bcs: None,
            display: None,
        });
    ...
    fn mgox_get_owned_objects(&mut self) -> Examples {
        let owner = MgoAddress::from(ObjectID::new(self.rng.gen()));
        ...
        let filter = Some(MgoObjectDataFilter::MatchAll(vec![
            MgoObjectDataFilter::StructType(
                StructTag::from_str("0x2::coin::Coin<0x2::mgo::MGO>").unwrap(),
            ),
            MgoObjectDataFilter::AddressOwner(owner),
            MgoObjectDataFilter::Version(version),
        ]));
        let query = json!(MgoObjectResponseQuery { filter, options });
        let object_id = ObjectID::new(self.rng.gen());
    ...
    fn mgox_query_events(&mut self) -> Examples {
    ...
    fn mgox_get_latest_mgo_system_state(&mut self) -> Examples {
    ...
    fn mgo_get_chain_identifier(&mut self) -> Examples {
    ...
    fn mgox_get_stakes(&mut self) -> Examples {
        let principal = 200000000000;
        let owner = MgoAddress::from(ObjectID::new(self.rng.gen()));
        let result = vec![
            DelegatedStake {
                validator_address: MgoAddress::from(ObjectID::new(self.rng.gen())),
                staking_pool: ObjectID::new(self.rng.gen()),
                stakes: vec![
                    Stake {
                        staked_mgo_id: ObjectID::new(self.rng.gen()),
    ...
    fn mgox_get_stakes_by_ids(&mut self) -> Examples {
        let principal = 200000000000;
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                "Returns the staking information for the address the request provides.",
                vec![("staked_mgo_ids", json!(vec![stake1, stake2]))],
                json!(result),
            )],
        )
    }

    fn mgox_resolve_name_service_address(&mut self) -> Examples {
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![("name", json!("example.mgo".to_string()))],
                json!(result),
            )],
        )
    }

    fn mgox_resolve_name_service_names(&mut self) -> Examples {
        let next_cursor = Some(ObjectID::new(self.rng.gen()));
        let object_id = ObjectID::new(self.rng.gen());
        let result = Page {
            data: vec!["example.mgo".to_string()],
            next_cursor,
            has_next_page: false,
        };
        Examples::new(
            ...
            vec![ExamplePairing::new(
                "Returns the MgoNS name for the address the request provides. Currently, the API returns only the first name in cases where there are multiple. Future support will use the cursor ID and limit values in the request to control pagination of the response for addresses with multiple names.",
                vec![
    ...
    fn mgo_try_multi_get_past_objects(&mut self) -> Examples {
        ...
        let result = vec![
            MgoPastObjectResponse::VersionFound(MgoObjectData {
                content: Some(
                    MgoParsedData::try_from_object(
                        coin.to_object(SequenceNumber::from_u64(1)),
                        GasCoin::layout(),
