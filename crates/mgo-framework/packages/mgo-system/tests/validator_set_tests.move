...
#[test_only]
module mgo_system::validator_set_tests {
    use mgo::balance;
    use mgo::coin;
    use mgo::tx_context::TxContext;
    use mgo_system::staking_pool::StakedMgo;
    use mgo_system::validator::{Self, Validator, staking_pool_id};
    use mgo_system::validator_set::{Self, ValidatorSet, active_validator_addresses};
    ...
    fun test_low_stake_departure() {
        ...
        let ctx = test_scenario::ctx(scenario);
        // Create 4 validators.
        let v1 = create_validator(@0x1, 1, 1, true, ctx); // 100 MGO of stake
        let v2 = create_validator(@0x2, 4, 1, true, ctx); // 400 MGO of stake
        let v3 = create_validator(@0x3, 10, 1, true, ctx); // 1000 MGO of stake
        let v4 = create_validator(@0x4, 4, 1, true, ctx); // 400 MGO of stake
    ...
    fun add_and_activate_validator(validator_set: &mut ValidatorSet, validator: Validator, scenario: &mut Scenario) {
        test_scenario::next_tx(scenario, validator::mgo_address(&validator));
        let ctx = test_scenario::ctx(scenario);
