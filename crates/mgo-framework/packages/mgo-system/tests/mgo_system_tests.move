...
// This file contains tests testing functionalities in `mgo_system` that are not
// already tested by the other more themed tests such as `stake_tests` or
// `rewards_distribution_tests`.

#[test_only]
module mgo_system::mgo_system_tests {
    use mgo::test_scenario::{Self, Scenario};
    use mgo::mgo::MGO;
    use mgo_system::governance_test_utils::{add_validator_full_flow, advance_epoch, remove_validator, set_up_mgo_system_state, create_mgo_system_state_for_testing};
    ...
    use std::vector;
    use mgo::balance;
    use mgo::test_utils::{assert_eq, destroy};
    use std::option::Self;
    use mgo::url;
    use std::string;
    use std::ascii;
    ...
    fun test_report_validator() {
        ...
        set_up_mgo_system_state(vector[@0x1, @0x2, @0x3]);
    ...
    fun test_validator_ops_by_stakee_ok() {
        let scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        set_up_mgo_system_state(vector[@0x1, @0x2]);
        ...
        let validator = mgo_system::active_validator_by_address(&mut system_state, @0x1);
        assert!(validator::next_epoch_gas_price(validator) == 666, 0);
        let pending_validator = mgo_system::pending_validator_by_address(&mut system_state, new_validator_addr);
    ...
    #[test]
    #[expected_failure(abort_code = mgo_system::validator_set::EInvalidCap)]
    ...
    fun test_set_commission_rate_failure() {
        ...
        mgo_system::request_set_commission_rate(&mut system_state, 2001, test_scenario::ctx(scenario));
        test_scenario::return_shared(system_state);
    ...
    #[test]
    #[expected_failure(abort_code = mgo_system_state_inner::ENotValidator)]
    ...
    #[test]
    #[expected_failure(abort_code = mgo_system_state_inner::ECannotReportOneself)]
    ...
    #[test]
    #[expected_failure(abort_code = mgo_system_state_inner::EReportRecordNotFound)]
    ...
    fun test_staking_pool_mappings() {
        ...
        set_up_mgo_system_state(vector[@0x1, @0x2, @0x3, @0x4]);
        test_scenario::next_tx(scenario, @0x1);
        let system_state = test_scenario::take_shared<MgoSystemState>(scenario);
        let pool_id_1 = mgo_system::validator_staking_pool_id(&mut system_state, @0x1);
        let pool_id_2 = mgo_system::validator_staking_pool_id(&mut system_state, @0x2);
        let pool_id_3 = mgo_system::validator_staking_pool_id(&mut system_state, @0x3);
        let pool_id_4 = mgo_system::validator_staking_pool_id(&mut system_state, @0x4);
        let pool_mappings = mgo_system::validator_staking_pool_mappings(&mut system_state);
        ...
        test_scenario::next_tx(scenario, @0x1);
        let system_state = test_scenario::take_shared<MgoSystemState>(scenario);
        let pool_id_5 = mgo_system::validator_staking_pool_id(&mut system_state, new_validator_addr);
        pool_mappings = mgo_system::validator_staking_pool_mappings(&mut system_state);
    ...
    fun report_helper(sender: address, reported: address, is_undo: bool, scenario: &mut Scenario) {
        ...
        let cap = test_scenario::take_from_sender<UnverifiedValidatorOperationCap>(scenario);
        if (is_undo) {
            mgo_system::undo_report_validator(&mut system_state, &cap, reported);
        } else {
            mgo_system::report_validator(&mut system_state, &cap, reported);
        };
    ...
    fun set_gas_price_helper(
        ...
        let cap = test_scenario::take_from_sender<UnverifiedValidatorOperationCap>(scenario);
        let system_state = test_scenario::take_shared<MgoSystemState>(scenario);
        mgo_system::request_set_gas_price(&mut system_state, &cap, new_gas_price);
        test_scenario::return_to_sender(scenario, cap);
        test_scenario::return_shared(system_state);
    }
    ...
    fun rotate_operation_cap(sender: address, scenario: &mut Scenario) {
        ...
        mgo_system::rotate_operation_cap(&mut system_state, ctx);
        test_scenario::return_shared(system_state);
    }
    ...
    fun get_reporters_of(addr: address, scenario: &mut Scenario): vector<address> {
        ...
        let system_state = test_scenario::take_shared<MgoSystemState>(scenario);
        let res = vec_set::into_keys(mgo_system::get_reporters_of(&mut system_state, addr));
        test_scenario::return_shared(system_state);
        res
    }
    ...
    fun update_candidate(
        scenario: &mut Scenario,
        system_state: &mut MgoSystemState,
        name: vector<u8>,
        ...
        let ctx = test_scenario::ctx(scenario);
        mgo_system::update_validator_name(system_state, name, ctx);
        mgo_system::update_validator_description(system_state, b"new_desc", ctx);
        mgo_system::update_validator_image_url(system_state, b"new_image_url", ctx);
        mgo_system::update_validator_project_url(system_state, b"new_project_url", ctx);
        mgo_system::update_candidate_validator_network_address(system_state, network_address, ctx);
        mgo_system::update_candidate_validator_p2p_address(system_state, p2p_address, ctx);
        mgo_system::update_candidate_validator_primary_address(system_state, b"/ip4/127.0.0.1/udp/80", ctx);
        mgo_system::update_candidate_validator_worker_address(system_state, b"/ip4/127.0.0.1/udp/80", ctx);
        mgo_system::update_candidate_validator_protocol_pubkey(
            system_state,
            protocol_pub_key,
            pop,
            ctx
        );
        mgo_system::update_candidate_validator_worker_pubkey(system_state, vector[68, 55, 206, 25, 199, 14, 169, 53, 68, 92, 142, 136, 174, 149, 54, 215, 101, 63, 249, 206, 197, 98, 233, 80, 60, 12, 183, 32, 216, 88, 103, 25], ctx);
        mgo_system::update_candidate_validator_network_pubkey(system_state, vector[32, 219, 38, 23, 242, 109, 116, 235, 225, 192, 219, 45, 40, 124, 162, 25, 33, 68, 52, 41, 123, 9, 98, 11, 184, 150, 214, 62, 60, 210, 121, 62], ctx);

        mgo_system::set_candidate_validator_commission_rate(system_state, commission_rate, ctx);
        let cap = test_scenario::take_from_sender<UnverifiedValidatorOperationCap>(scenario);
        mgo_system::set_candidate_validator_gas_price(system_state, &cap, gas_price);
        test_scenario::return_to_sender(scenario, cap);
    }
    ...
    // Note: `pop` MUST be a valid signature using mgo_address and protocol_pubkey_bytes.
    ...
    fun update_metadata(
        ...
        mgo_system::update_validator_image_url(system_state, b"new_image_url", ctx);
        mgo_system::update_validator_project_url(system_state, b"new_project_url", ctx);
        mgo_system::update_validator_next_epoch_network_address(system_state, network_address, ctx);
        mgo_system::update_validator_next_epoch_p2p_address(system_state, p2p_address, ctx);
        mgo_system::update_validator_next_epoch_primary_address(system_state, b"/ip4/168.168.168.168/udp/80", ctx);
        mgo_system::update_validator_next_epoch_worker_address(system_state, b"/ip4/168.168.168.168/udp/80", ctx);
        mgo_system::update_validator_next_epoch_protocol_pubkey(
            system_state,
            protocol_pub_key,
            pop,
            ctx
        );
        mgo_system::update_validator_next_epoch_network_pubkey(system_state, network_pubkey, ctx);
        mgo_system::update_validator_next_epoch_worker_pubkey(system_state, worker_pubkey, ctx);
    }
    ...
    fun test_active_validator_update_metadata() {
        ...
        // Set up MgoSystemState with an active validator
        ...
        let validator = validator::new_for_testing(
            ...
            b"/ip4/127.0.0.1/udp/80",
            b"/ip4/127.0.0.1/udp/80",
            option::some(balance::create_for_testing<MGO>(100_000_000_000)),
            1,
            0,
            true,
            ctx
        ...
        test_scenario::next_tx(scenario, validator_addr);
        let validator = mgo_system::active_validator_by_address(&mut system_state, validator_addr);
        verify_metadata(
            validator,
            b"validator_new_name",
            pubkey,
        ...
        test_scenario::next_tx(scenario, new_validator_addr);
        let validator = mgo_system::pending_validator_by_address(&mut system_state, new_validator_addr);
        verify_metadata(
            validator,
            b"new_validator_new_name",
            new_pubkey,
        ...
        let validator = mgo_system::active_validator_by_address(&mut system_state, new_validator_addr);
        verify_metadata_after_advancing_epoch(
            validator,
            b"new_validator_new_name",
            new_pubkey1,
    ...
    fun test_validator_candidate_update() {
        ...
        test_scenario::next_tx(scenario, validator_addr);
        {
            mgo_system::request_add_validator_candidate_for_testing(
                &mut system_state,
                pubkey,
                vector[215, 64, 85, 185, 231, 116, 69, 151, 97, 79, 4, 183, 20, 70, 84, 51, 211, 162, 115, 221, 73, 241, 240, 171, 192, 25, 232, 106, 175, 162, 176, 43],
                vector[148, 117, 212, 171, 44, 104, 167, 11, 177, 100, 4, 55, 17, 235, 117, 45, 117, 84, 159, 49, 14, 159, 239, 246, 237, 21, 83, 166, 112, 53, 62, 199],
        ...
        let validator = mgo_system::candidate_validator_by_address(&mut system_state, validator_addr);
        verify_candidate(
            validator,
            b"validator_new_name",
            pubkey1,
    ...
    fun test_add_validator_candidate_failure_duplicate_with_active() {
        ...
        create_mgo_system_state_for_testing(vector[validator], 1000, 0, ctx);
    ...
    fun advance_epoch_and_check_distribution_counter(scenario: &mut Scenario, epoch_length: u64, should_increment_counter: bool) {
        ...
        let system_state = test_scenario::take_shared<MgoSystemState>(scenario);
        let prev_epoch_time = mgo_system::epoch_start_timestamp_ms(&mut system_state);
        let prev_counter = mgo_system::get_stake_subsidy_distribution_counter(&mut system_state);

        let rebate = mgo_system::advance_epoch_for_testing(
            &mut system_state, new_epoch, 1, 0, 0, 0, 0, 0, 0, prev_epoch_time + epoch_length, test_scenario::ctx(scenario)
        );
        destroy(rebate);
        assert_eq(mgo_system::get_stake_subsidy_distribution_counter(&mut system_state), prev_counter + (if (should_increment_counter) 1 else 0));
        test_scenario::return_shared(system_state);
