...
module mgo_system::validator {
    ...
    use mgo_system::validator_cap::{Self, ValidatorOperationCap};
    use mgo::object::{Self, ID};
    use std::option::{Option, Self};
    use mgo_system::staking_pool::{Self, PoolTokenExchangeRate, StakedMgo, StakingPool};
    ...
    friend mgo_system::voting_power;

    #[test_only]
    friend mgo_system::validator_tests;
    #[test_only]
    ...
    public(friend) fun request_add_stake(
        ...
        let staked_mgo = staking_pool::request_add_stake(
    ...
    public(friend) fun request_add_stake_at_genesis(
        ...
        transfer::public_transfer(staked_mgo, staker_address);
    ...
    public(friend) fun request_withdraw_stake(
        ...
        let principal_amount = staking_pool::staked_mgo_amount(&staked_mgo);
        let stake_activation_epoch = staking_pool::stake_activation_epoch(&staked_mgo);
        let withdrawn_stake = staking_pool::request_withdraw_stake(
                &mut self.staking_pool, staked_mgo, ctx);
    ...
    public(friend) fun request_set_gas_price(
        ...
        assert!(validator_address == self.metadata.mgo_address, EInvalidCap);
    ...
    public(friend) fun deposit_stake_rewards(self: &mut Validator, reward: Balance<MGO>) {
    ...
    public fun mgo_address(self: &Validator): address {
    ...
    public fun total_stake_amount(self: &Validator): u64 {
        staking_pool::mgo_balance(&self.staking_pool)
    }
    ...
    public(friend) fun new_unverified_validator_operation_cap_and_transfer(self: &mut Validator, ctx: &mut TxContext) {
        ...
        assert!(address == self.metadata.mgo_address, ENewCapNotCreatedByValidatorItself);
    ...
    fun new_from_metadata(
        ...
        let mgo_address = metadata.mgo_address;

        let staking_pool = staking_pool::new(ctx);

        let operation_cap_id = validator_cap::new_unverified_validator_operation_cap_and_transfer(mgo_address, ctx);
        Validator {
            metadata,
    ...
    // Note: `proof_of_possession` MUST be a valid signature using mgo_address and
    ...
    public(friend) fun new_for_testing(
        ...
        primary_address: vector<u8>,
        worker_address: vector<u8>,
        initial_stake_option: Option<Balance<MGO>>,
        gas_price: u64,
        commission_rate: u64,
        ...
        if (option::is_some(&initial_stake_option)) {
            request_add_stake_at_genesis(
                ...
                mgo_address, // give the stake to the validator
                ctx
            );
        };
