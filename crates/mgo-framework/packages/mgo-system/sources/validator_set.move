...
module mgo_system::validator_set {
    ...
    use mgo_system::validator::{Self, Validator, staking_pool_id, mgo_address};
    use mgo_system::validator_cap::{Self, UnverifiedValidatorOperationCap, ValidatorOperationCap};
    use mgo_system::staking_pool::{Self, PoolTokenExchangeRate, StakedMgo, pool_id};
    use mgo::object::{Self, ID};
    use mgo::priority_queue as pq;
    use mgo::vec_map::{Self, VecMap};
    use mgo::vec_set::{Self, VecSet};
    use mgo::table::{Self, Table};
    use mgo::event;
    use mgo::table_vec::{Self, TableVec};
    use mgo::transfer;
    use mgo_system::voting_power;
    ...
    #[test_only]
    friend mgo_system::validator_set_tests;

    #[test_only]
    friend mgo_system::stake_tests;
    ...
    public(friend) fun new(init_active_validators: vector<Validator>, ctx: &mut TxContext): ValidatorSet {
        ...
        while (i < num_validators) {
            ...
            table::add(&mut staking_pool_mappings, staking_pool_id(validator), mgo_address(validator));
            i = i + 1;
        };
    ...
    /// Called by `mgo_system` to add a new validator candidate.
    public(friend) fun request_add_validator_candidate(
        ...
        let validator_address = mgo_address(&validator);
        assert!(
        ...
        table::add(
            ...
            mgo_address(&validator),
            validator_wrapper::create_v1(validator, ctx),
        );
    }

    /// Called by `mgo_system` to remove a validator candidate, and move them to `inactive_validators`.
    ...
    /// Called by `mgo_system` to add a new validator to `pending_active_validators`, which will be
    ...
    /// Called by `mgo_system`, to remove a validator.
    ...
    /// Called by `mgo_system`, to add a new stake to the validator.
    ...
    public(friend) fun request_add_stake(
        ...
        assert!(mgo_amount >= MIN_STAKING_THRESHOLD, EStakingBelowThreshold);
    ...
    /// Called by `mgo_system`, to withdraw some share of a stake from the validator. The share to withdraw
    /// is denoted by `principal_withdraw_amount`. One of two things occurs in this function:
    /// 1. If the `staked_mgo` is staked with an active validator, the request is added to the validator's
    ///    staking pool's pending stake withdraw entries, processed at the end of the epoch.
    /// 2. If the `staked_mgo` was staked with a validator that is no longer active,
    ...
    public(friend) fun request_withdraw_stake(
        ...
        let staking_pool_id = pool_id(&staked_mgo);
        let validator =
            if (table::contains(&self.staking_pool_mappings, staking_pool_id)) { // This is an active validator.
                let validator_address = *table::borrow(&self.staking_pool_mappings, pool_id(&staked_mgo));
        ...
        validator::request_withdraw_stake(validator, staked_mgo, ctx)
    }
    ...
    public(friend) fun advance_epoch(
        self: &mut ValidatorSet,
        computation_reward: &mut Balance<MGO>,
        storage_fund_reward: &mut Balance<MGO>,
    ...
    fun update_and_process_low_stake_departures(
        ...
        while (i > 0) {
            ...
            let validator_address = validator::mgo_address(validator_ref);
    ...
    /// Called by `mgo_system` to derive reference gas price for the new epoch.
    ...
    public(friend) fun is_active_validator_by_mgo_address(
        self: &ValidatorSet,
        validator_address: address,
    ): bool {
    ...
    /// It differs from `is_active_validator_by_mgo_address` in that the former checks
    ...
    fun find_validator(validators: &vector<Validator>, validator_address: address): Option<u64> {
        ...
        while (i < length) {
            let v = vector::borrow(validators, i);
            if (validator::mgo_address(v) == validator_address) {
    ...
    fun process_validator_departure(
        ...
        let new_epoch = tx_context::epoch(ctx) + 1;
        let validator_address = validator::mgo_address(&validator);
    ...
    fun process_pending_validators(
        ...
        while (!table_vec::is_empty(&self.pending_active_validators)) {
            ...
            event::emit(
                ValidatorJoinEvent {
                    epoch: new_epoch,
                    validator_address: validator::mgo_address(&validator),
                    staking_pool_id: staking_pool_id(&validator),
                }
            );
    ...
    fun compute_slashed_validators(
        ...
        while (!vec_map::is_empty(&validator_report_records)) {
            ...
            assert!(
                is_active_validator_by_mgo_address(self, validator_address),
    ...
    fun distribute_reward(
        ...
        staking_rewards: &mut Balance<MGO>,
        storage_fund_reward: &mut Balance<MGO>,
        ctx: &mut TxContext
    ) {
        let length = vector::length(validators);
    ...
    fun emit_validator_epoch_events(
        ...
        while (i < num_validators) {
            let v = vector::borrow(vs, i);
            let validator_address = validator::mgo_address(v);
    ...
    public(friend) fun active_validator_addresses(self: &ValidatorSet): vector<address> {
        ...
        while (i < length) {
            let validator_address = validator::mgo_address(vector::borrow(vs, i));
