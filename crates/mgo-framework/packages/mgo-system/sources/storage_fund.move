...
module mgo_system::storage_fund {
    ...
    struct StorageFund has store {
        total_object_storage_rebates: Balance<MGO>,
        non_refundable_balance: Balance<MGO>,
    }

    /// Called by `mgo_system` at genesis time.
    public(friend) fun new(initial_fund: Balance<MGO>) : StorageFund {
        StorageFund {
    ...
    /// Called by `mgo_system` at epoch change times to process the inflows and outflows of storage fund.
