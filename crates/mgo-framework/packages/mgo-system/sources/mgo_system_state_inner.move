...
module mgo_system::mgo_system_state_inner {
    ...
    use mgo::object::{ID};
    use mgo_system::staking_pool::{stake_activation_epoch, StakedMgo};
    use mgo::mgo::MGO;
    use mgo::transfer;
    use mgo::tx_context::{Self, TxContext};
    use mgo_system::validator::{Self, Validator};
    use mgo_system::validator_set::{Self, ValidatorSet};
    use mgo_system::validator_cap::{Self, UnverifiedValidatorOperationCap, ValidatorOperationCap};
    use mgo_system::stake_subsidy::{Self, StakeSubsidy};
    use mgo_system::storage_fund::{Self, StorageFund};
    use mgo_system::staking_pool::PoolTokenExchangeRate;
    use mgo::vec_map::{Self, VecMap};
    ...
    struct MgoSystemStateInner has store {
        ...
        safe_mode: bool,
        safe_mode_storage_rewards: Balance<MGO>,
        safe_mode_computation_rewards: Balance<MGO>,
    ...
    public(friend) fun v1_to_v2(self: MgoSystemStateInner): MgoSystemStateInnerV2 {
    ...
    public(friend) fun request_withdraw_stake(
        ...
        assert!(
            stake_activation_epoch(&staked_mgo) <= tx_context::epoch(ctx),
            EStakeWithdrawBeforeActivation
        );
        validator_set::request_withdraw_stake(
            &mut self.validators, staked_mgo, ctx,
        )
    }
    ...
    public(friend) fun report_validator(
        ...
        assert!(validator_set::is_active_validator_by_mgo_address(&self.validators, reportee_addr), ENotValidator);
    ...
    public(friend) fun epoch(self: &MgoSystemStateInnerV2): u64 {
        self.epoch
    }

    public(friend) fun protocol_version(self: &MgoSystemStateInnerV2): u64 {
        self.protocol_version
    }

    public(friend) fun system_state_version(self: &MgoSystemStateInnerV2): u64 {
    ...
    public(friend) fun epoch_start_timestamp_ms(self: &MgoSystemStateInnerV2): u64 {
        self.epoch_start_timestamp_ms
    }

    /// Returns the total amount staked with `validator_addr`.
    /// Aborts if `validator_addr` is not an active validator.
    public(friend) fun validator_stake_amount(self: &MgoSystemStateInnerV2, validator_addr: address): u64 {
        validator_set::validator_total_stake_amount(&self.validators, validator_addr)
    }

    /// Returns the staking pool id of a given validator.
    /// Aborts if `validator_addr` is not an active validator.
    public(friend) fun validator_staking_pool_id(self: &MgoSystemStateInnerV2, validator_addr: address): ID {

        validator_set::validator_staking_pool_id(&self.validators, validator_addr)
    }

    /// Returns reference to the staking pool mappings that map pool ids to active validator addresses
    public(friend) fun validator_staking_pool_mappings(self: &MgoSystemStateInnerV2): &Table<ID, address> {

        validator_set::staking_pool_mappings(&self.validators)
    }

    /// Returns all the validators who are currently reporting `addr`
    public(friend) fun get_reporters_of(self: &MgoSystemStateInnerV2, addr: address): VecSet<address> {
    ...
    public(friend) fun get_storage_fund_total_balance(self: &MgoSystemStateInnerV2): u64 {
        storage_fund::total_balance(&self.storage_fund)
    }

    public(friend) fun get_storage_fund_object_rebates(self: &MgoSystemStateInnerV2): u64 {
    ...
    public(friend) fun active_validator_addresses(self: &MgoSystemStateInnerV2): vector<address> {
    ...
    fun extract_coin_balance(coins: vector<Coin<MGO>>, amount: option::Option<u64>, ctx: &mut TxContext): Balance<MGO> {
    ...
    public(friend) fun validators(self: &MgoSystemStateInnerV2): &ValidatorSet {
        &self.validators
    }

    #[test_only]
    /// Return the currently active validator by address
    public(friend) fun active_validator_by_address(self: &MgoSystemStateInnerV2, validator_address: address): &Validator {
    ...
    public(friend) fun pending_validator_by_address(self: &MgoSystemStateInnerV2, validator_address: address): &Validator {
    ...
    public(friend) fun candidate_validator_by_address(self: &MgoSystemStateInnerV2, validator_address: address): &Validator {
        validator_set::get_candidate_validator_ref(validators(self), validator_address)
    }

    #[test_only]
    public(friend) fun get_stake_subsidy_distribution_counter(self: &MgoSystemStateInnerV2): u64 {
        stake_subsidy::get_distribution_counter(&self.stake_subsidy)
    }

    #[test_only]
    public(friend) fun set_epoch_for_testing(self: &mut MgoSystemStateInnerV2, epoch_num: u64) {
