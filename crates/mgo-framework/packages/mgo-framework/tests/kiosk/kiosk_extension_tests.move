...
#[test_only]
module mgo::kiosk_marketplace_ext {
    use mgo::bag;
    use mgo::mgo::MGO;
    use mgo::coin::{Self, Coin};
    use mgo::object::{Self, ID};
    use mgo::tx_context::TxContext;
    use mgo::kiosk_extension as ext;
    use mgo::kiosk::{Self, KioskOwnerCap, Kiosk, PurchaseCap};
    use mgo::transfer_policy::{Self as policy, TransferPolicy, TransferRequest};

    /// Trying to access an owner-only action.
    const ENotOwner: u64 = 0;
    /// Trying to purchase an item with an incorrect amount of MGO.
    ...
    /// Collection bidding: the Kiosk Owner offers a bid (in MGO) for an item of type `T`.
    ///
    /// There can be only one bid per type.
    public fun bid<Market, T: key + store>(
        kiosk: &mut Kiosk, cap: &KioskOwnerCap, bid: Coin<MGO>
    ) {
...
#[test_only]
module mgo::kiosk_extensions_tests {
    use mgo::kiosk_test_utils::{Self as test};
    use mgo::kiosk_extension as ext;
    use mgo::kiosk;
    ...
    #[test, expected_failure(abort_code = mgo::kiosk_extension::EExtensionNotAllowed)]
    ...
    #[test, expected_failure(abort_code = mgo::kiosk_extension::EExtensionNotInstalled)]
