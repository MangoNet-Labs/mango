...
#[test_only]
module mgo::test_scenario_tests {
    use mgo::object;
    use mgo::test_scenario as ts;
    use mgo::transfer;
    use mgo::tx_context;
    ...
    fun test_dynamic_field_still_borrowed() {
        ...
        mgo::dynamic_field::add(&mut parent, b"", 10);
        let r = mgo::dynamic_field::borrow<vector<u8>, u64>(&parent, b"");
        ts::end(scenario);
        assert!(*r == 10, 0);
    ...
    fun test_dynamic_object_field_still_borrowed() {
        ...
        let parent = ts::new_object(&mut scenario);
        let id = ts::new_object(&mut scenario);
        mgo::dynamic_object_field::add(&mut parent, b"", Object { id, value: 10});
        let obj = mgo::dynamic_object_field::borrow<vector<u8>, Object>(&parent, b"");
        ts::end(scenario);
        assert!(obj.value == 10, 0);
        object::delete(parent);
    }
    ...
    fun test_dynamic_object_field_not_retrievable() {
        ...
        assert!(!ts::has_most_recent_for_address<Object>(sender), 0);
        mgo::dynamic_object_field::add(&mut parent, b"", obj);
        ts::next_tx(&mut scenario, sender);
        assert!(!ts::has_most_recent_for_address<Object>(sender), 0);
        ts::end(scenario);
        object::delete(parent);
    ...
    fun test_dynamic_field_shared_misuse() {
        ...
        assert!(object::id(&obj) == id, 0);
        // wraps the object
        mgo::dynamic_field::add(&mut parent, b"", obj);
        ts::next_tx(&mut scenario, sender);
        abort 42
    }
