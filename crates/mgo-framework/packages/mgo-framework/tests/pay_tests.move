...
#[test_only]
module mgo::pay_tests {
    use std::vector;
    use mgo::test_scenario::{Self};
    ...
    fun test_coin_split_n() {
        ...
        let coin = coin::mint_for_testing<MGO>(10, ctx);

        test_scenario::next_tx(scenario, TEST_SENDER_ADDR);
        pay::divide_and_keep(&mut coin, 3, test_scenario::ctx(scenario));

        test_scenario::next_tx(scenario, TEST_SENDER_ADDR);
        let coin1 = test_scenario::take_from_sender<Coin<MGO>>(scenario);

        test_scenario::next_tx(scenario, TEST_SENDER_ADDR);
        let coin2 = test_scenario::take_from_sender<Coin<MGO>>(scenario);
        ...
        assert!(
            !test_scenario::has_most_recent_for_sender<Coin<MGO>>(scenario),
            1
        );
    ...
    fun test_split_and_transfer_fail() {
        ...
        test_scenario::next_tx(scenario, TEST_SENDER_ADDR);
        let coin_transfer_fail = test_scenario::take_from_sender<Coin<MGO>>(scenario);
