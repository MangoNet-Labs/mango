...
#[test_only]
module mgo::package_tests {
    use std::ascii;
    use mgo::address;
    use std::vector;
    use mgo::object::id_from_address as id;
    use mgo::package::{Self, UpgradeCap, UpgradeTicket};
    use mgo::test_utils;
    use mgo::test_scenario::{Self as test, Scenario, ctx};
    ...
    fun test_upgrade_policy_reflected_in_ticket() {
        ...
        while (!vector::is_empty(&policies)) {
            ...
            let ticket = check_ticket(&mut cap, policy, mgo::hash::blake2b256(&vector[policy]));
    ...
    fun test_full_upgrade_flow() {
        ...
        let ticket = package::authorize_upgrade(
            &mut cap,
            package::dep_only_policy(),
            mgo::hash::blake2b256(&b"package contents"),
        );
    ...
    #[test]
    #[expected_failure(abort_code = mgo::package::ETooPermissive)]
    ...
    #[test]
    #[expected_failure(abort_code = mgo::package::EAlreadyAuthorized)]
    fun test_failure_to_authorize_multiple_upgrades() {
        ...
        let _ticket0 = package::authorize_upgrade(
            &mut cap,
            package::compatible_policy(),
            mgo::hash::blake2b256(&b"package contents 0"),
        );
        ...
        let _ticket1 = package::authorize_upgrade(
            &mut cap,
            package::compatible_policy(),
            mgo::hash::blake2b256(&b"package contents 1"),
        );
    ...
    #[test]
    #[expected_failure(abort_code = mgo::package::EWrongUpgradeCap)]
