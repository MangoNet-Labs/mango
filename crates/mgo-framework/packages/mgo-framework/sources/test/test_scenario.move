...
module mgo::test_scenario {
    ...
    /// Utility for mocking a multi-transaction Mgo execution in a single Move procedure.
    ...
    public fun return_to_address<T: key>(account: address, t: T) {
        ...
        mgo::transfer::transfer_impl(t, account)
    }
    ...
    public fun return_immutable<T: key>(t: T) {
        ...
        mgo::transfer::freeze_object_impl(t)
    }
    ...
    public fun return_shared<T: key>(t: T) {
        ...
        mgo::transfer::share_object_impl(t)
    }
