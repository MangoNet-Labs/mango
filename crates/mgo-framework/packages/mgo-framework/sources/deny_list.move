...
module mgo::deny_list {
    ...
    /// The index into the deny list vector for the `mgo::coin::Coin` type.
    ...
    struct PerTypeList has key, store {
        ...
        /// For example with `mgo::coin::Coin`: If addresses A and B are banned from using
