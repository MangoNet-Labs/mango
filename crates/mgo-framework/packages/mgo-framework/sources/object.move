...
module mgo::object {
    ...
    friend mgo::clock;
    friend mgo::coin;
    friend mgo::dynamic_field;
    friend mgo::dynamic_object_field;
    friend mgo::transfer;
    friend mgo::authenticator_state;
    friend mgo::random;
    friend mgo::deny_list;

    #[test_only]
    friend mgo::test_scenario;

    /// The hardcoded ID for the singleton Mgo System State Object.
    const MGO_SYSTEM_STATE_OBJECT_ID: address = @0x5;

    /// The hardcoded ID for the singleton Clock Object.
    const MGO_CLOCK_OBJECT_ID: address = @0x6;

    /// The hardcoded ID for the singleton AuthenticatorState Object.
    const MGO_AUTHENTICATOR_STATE_ID: address = @0x7;

    /// The hardcoded ID for the singleton Random Object.
    const MGO_RANDOM_ID: address = @0x8;

    /// The hardcoded ID for the singleton DenyList.
    const MGO_DENY_LIST_OBJECT_ID: address = @0x403;
    ...
    /// Create the `UID` for the singleton `MgoSystemState` object.
    /// This should only be called once from `mgo_system`.
    ...
    /// Create a new object. Returns the `UID` that must be stored in a Mgo object.
