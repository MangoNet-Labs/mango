...
/// `mgo::table`, the values bound to these dynamic fields _must_ be objects themselves. This allows
...
module mgo::object_table {
    ...
    /// Aborts with `mgo::dynamic_field::EFieldAlreadyExists` if the table already has an entry with
    /// that key `k: K`.
    ...
    /// Aborts with `mgo::dynamic_field::EFieldDoesNotExist` if the table does not have an entry with
    /// that key `k: K`.
