...
/// A bag is a heterogeneous map-like collection. The collection is similar to `mgo::table` in that
/// its keys and values are not stored within the `Bag` value, but instead are stored using Mgo's
...
/// assert!(&bag1 != &bag2, 0);
/// ```
/// At it's core, `mgo::bag` is a wrapper around `UID` that allows for access to
/// `mgo::dynamic_field` while preventing accidentally stranding field values. A `UID` can be
...
module mgo::bag {
    ...
    /// Aborts with `mgo::dynamic_field::EFieldAlreadyExists` if the bag already has an entry with
    /// that key `k: K`.
    ...
    /// Aborts with `mgo::dynamic_field::EFieldDoesNotExist` if the bag does not have an entry with
    /// that key `k: K`.
    /// Aborts with `mgo::dynamic_field::EFieldTypeMismatch` if the bag has an entry for the key, but
