...
module mgo::kiosk {
    ...
    entry fun default(ctx: &mut TxContext) {
        ...
        mgo::transfer::share_object(kiosk);
    }
    ...
    public fun purchase<T: key + store>(
        self: &mut Kiosk, id: ID, payment: Coin<MGO>
    ): (T, TransferRequest<T>) {
    ...
    public fun purchase_with_cap<T: key + store>(
        self: &mut Kiosk, purchase_cap: PurchaseCap<T>, payment: Coin<MGO>
    ): (T, TransferRequest<T>) {
    ...
    public fun profits_mut(self: &mut Kiosk, cap: &KioskOwnerCap): &mut Balance<MGO> {
        assert!(has_access(self, cap), ENotOwner);
        &mut self.profits
    }
