...
A bag is a heterogeneous map-like collection. The collection is similar to <code>mgo::table</code> in that
its keys and values are not stored within the <code><a href="bag.md#0x2_bag_Bag">Bag</a></code> value, but instead are stored using Mgo's
...
assert!(&bag1 != &bag2, 0);
```
At it's core, <code>mgo::bag</code> is a wrapper around <code>UID</code> that allows for access to
<code>mgo::dynamic_field</code> while preventing accidentally stranding field values. A <code>UID</code> can be
