...
//! provides the [`Simulacrum`] type which is a implementation or instantiation of a mgo
...
use rand::rngs::OsRng;
use mgo_config::{genesis, transaction_deny_config::TransactionDenyConfig};
...
use mgo_types::base_types::{AuthorityName, ObjectID, VersionNumber};
use mgo_types::crypto::AuthoritySignature;
use mgo_types::digests::ConsensusCommitDigest;
use mgo_types::object::Object;
use mgo_types::storage::{ObjectStore, ReadStore};
use mgo_types::mgo_system_state::epoch_start_mgo_system_state::EpochStartSystemState;
use mgo_types::transaction::EndOfEpochTransactionKind;
use mgo_types::{
...
use mgo_types::mock_checkpoint_builder::{MockCheckpointBuilder, ValidatorKeypairProvider};
use mgo_types::{
    gas_coin::GasCoin,
...
/// A `Simulacrum` of Mgo.
///
/// This type represents a simulated instantiation of a Mgo blockchain that needs to be driven
...
impl<R, S: store::SimulatorStore> Simulacrum<R, S> {
    pub fn new_with_network_config_store(config: &NetworkConfig, rng: R, store: S) -> Self {
        ...
        let epoch_state = EpochState::new(genesis.mgo_system_object());
    ...
    /// # }
    /// ```
    pub fn request_gas(&mut self, address: MgoAddress, amount: u64) -> Result<TransactionEffects> {
        ...
        let pt = {
            let mut builder =
                mgo_types::programmable_transaction_builder::ProgrammableTransactionBuilder::new();
            builder.transfer_mgo(address, Some(amount));
            builder.finish()
        };

        let kind = mgo_types::transaction::TransactionKind::ProgrammableTransaction(pt);
        let tx_data =
            mgo_types::transaction::TransactionData::new_with_gas_data(kind, *sender, gas_data);
...
impl<T, V: store::SimulatorStore> ReadStore for Simulacrum<T, V> {
    ...
    ) -> mgo_types::storage::error::Result<Option<std::sync::Arc<Committee>>> {
        todo!()
    }
    ...
    fn get_checkpoint_contents_by_digest(
        &self,
        digest: &mgo_types::messages_checkpoint::CheckpointContentsDigest,
    ...
    fn get_transaction(
        &self,
        tx_digest: &mgo_types::digests::TransactionDigest,
    ...
    fn get_events(
        &self,
        event_digest: &mgo_types::digests::TransactionEventsDigest,
    ) -> mgo_types::storage::error::Result<Option<mgo_types::effects::TransactionEvents>> {
    ...
    fn get_full_checkpoint_contents(
        &self,
        _digest: &mgo_types::messages_checkpoint::CheckpointContentsDigest,
    ) -> mgo_types::storage::error::Result<
        Option<mgo_types::messages_checkpoint::FullCheckpointContents>,
    > {
        todo!()
...
impl Simulacrum {
    ...
    /// existing code for generating transactions in mgo-test-transaction-builder by defining a trait
    /// that both WalletContext and Simulacrum implement. Then we can remove this function.
    pub fn transfer_txn(&mut self, recipient: MgoAddress) -> (Transaction, u64) {
        let (sender, key) = self.keystore().accounts().next().unwrap();
        let sender = *sender;
        ...
        let pt = {
            ...
            builder.transfer_mgo(recipient, Some(transfer_amount));
            builder.finish()
        };
...
mod tests {
    ...
    use mgo_types::{
        base_types::MgoAddress, effects::TransactionEffectsAPI, gas_coin::GasCoin,
