...
use mgo_types::transaction::InputObjects;
use mgo_types::transaction::ObjectReadResult;
use mgo_types::transaction::ReceivingObjectReadResult;
use mgo_types::transaction::ReceivingObjects;
use mgo_types::{
...
pub trait SimulatorStore:
    mgo_types::storage::BackingPackageStore
    + mgo_types::storage::ObjectStore
    + ParentSync
    + ChildObjectResolver
{
    ...
    // mgo_core::TransactionInputLoad using an appropriate cache implementation.
    fn read_objects_for_synchronous_execution(
        &self,
        _tx_digest: &TransactionDigest,
        input_object_kinds: &[InputObjectKind],
        receiving_object_refs: &[ObjectRef],
    ) -> MgoResult<(InputObjects, ReceivingObjects)> {
