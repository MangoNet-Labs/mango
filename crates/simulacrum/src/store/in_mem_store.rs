...
use mgo_types::storage::{get_module, load_package_object_from_object_store, PackageObject};
use mgo_types::{
    base_types::{AuthorityName, ObjectID, SequenceNumber, MgoAddress},
...
impl InMemoryStore {
    ...
    pub fn get_system_state(&self) -> mgo_types::mgo_system_state::MgoSystemState {
        mgo_types::mgo_system_state::get_mgo_system_state(self).expect("system state must exist")
    }

    pub fn get_clock(&self) -> mgo_types::clock::Clock {
        self.get_object(&mgo_types::MGO_CLOCK_OBJECT_ID)
            .expect("clock should exist")
            .to_rust()
            .expect("clock object should deserialize")
    }

    pub fn owned_objects(&self, owner: MgoAddress) -> impl Iterator<Item = &Object> {
        self.live_objects
            .iter()
...
pub struct KeyStore {
    ...
    #[allow(unused)]
    account_keys: BTreeMap<MgoAddress, AccountKeyPair>,
}

impl KeyStore {
    pub fn from_network_config(
        network_config: &mgo_swarm_config::network_config::NetworkConfig,
    ) -> Self {
    ...
    pub fn accounts(&self) -> impl Iterator<Item = (&MgoAddress, &AccountKeyPair)> {
