...
use mgo_types::{
    ...
    mgo_system_state::{
        epoch_start_mgo_system_state::{EpochStartSystemState, EpochStartSystemStateTrait},
        MgoSystemState, MgoSystemStateTrait,
    },
...
impl EpochState {
    pub fn new(system_state: MgoSystemState) -> Self {
        let epoch_start_state = system_state.into_epoch_start_state();
        let committee = epoch_start_state.get_mgo_committee();
        let protocol_config =
        ...
        let executor = mgo_execution::executor(&protocol_config, true, None).unwrap();
    ...
    pub fn execute_transaction(
        ...
        MgoGasStatus,
        TransactionEffects,
        Result<(), mgo_types::error::ExecutionError>,
    )> {
        ...
        // for signing
        let (gas_status, checked_input_objects) = mgo_transaction_checks::check_transaction_input(
            &self.protocol_config,
            self.epoch_start_state.reference_gas_price(),
