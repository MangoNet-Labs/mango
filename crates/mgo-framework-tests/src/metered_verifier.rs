...
use mgo_adapter::adapter::run_metered_move_bytecode_verifier;
use mgo_framework::BuiltInFramework;
use mgo_move_build::{CompiledPackage, MgoPackageHooks};
use mgo_protocol_config::ProtocolConfig;
use mgo_types::{
    error::{MgoError, MgoResult},
    metrics::BytecodeVerifierMetrics,
};
use mgo_verifier::{default_verifier_config, meter::MgoVerifierMeter};

fn build(path: PathBuf) -> MgoResult<CompiledPackage> {
    let mut config = mgo_move_build::BuildConfig::new_for_testing();
...
fn test_metered_move_bytecode_verifier() {
    ...
    let path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../mgo-framework/packages/mgo-framework");
    ...
    let mut meter = MgoVerifierMeter::new(&metered_verifier_config);
    ...
    assert_eq!(
        ...
        bytecode_verifier_metrics
            .verifier_timeout_metrics
            .with_label_values(&[
                BytecodeVerifierMetrics::MGO_VERIFIER_TAG,
                BytecodeVerifierMetrics::TIMEOUT_TAG,
            ])
            .get(),
    );
    ...
    let path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../mgo_programmability/examples/basics");
    let package = build(path).unwrap();
    packages.push(package.get_dependency_sorted_modules(with_unpublished_deps));
    packages.push(package.get_dependency_sorted_modules(with_unpublished_deps));

    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../mgo_programmability/examples/fungible_tokens");
    let package = build(path).unwrap();
    packages.push(package.get_dependency_sorted_modules(with_unpublished_deps));
...
fn test_build_and_verify_programmability_examples() {
    ...
    let bytecode_verifier_metrics = Arc::new(BytecodeVerifierMetrics::new(registry));
    let examples =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../mgo_programmability/examples");
