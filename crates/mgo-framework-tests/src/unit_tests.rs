...
use mgo_move::unit_test::run_move_unit_tests;
...
fn run_mgo_framework_tests() {
    check_move_unit_tests({
        let mut buf = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        buf.extend(["..", "mgo-framework", "packages", "mgo-framework"]);
        buf
    });
}
...
#[test]
#[cfg_attr(msim, ignore)]
fn run_mgo_system_tests() {
    check_move_unit_tests({
        let mut buf = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        buf.extend(["..", "mgo-framework", "packages", "mgo-system"]);
        buf
    });
}
...
fn run_examples_move_unit_tests() {
    for example in [
        ...
        let path = {
            let mut buf = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
            buf.extend(["..", "..", "mgo_programmability", "examples", example]);
            buf
        };
