...
pub struct BridgeOrchestratorTables {
    ...
    pub(crate) mgo_syncer_cursors: DBMap<Identifier, EventID>,
...
impl BridgeOrchestratorTables {
    ...
    pub(crate) fn update_mgo_event_cursor(
        &self,
        module: Identifier,
        cursor: EventID,
    ) -> BridgeResult<()> {
        let mut batch = self.mgo_syncer_cursors.batch();

        batch
            .insert_batch(&self.mgo_syncer_cursors, [(module, cursor)])
            .map_err(|e| {
                BridgeError::StorageError(format!(
                    "Coudln't insert into mgo_syncer_cursors: {:?}",
                    e
                ))
            })?;
        batch
    ...
    pub fn get_mgo_event_cursors(
        &self,
        identifiers: &[Identifier],
    ) -> BridgeResult<Vec<Option<EventID>>> {
        self.mgo_syncer_cursors.multi_get(identifiers).map_err(|e| {
            BridgeError::StorageError(format!("Couldn't get mgo_syncer_cursors: {:?}", e))
        })
    }
...
mod tests {
    ...
    use crate::test_utils::get_test_mgo_to_eth_bridge_action;
    ...
    async fn test_bridge_storage_basic() {
        ...
        let action1 = get_test_mgo_to_eth_bridge_action(None, Some(0), Some(99), Some(10000));

        let action2 = get_test_mgo_to_eth_bridge_action(None, Some(2), Some(100), Some(10000));
        ...
        // update mgo event cursor
        let mgo_module = Identifier::from_str("test").unwrap();
        let mgo_cursor = EventID {
            tx_digest: TransactionDigest::random(),
            event_seq: 1,
        };
        assert!(store.get_mgo_event_cursors(&[mgo_module.clone()]).unwrap()[0].is_none());
        store
            .update_mgo_event_cursor(mgo_module.clone(), mgo_cursor)
            .unwrap();
        assert_eq!(
            store.get_mgo_event_cursors(&[mgo_module.clone()]).unwrap()[0].unwrap(),
            mgo_cursor
        );
    }
}
