...
use mgo_types::{base_types::ConciseableName, message_envelope::VerifiedEnvelope};
use tap::TapFallible;
...
mod tests {
    ...
    use crate::test_utils::{get_test_authority_and_key, get_test_mgo_to_eth_bridge_action};
    use crate::types::SignedBridgeAction;
    use crate::types::{
        BridgeAction, BridgeAuthority, BridgeChainId, MgoToEthBridgeAction, TokenId,
    };
    ...
    fn test_sign_and_verify_bridge_event_basic() -> anyhow::Result<()> {
        ...
        let action: BridgeAction =
            get_test_mgo_to_eth_bridge_action(None, Some(1), Some(1), Some(100));
        ...
        let mismatched_action: BridgeAction =
            get_test_mgo_to_eth_bridge_action(None, Some(2), Some(3), Some(4));
        ...
        let action2: BridgeAction =
            get_test_mgo_to_eth_bridge_action(None, Some(3), Some(5), Some(77));
    ...
    fn test_bridge_sig_verification_regression_test() {
        ...
        let action = BridgeAction::MgoToEthBridgeAction(MgoToEthBridgeAction {
            mgo_tx_digest: TransactionDigest::random(),
            mgo_tx_event_index: 0,
            mgo_bridge_event: EmittedMgoToEthTokenBridgeV1 {
                nonce: 1,
                mgo_chain_id: BridgeChainId::MgoTestnet,
                mgo_address: MgoAddress::from_str(
