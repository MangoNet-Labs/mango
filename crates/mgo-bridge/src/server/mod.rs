...
pub const ETH_TO_MGO_TX_PATH: &str = "/sign/bridge_tx/eth/mgo/:tx_hash/:event_index";
pub const MGO_TO_ETH_TX_PATH: &str = "/sign/bridge_tx/mgo/eth/:tx_digest/:event_index";
...
pub(crate) fn make_router(
    ...
    Router::new()
        ...
        .route(ETH_TO_MGO_TX_PATH, get(handle_eth_tx_hash))
        .route(MGO_TO_ETH_TX_PATH, get(handle_mgo_tx_digest))
        .with_state(handler)
}
...
async fn handle_mgo_tx_digest(
    ...
    let sig: Json<
        mgo_types::message_envelope::Envelope<
            crate::types::BridgeAction,
            crate::crypto::BridgeAuthoritySignInfo,
        >,
    > = handler
        .handle_mgo_tx_digest(tx_digest_base58, event_idx)
        .await?;
    Ok(sig)
}
