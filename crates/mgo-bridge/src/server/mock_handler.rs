...
pub struct BridgeRequestMockHandler {
    ...
    mgo_token_events_requested: Arc<Mutex<HashMap<(TransactionDigest, u16), u64>>>,
}
...
impl BridgeRequestMockHandler {
    ...
    pub fn add_mgo_event_response(
        &self,
        tx_digest: TransactionDigest,
        idx: u16,
        response: BridgeResult<SignedBridgeAction>,
    ) {
        self.mgo_token_events
            .lock()
            .unwrap()
            .insert((tx_digest, idx), response);
    }

    pub fn get_mgo_token_events_requested(
        &self,
        tx_digest: TransactionDigest,
        event_index: u16,
    ) -> u64 {
        *self
            .mgo_token_events_requested
            .lock()
            .unwrap()
...
impl BridgeRequestHandlerTrait for BridgeRequestMockHandler {
    ...
    async fn handle_mgo_tx_digest(
        ...
        let preset = self.mgo_token_events.lock().unwrap();
        if !preset.contains_key(&(tx_digest, event_idx)) {
            // Ok to panic in test
            panic!(
                "No preset handle_mgo_tx_digest result for tx_digest: {}, event_idx: {}",
                tx_digest, event_idx
            );
        }
        let mut requested = self.mgo_token_events_requested.lock().unwrap();
        ...
        *entry += 1;
        ...
        let signed_action: &mgo_types::message_envelope::Envelope<
