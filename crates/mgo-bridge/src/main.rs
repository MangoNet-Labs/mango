...
use clap::Parser;
use mango_metrics::start_prometheus_server;
use std::{
    collections::HashMap,
...
use mgo_bridge::{
    ...
    mgo_syncer::MgoSyncer,
};
use mgo_config::Config;
use tokio::task::JoinHandle;
use tracing::info;

// TODO consolidate this with mgo-node/src/main.rs, but where to put it?
...
async fn start_client_components(
    ...
    let stored_module_cursors = store
        .get_mgo_event_cursors(&client_config.mgo_bridge_modules)
        .map_err(|e| anyhow::anyhow!("Unable to get mgo event cursors from storage: {e:?}"))?;
    let mut mgo_modules_to_watch = HashMap::new();
        ...
        .zip(stored_module_cursors)
    {
        if client_config
            ...
            .contains_key(module)
        {
            mgo_modules_to_watch.insert(
                module.clone(),
                client_config.mgo_bridge_modules_last_processed_event_id_override[module],
            );
            info!(
                "Overriding cursor for mgo bridge module {} to {:?}. Stored cursor: {:?}",
                module,
                client_config.mgo_bridge_modules_last_processed_event_id_override[module],
                cursor
            );
        } else if let Some(cursor) = cursor {
            mgo_modules_to_watch.insert(module.clone(), cursor);
        } else {
            return Err(anyhow::anyhow!(
                "No cursor found for mgo bridge module {} in storage or config override",
                module
            ));
        }
    }
    ...
    let mgo_client = client_config.mgo_client.clone();
    ...
    let (task_handles, mgo_events_rx) =
        MgoSyncer::new(client_config.mgo_client, mgo_modules_to_watch)
            .run(Duration::from_secs(2))
            .await
            .expect("Failed to start mgo syncer");
    all_handles.extend(task_handles);
    ...
    let bridge_action_executor = BridgeActionExecutor::new(
        ...
        store.clone(),
        client_config.key,
        client_config.mgo_address,
        client_config.gas_object_ref.0,
    );

    let orchestrator =
        BridgeOrchestrator::new(mgo_client, mgo_events_rx, eth_events_rx, store.clone());
