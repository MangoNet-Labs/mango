...
use mgo_types::{
    base_types::{ObjectID, ObjectRef, SequenceNumber, MgoAddress},
...
// TODO: once we have bridge package on mgo framework, we can hardcode the actual package id.
...
// TODO: this should be hardcoded once we have bridge package on mgo framework.
...
pub fn get_mgo_token_type_tag(token_id: TokenId) -> TypeTag {
    ...
    let type_tags = TYPE_TAGS.get_or_init(|| {
        ...
        type_tags.insert(TokenId::Mgo, GAS::type_tag());
        type_tags.insert(
            TokenId::BTC,
    ...
    let (source_chain, seq_num, sender, target_chain, target, token_type, amount) =
        match bridge_action {
            BridgeAction::MgoToEthBridgeAction(a) => {
                let bridge_event = a.mgo_bridge_event;
                (
                    bridge_event.mgo_chain_id,
                    bridge_event.nonce,
                    bridge_event.mgo_address.to_vec(),
    ...
    if claim {
        builder.programmable_move_call(
            *get_bridge_package_id(),
            ...
            vec![get_mgo_token_type_tag(token_type)],
