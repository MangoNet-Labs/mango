...
use crate::mgo_client::MgoClient;
use anyhow::anyhow;
...
use mgo_sdk::MgoClient as MgoSdkClient;
...
pub struct BridgeNodeConfig {
    ...
    /// Rpc url for Mgo fullnode, used for query stuff and submit transactions.
    pub mgo_rpc_url: String,
    ...
    /// Path of the file where bridge client key (any MgoKeyPair) is stored as Base64 encoded `flag || privkey`.
    /// If `run_client` is true, and this is None, then use `bridge_authority_key_path_base64_raw` as client key.
    pub bridge_client_key_path_base64_mgo_key: Option<PathBuf>,
    /// Whether to run client. If true, `bridge_client_key_path_base64_mgo_key`,
    ...
    /// The mgo modules of bridge packages for client to watch for. Need to contain at least one item when `run_client` is true.
    pub mgo_bridge_modules: Option<Vec<String>>,
    ...
    /// Override the last processed EventID for each bridge module. Key must be in `mgo_bridge_modules`.
    /// When set, MgoSyncer will start from this cursor (exclusively) instead of the one in storage.
    /// Key: mgo module, Value: last processed EventID (tx_digest, event_seq).
    ...
    pub mgo_bridge_modules_last_processed_event_id_override: Option<BTreeMap<String, EventID>>,
}
...
impl BridgeNodeConfig {
    pub async fn validate(
        ...
        let mgo_client = Arc::new(MgoClient::<MgoSdkClient>::new(&self.mgo_rpc_url).await?);
        ...
        let bridge_client_key = if self.bridge_client_key_path_base64_mgo_key.is_none() {
            let bridge_client_key =
                read_bridge_authority_key(&self.bridge_authority_key_path_base64_raw)?;
            Ok(MgoKeyPair::from(bridge_client_key))
        } else {
            read_bridge_client_key(self.bridge_client_key_path_base64_mgo_key.as_ref().unwrap())
        }?;

        let client_mgo_address = MgoAddress::from(&bridge_client_key.public());
        info!("Bridge client mgo address: {:?}", client_mgo_address);
        ...
        let mgo_bridge_modules = match &self.mgo_bridge_modules {
            Some(modules) => {
                if modules.is_empty() {
                    return Err(anyhow!(
                        "`mgo_bridge_modules` is required when `run_client` is true"
                    ));
                }
                modules
                    .iter()
                    ...
                    .collect::<Result<Vec<_>, _>>()
                    .map_err(|e| anyhow!("Error parsing mgo module: {:?}", e))?
            }
            None => {
                return Err(anyhow!(
                    "`mgo_bridge_modules` is required when `run_client` is true"
        ...
        let mut mgo_bridge_modules_last_processed_event_id_override = BTreeMap::new();
        match &self.mgo_bridge_modules_last_processed_event_id_override {
            Some(overrides) => {
                for (module, cursor) in overrides {
                    let module = Identifier::from_str(module)?;
                    if mgo_bridge_modules.contains(&module) {
                        mgo_bridge_modules_last_processed_event_id_override.insert(module, *cursor);
                    } else {
                        return Err(anyhow!(
                            "Override start tx digest for module {:?} is not in `mgo_bridge_modules`",
                            module
                        ));
                    }
                }
        ...
        let (gas_coin, gas_object_ref, owner) = mgo_client
            .get_gas_data_panic_if_not_gas(gas_object_id)
            .await;
        if owner != Owner::AddressOwner(client_mgo_address) {
            return Err(anyhow!("Gas object {:?} is not owned by bridge client key's associated mgo address {:?}, but {:?}", gas_object_id, client_mgo_address, owner));
        }
        info!(
        ...
        let bridge_client_config = BridgeClientConfig {
            mgo_address: client_mgo_address,
            ...
            mgo_bridge_modules_last_processed_event_id_override,
        };
...
pub struct BridgeServerConfig {
    ...
    pub server_listen_port: u16,
    pub metrics_port: u16,
    pub mgo_client: Arc<MgoClient<MgoSdkClient>>,
...
pub struct BridgeClientConfig {
    ...
    pub mgo_bridge_modules: Vec<Identifier>,
    pub eth_bridge_contracts_start_block_override: BTreeMap<EthAddress, u64>,
    pub mgo_bridge_modules_last_processed_event_id_override: BTreeMap<Identifier, EventID>,
}
...
/// Read Bridge client key (any MgoKeyPair) from a file.
/// Read from file as Base64 encoded `flag || privkey`.
pub fn read_bridge_client_key(path: &PathBuf) -> Result<MgoKeyPair, anyhow::Error> {
    if !path.exists() {
        return Err(anyhow::anyhow!(
    ...
    MgoKeyPair::decode_base64(contents.as_str().trim())
        .map_err(|e| anyhow!("Error decoding authority key: {:?}", e))
}
