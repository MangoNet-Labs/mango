...
use mgo_json_rpc_types::{EventFilter, Page, MgoData, MgoEvent};
use mgo_json_rpc_types::{
    EventPage, MgoObjectDataOptions, MgoTransactionBlockResponse,
    MgoTransactionBlockResponseOptions,
};
use mgo_sdk::{MgoClient as MgoSdkClient, MgoClientBuilder};
use mgo_types::base_types::ObjectRef;
use mgo_types::collection_types::LinkedTableNode;
...
// TODO: once we have bridge package on mgo framework, we can hardcode the actual
...
// TODO: once we have bridge package on mgo framework, we can hardcode the actual id.
...
pub struct MgoClient<P> {
    inner: P,
}

impl MgoClient<MgoSdkClient> {
    pub async fn new(rpc_url: &str) -> anyhow::Result<Self> {
        let inner = MgoClientBuilder::default().build(rpc_url).await?;
    ...
    P: MgoClientInner,
{
    ...
    async fn describe(&self) -> anyhow::Result<()> {
        ...
        tracing::info!(
            "MgoClient is connected to chain {chain_id}, current block number: {block_number}"
        );
        Ok(())
    }
    ...
    pub async fn query_events_by_module(
        ...
        // cursor is exclusive
        cursor: EventID,
    ) -> BridgeResult<Page<MgoEvent, EventID>> {
    ...
    /// Returns BridgeAction from a Mgo Transaction with transaction hash
    ...
    pub async fn get_bridge_action_by_tx_digest_and_event_idx_maybe(
        ...
        if event.type_.address.as_ref() != get_bridge_package_id().as_ref() {
            return Err(BridgeError::BridgeEventInUnrecognizedMgoPackage);
        }
        let bridge_event = MgoBridgeEvent::try_from_mgo_event(event)?
            .ok_or(BridgeError::NoBridgeEventsInTxPosition)?;
        ...
        for member in move_type_bridge_committee.members.contents {
            ...
            let base_url = from_utf8(&http_rest_url).unwrap_or_else(|e| {
                warn!(
                    ...
                    mgo_address, bridge_pubkey_bytes, http_rest_url
                );
                ""
            });
    ...
    pub async fn execute_transaction_block_with_effects(
        &self,
        tx: mgo_types::transaction::Transaction,
    ) -> BridgeResult<MgoTransactionBlockResponse> {
...
/// Use a trait to abstract over the MgoSDKClient and MgoMockClient for testing.
#[async_trait]
pub trait MgoClientInner: Send + Sync {
...
#[async_trait]
impl MgoClientInner for MgoSdkClient {
    ...
    async fn get_action_onchain_status(
            ...
            .into_object()
        {
            ...
            Err(MgoObjectResponseError::DynamicFieldNotFound { .. }) => {
    ...
    async fn execute_transaction_block_with_effects(
        ...
        match self.quorum_driver_api().execute_transaction_block(
            ...
            Some(mgo_types::quorum_driver_types::ExecuteTransactionRequestType::WaitForEffectsCert),
        ).await {
            Ok(response) => Ok(response),
            Err(e) => return Err(BridgeError::MgoTxFailureGeneric(e.to_string())),
        }
    }
        ...
        loop {
            match self
                ...
                .get_object_with_options(
                    gas_object_id,
                    MgoObjectDataOptions::default().with_owner().with_content(),
                )
                .await
                .map(|resp| resp.data)
            {
...
mod tests {
    use crate::{
        events::{EmittedMgoToEthTokenBridgeV1, MoveTokenBridgeEvent},
        mgo_mock_client::MgoMockClient,
        test_utils::{
            bridge_token, get_test_mgo_to_eth_bridge_action, mint_tokens, publish_bridge_package,
            transfer_treasury_cap,
        },
        types::{BridgeActionType, BridgeChainId, MgoToEthBridgeAction, TokenId},
    };
    use ethers::{
        abi::Token,
        types::{
    ...
    use super::*;
    use crate::events::{init_all_struct_tags, MgoToEthTokenBridgeV1};
    ...
    async fn get_bridge_action_by_tx_digest_and_event_idx_maybe() {
        ...
        let sanitized_event_1 = EmittedMgoToEthTokenBridgeV1 {
            nonce: 1,
        ...
        let emitted_event_1 = MoveTokenBridgeEvent {
            ...
            source_chain: sanitized_event_1.mgo_chain_id as u8,
            sender_address: sanitized_event_1.mgo_address.to_vec(),
        ...
        mgo_event_2.type_.module = Identifier::from_str("unrecognized_module").unwrap();
        mgo_event_2.bcs = bcs::to_bytes(&event_2).unwrap();

        // Event 3 is defined in non-bridge package
        let mut mgo_event_3 = mgo_event_1.clone();
        mgo_event_3.type_.address = AccountAddress::random();
        ...
        let mut expected_action_1 = BridgeAction::MgoToEthBridgeAction(MgoToEthBridgeAction {
            mgo_tx_digest: tx_digest,
            mgo_tx_event_index: 0,
            mgo_bridge_event: sanitized_event_1.clone(),
        });
        assert_eq!(
            mgo_client
        ...
        let mut expected_action_2 = BridgeAction::MgoToEthBridgeAction(MgoToEthBridgeAction {
            mgo_tx_digest: tx_digest,
            mgo_tx_event_index: 2,
            mgo_bridge_event: sanitized_event_1.clone(),
        });
        assert_eq!(
            mgo_client
        ...
        mgo_event_2.type_ = MgoToEthTokenBridgeV1.get().unwrap().clone();
        mock_client.add_events_by_tx_digest(tx_digest, vec![mgo_event_2]);
        mgo_client
            .get_bridge_action_by_tx_digest_and_event_idx_maybe(&tx_digest, 2)
            .await
            .unwrap_err();
    ...
    #[tokio::test]
    async fn test_get_action_onchain_status_for_mgo_to_eth_transfer() {
        let mut test_cluster = TestClusterBuilder::new().build().await;
        let context = &mut test_cluster.wallet;
        let sender = context.active_address().unwrap();

        let treasury_caps = publish_bridge_package(context).await;
        let mgo_client = MgoClient::new(&test_cluster.fullnode_handle.rpc_url)
            .await
            .unwrap();

        let action = get_test_mgo_to_eth_bridge_action(None, None, None, None);
    ...
    #[tokio::test]
    async fn test_get_action_onchain_status_for_eth_to_mgo_transfer() {
        // TODO: init an eth -> mgo transfer, run bridge committee, approve the action, then assert status is Approved/Claimed
    }
}
