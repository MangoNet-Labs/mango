...
use mgo_json_rpc_types::MgoProgrammableMoveCall;
use mgo_json_rpc_types::MgoProgrammableTransactionBlock;
use mgo_json_rpc_types::{BalanceChange, MgoArgument};
use mgo_json_rpc_types::{MgoCallArg, MgoCommand};
use mgo_sdk::rpc_types::{
    MgoTransactionBlockData, MgoTransactionBlockDataAPI, MgoTransactionBlockEffectsAPI,
    MgoTransactionBlockKind, MgoTransactionBlockResponse,
};
use mgo_types::base_types::{ObjectID, SequenceNumber, MgoAddress};
use mgo_types::gas_coin::{GasCoin, GAS};
use mgo_types::governance::{ADD_STAKE_FUN_NAME, WITHDRAW_STAKE_FUN_NAME};
use mgo_types::object::Owner;
use mgo_types::mgo_system_state::MGO_SYSTEM_MODULE_NAME;
use mgo_types::transaction::TransactionData;
use mgo_types::{MGO_SYSTEM_ADDRESS, MGO_SYSTEM_PACKAGE_ID};
...
impl Operations {
    ...
    pub fn into_internal(self) -> Result<InternalOperation, Error> {
        ...
        match type_ {
            OperationType::PayMgo => self.pay_mgo_ops_to_internal(),
    ...
    fn pay_mgo_ops_to_internal(self) -> Result<InternalOperation, Error> {
    ...
    fn stake_ops_to_internal(self) -> Result<InternalOperation, Error> {
        ...
        // Total issued MGO is less than u64, safe to cast.
    ...
    fn parse_programmable_transaction(
        sender: MgoAddress,
        status: Option<OperationStatus>,
        pt: MgoProgrammableTransactionBlock,
    ) -> Result<Vec<Operation>, Error> {
        #[derive(Debug)]
        enum KnownValue {
            GasCoin(u64),
            ...
            let amounts = amounts
                ...
                .map(|amount| {
                    let value: u64 = match *amount {
                        ...
                        MgoArgument::GasCoin
                        | MgoArgument::Result(_)
                        | MgoArgument::NestedResult(_, _) => return None,
                    };
        ...
        fn transfer_object(
            aggregated_recipients: &mut HashMap<MgoAddress, u64>,
            inputs: &[MgoCallArg],
            known_results: &[Vec<KnownValue>],
            objs: &[MgoArgument],
            recipient: MgoArgument,
        ) -> Option<Vec<KnownValue>> {
            let addr = match recipient {
                MgoArgument::Input(i) => inputs[i as usize].pure()?.to_mgo_address().ok()?,
                MgoArgument::GasCoin | MgoArgument::Result(_) | MgoArgument::NestedResult(_, _) => {
                    return None
                }
            };
            for obj in objs {
                let value = match *obj {
                    ...
                    MgoArgument::GasCoin | MgoArgument::Input(_) => return None,
                };
        ...
        fn stake_call(
            inputs: &[MgoCallArg],
            known_results: &[Vec<KnownValue>],
            call: &MgoProgrammableMoveCall,
        ) -> Result<Option<(Option<u64>, MgoAddress)>, Error> {
            let MgoProgrammableMoveCall { arguments, .. } = call;
            let (amount, validator) = match &arguments[..] {
                [_, coin, validator] => {
                    ...
                    let (some_amount, validator) = match validator {
                        ...
                        MgoArgument::Input(i) => (*i==1, inputs[*i as usize].pure().map(|v|v.to_mgo_address()).transpose()),
                        _=> return Ok(None),
                    };
        ...
        let MgoProgrammableTransactionBlock { inputs, commands } = &pt;
        let mut known_results: Vec<Vec<KnownValue>> = vec![];
        let mut aggregated_recipients: HashMap<MgoAddress, u64> = HashMap::new();
        ...
        for command in commands {
            let result = match command {
                ...
                MgoCommand::TransferObjects(objs, addr) => transfer_object(
                ...
                MgoCommand::MoveCall(m) if Self::is_stake_call(m) => {
                ...
                MgoCommand::MoveCall(m) if Self::is_unstake_call(m) => {
            ...
            operations.extend(
                aggregated_recipients
                    ...
                    .map(|(recipient, amount)| {
                        Operation::pay_mgo(status, recipient, amount.into())
                    }),
            );
            operations.push(Operation::pay_mgo(status, sender, -(total_paid as i128)));
    ...
    fn is_stake_call(tx: &MgoProgrammableMoveCall) -> bool {
        tx.package == MGO_SYSTEM_PACKAGE_ID
            && tx.module == MGO_SYSTEM_MODULE_NAME.as_str()
            && tx.function == ADD_STAKE_FUN_NAME.as_str()
    }

    fn is_unstake_call(tx: &MgoProgrammableMoveCall) -> bool {
        tx.package == MGO_SYSTEM_PACKAGE_ID
            && tx.module == MGO_SYSTEM_MODULE_NAME.as_str()
    ...
    fn process_balance_change(
        gas_owner: MgoAddress,
        gas_used: i128,
        balance_changes: &[BalanceChange],
        status: Option<OperationStatus>,
        balances: HashMap<MgoAddress, i128>,
...
impl TryFrom<MgoTransactionBlockData> for Operations {
    type Error = Error;
    fn try_from(data: MgoTransactionBlockData) -> Result<Self, Self::Error> {
...
impl TryFrom<MgoTransactionBlockResponse> for Operations {
    type Error = Error;
...
fn is_unstake_event(tag: &StructTag) -> bool {
    tag.address == MGO_SYSTEM_ADDRESS
...
impl TryFrom<TransactionData> for Operations {
    ...
    fn try_from(data: TransactionData) -> Result<Self, Self::Error> {
        ...
        MgoTransactionBlockData::try_from(data, &&mut NoOpsModuleResolver)?.try_into()
    }
}
...
pub enum OperationMetadata {
    GenericTransaction(MgoTransactionBlockKind),
    Stake { validator: MgoAddress },
...
impl Operation {
    ...
    pub fn genesis(index: u64, sender: MgoAddress, coin: GasCoin) -> Self {
        Operation {
    ...
    fn pay_mgo(status: Option<OperationStatus>, address: MgoAddress, amount: i128) -> Self {
        Operation {
            operation_identifier: Default::default(),
            type_: OperationType::PayMgo,
            status,
    ...
    fn balance_change(status: Option<OperationStatus>, addr: MgoAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
            type_: OperationType::MgoBalanceChange,
            status,
    ...
    fn gas(addr: MgoAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
    ...
    fn stake_reward(status: Option<OperationStatus>, addr: MgoAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
    ...
    fn stake_principle(status: Option<OperationStatus>, addr: MgoAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
