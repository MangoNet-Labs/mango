...
use mgo_json_rpc_types::{
    StakeStatus, MgoObjectDataOptions, MgoTransactionBlockEffectsAPI,
    MgoTransactionBlockResponseOptions,
};
use mgo_sdk::rpc_types::MgoExecutionStatus;
use mgo_types::base_types::MgoAddress;
use mgo_types::crypto::{DefaultHash, SignatureScheme, ToFromBytes};
use mgo_types::error::MgoError;
use mgo_types::signature::{GenericSignature, VerifyParams};
...
use crate::{OnlineServerContext, MgoEnv};
...
pub async fn derive(
    Extension(env): Extension<MgoEnv>,
    ...
    let address: MgoAddress = request.public_key.try_into()?;
...
pub async fn submit(
    ...
    if let MgoExecutionStatus::Failure { error } = response
        .effects
...
/// For Mgo, we are returning the latest object refs for all the input objects,
...
pub async fn metadata(
    ...
    let (total_required_amount, objects) = match &option.internal_operation {
        InternalOperation::PayMgo { amounts, .. } => {
        ...
        InternalOperation::WithdrawStake { sender, stake_ids } => {
            ...
            let responses = context
                .client
                .read_api()
                .multi_get_object_with_options(stake_ids, MgoObjectDataOptions::default())
                .await?;
    ...
    // If required amount is None (all MGO) or failed to select coin (might not have enough MGO), select all coins.
