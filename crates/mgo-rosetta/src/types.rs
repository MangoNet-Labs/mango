...
use mgo_sdk::rpc_types::{MgoExecutionStatus, MgoTransactionBlockKind};
use mgo_types::base_types::{ObjectID, ObjectRef, SequenceNumber, MgoAddress, TransactionDigest};
use mgo_types::crypto::PublicKey as MgoPublicKey;
...
use mgo_types::transaction::{Argument, CallArg, Command, ObjectArg, TransactionData};
use mgo_types::MGO_SYSTEM_PACKAGE_ID;
...
impl MgoEnv {
    pub fn check_network_identifier(
        ...
        if &network_identifier.blockchain != "mgo" {
...
impl From<MgoAddress> for AccountIdentifier {
...
impl From<mgo_sdk::rpc_types::Coin> for Coin {
    fn from(coin: mgo_sdk::rpc_types::Coin) -> Self {
        Self {
...
impl From<MgoPublicKey> for PublicKey {
    fn from(pk: MgoPublicKey) -> Self {
        match pk {
            MgoPublicKey::Ed25519(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Edwards25519,
            },
            MgoPublicKey::Secp256k1(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256k1,
            },
            MgoPublicKey::Secp256r1(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256r1,
            },
            MgoPublicKey::ZkLogin(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
...
impl TryInto<MgoAddress> for PublicKey {
    type Error = Error;

    fn try_into(self) -> Result<MgoAddress, Self::Error> {
        let key_bytes = self.hex_bytes.to_vec()?;
        let pub_key = MgoPublicKey::try_from_bytes(self.curve_type.into(), &key_bytes)?;
        Ok((&pub_key).into())
    }
}
...
pub enum OperationType {
    ...
    StakeReward,
    StakePrinciple,
    // mgo-rosetta supported operation type
    PayMgo,
    Stake,
    WithdrawStake,
    // All other Mgo transaction types, readonly
    EpochChange,
    Genesis,
...
impl From<&MgoTransactionBlockKind> for OperationType {
    fn from(tx: &MgoTransactionBlockKind) -> Self {
        match tx {
            MgoTransactionBlockKind::ChangeEpoch(_) => OperationType::EpochChange,
            MgoTransactionBlockKind::Genesis(_) => OperationType::Genesis,
            MgoTransactionBlockKind::ConsensusCommitPrologue(_)
            | MgoTransactionBlockKind::ConsensusCommitPrologueV2(_) => {
                OperationType::ConsensusCommitPrologue
            }
            MgoTransactionBlockKind::ProgrammableTransaction(_) => {
                OperationType::ProgrammableTransaction
            }
            MgoTransactionBlockKind::AuthenticatorStateUpdate(_) => {
                OperationType::AuthenticatorStateUpdate
            }
            MgoTransactionBlockKind::RandomnessStateUpdate(_) => {
...
impl From<MgoExecutionStatus> for OperationStatus {
    fn from(es: MgoExecutionStatus) -> Self {
        match es {
            MgoExecutionStatus::Success => OperationStatus::Success,
            MgoExecutionStatus::Failure { .. } => OperationStatus::Failure,
        }
    }
}
...
impl InternalOperation {
    pub fn sender(&self) -> MgoAddress {
        match self {
            InternalOperation::PayMgo { sender, .. }
        ...
        let pt = match self {
            ...
            InternalOperation::Stake {
                ...
                let (validator, system_state, amount) = if let Some(amount) = amount {
                    ...
                    let state = builder.input(CallArg::MGO_SYSTEM_MUT)?;
            ...
            InternalOperation::WithdrawStake { stake_ids, .. } => {
                ...
                for stake_id in metadata.objects {
                    ...
                    let (system_state, id) = if !stake_ids.is_empty() {
                        let system_state = builder.input(CallArg::MGO_SYSTEM_MUT)?;
