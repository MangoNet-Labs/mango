...
use crate::types::{Currency, MgoEnv};
...
pub static MGO: Lazy<Currency> = Lazy::new(|| Currency {
    symbol: "MGO".to_string(),
    decimals: 9,
});
...
impl RosettaOnlineServer {
    pub fn new(env: MgoEnv, client: MgoClient) -> Self {
    ...
    pub fn serve(self, addr: SocketAddr) -> JoinHandle<hyper::Result<()>> {
        ...
        info!(
            "Mgo Rosetta online server listening on {}",
            server.local_addr()
        );
...
impl RosettaOfflineServer {
    ...
    pub fn serve(self, addr: SocketAddr) -> JoinHandle<hyper::Result<()>> {
        ...
        let server = axum::Server::bind(&addr).serve(app.into_make_service());
        info!(
            "Mgo Rosetta offline server listening on {}",
            server.local_addr()
        );
        spawn_monitored_task!(server)
    }
