...
use mgo_config::{mgo_config_dir, Config, NodeConfig, MGO_FULLNODE_CONFIG, MGO_KEYSTORE_FILENAME};
use mgo_node::MgoNode;
use mgo_rosetta::types::{CurveType, PrefundedAccount, MgoEnv};
use mgo_rosetta::{RosettaOfflineServer, RosettaOnlineServer, MGO};
use mgo_sdk::{MgoClient, MgoClientBuilder};
use mgo_types::base_types::MgoAddress;
use mgo_types::crypto::{KeypairTraits, MgoKeyPair, ToFromBytes};
use tracing::info;
use tracing::log::warn;

#[derive(Parser)]
#[clap(name = "mgo-rosetta", rename_all = "kebab-case", author, version)]
        ...
        match self {
            RosettaServerCommand::GenerateRosettaCLIConfig {
                ...
                let path = keystore_path
                    .unwrap_or_else(|| mgo_config_dir().unwrap().join(MGO_KEYSTORE_FILENAME));

                let prefunded_accounts = read_prefunded_account(&path)?;

                info!(
                    "Retrieved {} Mgo address from keystore file {:?}",
                ...
                let dsl_path = PathBuf::from(".").join("mgo.ros");
                let dsl = include_str!("../resources/mgo.ros");
                fs::write(
                    &dsl_path,
                    dsl.replace("{{mgo.env}}", json!(env).as_str().unwrap()),
                )?;
            ...
            RosettaServerCommand::StartOnlineRemoteServer {
                ...
                info!(
                    "Starting Rosetta Online Server with remove Mgo full node [{full_node_url}]."
                );
                let mgo_client = wait_for_mgo_client(full_node_url).await;
                let rosetta_path = data_path.join("rosetta_db");
                info!("Rosetta db path : {rosetta_path:?}");
                let rosetta = RosettaOnlineServer::new(env, mgo_client);
            ...
            RosettaServerCommand::StartOnlineServer {
                ...
                info!("Starting Rosetta Online Server with embedded Mgo full node.");
                info!("Data directory path: {data_path:?}");

                let node_config = node_config.unwrap_or_else(|| {
                    let path = mgo_config_dir().unwrap().join(MGO_FULLNODE_CONFIG);
                    info!("Using default node config from {path:?}");
                    path
                });

                let mut config = NodeConfig::load(&node_config)?;
                config.db_path = data_path.join("mgo_db");
                info!("Overriding Mgo db path to : {:?}", config.db_path);
...
async fn wait_for_mgo_client(rpc_address: String) -> MgoClient {
    loop {
        match MgoClientBuilder::default()
            ...
            .await
        {
            ...
            Err(e) => {
                warn!("Error connecting to Mgo RPC server [{rpc_address}]: {e}, retrying in 5 seconds.");
...
/// This method reads the keypairs from the Mgo keystore to create the PrefundedAccount objects,
...
fn read_prefunded_account(path: &Path) -> Result<Vec<PrefundedAccount>, anyhow::Error> {
    ...
    let keys = kp_strings
        .iter()
        .map(|kpstr| {
            let key = MgoKeyPair::decode_base64(kpstr);
            key.map(|k| (MgoAddress::from(&k.public()), k))
        })
