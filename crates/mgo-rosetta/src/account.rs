...
use mgo_sdk::rpc_types::StakeStatus;
use mgo_sdk::{MgoClient, MGO_COIN_TYPE};
    ...
    if let Some(SubAccount { account_type }) = request.account_identifier.sub_account {
        ...
        while retry_attempts > 0 {
            let balances_first = ctx
                .client
                .coin_read_api()
                .get_balance(address, Some(MGO_COIN_TYPE.to_string()))
                .await?
