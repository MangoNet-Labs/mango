...
use mgo_core::authority::test_authority_builder::TestAuthorityBuilder;
use mgo_core::authority::AuthorityState;
use mgo_framework::DEFAULT_FRAMEWORK_PATH;
use mgo_graphql_rpc::config::ConnectionConfig;
use mgo_graphql_rpc::test_infra::cluster::ExecutorCluster;
use mgo_graphql_rpc::test_infra::cluster::{serve_executor, SnapshotLagConfig};
use mgo_json_rpc_api::QUERY_MAX_RESULT_LIMIT;
use mgo_json_rpc_types::{DevInspectResults, MgoExecutionStatus, MgoTransactionBlockEffectsAPI};
use mgo_protocol_config::{Chain, ProtocolConfig};
use mgo_storage::{
    key_value_store::TransactionKeyValueStore, key_value_store_metrics::KeyValueStoreMetrics,
};
use mgo_swarm_config::genesis_config::AccountConfig;
use mgo_types::base_types::{SequenceNumber, VersionNumber};
use mgo_types::crypto::get_authority_key_pair;
use mgo_types::digests::{ConsensusCommitDigest, TransactionDigest, TransactionEventsDigest};
...
use mgo_types::{
    base_types::{ObjectID, ObjectRef, MgoAddress, MGO_ADDRESS_LENGTH},
    crypto::{get_key_pair_from_rng, AccountKeyPair},
    event::Event,
    object::{self, Object},
    transaction::{Transaction, TransactionData, TransactionDataAPI, VerifiedTransaction},
    MOVE_STDLIB_ADDRESS, MGO_CLOCK_OBJECT_ID, MGO_FRAMEWORK_ADDRESS, MGO_SYSTEM_STATE_OBJECT_ID,
};
use mgo_types::{execution_status::ExecutionStatus, transaction::TransactionKind};
use mgo_types::{gas::GasCostSummary, object::GAS_VALUE_FOR_TESTING};
use mgo_types::{
...
#[async_trait]
impl<'a> MoveTestAdapter<'a> for MgoTestAdapter<'a> {
    type ExtraPublishArgs = MgoPublishArgs;
    type ExtraRunArgs = MgoRunArgs;
    type Subcommand = MgoSubcommand;
    type ExtraInitArgs = MgoInitArgs;
    type ExtraValueArgs = MgoExtraValueArgs;
    ...
    async fn init(
        ...
        assert!(
            ...
            "Must populate 'pre_compiled_deps' with Mgo framework"
        );
    ...
    async fn publish_modules(
        ...
        // we are assuming that all packages depend on Move Stdlib and Mgo Framework, so these
        // don't have to be provided explicitly as parameters
        dependencies.extend([MOVE_STDLIB_PACKAGE_ID, MGO_FRAMEWORK_PACKAGE_ID]);
        let data = |sender, gas| {
    ...
    async fn handle_subcommand(
        ...
        match command {
            MgoSubcommand::ForceObjectSnapshotCatchup(ForceObjectSnapshotCatchup {
                start_cp,
                end_cp,
            }) => {
            ...
            MgoSubcommand::RunGraphql(RunGraphqlCommand {
                show_usage,
                show_headers,
            ...
            MgoSubcommand::CreateCheckpoint(CreateCheckpointCommand { count }) => {
            ...
            MgoSubcommand::AdvanceEpoch(AdvanceEpochCommand {
                count,
                create_random_state,
            }) => {
                for _ in 0..count.unwrap_or(1) {
            ...
            MgoSubcommand::AdvanceClock(AdvanceClockCommand { duration_ns }) => {
                self.executor
                    .advance_clock(Duration::from_nanos(duration_ns))
                    .await?;
                Ok(None)
            }
            MgoSubcommand::SetRandomState(SetRandomStateCommand {
                randomness_round,
                random_bytes,
            ...
            MgoSubcommand::ViewObject(ViewObjectCommand { id: fake_id }) => {
            ...
            MgoSubcommand::TransferObject(TransferObjectCommand {
                id: fake_id,
                recipient,
                sender,
                gas_budget,
                ...
                let mut builder = ProgrammableTransactionBuilder::new();
                let obj_arg = MgoValue::Object(fake_id, None).into_argument(&mut builder, self)?;
                ...
                let transaction = self.sign_txn(sender, |sender, gas| {
                    ...
                    builder.command(mgo_types::transaction::Command::TransferObjects(
                        vec![obj_arg],
                        rec_arg,
                    ));
                    let pt = builder.finish();
            ...
            MgoSubcommand::ConsensusCommitPrologue(ConsensusCommitPrologueCommand {
                timestamp_ms,
            }) => {
            ...
            MgoSubcommand::ProgrammableTransaction(ProgrammableTransactionCommand {
                sender,
                gas_budget,
                gas_price,
                dev_inspect,
            ...
            MgoSubcommand::UpgradePackage(UpgradePackageCommand {
                package,
                upgrade_capability,
                dependencies,
                sender,
            ...
            MgoSubcommand::StagePackage(StagePackageCommand {
                syntax,
                dependencies,
            }) => {
                let syntax = syntax.unwrap_or_else(|| self.default_syntax());
            ...
            MgoSubcommand::SetAddress(SetAddressCommand { address, input }) => {
                ...
                let (value, package) = match input {
                    ...
                    MgoValue::MoveValue(v) => {
                        let bytes = v.simple_serialize().unwrap();
                        let value: AccountAddress = bcs::from_bytes(&bytes)?;
                        (value, None)
                    }
                    MgoValue::Digest(_) => bail!("digest is not supported as an input"),
                    MgoValue::ObjVec(_) => bail!("obj vec is not supported as an input"),
                    MgoValue::Receiving(_, _) => bail!("receiving is not supported as an input"),
                    MgoValue::ImmShared(_, _) => {
...
impl<'a> MgoTestAdapter<'a> {
    ...
    async fn upgrade_package(
        ...
        // Argument::Input(0)
        MgoValue::Object(upgrade_capability, None).into_argument(&mut builder, self)?;
    ...
    fn sign_txn(
        ...
        txn_data: impl FnOnce(/* sender */ MgoAddress, /* gas */ ObjectRef) -> TransactionData,
    ) -> Transaction {
    ...
    async fn dev_inspect(
        ...
        match effects.status() {
            ...
            MgoExecutionStatus::Failure { error } => Err(anyhow::anyhow!(self.stabilize_str(
    ...
    fn stabilize_str(&self, input: impl AsRef<str>) -> String {
        fn candidate_is_hex(s: &str) -> bool {
            const HEX_STR_LENGTH: usize = MGO_ADDRESS_LENGTH * 2;
            let n = s.len();
...
impl<'a> GetModule for &'a MgoTestAdapter<'_> {
...
static NAMED_ADDRESSES: Lazy<BTreeMap<String, NumericalAddress>> = Lazy::new(|| {
    ...
    // TODO fix Mgo framework constants
    map.insert(
        "mgo".to_string(),
        NumericalAddress::new(
            MGO_FRAMEWORK_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
        "mgo_system".to_string(),
        NumericalAddress::new(
            MGO_SYSTEM_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map
...
pub static PRE_COMPILED: Lazy<FullyCompiledProgram> = Lazy::new(|| {
    ...
    let mgo_files: &Path = Path::new(DEFAULT_FRAMEWORK_PATH);
    let mgo_system_sources = {
        let mut buf = mgo_files.to_path_buf();
        buf.extend(["packages", "mgo-system", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let mgo_sources = {
        let mut buf = mgo_files.to_path_buf();
        buf.extend(["packages", "mgo-framework", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let mgo_deps = {
        let mut buf = mgo_files.to_path_buf();
    ...
    match fully_compiled_res {
        Err((files, diags)) => {
            eprintln!("!!!Mgo framework failed to compile!!!");
...
async fn init_val_fullnode_executor(
    ...
    // For each named Mgo account without an address value, create an account with an address
    // and a gas object
...
async fn init_sim_executor(
    ...
    // For each named Mgo account without an address value, create a key pair
    for n in account_names {
    ...
    if custom_validator_account {
        ...
        let (a, b): (MgoAddress, Ed25519KeyPair) = get_key_pair_from_rng(&mut rng);
    ...
    let db_name = format!("mgo_graphql_test_{}", hash);
...
impl ObjectStore for MgoTestAdapter<'_> {
    fn get_object(
        &self,
        object_id: &ObjectID,
    ) -> mgo_types::storage::error::Result<Option<Object>> {
...
impl ReadStore for MgoTestAdapter<'_> {
    fn get_committee(
        &self,
        epoch: mgo_types::committee::EpochId,
    ...
    fn get_lowest_available_checkpoint(
        &self,
    ) -> mgo_types::storage::error::Result<CheckpointSequenceNumber> {
        self.executor.get_lowest_available_checkpoint()
    }

    fn get_checkpoint_by_digest(
        &self,
        digest: &mgo_types::messages_checkpoint::CheckpointDigest,
    ...
    ) -> mgo_types::storage::error::Result<Option<CheckpointContents>> {
    ...
    ) -> mgo_types::storage::error::Result<Option<VerifiedTransaction>> {
