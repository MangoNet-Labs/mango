...
use mgo_types::{
    base_types::{ObjectID, SequenceNumber, MgoAddress, VersionNumber},
    committee::{Committee, EpochId},
    crypto::AccountKeyPair,
    digests::{ObjectDigest, TransactionDigest, TransactionEventsDigest},
    effects::{TransactionEffects, TransactionEffectsAPI, TransactionEvents},
    error::{MgoError, UserInputError},
    messages_checkpoint::{
...
pub struct PersistedStoreInner {
    // Checkpoint data
    checkpoints: DBMap<CheckpointSequenceNumber, mgo_types::messages_checkpoint::TrustedCheckpoint>,
    ...
    // Transaction data
    transactions: DBMap<TransactionDigest, mgo_types::transaction::TrustedTransaction>,
...
impl SimulatorStore for PersistedStore {
    ...
    fn get_system_state(&self) -> mgo_types::mgo_system_state::MgoSystemState {
        mgo_types::mgo_system_state::get_mgo_system_state(self).expect("system state must exist")
    }

    fn get_clock(&self) -> mgo_types::clock::Clock {
        SimulatorStore::get_object(self, &mgo_types::MGO_CLOCK_OBJECT_ID)
            .expect("clock should exist")
            .to_rust()
            .expect("clock object should deserialize")
    }

    fn owned_objects(&self, owner: MgoAddress) -> Box<dyn Iterator<Item = Object> + '_> {
    ...
    fn backing_store(&self) -> &dyn mgo_types::storage::BackingStore {
        self
    }
}
...
impl BackingPackageStore for PersistedStore {
    fn get_package_object(
        &self,
        package_id: &ObjectID,
    ) -> mgo_types::error::MgoResult<Option<PackageObject>> {
...
impl ChildObjectResolver for PersistedStore {
    ...
    ) -> mgo_types::error::MgoResult<Option<Object>> {
...
impl ParentSync for PersistedStore {
    fn get_latest_parent_entry_ref_deprecated(
        &self,
        _object_id: ObjectID,
    ) -> mgo_types::error::MgoResult<Option<mgo_types::base_types::ObjectRef>> {
