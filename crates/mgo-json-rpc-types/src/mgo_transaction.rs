...
use crate::mgo_transaction::GenericSignature::Signature;
use crate::{Filter, Page, MgoEvent, MgoObjectRef};
...
use mango_metrics::monitored_scope;
use schemars::JsonSchema;
use serde::{Deserialize, Serialize};
use serde_with::serde_as;
use std::fmt::{self, Display, Formatter, Write};
use mgo_json::{primitive_type, MgoJsonValue};
use mgo_types::authenticator_state::ActiveJwk;
use mgo_types::base_types::{
    EpochId, ObjectID, ObjectRef, SequenceNumber, MgoAddress, TransactionDigest,
};
use mgo_types::crypto::MgoSignature;
use mgo_types::digests::{ConsensusCommitDigest, ObjectDigest, TransactionEventsDigest};
use mgo_types::effects::{TransactionEffects, TransactionEffectsAPI, TransactionEvents};
use mgo_types::error::{ExecutionError, MgoError, MgoResult};
use mgo_types::execution_status::ExecutionStatus;
...
use mgo_types::parse_mgo_type_tag;
use mgo_types::quorum_driver_types::ExecuteTransactionRequestType;
use mgo_types::signature::GenericSignature;
use mgo_types::storage::{DeleteKind, WriteKind};
use mgo_types::mgo_serde::Readable;
use mgo_types::mgo_serde::{
    BigInt, SequenceNumber as AsSequenceNumber, MgoTypeTag as AsMgoTypeTag,
};
...
use mgo_types::type_resolver::LayoutResolver;
...
// similar to EpochId of mgo-types but BigInt
pub type MgoEpochId = BigInt<u64>;
...
pub struct MgoTransactionBlockResponseQuery {
    /// If None, no filter will be applied
    pub filter: Option<TransactionFilter>,
    /// config which fields to include in the response, by default only digest is included
    pub options: Option<MgoTransactionBlockResponseOptions>,
}

impl MgoTransactionBlockResponseQuery {
    pub fn new(
...
pub type TransactionBlocksPage = Page<MgoTransactionBlockResponse, TransactionDigest>;
...
pub struct MgoTransactionBlockResponse {
    ...
    pub transaction: Option<MgoTransactionBlock>,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub effects: Option<MgoTransactionBlockEffects>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub events: Option<MgoTransactionBlockEvents>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl MgoTransactionBlockResponse {
...
impl PartialEq for MgoTransactionBlockResponse {
...
impl Display for MgoTransactionBlockResponse {
...
impl MgoTransactionBlockKind {
    fn try_from(tx: TransactionKind, module_cache: &impl GetModule) -> Result<Self, anyhow::Error> {
        Ok(match tx {
            ...
            TransactionKind::Genesis(g) => Self::Genesis(MgoGenesisTransaction {
                objects: g.objects.iter().map(GenesisObject::id).collect(),
            }),
            TransactionKind::ConsensusCommitPrologue(p) => {
                Self::ConsensusCommitPrologue(MgoConsensusCommitPrologue {
                    epoch: p.epoch,
                    round: p.round,
            ...
            TransactionKind::ConsensusCommitPrologueV2(p) => {
                Self::ConsensusCommitPrologueV2(MgoConsensusCommitPrologueV2 {
                    epoch: p.epoch,
                    round: p.round,
                    commit_timestamp_ms: p.commit_timestamp_ms,
            ...
            TransactionKind::ProgrammableTransaction(p) => Self::ProgrammableTransaction(
                MgoProgrammableTransactionBlock::try_from(p, module_cache)?,
            ),
            TransactionKind::AuthenticatorStateUpdate(update) => {
                Self::AuthenticatorStateUpdate(MgoAuthenticatorStateUpdate {
                    epoch: update.epoch,
                    round: update.round,
                    new_active_jwks: update
                        .new_active_jwks
                        .into_iter()
                        .map(MgoActiveJwk::from)
                        .collect(),
                })
            }
            TransactionKind::RandomnessStateUpdate(update) => {
                Self::RandomnessStateUpdate(MgoRandomnessStateUpdate {
                    epoch: update.epoch,
            ...
            TransactionKind::EndOfEpochTransaction(end_of_epoch_tx) => {
                Self::EndOfEpochTransaction(MgoEndOfEpochTransaction {
                    transactions: end_of_epoch_tx
                        .into_iter()
                        .map(|tx| match tx {
                            EndOfEpochTransactionKind::ChangeEpoch(e) => {
                                MgoEndOfEpochTransactionKind::ChangeEpoch(e.into())
                            }
                            EndOfEpochTransactionKind::AuthenticatorStateCreate => {
                                MgoEndOfEpochTransactionKind::AuthenticatorStateCreate
                            }
                            EndOfEpochTransactionKind::AuthenticatorStateExpire(expire) => {
                                MgoEndOfEpochTransactionKind::AuthenticatorStateExpire(
                                    MgoAuthenticatorStateExpire {
                                        min_epoch: expire.min_epoch,
                                    },
                                )
                            }
                            EndOfEpochTransactionKind::RandomnessStateCreate => {
                                MgoEndOfEpochTransactionKind::RandomnessStateCreate
                            }
                            EndOfEpochTransactionKind::DenyListStateCreate => {
                                MgoEndOfEpochTransactionKind::CoinDenyListStateCreate
                            }
                        })
                        .collect(),
                })
...
impl From<ChangeEpoch> for MgoChangeEpoch {
...
#[enum_dispatch(MgoTransactionBlockEffectsAPI)]
#[serde(
    rename = "TransactionBlockEffects",
    rename_all = "camelCase",
    tag = "messageVersion"
)]
pub enum MgoTransactionBlockEffects {
    V1(MgoTransactionBlockEffectsV1),
}

#[enum_dispatch]
pub trait MgoTransactionBlockEffectsAPI {
    fn status(&self) -> &MgoExecutionStatus;
    fn into_status(self) -> MgoExecutionStatus;
    fn shared_objects(&self) -> &[MgoObjectRef];
    fn created(&self) -> &[OwnedObjectRef];
    fn mutated(&self) -> &[OwnedObjectRef];
    fn unwrapped(&self) -> &[OwnedObjectRef];
    fn deleted(&self) -> &[MgoObjectRef];
    fn unwrapped_then_deleted(&self) -> &[MgoObjectRef];
    fn wrapped(&self) -> &[MgoObjectRef];
    ...
    fn all_deleted_objects(&self) -> Vec<(&MgoObjectRef, DeleteKind)>;
}
...
pub struct MgoTransactionBlockEffectsModifiedAtVersions {
    object_id: ObjectID,
...
pub struct MgoTransactionBlockEffectsV1 {
    /// The status of the execution
    pub status: MgoExecutionStatus,
    ...
    pub modified_at_versions: Vec<MgoTransactionBlockEffectsModifiedAtVersions>,
    /// The object references of the shared objects used in this transaction. Empty if no shared objects were used.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub shared_objects: Vec<MgoObjectRef>,
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub deleted: Vec<MgoObjectRef>,
    /// Object refs of objects previously wrapped in other objects but now deleted.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub unwrapped_then_deleted: Vec<MgoObjectRef>,
    /// Object refs of objects now wrapped in other objects.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub wrapped: Vec<MgoObjectRef>,
...
impl MgoTransactionBlockEffectsAPI for MgoTransactionBlockEffectsV1 {
    fn status(&self) -> &MgoExecutionStatus {
        &self.status
    }
    fn into_status(self) -> MgoExecutionStatus {
...
impl MgoTransactionBlockEffects {
    #[cfg(any(feature = "test-utils", test))]
    pub fn new_for_testing(
        transaction_digest: TransactionDigest,
        status: MgoExecutionStatus,
    ) -> Self {
        Self::V1(MgoTransactionBlockEffectsV1 {
            transaction_digest,
            status,
            gas_object: OwnedObjectRef {
                owner: Owner::AddressOwner(MgoAddress::random_for_testing_only()),
                reference: mgo_types::base_types::random_object_ref().into(),
            },
            executed_epoch: 0,
...
impl TryFrom<TransactionEffects> for MgoTransactionBlockEffects {
    type Error = MgoError;

    fn try_from(effect: TransactionEffects) -> Result<Self, Self::Error> {
        Ok(MgoTransactionBlockEffects::V1(
            MgoTransactionBlockEffectsV1 {
                ...
                modified_at_versions: effect
                    ...
                    .map(|(object_id, sequence_number)| {
                        MgoTransactionBlockEffectsModifiedAtVersions {
                            object_id,
                            sequence_number,
                        }
                    })
                ...
                shared_objects: to_mgo_object_ref(
                    effect
                ...
                deleted: to_mgo_object_ref(effect.deleted().to_vec()),
                unwrapped_then_deleted: to_mgo_object_ref(effect.unwrapped_then_deleted().to_vec()),
                wrapped: to_mgo_object_ref(effect.wrapped().to_vec()),
                gas_object: OwnedObjectRef {
...
fn objref_string(obj: &MgoObjectRef) -> String {
    format!(
        " ┌──\n │ ID: {} \n │ Version: {} \n │ Digest: {}\n └──",
        obj.object_id,
...
impl Display for MgoTransactionBlockEffects {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut builder = TableBuilder::default();
...
pub struct DryRunTransactionBlockResponse {
    ...
    pub input: MgoTransactionBlockData,
}

#[derive(Eq, PartialEq, Clone, Debug, Default, Serialize, Deserialize, JsonSchema)]
#[serde(rename = "TransactionBlockEvents", transparent)]
pub struct MgoTransactionBlockEvents {
    pub data: Vec<MgoEvent>,
}

impl MgoTransactionBlockEvents {
    pub fn try_from(
        ...
        Ok(Self {
            data: events
                ...
                .map(|(seq, event)| {
                    ...
                    MgoEvent::try_from(event, tx_digest, seq as u64, timestamp_ms, layout)
                })
                .collect::<Result<_, _>>()?,
        })
    }
...
impl Display for MgoTransactionBlockEvents {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
...
pub struct DevInspectArgs {
    ...
    pub gas_sponsor: Option<MgoAddress>,
...
pub struct DevInspectResults {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub results: Option<Vec<MgoExecutionResult>>,
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename = "MgoExecutionResult", rename_all = "camelCase")]
pub struct MgoExecutionResult {
    /// The value of any arguments that were mutably borrowed.
    /// Non-mut borrowed values are not included
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub mutable_reference_outputs: Vec<(/* argument */ MgoArgument, Vec<u8>, MgoTypeTag)>,
    /// The return values from the transaction
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub return_values: Vec<(Vec<u8>, MgoTypeTag)>,
}
...
impl DevInspectResults {
    pub fn new(
                    ...
                    srvs.into_iter()
                        .map(|srv| {
                            ...
                            let mutable_reference_outputs = mutable_reference_outputs
                                .into_iter()
                                .map(|(a, bytes, tag)| (a.into(), bytes, MgoTypeTag::from(tag)))
                                .collect();
                            let return_values = return_values
                                .into_iter()
                                .map(|(bytes, tag)| (bytes, MgoTypeTag::from(tag)))
                                .collect();
                            MgoExecutionResult {
        ...
        Ok(Self {
            ...
            events: MgoTransactionBlockEvents::try_from(events, tx_digest, None, resolver)?,
            results,
            error,
            raw_txn_data,
            raw_effects,
...
#[derive(Eq, PartialEq, Clone, Debug, Serialize, Deserialize, JsonSchema)]
pub enum MgoTransactionBlockBuilderMode {
    /// Regular Mgo Transactions that are committed on chain
    Commit,
...
impl MgoExecutionStatus {
    pub fn is_ok(&self) -> bool {
        matches!(self, MgoExecutionStatus::Success { .. })
    }
    pub fn is_err(&self) -> bool {
        matches!(self, MgoExecutionStatus::Failure { .. })
    }
}

impl From<ExecutionStatus> for MgoExecutionStatus {
...
fn to_mgo_object_ref(refs: Vec<ObjectRef>) -> Vec<MgoObjectRef> {
    refs.into_iter().map(MgoObjectRef::from).collect()
}
...
pub struct MgoGasData {
    pub payment: Vec<MgoObjectRef>,
...
#[derive(Debug, Deserialize, Serialize, JsonSchema, Clone, PartialEq, Eq)]
#[enum_dispatch(MgoTransactionBlockDataAPI)]
#[serde(
    rename = "TransactionBlockData",
    rename_all = "camelCase",
    tag = "messageVersion"
)]
pub enum MgoTransactionBlockData {
    V1(MgoTransactionBlockDataV1),
}

#[enum_dispatch]
pub trait MgoTransactionBlockDataAPI {
    fn transaction(&self) -> &MgoTransactionBlockKind;
    fn sender(&self) -> &MgoAddress;
    fn gas_data(&self) -> &MgoGasData;
}

#[derive(Debug, Deserialize, Serialize, JsonSchema, Clone, PartialEq, Eq)]
#[serde(rename = "TransactionBlockDataV1", rename_all = "camelCase")]
pub struct MgoTransactionBlockDataV1 {
    pub transaction: MgoTransactionBlockKind,
    pub sender: MgoAddress,
    pub gas_data: MgoGasData,
}

impl MgoTransactionBlockDataAPI for MgoTransactionBlockDataV1 {
    fn transaction(&self) -> &MgoTransactionBlockKind {
        &self.transaction
    }
    fn sender(&self) -> &MgoAddress {
...
impl MgoTransactionBlockData {
    pub fn move_calls(&self) -> Vec<&MgoProgrammableMoveCall> {
        match self {
            Self::V1(data) => match &data.transaction {
                MgoTransactionBlockKind::ProgrammableTransaction(pt) => pt
                    .commands
                    .iter()
                    .filter_map(|command| match command {
                        MgoCommand::MoveCall(c) => Some(&**c),
                        _ => None,
                    })
                    .collect(),
                _ => vec![],
...
impl Display for MgoTransactionBlockData {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            Self::V1(data) => {
...
impl MgoTransactionBlockData {
    pub fn try_from(
        ...
        let gas_data = MgoGasData {
            payment: data
                .gas()
                .iter()
                .map(|obj_ref| MgoObjectRef::from(*obj_ref))
                .collect(),
            owner: data.gas_owner(),
            price: data.gas_price(),
            budget: data.gas_budget(),
        };
        let transaction = MgoTransactionBlockKind::try_from(data.into_kind(), module_cache)?;
        match message_version {
            1 => Ok(MgoTransactionBlockData::V1(MgoTransactionBlockDataV1 {
                transaction,
                sender,
                gas_data,
            })),
...
pub struct MgoTransactionBlock {
    pub data: MgoTransactionBlockData,
    pub tx_signatures: Vec<GenericSignature>,
}

impl MgoTransactionBlock {
    pub fn try_from(
        data: SenderSignedData,
        module_cache: &impl GetModule,
    ) -> Result<Self, anyhow::Error> {
        Ok(Self {
            data: MgoTransactionBlockData::try_from(
...
impl Display for MgoTransactionBlock {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut builder = TableBuilder::default();
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MgoGenesisTransaction {
    pub objects: Vec<ObjectID>,
}

#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MgoConsensusCommitPrologue {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MgoConsensusCommitPrologueV2 {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MgoAuthenticatorStateUpdate {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
    ...
    pub new_active_jwks: Vec<MgoActiveJwk>,
}

#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MgoRandomnessStateUpdate {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MgoEndOfEpochTransaction {
    pub transactions: Vec<MgoEndOfEpochTransactionKind>,
}

#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub enum MgoEndOfEpochTransactionKind {
    ChangeEpoch(MgoChangeEpoch),
    AuthenticatorStateCreate,
    AuthenticatorStateExpire(MgoAuthenticatorStateExpire),
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MgoAuthenticatorStateExpire {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub min_epoch: u64,
}
...
impl From<ActiveJwk> for MgoActiveJwk {
...
/// commands
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MgoProgrammableTransactionBlock {
    /// Input objects or primitive values
    pub inputs: Vec<MgoCallArg>,
    ...
    pub commands: Vec<MgoCommand>,
}

impl Display for MgoProgrammableTransactionBlock {
...
impl MgoProgrammableTransactionBlock {
    fn try_from(
        ...
        Ok(MgoProgrammableTransactionBlock {
            inputs: inputs
                .into_iter()
                .zip(input_types)
                .map(|(arg, layout)| MgoCallArg::try_from(arg, layout.as_ref()))
                .collect::<Result<_, _>>()?,
            commands: commands.into_iter().map(MgoCommand::from).collect(),
        })
    }
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(rename = "MgoTransaction")]
pub enum MgoCommand {
    /// A call to either an entry or a public Move function
    MoveCall(Box<MgoProgrammableMoveCall>),
    /// `(Vec<forall T:key+store. T>, address)`
    /// It sends n-objects to the specified address. These objects must have store
    /// (public transfer) and either the previous owner must be an address or the object must
    /// be newly created.
    TransferObjects(Vec<MgoArgument>, MgoArgument),
    /// `(&mut Coin<T>, Vec<u64>)` -> `Vec<Coin<T>>`
    /// It splits off some amounts into a new coins with those amounts
    SplitCoins(MgoArgument, Vec<MgoArgument>),
    /// `(&mut Coin<T>, Vec<Coin<T>>)`
    /// It merges n-coins into the first coin
    MergeCoins(MgoArgument, Vec<MgoArgument>),
    /// Publishes a Move package. It takes the package bytes and a list of the package's transitive
    /// dependencies to link against on-chain.
    Publish(Vec<ObjectID>),
    /// Upgrades a Move package
    Upgrade(Vec<ObjectID>, ObjectID, MgoArgument),
    /// `forall T: Vec<T> -> vector<T>`
    /// Given n-values of the same type, it constructs a vector. For non objects or an empty vector,
    /// the type tag must be specified.
    MakeMoveVec(Option<String>, Vec<MgoArgument>),
}

impl Display for MgoCommand {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MoveCall(p) => {
                write!(f, "MoveCall({p})")
...
impl From<Command> for MgoCommand {
    fn from(value: Command) -> Self {
        match value {
            Command::MoveCall(m) => MgoCommand::MoveCall(Box::new((*m).into())),
            Command::TransferObjects(args, arg) => MgoCommand::TransferObjects(
                args.into_iter().map(MgoArgument::from).collect(),
                arg.into(),
            ),
            Command::SplitCoins(arg, args) => MgoCommand::SplitCoins(
                arg.into(),
                args.into_iter().map(MgoArgument::from).collect(),
            ),
            Command::MergeCoins(arg, args) => MgoCommand::MergeCoins(
                arg.into(),
                args.into_iter().map(MgoArgument::from).collect(),
            ),
            Command::Publish(_modules, dep_ids) => MgoCommand::Publish(dep_ids),
            Command::MakeMoveVec(tag_opt, args) => MgoCommand::MakeMoveVec(
                tag_opt.map(|tag| tag.to_string()),
                args.into_iter().map(MgoArgument::from).collect(),
            ),
            Command::Upgrade(_modules, dep_ids, current_package_id, ticket) => {
                MgoCommand::Upgrade(dep_ids, current_package_id, MgoArgument::from(ticket))
            }
        }
    }
}
...
impl From<Argument> for MgoArgument {
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct MgoProgrammableMoveCall {
...
impl Display for MgoProgrammableMoveCall {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let Self {
            package,
            module,
...
impl From<ProgrammableMoveCall> for MgoProgrammableMoveCall {
    fn from(value: ProgrammableMoveCall) -> Self {
        ...
        Self {
            ...
            arguments: arguments.into_iter().map(MgoArgument::from).collect(),
        }
    }
}
...
impl From<InputObjectKind> for MgoInputObjectKind {
...
impl TryInto<TypeTag> for MgoTypeTag {
    type Error = anyhow::Error;
    fn try_into(self) -> Result<TypeTag, Self::Error> {
        parse_mgo_type_tag(&self.0)
    }
}

impl From<TypeTag> for MgoTypeTag {
...
pub struct MoveCallParams {
    ...
    pub function: String,
    #[serde(default)]
    pub type_arguments: Vec<MgoTypeTag>,
    pub arguments: Vec<MgoJsonValue>,
}
...
pub struct TransactionBlockBytes {
    ...
    pub gas: Vec<MgoObjectRef>,
    /// objects to be used in this transaction
    pub input_objects: Vec<MgoInputObjectKind>,
}
...
impl MgoCallArg {
    pub fn try_from(
        ...
        Ok(match value {
            CallArg::Pure(p) => MgoCallArg::Pure(MgoPureValue {
                value_type: layout.map(|l| l.into()),
                value: MgoJsonValue::from_bcs_bytes(layout, &p)?,
            }),
            CallArg::Object(ObjectArg::ImmOrOwnedObject((id, version, digest))) => {
                MgoCallArg::Object(MgoObjectArg::ImmOrOwnedObject {
                    object_id: id,
                    version,
                    digest,
                })
            ...
            CallArg::Object(ObjectArg::SharedObject {
                ...
                initial_shared_version,
                mutable,
            }) => MgoCallArg::Object(MgoObjectArg::SharedObject {
                object_id: id,
                initial_shared_version,
                mutable,
            }),
            CallArg::Object(ObjectArg::Receiving((object_id, version, digest))) => {
                MgoCallArg::Object(MgoObjectArg::Receiving {
                    object_id,
                    version,
                    digest,
                })
    ...
    pub fn pure(&self) -> Option<&MgoJsonValue> {
        match self {
            MgoCallArg::Pure(v) => Some(&v.value),
            _ => None,
        }
    }
    ...
    pub fn object(&self) -> Option<&ObjectID> {
        match self {
            ...
            | MgoCallArg::Object(MgoObjectArg::Receiving { object_id, .. }) => Some(object_id),
            _ => None,
        }
    }
}
...
#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct MgoPureValue {
    #[schemars(with = "Option<String>")]
    #[serde_as(as = "Option<AsMgoTypeTag>")]
    value_type: Option<TypeTag>,
    value: MgoJsonValue,
}

impl MgoPureValue {
    pub fn value(&self) -> MgoJsonValue {
        self.value.clone()
    }
...
pub struct MgoLoadedChildObject {
    object_id: ObjectID,
    #[schemars(with = "AsSequenceNumber")]
    #[serde_as(as = "AsSequenceNumber")]
    sequence_number: SequenceNumber,
...
pub struct MgoLoadedChildObjectsResponse {
    pub loaded_child_objects: Vec<MgoLoadedChildObject>,
}
...
impl From<EffectsWithInput> for MgoTransactionBlockEffects {
...
pub enum TransactionFilter {
    ...
    FromAddress(MgoAddress),
    /// Query by recipient address.
    ToAddress(MgoAddress),
    /// Query by sender and recipient address.
    FromAndToAddress { from: MgoAddress, to: MgoAddress },
    /// Query txs that have a given address as sender or recipient.
    FromOrToAddress { addr: MgoAddress },
