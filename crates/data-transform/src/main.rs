...
use mgo_indexer::store::module_resolver::IndexerModuleResolver;
    ...
    for target_id in start_id.. {
        ...
        match event {
            Ok(Some(event)) => {
                ...
                let type_ = parse_mgo_struct_tag(&event.event_type).expect("cannot load StructTag");
                ...
                match layout {
                    Ok(l) => {
                        ...
                        match move_object {
                            Ok(m) => {
                                let parsed_json = MgoMoveStruct::from(m).to_json_value();
                                let final_result =
