...
use mgo_json_rpc_types::{
    MgoObjectDataOptions, MgoObjectResponse, MgoTransactionBlockEffectsAPI,
...
pub(crate) async fn check_transactions(
    ...
    cross_validate: bool,
    verify_objects: bool,
) -> Vec<Vec<MgoTransactionBlockResponse>> {
    let transactions: Vec<Vec<MgoTransactionBlockResponse>> =
        join_all(clients.iter().map(|client| async move {
            client
                .read_api()
                .multi_get_transactions_with_options(
                    digests.to_vec(),
                    MgoTransactionBlockResponseOptions::full_content(), // todo(Will) support options for this
                )
                .await
        }))
        .await
...
pub(crate) fn get_all_object_ids(response: &MgoTransactionBlockResponse) -> Vec<ObjectID> {
...
pub(crate) async fn multi_get_object(
    clients: &[MgoClient],
    object_ids: &[ObjectID],
) -> Vec<Vec<MgoObjectResponse>> {
    let objects: Vec<Vec<MgoObjectResponse>> = join_all(clients.iter().map(|client| async move {
        ...
        client
            ...
            .multi_get_object_with_options(
                object_ids.to_vec(),
                MgoObjectDataOptions::full_content(), // todo(Will) support options for this
            )
            .await
    }))
    .await
