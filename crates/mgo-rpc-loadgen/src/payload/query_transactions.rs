...
use mgo_json_rpc_types::{
    Page, MgoTransactionBlockResponse, MgoTransactionBlockResponseOptions,
...
impl<'a> ProcessPayload<'a, &'a QueryTransactionBlocks> for RpcCommandProcessor {
    async fn process(
        ...
        let queries: Vec<MgoTransactionBlockResponseQuery> = filters
            .into_iter()
            .map(|filter| MgoTransactionBlockResponseQuery {
                filter,
                options: Some(MgoTransactionBlockResponseOptions::full_content()),
            })
            .collect();
...
) -> Result<Page<MgoTransactionBlockResponse, TransactionDigest>> {
