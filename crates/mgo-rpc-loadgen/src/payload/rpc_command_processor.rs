...
use mgo_json_rpc_types::{
    MgoExecutionStatus, MgoObjectDataOptions, MgoTransactionBlockDataAPI,
...
use mgo_types::crypto::{get_key_pair, AccountKeyPair, EncodeDecodeBase64, Signature, MgoKeyPair};
...
pub struct RpcCommandProcessor {
    clients: Arc<RwLock<Vec<MgoClient>>>,
    ...
    addresses: Arc<DashSet<MgoAddress>>,
    data_dir: String,
}
...
impl RpcCommandProcessor {
    ...
    async fn process_command_data(
        ...
        match command {
            ...
            CommandData::PayMgo(ref v) => self.process(v, signer_info).await,
    ...
    pub(crate) async fn get_clients(&self) -> Result<Vec<MgoClient>> {
    ...
    pub(crate) fn add_addresses_from_response(&self, responses: &[MgoTransactionBlockResponse]) {
    ...
    pub(crate) fn add_object_ids_from_response(&self, responses: &[MgoTransactionBlockResponse]) {
        for response in responses {
    ...
    pub(crate) fn dump_cache_to_file(&self) {
        ...
        let addresses: Vec<MgoAddress> = self.addresses.iter().map(|x| *x).collect();
        if !addresses.is_empty() {
            debug!("dumping addresses to file {:?}", addresses.len());
            write_data_to_file(
                &addresses,
                &format!("{}/{}", &self.data_dir, CacheType::MgoAddress),
            )
            .unwrap();
        }
...
impl fmt::Display for CacheType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CacheType::MgoAddress => write!(f, "MgoAddress"),
...
pub fn load_addresses_from_file(filepath: String) -> Vec<MgoAddress> {
    let path = format!("{}/{}", filepath, CacheType::MgoAddress);
    let addresses: Vec<MgoAddress> = read_data_from_file(&path).expect("Failed to read addresses");
    addresses
}
...
async fn prepare_new_signer_and_coins(
    ...
    let (gas_fee_for_split, gas_fee_for_pay_mgo) = (
        DEFAULT_LARGE_GAS_BUDGET * num_split_txns as u64,
        DEFAULT_GAS_BUDGET,
    );

    let primary_keypair = MgoKeyPair::decode_base64(&signer_info.encoded_keypair)
        .expect("Decoding keypair should not fail");
    let sender = MgoAddress::from(&primary_keypair.public());
    let (coin, balance) = get_coin_with_max_balance(client, sender).await;
    // The balance needs to cover `pay_amount` plus
    // 1. gas fee for pay_mgo from the primary address to the burner address
    ...
    let burner_keypair = MgoKeyPair::Ed25519(burner_keypair);
    ...
    let coins = get_mgo_coin_ids(client, burner_address).await;
...
async fn get_coin_with_max_balance(client: &MgoClient, address: MgoAddress) -> (ObjectID, u64) {
    let coins = get_mgo_coin_ids(client, address).await;
    assert!(!coins.is_empty());
...
async fn get_mgo_coin_ids(client: &MgoClient, address: MgoAddress) -> Vec<(ObjectID, u64)> {
    match client
        .coin_read_api()
        ...
        .await
    {
        Ok(page) => page
            ...
            .collect::<Vec<_>>(),
        Err(e) => {
            panic!("get_mgo_coin_ids error for address {address} {e}")
        }
    }
...
async fn pay_mgo(
    ...
    let tx = client
        ...
        .await
        .expect("Failed to construct pay mgo transaction");
    sign_and_execute(
        client,
        keypair,
        tx,
...
pub(crate) async fn sign_and_execute(
    ...
    let signature = Signature::new_secure(
        &IntentMessage::new(Intent::mgo_transaction(), &txn_data),
        keypair,
    );
    ...
    match &transaction_response.effects {
        Some(effects) => {
            if let MgoExecutionStatus::Failure { error } = effects.status() {
                panic!(
