...
#[derive(Parser)]
#[clap(
    name = "Mgo RPC Load Generator",
    version = "0.1",
    about = "A load test application for Mgo RPC"
)]
struct Opts {
    ...
    #[clap(long, default_value = "~/.mgo/mgo_config/logs")]
    logs_directory: String,

    #[clap(long, default_value = "~/.mgo/loadgen/data")]
...
fn get_keypair() -> Result<SignerInfo> {
    ...
    // Load keystore from ~/.mgo/mgo_config/mgo.keystore
    let keystore_path = get_mgo_config_directory().join("mgo.keystore");
    let keystore = Keystore::from(FileBasedKeystore::new(&keystore_path)?);
    let active_address = keystore.addresses().pop().unwrap();
    let keypair: &MgoKeyPair = keystore.get_key(&active_address)?;
    println!("using address {active_address} for signing");
    Ok(SignerInfo::new(keypair.encode_base64()))
}

fn get_mgo_config_directory() -> PathBuf {
    match dirs::home_dir() {
        Some(v) => v.join(".mgo").join("mgo_config"),
...
fn get_log_file_path(dir_path: String) -> String {
    ...
    let log_filename = format!("mgo-rpc-loadgen.{}.log", timestamp);
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (command, common, need_keystore) = match opts.command {
        ...
        ClapCommand::PayMgo { common } => (Command::new_pay_mgo(), common, true),
