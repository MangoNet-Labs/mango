...
use mgo_indexer::framework::Handler;
use mgo_json_rpc_types::MgoMoveStruct;
...
impl ObjectHandler {
    ...
    async fn process_object(
        ...
        let (struct_tag, mgo_move_struct) = if let Some(move_struct) = move_struct {
            match move_struct.into() {
                MgoMoveStruct::WithTypes { type_, fields } => {
                    (Some(type_), Some(MgoMoveStruct::WithFields(fields)))
                }
        ...
        let entry = ObjectEntry {
            ...
            object_json: mgo_move_struct.map(|x| x.to_json_value().to_string()),
        };
