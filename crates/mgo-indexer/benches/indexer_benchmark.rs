...
use mgo_indexer::metrics::IndexerMetrics;
use mgo_indexer::models::checkpoints::Checkpoint;
use mgo_indexer::models::transactions::Transaction;
use mgo_indexer::new_pg_connection_pool;
use mgo_indexer::store::{IndexerStore, PgIndexerStore, TemporaryCheckpointStore};
use mgo_indexer::utils::reset_database;
use mgo_json_rpc_types::CheckpointId;
...
use mgo_types::transaction::{TransactionData, TEST_ONLY_GAS_UNIT_FOR_TRANSFER};
...
fn create_transaction(sequence_number: i64) -> Transaction {
    let gas_price = 1000;
    let tx = TransactionData::new_pay_mgo(
    ...
    Transaction {
        ...
        sender: MgoAddress::random_for_testing_only().to_string(),
