...
pub mod pg_integration_test {
    ...
    use mgo_indexer::models::owners::OwnerType;
    use mgo_indexer::schema::objects;
    use mgo_indexer::store::{IndexerStore, PgIndexerStore};
    use mgo_indexer::test_utils::{start_test_indexer, MgoTransactionBlockResponseBuilder};
    use mgo_indexer::{get_pg_pool_connection, new_pg_connection_pool, IndexerConfig};
    use mgo_json_rpc_api::ExtendedApiClient;
    use mgo_json_rpc_api::IndexerApiClient;
    use mgo_json_rpc_api::{ReadApiClient, TransactionBuilderClient, WriteApiClient};
    use mgo_json_rpc_types::{
        CheckpointId, EventFilter, MgoMoveObject, MgoObjectData, MgoObjectDataFilter,
        MgoObjectDataOptions, MgoObjectResponse, MgoObjectResponseQuery, MgoParsedMoveObject,
        MgoTransactionBlockResponse, MgoTransactionBlockResponseOptions,
        MgoTransactionBlockResponseQuery, TransactionBlockBytes, TransactionFilter,
    };
    ...
    async fn sign_and_execute_transaction_block(
        ...
        let tx_response = indexer_rpc_client
            .execute_transaction_block(
                tx_bytes,
                signatures,
                Some(MgoTransactionBlockResponseOptions::full_content()),
    ...
    // TODO: we should use MgoClient for tests like this
    ...
    > {
        ...
        let gas_objects: Vec<ObjectID> = test_cluster
            ...
            .get_owned_objects(
                sender,
                Some(MgoObjectResponseQuery::new_with_filter(
                    MgoObjectDataFilter::gas_coin(),
                )),
                None,
                None,
            )
    ...
    async fn test_genesis_sync() {
        ...
        for tx_digest in checkpoint.transactions {
            ...
            let _fullnode_rpc_tx = test_cluster
                .rpc_client()
                .get_transaction_block(tx_digest, Some(MgoTransactionBlockResponseOptions::new()))
                .await
                .unwrap();
    ...
    async fn test_simple_transaction_e2e() -> Result<(), anyhow::Error> {
        ...
        let checkpoint_seq_query =
            MgoTransactionBlockResponseQuery::new_with_filter(TransactionFilter::Checkpoint(2u64));
        ...
        let from_query = MgoTransactionBlockResponseQuery::new_with_filter(
        ...
        let tx_kind_query = MgoTransactionBlockResponseQuery::new_with_filter(
        ...
        let to_query = MgoTransactionBlockResponseQuery::new_with_filter(
        ...
        let from_to_query = MgoTransactionBlockResponseQuery::new_with_filter(
        ...
        let mutation_query = MgoTransactionBlockResponseQuery::new_with_filter(
        ...
        let input_query = MgoTransactionBlockResponseQuery::new_with_filter(
        ...
        let move_call_query =
            MgoTransactionBlockResponseQuery::new_with_filter(TransactionFilter::MoveFunction {
                package: package_id,
    ...
    async fn test_get_object_with_options() -> Result<(), anyhow::Error> {
        ...
        let show_all_content = MgoObjectDataOptions {
            show_type: true,
            show_owner: true,
        ...
        let object_required_fields = MgoObjectData {
            type_: None,
            owner: None,
        ...
        let show_some_content = MgoObjectDataOptions::new();
        let futures = vec![
            indexer_rpc_client
                .get_object(source_object_id, Some(MgoObjectDataOptions::bcs_lossless())),
            indexer_rpc_client
                .get_object(source_object_id, Some(MgoObjectDataOptions::full_content())),
        ...
        let results: Vec<MgoObjectResponse> = join_all(futures)
            .await
            .into_iter()
            .collect::<Result<_, _>>()
            .unwrap();
        ...
        // deleted object - returns MgoObjectRef
        let gas_objects =
        ...
        match (&resp.data, &resp.error) {
            ...
            _ => {
                panic!(
                    "Expected MgoObjectResponse::Deleted, but got {:?}",
                    resp.error
                );
            }
        }
        ...
        if let Some(MgoObjectResponseError::NotExists { object_id }) = resp.error {
            assert_eq!(object_id, obj_id)
        } else {
            panic!(
                "Expected MgoObjectResponse::NotExists, but got {:?}",
                resp.error
            );
        }
    ...
    async fn test_module_cache() {
        ...
        let parsed_coin = MgoParsedMoveObject::try_from_layout(
    ...
    async fn test_query_objects_cross_check() -> Result<(), anyhow::Error> {
        ...
        let object_from_indexer = indexer_rpc_client
            .query_objects(
                MgoObjectResponseQuery::new_with_filter(MgoObjectDataFilter::AddressOwner(address)),
                None,
                None,
            )
            .await
    ...
    async fn test_query_objects() -> Result<(), anyhow::Error> {
        ...
        let all_coins = indexer_rpc_client
            .query_objects(
                MgoObjectResponseQuery::new_with_filter(MgoObjectDataFilter::StructType(
                    parse_struct_tag("0x2::coin::Coin<0x2::mgo::MGO>").unwrap(),
                )),
                None,
                None,
            )
    ...
    async fn pg_unnest_bulk_insert_update_test() {
        ...
        let bulk_data = (1..=10000)
            .map(|_| Object {
                ...
                owner_type: OwnerType::AddressOwner,
                owner_address: Some(MgoAddress::random_for_testing_only().to_string()),
                initial_shared_version: None,
                previous_transaction: TransactionDigest::random().to_string(),
                object_type: "0x2::coin::Coin<0x2::mgo::MGO>".to_string(),
                object_status: ObjectStatus::Created,
                has_public_transfer: false,
                storage_rebate: 0,
    ...
    async fn test_get_transaction_with_options() -> Result<(), anyhow::Error> {
        ...
        let full_transaction_response = indexer_rpc_client
            ...
            .await?;
        let mgo_transaction_response_options = vec![
            MgoTransactionBlockResponseOptions::new().with_input(),
            MgoTransactionBlockResponseOptions::new().with_raw_input(),
            MgoTransactionBlockResponseOptions::new().with_effects(),
            MgoTransactionBlockResponseOptions::new().with_events(),
            MgoTransactionBlockResponseOptions::new().with_balance_changes(),
            MgoTransactionBlockResponseOptions::new().with_object_changes(),
            MgoTransactionBlockResponseOptions::new()
                .with_input()
                .with_balance_changes()
                .with_object_changes(),
        ];
        let futures = mgo_transaction_response_options
            .into_iter()
            .map(|option| {
        ...
        let received_transaction_results: Vec<MgoTransactionBlockResponse> = join_all(futures)
            .await
            .into_iter()
            .collect::<Result<_, _>>()
            .unwrap();
        ...
        let expected_transaction_results = vec![
            MgoTransactionBlockResponseBuilder::new(&full_transaction_response)
                .with_input()
                .build(),
            MgoTransactionBlockResponseBuilder::new(&full_transaction_response)
                .with_raw_input()
    ...
    async fn test_get_checkpoint() -> Result<(), anyhow::Error> {
        ...
        let tx_response = indexer_rpc_client
            .get_transaction_block(
                tx_response.digest,
                Some(MgoTransactionBlockResponseOptions::new()),
            )
            .await?;
