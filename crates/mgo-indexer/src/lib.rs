...
use mgo_json_rpc::{JsonRpcServerBuilder, ServerHandle, ServerType};
use mgo_json_rpc_api::CLIENT_SDK_TYPE_HEADER;
...
#[clap(
    name = "Mgo indexer",
    about = "An off-fullnode service serving data from Mgo protocol",
...
impl Default for IndexerConfig {
    fn default() -> Self {
        Self {
            db_url: Some("postgres://postgres:postgres@localhost:5432/mgo_indexer".to_string()),
            db_user_name: None,
            db_password: None,
            db_host: None,
            db_port: None,
...
impl Indexer {
    pub async fn start<S: IndexerStore + Sync + Send + Clone + 'static>(
        ...
        info!(
            "Mgo indexer of version {:?} started...",
...
pub async fn new_rpc_client(http_url: &str) -> Result<MgoClient, IndexerError> {
