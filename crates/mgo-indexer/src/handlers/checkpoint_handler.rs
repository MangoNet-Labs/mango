...
use mango_metrics::{get_metrics, spawn_monitored_task};
use std::collections::HashMap;
use mgo_rest_api::{CheckpointData, CheckpointTransaction};
...
use mgo_types::messages_checkpoint::{CheckpointCommitment, CheckpointSequenceNumber};
use mgo_types::mgo_system_state::mgo_system_state_summary::MgoSystemStateSummary;
use mgo_types::mgo_system_state::{get_mgo_system_state, MgoSystemStateTrait};
    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    ...
    let (tx_indexing_sender, tx_indexing_receiver) = mango_metrics::metered_channel::channel(
    ...
    let (object_indexing_sender, object_indexing_receiver) =
        mango_metrics::metered_channel::channel(
            checkpoint_queue_size,
            &global_metrics
                .channels
    ...
    let (epoch_indexing_sender, epoch_indexing_receiver) = mango_metrics::metered_channel::channel(
        EPOCH_QUEUE_LIMIT,
        &global_metrics
            .channels
...
pub struct CheckpointProcessor<S> {
    state: S,
    metrics: IndexerMetrics,
    epoch_indexing_sender: mango_metrics::metered_channel::Sender<TemporaryEpochStore>,
    checkpoint_sender: mango_metrics::metered_channel::Sender<TemporaryCheckpointStore>,
}
...
struct CheckpointDataObjectStore<'a> {
    objects: Vec<&'a mgo_types::object::Object>,
}
...
impl<'a> mgo_types::storage::ObjectStore for CheckpointDataObjectStore<'a> {
    fn get_object(
        &self,
        object_id: &ObjectID,
    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    async fn index_epoch(
        ...
        {
            // very first epoch
            let system_state = get_mgo_system_state(&checkpoint_object_store)?;
            let system_state: MgoSystemStateSummary = system_state.into_mgo_system_state_summary();
            ...
            let epoch_event = transactions
                ...
                .flatten()
                .find(|ev| {
                    ev.type_.address == MGO_SYSTEM_ADDRESS
                        && ev.type_.module.as_ident_str() == ident_str!("mgo_system_state_inner")
    ...
    async fn index_checkpoint_and_epoch(
        ...
        } in transactions
        {
            ...
            db_events.extend(events.iter().flat_map(|events| &events.data).map(|event| {
                Event::from_mgo_event(event, transaction_digest, checkpoint_summary.timestamp_ms)
            }));
            ...
            // Move Calls
            if let mgo_types::transaction::TransactionKind::ProgrammableTransaction(pt) = tx.kind()
            {
                db_move_calls.extend(pt.commands.clone().into_iter().filter_map(move |command| {
                    match command {
                        mgo_types::transaction::Command::MoveCall(m) => Some(MoveCall {
                            id: None,
                            transaction_digest: transaction_digest.to_string(),
        ...
        Ok((
            TemporaryCheckpointStore {
                checkpoint: Checkpoint::from_mgo_checkpoint(
                    checkpoint_summary,
                    checkpoint_contents,
                    total_transactions,
...
pub async fn start_tx_checkpoint_commit_task<S>(
    ...
    metrics: IndexerMetrics,
    config: IndexerConfig,
    tx_indexing_receiver: mango_metrics::metered_channel::Receiver<TemporaryCheckpointStore>,
) where
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    ...
    let mut stream = mango_metrics::metered_channel::ReceiverStream::new(tx_indexing_receiver)
...
pub async fn start_epoch_commit_task<S>(
    state: S,
    metrics: IndexerMetrics,
    epoch_indexing_receiver: mango_metrics::metered_channel::Receiver<TemporaryEpochStore>,
) where
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    use futures::StreamExt;
    ...
    let mut stream = mango_metrics::metered_channel::ReceiverStream::new(epoch_indexing_receiver);
...
pub async fn start_object_checkpoint_commit_task<S>(
    ...
    metrics: IndexerMetrics,
    config: IndexerConfig,
    object_indexing_receiver: mango_metrics::metered_channel::Receiver<(
        mgo_types::messages_checkpoint::CheckpointSequenceNumber,
    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    ...
    let mut stream = mango_metrics::metered_channel::ReceiverStream::new(object_indexing_receiver)
        .ready_chunks(checkpoint_commit_batch_size);
...
pub struct ObjectsProcessor<S> {
    metrics: IndexerMetrics,
    object_indexing_sender: mango_metrics::metered_channel::Sender<(
        mgo_types::messages_checkpoint::CheckpointSequenceNumber,
    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    ...
    fn index_packages(checkpoint_data: &CheckpointData) -> Vec<Package> {
        ...
        checkpoint_data
            ...
            .flat_map(|tx| tx.output_objects.iter())
            .filter_map(|o| {
                if let mgo_types::object::Data::Package(p) = &o.data {
                    let sender = senders
    ...
    deleted
        ...
        .map(|(status, oref)| {
            DeletedObject::from(
                epoch,
                Some(checkpoint),
                &mgo_json_rpc_types::MgoObjectRef::from(oref.to_owned()),
