...
use mgo_rest_api::CheckpointData;
use mgo_rest_api::CheckpointTransaction;
...
use mgo_types::messages_checkpoint::{CertifiedCheckpointSummary, CheckpointContents};
...
pub struct CheckpointHandler<S> {
    state: S,
    metrics: IndexerMetrics,
    indexed_checkpoint_sender: mango_metrics::metered_channel::Sender<CheckpointDataToCommit>,
        ...
        S: IndexerStoreV2 + Clone + Sync + Send + 'static,
{
    async fn index_epoch(
        ...
        if *checkpoint_summary.sequence_number() == 0 {
            ...
            let system_state: MgoSystemStateSummary =
                get_mgo_system_state(&checkpoint_object_store)?.into_mgo_system_state_summary();
    ...
    async fn index_one_checkpoint(
        ...
        let (checkpoint, db_transactions, db_events, db_indices, db_displays) = {
            ...
            (
                IndexedCheckpoint::from_mgo_checkpoint(
                    &checkpoint_summary,
                    &checkpoint_contents,
