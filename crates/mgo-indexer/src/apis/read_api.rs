...
use mgo_json_rpc_api::{ReadApiClient, ReadApiServer};
...
impl<S: IndexerStore> ReadApi<S> {
    ...
    async fn get_transaction_block_internal(
        ...
        let tx = self
            ...
            .await?;
        let mgo_tx_resp = self
            .state
            .compose_mgo_transaction_block_response(tx, options.as_ref())
            .await?;
        let mgo_transaction_response = MgoTransactionBlockResponseWithOptions {
            response: mgo_tx_resp,
            options: options.unwrap_or_default(),
        }
        .into();
        Ok(mgo_transaction_response)
    }
    ...
    async fn multi_get_transaction_blocks_internal(
        ...
        options: Option<MgoTransactionBlockResponseOptions>,
    ) -> Result<Vec<MgoTransactionBlockResponse>, IndexerError> {
        ...
        let mgo_tx_resp_futures = ordered_tx_vec.into_iter().map(|tx| {
            self.state
                .compose_mgo_transaction_block_response(tx, options.as_ref())
        });
        let mgo_tx_resp_vec = join_all(mgo_tx_resp_futures)
            .await
            .into_iter()
    ...
    ) -> Result<MgoObjectResponse, IndexerError> {
...
impl<S> MgoRpcModule for ReadApi<S>
where
    S: IndexerStore + Sync + Send + 'static,
{
