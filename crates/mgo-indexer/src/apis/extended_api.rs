...
use mgo_json_rpc_types::{
    ...
    NetworkMetrics, Page, QueryObjectsPage, MgoObjectDataFilter, MgoObjectResponse,
...
impl<S: IndexerStore> ExtendedApi<S> {
    ...
    async fn query_objects_internal(
        ...
        let MgoObjectResponseQuery { filter, options } = query;
        let filter = filter.unwrap_or_else(|| MgoObjectDataFilter::MatchAll(vec![]));
        ...
        let mut data = objects_from_db
            .into_iter()
            .map(|obj_read| {
                MgoObjectResponse::try_from((obj_read, options.clone().unwrap_or_default()))
            })
...
impl<S> MgoRpcModule for ExtendedApi<S>
where
    S: IndexerStore + Sync + Send + 'static,
{
    ...
    fn rpc_doc_module() -> Module {
        mgo_json_rpc_api::ExtendedApiOpenRpc::module_doc()
    }
}
