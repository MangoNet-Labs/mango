...
use mgo_json_rpc::{governance_api::ValidatorExchangeRates, MgoRpcModule};
use mgo_json_rpc_api::GovernanceReadApiServer;
use mgo_json_rpc_types::{
    DelegatedStake, EpochInfo, StakeStatus, MgoCommittee, MgoObjectDataFilter, ValidatorApys,
};
use mgo_open_rpc::Module;
use mgo_types::{
    base_types::{MoveObjectType, ObjectID, MgoAddress},
    committee::EpochId,
    governance::StakedMgo,
    mgo_serde::BigInt,
    mgo_system_state::{mgo_system_state_summary::MgoSystemStateSummary, PoolTokenExchangeRate},
};
...
impl GovernanceReadApiV2 {
    ...
    async fn get_validators_apy(&self) -> Result<ValidatorApys, IndexerError> {
        ...
        let apys = mgo_json_rpc::governance_api::calculate_apys(
    ...
    async fn get_latest_mgo_system_state(&self) -> Result<MgoSystemStateSummary, IndexerError> {
        self.inner
            .spawn_blocking(|this| this.get_latest_mgo_system_state())
            .await
    }
    ...
    async fn get_stakes_by_ids(
        ...
        for stored_object in self.inner.multi_get_objects_in_blocking_task(ids).await? {
            let object = mgo_types::object::Object::try_from(stored_object)?;
            let stake_object = StakedMgo::try_from(&object)?;
    ...
    async fn get_staked_by_owner(
        ...
        for stored_object in self
            ...
            .get_owned_objects_in_blocking_task(
                owner,
                Some(MgoObjectDataFilter::StructType(
                    MoveObjectType::staked_mgo().into(),
                )),
                None,
    ...
    pub async fn get_delegated_stakes(
        ...
        let system_state_summary = self.get_latest_mgo_system_state().await?;
        let epoch = system_state_summary.epoch;
        ...
        for (pool_id, stakes) in pools {
            ...
            for stake in stakes {
                ...
                delegations.push(mgo_json_rpc_types::Stake {
...
async fn exchange_rates(
    ...
    system_state_summary: MgoSystemStateSummary,
    ...
    for df in state
        ...
        .await?
    {
        let pool_id: mgo_types::id::ID = bcs::from_bytes(&df.bcs_name).map_err(|e| {
            mgo_types::error::MgoError::ObjectDeserializationError {
                error: e.to_string(),
            }
        })?;
        ...
        let validator = state
            ...
            .spawn_blocking(move |this| {
                mgo_types::mgo_system_state::get_validator_from_table(
                    &this,
                    inactive_pools_id,
                    &pool_id,
                )
    ...
    for (address, pool_id, exchange_rates_id, exchange_rates_size, active) in tables {
            ...
            .await?
        {
            let dynamic_field = df
                ...
                .ok_or_else(|| mgo_types::error::MgoError::ObjectDeserializationError {
...
#[cached(
    type = "SizedCache<EpochId, BTreeMap<MgoAddress, f64>>",
    create = "{ SizedCache::with_size(1) }",
    convert = " {apys.epoch} "
)]
fn validators_apys_map(apys: ValidatorApys) -> BTreeMap<MgoAddress, f64> {
...
impl GovernanceReadApiServer for GovernanceReadApiV2 {
    async fn get_stakes_by_ids(
        ...
        self.get_stakes_by_ids(staked_mgo_ids)
            .await
            .map_err(Into::into)
    }
    ...
    async fn get_latest_mgo_system_state(&self) -> RpcResult<MgoSystemStateSummary> {
        self.get_latest_mgo_system_state().await.map_err(Into::into)
    }
...
impl MgoRpcModule for GovernanceReadApiV2 {
