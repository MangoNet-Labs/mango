...
use mgo_json_rpc_api::{cap_page_limit, IndexerApiClient, IndexerApiServer};
use mgo_json_rpc_types::{
    DynamicFieldPage, EventFilter, EventPage, ObjectsPage, Page, MgoObjectDataFilter,
    MgoObjectResponse, MgoObjectResponseQuery, MgoTransactionBlockResponseQuery,
...
impl<S: IndexerStore> IndexerApi<S> {
    ...
    async fn query_transaction_blocks_internal(
        ...
        let tx_resp_futures = tx_vec_from_db.into_iter().map(|tx| {
            self.state
                .compose_mgo_transaction_block_response(tx, query.options.as_ref())
        });
        let mgo_tx_resp_vec = join_all(tx_resp_futures)
            .await
            .into_iter()
            .collect::<Result<Vec<_>, _>>()?;

        Ok(Page {
            data: mgo_tx_resp_vec,
            next_cursor,
            has_next_page,
        })
    }
    ...
    async fn get_owned_objects_internal(
        ...
        let address = MgoObjectDataFilter::AddressOwner(address);
        // MUSTFIX(gegaowp): implement other filters beside address owner filter.
        let (filter, options) = match query {
            Some(MgoObjectResponseQuery {
                filter: Some(filter),
                options,
            }) => match filter {
                MgoObjectDataFilter::AddressOwner(_) => Ok((address, options)),
                _ => Err(anyhow!(
                    "Only address filter is supported on indexer for now."
                )),
            },
            Some(MgoObjectResponseQuery { filter: _, options }) => Ok((address, options)),
        ...
        let data: Vec<MgoObjectResponse> = objects
            .into_iter()
...
impl<S> MgoRpcModule for IndexerApi<S>
where
    S: IndexerStore + Sync + Send + 'static,
{
