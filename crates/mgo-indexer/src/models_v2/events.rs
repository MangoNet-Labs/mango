...
impl StoredEvent {
    pub fn try_into_mgo_event(
        self,
        module_cache: &impl GetModule,
    ) -> Result<MgoEvent, IndexerError> {
        ...
        // Note: MgoEvent only has one sender today, so we always use the first one.
        ...
        let sender = match sender {
            Some(s) => MgoAddress::from_bytes(s).map_err(|_e| {
                IndexerError::PersistentStorageDataCorruptionError(format!(
