...
use mgo_json_rpc_types::BalanceChange;
use mgo_json_rpc_types::ObjectChange;
use mgo_json_rpc_types::MgoTransactionBlock;
use mgo_json_rpc_types::MgoTransactionBlockEffects;
use mgo_json_rpc_types::MgoTransactionBlockEvents;
...
impl StoredTransaction {
    pub fn try_into_mgo_transaction_block_response(
        self,
        options: &MgoTransactionBlockResponseOptions,
        module: &impl GetModule,
    ) -> IndexerResult<MgoTransactionBlockResponse> {
        let tx_digest =
        ...
        let transaction = if options.show_input {
            ...
            let tx_block = MgoTransactionBlock::try_from(sender_signed_data, module)?;
            Some(tx_block)
        } else {
            None
        };
        ...
        let effects = if options.show_effects {
            let effects = self.try_into_mgo_transaction_effects()?;
            Some(effects)
        } else {
            None
        };
        ...
        let events = if options.show_events {
            ...
            let tx_events = MgoTransactionBlockEvents::try_from_using_module_resolver(
                tx_events,
                tx_digest,
                Some(timestamp),
                module,
    ...
    pub fn try_into_mgo_transaction_effects(&self) -> IndexerResult<MgoTransactionBlockEffects> {
        ...
        let effects = MgoTransactionBlockEffects::try_from(effects)?;
        Ok(effects)
    }
}
