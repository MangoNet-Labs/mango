...
use mgo_json_rpc_types::{
    ...
    NetworkMetrics, MgoEvent, MgoObjectDataFilter,
};
use mgo_json_rpc_types::{
    MgoTransactionBlock, MgoTransactionBlockEffects, MgoTransactionBlockEvents,
...
use mgo_types::digests::CheckpointDigest;
...
impl PgIndexerStore {
    ...
    pub fn get_mgo_types_object(
        &self,
        object_id: &ObjectID,
        version: &SequenceNumber,
    ) -> Result<mgo_types::object::Object, IndexerError> {
        ...
        match pg_object {
            Ok(pg_object) => {
                let object = mgo_types::object::Object::try_from(pg_object)?;
                Ok(object)
            }
            Err(e) => Err(e),
        }
    ...
    pub fn find_mgo_types_object_lt_or_eq_version(
        &self,
        id: &ObjectID,
        version: &SequenceNumber,
    ) -> Result<Option<mgo_types::object::Object>, IndexerError> {
        let pg_object = read_only_blocking!(&self.blocking_cp, |conn| {
            objects_history::dsl::objects_history
                .select((
                    objects_history::epoch,
    ...
    fn get_checkpoint(
        &self,
        id: CheckpointId,
    ) -> Result<mgo_json_rpc_types::Checkpoint, IndexerError> {
        read_only_blocking!(&self.blocking_cp, |conn| {
    ...
    fn get_events(
        ...
        let mut mgo_event_vec = events_vec
            .into_iter()
            .map(|event| event.try_into(&self.module_cache))
            .collect::<Result<Vec<MgoEvent>, _>>()?;
        // reset to original limit for checking and truncating
        page_limit -= 1;
        let has_next_page = mgo_event_vec.len() > page_limit;
        mgo_event_vec.truncate(page_limit);
        let next_cursor = mgo_event_vec.last().map(|e| e.id);
        Ok(EventPage {
            data: mgo_event_vec,
            next_cursor,
            has_next_page,
    ...
    fn compose_mgo_transaction_block_response(
        &self,
        tx: Transaction,
        options: Option<&MgoTransactionBlockResponseOptions>,
    ) -> Result<MgoTransactionBlockResponse, IndexerError> {
        ...
        let transaction = MgoTransactionBlock::try_from(sender_signed_data, &self.module_cache)?;
        let effects: MgoTransactionBlockEffects = serde_json::from_str(&tx.transaction_effects_content).map_err(|err| {
        IndexerError::InsertableParsingError(format!(
            "Failed converting transaction effect JSON {:?} to MgoTransactionBlockEffects with error: {:?}",
        ...
        if let Some(options) = options {
            ...
            if options.show_events {
                ...
                events = Some(MgoTransactionBlockEvents {
...
impl IndexerStore for PgIndexerStore {
    ...
    async fn compose_mgo_transaction_block_response(
        &self,
        tx: Transaction,
        options: Option<&MgoTransactionBlockResponseOptions>,
    ) -> Result<MgoTransactionBlockResponse, IndexerError> {
        ...
        self.spawn_blocking(move |this| {
            this.compose_mgo_transaction_block_response(tx, options.as_ref())
        })
        .await
    }
    ...
    async fn get_transaction_page_by_recipient_address(
        &self,
        sender_address: Option<MgoAddress>,
        recipient_address: MgoAddress,
        start_sequence: Option<i64>,
        limit: usize,
        is_descending: bool,
    ) -> Result<Vec<Transaction>, IndexerError> {
