...
use mgo_indexer::new_rpc_client;
use mgo_json_rpc_types::{
    CheckpointId, ObjectChange, MgoObjectDataOptions, MgoTransactionBlockResponseOptions,
};
...
pub async fn check_transactions(
    ...
    for digest in tx_vec {
        let fetch_options = MgoTransactionBlockResponseOptions::full_content();
        let fn_mgo_tx_response = fn_client
            .read_api()
            .get_transaction_with_options(digest, fetch_options.clone())
            .await?;
        let indexer_mgo_tx_response = indexer_client
            .read_api()
            .get_transaction_with_options(digest, fetch_options)
            .await?;
        if fn_mgo_tx_response != indexer_mgo_tx_response {
            error!("Checkpoint transactions mismatch found in {}", digest);
            warn!(
                "Transaction response mismatch with digest {:?}:\nFN:\n{:?}\nIndexer:\n{:?} ",
                digest, fn_mgo_tx_response, indexer_mgo_tx_response
            );
            continue;
        }
        ...
        if config.check_objects {
            let objects = fn_mgo_tx_response.object_changes.unwrap();
...
pub async fn check_objects(
    ...
    let options = MgoObjectDataOptions::full_content();
