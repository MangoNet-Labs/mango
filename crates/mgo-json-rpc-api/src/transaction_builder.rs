...
pub trait TransactionBuilder {
    ...
    /// Create an unsigned transaction to send MGO coin object to a Mgo address. The MGO object is also used as the gas object.
    #[method(name = "transferMgo")]
    ...
    /// Send MGO coins to a list of addresses, following a list of amounts.
    ...
    /// 2. accumulate all residual MGO from input coins left and deposit all MGO to the first
    ...
    /// Send all MGO coins to one recipient.
    /// This is for MGO coin only and does not require a separate gas coin object.
    ...
    async fn move_call(
        ...
        /// the arguments to be passed into the Move function, in [MgoJson](https://docs.mangonetwork.io/build/mgo-json) format
        arguments: Vec<MgoJsonValue>,
        /// gas object to be used in this transaction, node will pick one from the signer's possession if not provided
        gas: Option<ObjectID>,
        /// the gas budget, the transaction will fail if the gas cost exceed the budget
        gas_budget: BigInt<u64>,
        /// Whether this is a Normal transaction or a Dev Inspect Transaction. Default to be `MgoTransactionBlockBuilderMode::Commit` when it's None.
        execution_mode: Option<MgoTransactionBlockBuilderMode>,
    ) -> RpcResult<TransactionBlockBytes>;
    ...
    async fn batch_transaction(
        ...
        txn_builder_mode: Option<MgoTransactionBlockBuilderMode>,
    ) -> RpcResult<TransactionBlockBytes>;
