...
use mgo_core::authority::authority_per_epoch_store::AuthorityEpochTables;
use mgo_core::authority::authority_store_tables::AuthorityPerpetualTables;
use mgo_core::checkpoints::CheckpointStore;
use mgo_types::base_types::{EpochId, ObjectID, SequenceNumber};
use mgo_types::digests::{CheckpointContentsDigest, TransactionDigest};
...
/// Dry run with: cargo run --package mgo-tool -- db-tool --db-path /opt/mgo/db/authorities_db/live remove-transaction --digest xxxx
...
pub fn remove_transaction(path: &Path, opt: RemoveTransactionOptions) -> anyhow::Result<()> {
    ...
    let epoch = if let Some(epoch) = opt.epoch {
        ...
        get_mgo_system_state(&perpetual_db)?.epoch()
    };
...
pub fn reset_db_to_genesis(path: &Path) -> anyhow::Result<()> {
    ...
    // Download the snapshot for the epoch you want to restore to the local disk. You will find one snapshot per epoch in the S3 bucket. We need to place the snapshot in the dir where config is pointing to. If db-config in fullnode.yaml is /opt/mgo/db/authorities_db and we want to restore from epoch 10, we want to copy the snapshot to /opt/mgo/db/authorities_dblike this:
    // aws s3 cp s3://myBucket/dir /opt/mgo/db/authorities_db/ --recursive —exclude “*” —include “epoch_10*”
    // Mark downloaded snapshot as live: mv  /opt/mgo/db/authorities_db/epoch_10  /opt/mgo/db/authorities_db/live
    // Reset the downloaded db to execute from genesis with: cargo run --package mgo-tool -- db-tool --db-path /opt/mgo/db/authorities_db/live reset-db
    // Start the mgo full node: cargo run --release --bin mgo-node -- --config-path ~/db_checkpoints/fullnode.yaml
...
/// Run with: cargo run --package mgo-tool -- db-tool --db-path /opt/mgo/db/authorities_db/live rewind-checkpoint-execution --epoch 3 --checkpoint-sequence-number 300000
