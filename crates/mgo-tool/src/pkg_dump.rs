...
use mgo_indexer::{models_v2::packages::StoredPackage, schema_v2::packages};
use mgo_types::{base_types::MgoAddress, move_package::MovePackage};
use tracing::info;
...
pub(crate) async fn dump(db_url: String, output_dir: PathBuf) -> Result<()> {
    ...
    for (i, pkg) in pkgs.into_iter().enumerate() {
        ...
        let id = MgoAddress::from_bytes(&pkg.package_id).context("Parsing package ID")?;
...
fn dump_package(output_dir: &Path, id: MgoAddress, pkg: &[u8]) -> Result<()> {
