...
use mgo_replay::{execute_replay_command, ReplayToolCommand};
...
use mgo_core::authority_aggregator::AuthorityAggregatorBuilder;
use mgo_types::messages_checkpoint::{
    CheckpointRequest, CheckpointResponse, CheckpointSequenceNumber,
};
use mgo_types::transaction::{SenderSignedData, Transaction};
...
#[command(
    name = "mgo-tool",
    about = "Debugging utilities for mgo",
...
pub enum ToolCommand {
    ...
    FetchObject {
        ...
        /// ```text
        /// $ mgo-tool fetch-object --id 0x260efde76ebccf57f4c5e951157f5c361cde822c \
        ///      --genesis $HOME/.mgo/mgo_config/genesis.blob \
    ...
    pub async fn execute(self, tracing_handle: TracingHandle) -> Result<(), anyhow::Error> {
        match self {
                ...
                if !concise {
                    ...
                    for (i, val_info) in genesis.validator_set_for_tooling().iter().enumerate() {
                        ...
                        println!(
                            ...
                            i,
                            metadata.name,
                            metadata.mgo_pubkey_bytes().concise(),
            ...
            ToolCommand::RestoreFromDBCheckpoint {
                ...
                let config = mgo_config::NodeConfig::load(config_path)?;
                    ...
                    snapshot_bucket.or_else(|| match (network, no_sign_request) {
                        (Chain::Mainnet, false) => Some(
                            env::var("MAINNET_FORMAL_SIGNED_BUCKET")
                                .unwrap_or("mango-mainnet-formal".to_string()),
                        ),
                        ...
                        (Chain::Testnet, _) => Some(
                            env::var("TESTNET_FORMAL_SIGNED_BUCKET")
                                .unwrap_or("mango-testnet-formal".to_string()),
                        ),
                        (Chain::Unknown, _) => {
                ...
                let aws_endpoint = env::var("AWS_SNAPSHOT_ENDPOINT").ok().or_else(|| {
                    if no_sign_request {
                        if network == Chain::Mainnet {
                            Some("https://formal-snapshot.mainnet.mangonetwork.io".to_string())
                        } else if network == Chain::Testnet {
                            Some("https://formal-snapshot.testnet.mangonetwork.io".to_string())
                        } else {
                            None
                        }
                    } else {
                ...
                let archive_bucket = archive_bucket.or_else(|| match network {
                    Chain::Mainnet => Some(
                        env::var("MAINNET_ARCHIVE_BUCKET")
                            .unwrap_or("mango-mainnet-archives".to_string()),
                    ),
                    Chain::Testnet => Some(
                        env::var("TESTNET_ARCHIVE_BUCKET")
                            .unwrap_or("mango-testnet-archives".to_string()),
                    ),
                    Chain::Unknown => {
                    ...
                    snapshot_bucket.or_else(|| match (formal, network, no_sign_request) {
                        ...
                        (false, Chain::Mainnet, false) => Some(
                            env::var("MAINNET_DB_SIGNED_BUCKET")
                                .unwrap_or("mango-mainnet-snapshots".to_string()),
                        ),
                        ...
                        (false, Chain::Testnet, _) => Some(
                            env::var("TESTNET_DB_SIGNED_BUCKET")
                                .unwrap_or("mango-testnet-snapshots".to_string()),
                        ),
                        (_, Chain::Unknown, _) => {
                            panic!("Cannot generate default snapshot bucket for unknown network");
                        }
                ...
                let snapshot_store_config = if no_sign_request {
                    let aws_endpoint = env::var("AWS_SNAPSHOT_ENDPOINT").ok().or_else(|| {
                        ...
                        if formal {
                            ...
                            Some("https://db-snapshot.mainnet.mangonetwork.io".to_string())
                        } else if network == Chain::Testnet {
                            Some("https://db-snapshot.testnet.mangonetwork.io".to_string())
                        } else {
                            None
                        }
                    });
