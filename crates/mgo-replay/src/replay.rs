...
use mgo_types::storage::{get_module, PackageObject};
use mgo_types::transaction::TransactionKind::ProgrammableTransaction;
use mgo_types::{
...
pub struct ExecutionSandboxState {
    ...
    pub local_exec_effects: MgoTransactionBlockEffects,
...
impl LocalExec {
    ...
    pub async fn multi_download(
        ...
        while num_retries_for_timeout >= 0 {
            match self.fetcher.multi_get_versioned(objs).await {
                ...
                Err(ReplayEngineError::MgoRpcRequestTimeout) => {
                    warn!(
    ...
    pub async fn replay_with_network_config(
        ...
        if let Some(url) = rpc_url.clone() {
            ...
            .await
            {
                ...
                Err(e) => Err(ReplayEngineError::MgoRpcError {
                    err: format!(
    ...
    pub fn download_latest_object(
        ...
        match resp {
            ...
            Err(err) => Err(ReplayEngineError::MgoRpcError {
                err: err.to_string(),
            }),
        }
    }
    ...
    pub async fn get_checkpoint_txs(
        ...
        self.fetcher
            ...
            .await
            .map_err(|e| ReplayEngineError::MgoRpcError { err: e.to_string() })
    }
    ...
    pub async fn execution_engine_execute_with_tx_info_impl(
        ...
        if *tx_digest == TransactionDigest::genesis_marker() || tx_info.sender == MgoAddress::ZERO {
            // Genesis.
            warn!(
        ...
        let (inner_store, gas_status, effects, result) = if let Ok(gas_status) =
            MgoGasStatus::new(tx_info.gas_budget, tx_info.gas_price, rgp, protocol_config)
        {
        ...
        let effects =
            MgoTransactionBlockEffects::try_from(effects).map_err(ReplayEngineError::from)?;
    ...
    pub async fn certificate_execute_with_sandbox_state(
        ...
        let effects =
            MgoTransactionBlockEffects::try_from(res.0).map_err(ReplayEngineError::from)?;
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `mgo_core::authority::AuthorityState::try_execute_immediately`
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `mgo_adapter::execution_engine::execute_transaction_to_effects`
    ...
    async fn resolve_tx_components(
        ...
        let sender = match tx_info.clone().transaction.unwrap().data {
            mgo_json_rpc_types::MgoTransactionBlockData::V1(tx) => tx.sender,
        };
        let MgoTransactionBlockEffects::V1(effects) = tx_info.clone().effects.unwrap();
        ...
        let gas_data = match tx_info.clone().transaction.unwrap().data {
            mgo_json_rpc_types::MgoTransactionBlockData::V1(tx) => tx.gas_data,
        };
        ...
        Ok(OnChainTransactionInfo {
            ...
            effects: MgoTransactionBlockEffects::V1(effects),
    ...
    async fn resolve_tx_components_from_dump(
        ...
        let effects = MgoTransactionBlockEffects::try_from(effects).unwrap();
    ...
    fn read_child_object(
        ...
        fn inner(
            ...
            if child_object.version() > child_version_upper_bound {
                return Err(MgoError::Unknown(format!(
            ...
            if child_object.owner != Owner::ObjectOwner(parent.into()) {
                return Err(MgoError::InvalidChildObjectAccess {
                    object: *child,
                    given_parent: parent,
...
impl ParentSync for LocalExec {
    ...
    fn get_latest_parent_entry_ref_deprecated(
        ...
        fn inner(self_: &LocalExec, object_id: ObjectID) -> MgoResult<Option<ObjectRef>> {
...
impl ModuleResolver for LocalExec {
    ...
    /// We do not download
    fn get_module(&self, module_id: &ModuleId) -> MgoResult<Option<Vec<u8>>> {
        fn inner(self_: &LocalExec, module_id: &ModuleId) -> MgoResult<Option<Vec<u8>>> {
...
impl GetModule for LocalExec {
    ...
    fn get_module_by_id(&self, id: &ModuleId) -> MgoResult<Option<Self::Item>> {
...
pub fn get_executor(
    ...
    let silent = true;
    mgo_execution::executor(&protocol_config, silent, enable_profiler)
