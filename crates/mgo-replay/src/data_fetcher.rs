...
use mgo_types::base_types::{ObjectID, SequenceNumber, VersionNumber};
...
use mgo_types::transaction::{EndOfEpochTransactionKind, TransactionKind};
use tracing::error;
...
pub(crate) trait DataFetcher {
    ...
    ) -> Result<MgoTransactionBlockResponse, ReplayEngineError>;
    ...
    async fn get_epoch_change_events(
        &self,
        reverse: bool,
    ) -> Result<Vec<MgoEvent>, ReplayEngineError>;
...
impl RemoteFetcher {
    pub fn new(rpc_client: MgoClient) -> Self {
        Self {
            rpc_client,
...
impl DataFetcher for RemoteFetcher {
    ...
    async fn multi_get_versioned(
        ...
        let options = MgoObjectDataOptions::bcs_lossless();

        let objs: Vec<_> = to_fetch
            .iter()
            .map(|(object_id, version)| MgoGetPastObjectRequest {
                object_id: *object_id,
                version: *version,
            })
            .collect();
    ...
    async fn get_checkpoint_txs(
        ...
        Ok(self
            ...
            .await
            .map_err(|q| ReplayEngineError::MgoRpcError { err: q.to_string() })?
            .transactions)
    }
    ...
    async fn get_transaction(
        ...
        let tx_fetch_opts = MgoTransactionBlockResponseOptions::full_content();
    ...
    async fn get_epoch_change_events(
        ...
        let mut epoch_change_events: Vec<MgoEvent> = vec![];
...
fn convert_past_obj_response(resp: MgoPastObjectResponse) -> Result<Object, ReplayEngineError> {
    match resp {
        MgoPastObjectResponse::VersionFound(o) => obj_from_mgo_obj_data(&o),
        MgoPastObjectResponse::ObjectDeleted(r) => Err(ReplayEngineError::ObjectDeleted {
            id: r.object_id,
            version: r.version,
            digest: r.digest,
        }),
        MgoPastObjectResponse::ObjectNotExists(id) => Err(ReplayEngineError::ObjectNotExist { id }),
        MgoPastObjectResponse::VersionNotFound(id, version) => {
            Err(ReplayEngineError::ObjectVersionNotFound { id, version })
        }
        MgoPastObjectResponse::VersionTooHigh {
            object_id,
            asked_version,
            latest_version,
...
fn obj_from_mgo_obj_response(o: &MgoObjectResponse) -> Result<Object, ReplayEngineError> {
    let o = o.object().map_err(ReplayEngineError::from)?.clone();
    obj_from_mgo_obj_data(&o)
}

fn obj_from_mgo_obj_data(o: &MgoObjectData) -> Result<Object, ReplayEngineError> {
...
pub fn extract_epoch_and_version(ev: MgoEvent) -> Result<(u64, u64), ReplayEngineError> {
