...
use futures::Stream;
use mango_metrics::metered_channel::Sender;
    ...
    F: Filter<T> + Clone + Send + Sync + 'static + Clone,
{
    pub fn spawn(
        ...
        let gauge = if let Some(metrics) = mango_metrics::get_metrics() {
            ...
            mango_metrics::init_metrics(&Registry::default());
        ...
        let (tx, rx) = mango_metrics::metered_channel::channel(buffer, &gauge);
        let streamer = Self {
            streamer_queue: tx,
    ...
    pub async fn send(&self, data: T) -> Result<(), MgoError> {
        self.streamer_queue
            .send(data)
            .await
            .map_err(|e| MgoError::FailedToDispatchSubscription {
                error: e.to_string(),
            })
    }
}
