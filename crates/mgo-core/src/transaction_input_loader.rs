...
use mgo_types::{
    ...
    error::{MgoError, MgoResult, UserInputError},
            ...
            match kind {
                ...
                InputObjectKind::MovePackage(id) => {
                    let Some(package) = self.cache.get_package_object(id)?.map(|o| o.into()) else {
                        return Err(MgoError::from(kind.object_not_found_error()));
                    };
            ...
            match input {
                ...
                InputObjectKind::SharedMoveObject { id, .. } => {
                    let shared_locks = shared_locks_cell.get_or_try_init(|| {
                        Ok::<HashMap<ObjectID, SequenceNumber>, MgoError>(
                            shared_lock_store
...
impl TransactionInputLoader {
    async fn read_objects_for_synchronous_execution_impl(
        ...
        for kind in input_object_kinds {
            ...
            .ok_or_else(|| MgoError::from(kind.object_not_found_error()))?;
