...
use mango_metrics::histogram::Histogram;
use mango_metrics::{monitored_future, spawn_monitored_task, GaugeGuard};
use mango_network::config::Config;
use std::convert::AsRef;
use mgo_authority_aggregation::ReduceOutput;
use mgo_authority_aggregation::{quorum_map_then_reduce_with_timeout, AsyncResult};
use mgo_config::genesis::Genesis;
use mgo_network::{
    default_mango_network_config, DEFAULT_CONNECT_TIMEOUT_SEC, DEFAULT_REQUEST_TIMEOUT_SEC,
};
use mgo_swarm_config::network_config::NetworkConfig;
use mgo_types::crypto::{AuthorityPublicKeyBytes, AuthoritySignInfo};
use mgo_types::error::UserInputError;
use mgo_types::fp_ensure;
use mgo_types::message_envelope::Message;
use mgo_types::object::Object;
use mgo_types::quorum_driver_types::GroupedErrors;
use mgo_types::mgo_system_state::{MgoSystemState, MgoSystemStateTrait};
use mgo_types::{
    base_types::*,
...
use mgo_types::committee::{CommitteeTrait, CommitteeWithNetworkMetadata, StakeUnit};
...
use mgo_types::messages_safe_client::PlainTransactionInfoResponse;
...
impl AuthAggMetrics {
    pub fn new(registry: &prometheus::Registry) -> Self {
        Self {
            ...
            .unwrap(),
            remaining_tasks_when_reaching_cert_quorum: mango_metrics::histogram::Histogram::new_in_registry(
                "auth_agg_remaining_tasks_when_reaching_cert_quorum",
                "Number of remaining tasks when reaching certificate quorum",
                registry,
            ),
            remaining_tasks_when_cert_broadcasting_post_quorum_timeout: mango_metrics::histogram::Histogram::new_in_registry(
...
pub fn group_errors(errors: Vec<(MgoError, Vec<AuthorityName>, StakeUnit)>) -> GroupedErrors {
...
struct ProcessTransactionState {
    ...
    errors: Vec<(MgoError, Vec<AuthorityName>, StakeUnit)>,
    // This is exclusively non-retryable stake.
    non_retryable_stake: StakeUnit,
    // This includes both object and package not found mgo errors.
...
impl ProcessTransactionState {
    ...
    pub fn record_conflicting_transaction_if_any(
        ...
        if let MgoError::ObjectLockConflict {
            obj_ref,
    ...
    pub fn check_if_error_indicates_tx_finalized_with_different_user_sig(
        ...
        // and return Mgo::FailedToVerifyTxCertWithExecutedEffects.
        ...
        let invalid_sig_stake: StakeUnit = self
            ...
            .filter_map(|(e, _, stake)| {
                if matches!(e, MgoError::FailedToVerifyTxCertWithExecutedEffects { .. }) {
                    Some(stake)
                } else {
                    None
                }
...
struct ProcessCertificateState {
    ...
    non_retryable_stake: StakeUnit,
    non_retryable_errors: Vec<(MgoError, Vec<AuthorityName>, StakeUnit)>,
    retryable_errors: Vec<(MgoError, Vec<AuthorityName>, StakeUnit)>,
    // As long as none of the exit criteria are met we consider the state retryable
    // 1) >= 2f+1 signatures
    // 2) >= f+1 non-retryable errors
    retryable: bool,
...
impl<A: Clone> AuthorityAggregator<A> {
    ...
    ) -> MgoResult<AuthorityAggregator<NetworkAuthorityClient>> {
        let network_clients =
            make_network_authority_clients_with_network_config(&committee, network_config)
                .map_err(|err| MgoError::GenericAuthorityError {
                    error: format!(
...
impl AuthorityAggregator<NetworkAuthorityClient> {
    ...
    pub fn new_from_local_system_state(
        ...
        let mgo_system_state = store.get_mgo_system_state_object_unsafe()?;
        let committee = mgo_system_state.get_current_epoch_committee();
        let validator_display_names = mgo_system_state
    ...
    pub fn new_from_committee(
        ...
        let net_config = default_mango_network_config();
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    async fn quorum_once_inner<'a, S, FMap>(
        ...
        authority_errors: &mut HashMap<AuthorityName, MgoError>,
    ) -> Result<S, MgoError>
    where
        FMap: Fn(AuthorityName, Arc<SafeClient<A>>) -> AsyncResult<'a, S, MgoError>
            + Send
            + Clone
            + 'a,
        S: Send,
    {
        ...
        loop {
            ...
            type RequestResult<S> = Result<Result<S, MgoError>, tokio::time::error::Elapsed>;
            ...
            let name = authorities_shuffled.next().ok_or_else(|| {
                ...
                MgoError::from("Available authorities list is empty")
            })?;
                    ...
                    Event::Request(name, res) => {
                        match res {
                            ...
                            Err(_) => {
                                ...
                                authority_errors.insert(name, MgoError::TimeoutError);
                            }
                            // request completed
                            Ok(inner_res) => {
        ...
        if let Some(t) = timeout_total {
            timeout(t, fut).await.map_err(|_timeout_error| {
                if authority_errors.is_empty() {
                    MgoError::TimeoutError
                } else {
                    MgoError::TooManyIncorrectAuthorities {
    ...
    pub async fn get_latest_object_version_for_testing(
        ...
        let result = quorum_map_then_reduce_with_timeout(
            ...
            .await.map_err(|_state| MgoError::from(UserInputError::ObjectNotFound {
                object_id,
                version: None,
            }))?;
        Ok(result.0)
    ...
    pub async fn get_latest_system_state_object_for_testing(
        &self,
    ) -> anyhow::Result<MgoSystemState> {
        #[derive(Debug, Default)]
        struct State {
            latest_system_state: Option<MgoSystemState>,
            total_weight: StakeUnit,
        }
        let initial_state = State::default();
        let result = quorum_map_then_reduce_with_timeout(
    ...
    fn record_rpc_error_maybe(
        ...
        if let MgoError::RpcError(_message, code) = error {
            metrics
                .total_rpc_err
    ...
    fn handle_process_transaction_response(
        ...
        response: MgoResult<PlainTransactionInfoResponse>,
        name: AuthorityName,
        weight: StakeUnit,
    ) -> MgoResult<Option<ProcessTransactionResult>> {
        match response {
    ...
    > {
        ...
        .map_err(|state| {
            ...
            for (mgo_err, _, _) in state.retryable_errors.iter().chain(state.non_retryable_errors.iter()) {
                self
                    .metrics
                    .total_aggregated_err
                    .with_label_values(&[
    ...
    fn handle_process_certificate_response(
        ...
        response: MgoResult<HandleCertificateResponseV2>,
        name: AuthorityName,
    ) -> MgoResult<Option<(VerifiedCertifiedTransactionEffects, TransactionEvents)>> {
        match response {
    ...
    pub async fn handle_transaction_info_request_from_some_validators(
        ...
        timeout_total: Option<Duration>,
    ) -> MgoResult<PlainTransactionInfoResponse> {
