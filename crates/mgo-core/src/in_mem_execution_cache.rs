...
use crate::authority::authority_store::MgoLockResult;
...
use std::sync::Arc;
use mgo_storage::package_object_cache::PackageObjectCache;
...
pub trait ExecutionCacheRead: Send + Sync {
    fn get_package_object(&self, id: &ObjectID) -> MgoResult<Option<PackageObject>>;
    fn force_reload_system_packages(&self, system_package_ids: &[ObjectID]);

    fn get_object(&self, id: &ObjectID) -> MgoResult<Option<Object>>;

    fn get_objects(&self, objects: &[ObjectID]) -> MgoResult<Vec<Option<Object>>> {
    ...
    fn get_latest_object_or_tombstone(
        &self,
        object_id: ObjectID,
    ) -> MgoResult<Option<(ObjectKey, ObjectOrTombstone)>>;
    ...
    fn multi_get_object_by_key(&self, object_keys: &[ObjectKey]) -> MgoResult<Vec<Option<Object>>>;
    ...
    fn multi_object_exists_by_key(&self, object_keys: &[ObjectKey]) -> MgoResult<Vec<bool>>;
        ...
        for (object_opt, object_ref) in objects.into_iter().zip(object_refs) {
            match object_opt {
                None => {
                    ...
                    return Err(MgoError::UserInputError { error });
                }
                Some(object) => {
                    result.push(object);
                }
    ...
    fn get_lock(&self, obj_ref: ObjectRef, epoch_id: EpochId) -> MgoLockResult;

    fn get_latest_lock_for_object_id(&self, object_id: ObjectID) -> MgoResult<ObjectRef>;

    fn multi_get_transaction_blocks(
        &self,
        digests: &[TransactionDigest],
    ) -> MgoResult<Vec<Option<Arc<VerifiedTransaction>>>>;

    fn get_transaction_block(
        &self,
        digest: &TransactionDigest,
    ) -> MgoResult<Option<Arc<VerifiedTransaction>>> {
    ...
    fn get_transactions_and_serialized_sizes(
        &self,
        digests: &[TransactionDigest],
    ) -> MgoResult<Vec<Option<(VerifiedTransaction, usize)>>> {
    ...
    fn is_tx_already_executed(&self, digest: &TransactionDigest) -> MgoResult<bool> {
    ...
    fn get_events(&self, digest: &TransactionEventsDigest) -> MgoResult<Option<TransactionEvents>> {
    ...
    ) -> BoxFuture<'a, MgoResult<Vec<TransactionEffectsDigest>>>;

    fn notify_read_executed_effects<'a>(
        &'a self,
        digests: &'a [TransactionDigest],
    ) -> BoxFuture<'a, MgoResult<Vec<TransactionEffects>>> {
        async move {
    ...
    fn get_mgo_system_state_object_unsafe(&self) -> MgoResult<MgoSystemState>;
    ...
    fn get_last_shared_object_deletion_info(
        ...
        object_id: &ObjectID,
        epoch_id: EpochId,
    ) -> MgoResult<Option<(SequenceNumber, TransactionDigest)>> {
    ...
    fn get_deleted_shared_object_previous_tx_digest(
        ...
        version: &SequenceNumber,
        epoch_id: EpochId,
    ) -> MgoResult<Option<TransactionDigest>> {
    ...
    ($implementor: ident) => {
        ...
        impl BackingPackageStore for $implementor {
            fn get_package_object(
                &self,
                package_id: &ObjectID,
            ) -> MgoResult<Option<PackageObject>> {
