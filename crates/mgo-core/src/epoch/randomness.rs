...
impl RandomnessManager {
    ...
    pub fn start_dkg(&self) -> MgoResult {
        self.inner.lock().unwrap().start_dkg()
    }

    pub fn advance_dkg(&self, batch: &mut DBBatch) -> MgoResult {
        self.inner.lock().unwrap().advance_dkg(batch)
    }

    pub fn add_message(&self, batch: &mut DBBatch, msg: dkg::Message<PkG, EncG>) -> MgoResult {
...
impl Inner {
    pub fn start_dkg(&mut self) -> MgoResult {
    ...
    pub fn advance_dkg(&mut self, batch: &mut DBBatch) -> MgoResult {
        let epoch_store = self.epoch_store()?;
    ...
    pub fn add_message(&mut self, batch: &mut DBBatch, msg: dkg::Message<PkG, EncG>) -> MgoResult {
        if self.used_messages.initialized() || self.dkg_output.initialized() {
    ...
    fn epoch_store(&self) -> MgoResult<Arc<AuthorityPerEpochStore>> {
        self.epoch_store.upgrade().ok_or(MgoError::EpochEnded)
    }

    fn tables(&self) -> MgoResult<Arc<AuthorityEpochTables>> {
