...
use mgo_types::execution_status::{
    CommandArgumentError, ExecutionFailureStatus, ExecutionStatus, PackageUpgradeError,
    TypeArgumentError,
};
use mgo_types::messages_grpc::ObjectInfoRequestKind;
use mgo_types::{
...
use mgo_types::{
    ...
    crypto::{
        ...
        AuthorityPublicKeyBytes, AuthoritySignature, KeypairTraits, Signature, MgoKeyPair,
    },
...
fn get_registry() -> Result<Registry> {
    ...
    let kp1: MgoKeyPair =
        MgoKeyPair::Ed25519(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp2: MgoKeyPair =
        MgoKeyPair::Secp256k1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp3: MgoKeyPair =
        MgoKeyPair::Secp256r1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    ...
    // ObjectID and MgoAddress are the same length
    ...
    let struct_tag = StructTag::from_str("0x2::coin::Coin<0x2::mgo::MGO>").unwrap();
    ...
    tracer.trace_type::<base_types::MgoAddress>(&samples)?;
...
#[derive(Debug, Parser)]
#[clap(
    name = "Mgo format generator",
    about = "Trace serde (de)serialization to generate format descriptions for Mgo types"
)]
struct Options {
...
const FILE_PATH: &str = "mgo-core/tests/staged/mgo.yaml";
