...
use mango_metrics::{spawn_logged_monitored_task, spawn_monitored_task};
...
use mgo_storage::write_path_pending_tx_log::WritePathPendingTransactionLog;
use mgo_types::base_types::TransactionDigest;
use mgo_types::effects::{TransactionEffectsAPI, VerifiedCertifiedTransactionEffects};
...
impl TransactiondOrchestrator<NetworkAuthorityClient> {
    pub fn new_with_network_clients(
        validator_state: Arc<AuthorityState>,
        reconfig_channel: Receiver<MgoSystemState>,
        parent_path: &Path,
    ...
    OnsiteReconfigObserver: ReconfigObserver<A>,
{
    ...
    ) -> MgoResult<impl Future<Output = MgoResult<QuorumDriverResult>> + '_> {
        let tx_digest = *transaction.digest();
    ...
    async fn execute_finalized_tx_locally_with_timeout(
        ...
        .await
        {
            ...
            Ok(Err(err)) => {
                ...
                Err(MgoError::TransactionOrchestratorLocalExecutionError {
                    error: err.to_string(),
                })
            }
            Ok(Ok(_)) => {
