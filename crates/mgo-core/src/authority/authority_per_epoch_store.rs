...
use std::sync::Arc;
use mgo_config::node::ExpensiveSafetyCheckConfig;
use mgo_types::accumulator::Accumulator;
use mgo_types::authenticator_state::{get_authenticator_state, ActiveJwk};
use mgo_types::base_types::ConciseableName;
use mgo_types::base_types::{AuthorityName, EpochId, ObjectID, SequenceNumber, TransactionDigest};
use mgo_types::committee::Committee;
use mgo_types::committee::CommitteeTrait;
use mgo_types::crypto::{AuthoritySignInfo, AuthorityStrongQuorumSignInfo};
use mgo_types::digests::ChainIdentifier;
use mgo_types::error::{MgoError, MgoResult};
use mgo_types::signature::GenericSignature;
use mgo_types::storage::InputKey;
...
use mgo_types::MGO_RANDOMNESS_STATE_OBJECT_ID;
...
use mango_common::sync::notify_once::NotifyOnce;
...
use std::str::FromStr;
use mgo_execution::{self, Executor};
use mgo_macros::fail_point;
use mgo_protocol_config::{Chain, ProtocolConfig, ProtocolVersion};
use mgo_storage::mutex_table::{MutexGuard, MutexTable};
use mgo_types::effects::{TransactionEffects, TransactionEffectsAPI};
use mgo_types::executable_transaction::{
    TrustedExecutableTransaction, VerifiedExecutableTransaction,
};
use mgo_types::message_envelope::TrustedEnvelope;
...
use mgo_types::mgo_system_state::epoch_start_mgo_system_state::{
...
pub enum ConsensusCertificateResult {
    ...
    MgoTransaction(VerifiedExecutableTransaction),
...
impl AuthorityEpochTables {
    ...
    fn load_reconfig_state(&self) -> MgoResult<ReconfigState> {
        let state = self
            .reconfig_state
    ...
    pub fn remove_executed_tx_subtle(&self, digest: &TransactionDigest) -> MgoResult {
        self.executed_transactions_to_checkpoint.remove(digest)?;
        Ok(())
    }

    pub fn get_last_consensus_index(&self) -> MgoResult<Option<ExecutionIndicesWithHash>> {
        Ok(self.last_consensus_index.get(&LAST_CONSENSUS_STATS_ADDR)?)
    }

    pub fn get_last_consensus_stats(&self) -> MgoResult<Option<ExecutionIndicesWithStats>> {
...
impl AuthorityPerEpochStore {
    ...
    pub fn store_reconfig_state(&self, new_state: &ReconfigState) -> MgoResult {
        self.tables()?
            .reconfig_state
    ...
    pub fn insert_signed_transaction(&self, transaction: VerifiedSignedTransaction) -> MgoResult {
        Ok(self
            .tables()?
            .signed_transactions
    ...
    pub fn get_effects_signature(
        &self,
        tx_digest: &TransactionDigest,
    ) -> MgoResult<Option<AuthoritySignInfo>> {
    ...
    pub fn get_transaction_cert_sig(
        &self,
        tx_digest: &TransactionDigest,
    ) -> MgoResult<Option<AuthorityStrongQuorumSignInfo>> {
    ...
    ) -> MgoResult<Vec<Option<SequenceNumber>>> {
    ...
    pub fn get_last_consensus_index(&self) -> MgoResult<ExecutionIndicesWithHash> {
        self.tables()?
    ...
    pub fn get_last_consensus_stats(&self) -> MgoResult<ExecutionIndicesWithStats> {
        match self
            .tables()?
    ...
    ) -> MgoResult<Vec<(CheckpointSequenceNumber, Accumulator)>> {
        self.tables()?
            .state_hash_by_checkpoint
    ...
    pub fn all_pending_execution(&self) -> MgoResult<Vec<VerifiedExecutableTransaction>> {
        Ok(self
            .tables()?
            .pending_execution
            .unbounded_iter()
    ...
    #[instrument(level = "trace", skip_all)]
    pub fn remove_pending_execution(&self, digest: &TransactionDigest) -> MgoResult<()> {
    ...
    ) -> MgoResult<HashMap<ObjectID, SequenceNumber>> {
    ...
    ) -> MgoResult<Vec<VerifiedSequencedConsensusTransaction>> {
    ...
    fn should_defer(
        ...
        if cert
            .shared_input_objects()
            .any(|obj| obj.id() == MGO_RANDOMNESS_STATE_OBJECT_ID)
        {
    ...
    pub fn remove_pending_consensus_transaction(&self, key: &ConsensusTransactionKey) -> MgoResult {
    ...
    pub fn has_sent_end_of_publish(&self, authority: &AuthorityName) -> MgoResult<bool> {
        Ok(self
            .end_of_publish
            .try_lock()
    ...
    pub fn user_signatures_for_checkpoint(
        &self,
        digests: &[TransactionDigest],
    ) -> MgoResult<Vec<Vec<GenericSignature>>> {
        let signatures = self
            .tables()?
    ...
    pub fn clear_override_protocol_upgrade_buffer_stake(&self) -> MgoResult {
        warn!(
            epoch = ?self.epoch(),
    ...
    pub fn set_override_protocol_upgrade_buffer_stake(&self, new_stake_bps: u64) -> MgoResult {
        warn!(
            ?new_stake_bps,
            epoch = ?self.epoch(),
    ...
    pub fn record_capabilities(&self, capabilities: &AuthorityCapabilities) -> MgoResult {
    ...
    pub fn get_capabilities(&self) -> MgoResult<Vec<AuthorityCapabilities>> {
    ...
    pub(crate) fn get_new_jwks(&self, round: u64) -> MgoResult<Vec<ActiveJwk>> {
    ...
    #[instrument(level = "trace", skip_all)]
    pub fn verify_transaction(&self, tx: Transaction) -> MgoResult<VerifiedTransaction> {
    ...
    fn verify_consensus_transaction(
        ...
        // Signatures are verified as part of narwhal payload verification in MgoTxValidator
    ...
    fn db_batch(&self) -> MgoResult<DBBatch> {
    ...
    >(
        ...
        skipped_consensus_txns: &IntCounter,
    ) -> MgoResult<Vec<VerifiedExecutableTransaction>> {
    ...
    pub(crate) async fn process_consensus_transactions<C: CheckpointServiceNotify>(
        ...
        for tx in transactions {
            ...
            match self
                ...
                .await?
            {
                ConsensusCertificateResult::MgoTransaction(cert) => {
    ...
    async fn process_consensus_transaction<C: CheckpointServiceNotify>(
        ...
        previously_deferred_tx_digests: &HashSet<TransactionDigest>,
        last_randomness_round: RandomnessRound,
    ) -> MgoResult<ConsensusCertificateResult> {
        ...
        match &transaction {
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                ...
                Ok(ConsensusCertificateResult::MgoTransaction(certificate))
            }
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
            ...
            SequencedConsensusTransactionKind::System(system_transaction) => {
                ...
                Ok(ConsensusCertificateResult::MgoTransaction(
    ...
    ) -> MgoResult<Vec<(CheckpointCommitHeight, PendingCheckpoint)>> {
        let tables = self.tables()?;
    ...
    pub fn last_built_checkpoint_commit_height(&self) -> MgoResult<Option<CheckpointCommitHeight>> {
        Ok(self
            .tables()?
            .builder_checkpoint_summary_v2
            .unbounded_iter()
    ...
    pub fn last_built_checkpoint_summary(
        &self,
    ) -> MgoResult<Option<(CheckpointSequenceNumber, CheckpointSummary)>> {
        Ok(self
            .tables()?
            .builder_checkpoint_summary_v2
            .unbounded_iter()
    ...
    pub fn get_built_checkpoint_summary(
        &self,
        sequence: CheckpointSequenceNumber,
    ) -> MgoResult<Option<CheckpointSummary>> {
        Ok(self
            .tables()?
            .builder_checkpoint_summary_v2
            .get(&sequence)?
    ...
    pub fn get_last_checkpoint_signature_index(&self) -> MgoResult<u64> {
        Ok(self
            .tables()?
            .pending_checkpoint_signatures
            .unbounded_iter()
    ...
    pub fn last_randomness_round_written(&self) -> MgoResult<RandomnessRound> {
        Ok(self
            .tables()?
...
impl GetSharedLocks for AuthorityPerEpochStore {
    ...
    ) -> Result<Vec<(ObjectID, SequenceNumber)>, MgoError> {
        Ok(self
            .tables()?
            .assigned_shared_object_versions
