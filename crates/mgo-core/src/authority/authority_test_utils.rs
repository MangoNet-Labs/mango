...
use mgo_types::crypto::{AccountKeyPair, AuthorityKeyPair};
...
pub async fn certify_transaction(
    authority: &AuthorityState,
    transaction: Transaction,
) -> Result<VerifiedCertificate, MgoError> {
...
pub async fn init_state_with_ids<I: IntoIterator<Item = (MgoAddress, ObjectID)>>(
    objects: I,
) -> Arc<AuthorityState> {
...
pub async fn init_state_with_ids_and_versions<
    I: IntoIterator<Item = (MgoAddress, ObjectID, SequenceNumber)>,
>(
    objects: I,
) -> Arc<AuthorityState> {
    let state = TestAuthorityBuilder::new().build().await;
...
pub async fn init_state_with_objects<I: IntoIterator<Item = Object>>(
    ...
    let network_config = mgo_swarm_config::network_config_builder::ConfigBuilder::new(&dir).build();
...
) -> Result<Vec<TransactionEffects>, MgoError> {
    authority
...
pub async fn execute_sequenced_certificate_to_effects(
    authority: &AuthorityState,
    certificate: VerifiedCertificate,
) -> Result<(TransactionEffects, Option<ExecutionError>), MgoError> {
    authority
        .enqueue_certificates_for_execution(
