...
use mgo_types::committee::{Committee, CommitteeTrait, StakeUnit};
use mgo_types::crypto::{AuthorityQuorumSignInfo, AuthoritySignInfo, AuthoritySignInfoTrait};
use mgo_types::error::MgoError;
use mgo_types::message_envelope::{Envelope, Message};
use tracing::warn;
...
impl<S: Clone + Eq, const STRENGTH: bool> StakeAggregator<S, STRENGTH> {
    ...
    pub fn insert_generic(
        ...
        match self.data.entry(authority) {
            Entry::Occupied(oc) => {
                return InsertResult::Failed {
                    error: MgoError::StakeAggregatorRepeatedSigner {
                        signer: authority,
        ...
        if votes > 0 {
            ...
            InsertResult::Failed {
                error: MgoError::InvalidAuthenticator,
            }
        }
    }
