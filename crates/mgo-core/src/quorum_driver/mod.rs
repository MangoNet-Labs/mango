...
use mgo_types::base_types::{AuthorityName, ObjectRef, TransactionDigest};
use mgo_types::committee::{Committee, EpochId, StakeUnit};
...
use mango_common::sync::notify_read::{NotifyRead, Registration};
use mango_metrics::{
...
use mgo_types::transaction::{CertifiedTransaction, Transaction};
...
impl<A: Clone> QuorumDriver<A> {
    ...
    async fn enqueue_task(&self, task: QuorumDriverTask) -> MgoResult<()> {
        self.task_sender
            .send(task.clone())
            .await
            ...
            .map_err(|e| MgoError::QuorumDriverCommunicationError {
                error: e.to_string(),
            })
    }
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn submit_transaction_no_ticket(&self, transaction: Transaction) -> MgoResult<()> {
        let tx_digest = transaction.digest();
        debug!(
            ?tx_digest,
