...
use mgo_types::crypto::{deterministic_random_account_key, get_key_pair, AccountKeyPair};
use mgo_types::effects::TransactionEffectsAPI;
use mgo_types::object::{generate_test_gas_objects, Object};
use mgo_types::quorum_driver_types::{QuorumDriverError, QuorumDriverResponse, QuorumDriverResult};
...
async fn test_quorum_driver_object_locked() -> Result<(), anyhow::Error> {
    ...
    let (sender, keypair): (MgoAddress, AccountKeyPair) = deterministic_random_account_key();
