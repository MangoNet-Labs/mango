...
use mango_metrics::{monitored_scope, spawn_monitored_task, MonitoredFutureExt};
...
use mgo_types::messages_checkpoint::{CheckpointRequestV2, SignedCheckpointSummary};
use mgo_types::messages_consensus::ConsensusTransactionKey;
use mgo_types::signature::GenericSignature;
use mgo_types::mgo_system_state::{MgoSystemState, MgoSystemStateTrait};
use mgo_types::transaction::{TransactionDataAPI, TransactionKind};
use tokio::{
    sync::{watch, Notify},
    time::timeout,
};
...
impl CheckpointStore {
    ...
    fn prune_local_summaries(&self) -> MgoResult {
...
impl CheckpointBuilder {
    ...
    async fn augment_epoch_last_checkpoint(
        ...
        // TODO: Check whether we must use anyhow::Result or can we use MgoResult.
...
impl CheckpointAggregator {
    ...
    async fn run_and_notify(&mut self) -> MgoResult {
    ...
    fn run_inner(&mut self) -> MgoResult<Vec<CertifiedCheckpointSummary>> {
...
async fn diagnose_split_brain(
    ...
    let committee = epoch_store
        .epoch_start_state()
        .get_mgo_committee_with_network_metadata();
    let network_config = default_mango_network_config();
    let network_clients =
...
pub trait CheckpointServiceNotify {
    ...
    fn notify_checkpoint(&self, checkpoint: &PendingCheckpoint) -> MgoResult;
}

/// This is a service used to communicate with other pieces of mgo(for ex. authority)
...
impl CheckpointServiceNotify for CheckpointServiceNoop {
    ...
    fn notify_checkpoint(&self, _: &PendingCheckpoint) -> MgoResult {
        Ok(())
    }
}
...
mod tests {
    ...
    use mgo_types::base_types::{ObjectID, SequenceNumber, TransactionEffectsDigest};
    use mgo_types::crypto::{AuthoritySignInfo, Signature};
    use mgo_types::effects::TransactionEffects;
    use mgo_types::messages_checkpoint::SignedCheckpointSummary;
    use mgo_types::move_package::MovePackage;
    use mgo_types::object;
    use mgo_types::transaction::{GenesisObject, VerifiedTransaction};
    use tokio::sync::mpsc;
    ...
    pub async fn checkpoint_builder_test() {
        ...
        for digest in all_digests {
            let signature = Signature::Ed25519MgoSignature(Default::default()).into();
            state
