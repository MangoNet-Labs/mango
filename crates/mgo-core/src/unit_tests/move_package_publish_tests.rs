...
use mgo_move_build::{check_unpublished_dependencies, gather_published_ids, BuildConfig};
use mgo_types::{
...
async fn test_generate_lock_file() {
    ...
    let expected = expect![[r#"
        ...
        [[move.package]]
        name = "MoveStdlib"
        source = { local = "../../../../../mgo-framework/packages/move-stdlib" }

        [[move.package]]
        name = "Mgo"
        source = { local = "../../../../../mgo-framework/packages/mgo-framework" }
...
async fn test_custom_property_check_unpublished_dependencies() {
    ...
    let MgoError::ModulePublishFailure { error } =
