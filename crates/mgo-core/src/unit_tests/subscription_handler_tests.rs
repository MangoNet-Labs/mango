...
use mgo_types::{MOVE_STDLIB_ADDRESS, MGO_FRAMEWORK_ADDRESS};
...
fn test_to_json_value() {
    ...
    let mgo_move_struct: MgoMoveStruct =
        MoveStruct::simple_deserialize(&event_bytes, &TestEvent::layout())
            .unwrap()
            .into();
    let json_value = mgo_move_struct.to_json_value();
    assert_eq!(
        Some(&json!("1000000")),
...
// TODO: Do we need this in the mgo-types lib?
