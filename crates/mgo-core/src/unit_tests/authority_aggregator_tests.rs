...
use crate::test_utils::make_transfer_mgo_transaction;
...
use mgo_authority_aggregation::quorum_map_then_reduce_with_timeout;
use mgo_macros::sim_test;
use mgo_move_build::BuildConfig;
use mgo_types::crypto::get_key_pair_from_rng;
use mgo_types::crypto::{get_key_pair, AccountKeyPair, AuthorityKeyPair};
use mgo_types::crypto::{AuthoritySignature, Signer};
use mgo_types::crypto::{KeypairTraits, Signature};
use mgo_types::object::Object;
use mgo_types::transaction::*;
use mgo_types::utils::create_fake_transaction;
...
#[cfg(msim)]
use mgo_simulator::configs::constant_latency_ms;
...
async fn test_map_reducer() {
    ...
    let res = quorum_map_then_reduce_with_timeout::<_, _, _, _, _, (), _, _, _>(
        ...
        |_name, _client| {
            Box::pin(async move {
                let res: Result<usize, MgoError> = Err(MgoError::TooManyIncorrectAuthorities {
                    errors: vec![],
                    action: "".to_string(),
                });
                res
        ...
        |mut accumulated_state, _authority_name, _authority_weight, result| {
            Box::pin(async move {
                assert!(matches!(
                    result,
                    Err(MgoError::TooManyIncorrectAuthorities { .. })
                ));
                accumulated_state += 1;
            ...
            |_name, client| {
                ...
                Box::pin(async move {
                    ...
                    match res {
                        ...
                        Err(MgoError::TransactionNotFound { .. }) => Ok(()),
                        Err(err) => Err(err),
                    }
                })
            },
...
async fn test_handle_transaction_fork() {
    ...
    let tx = make_transfer_mgo_transaction(
        gas_object,
        MgoAddress::default(),
        None,
        sender,
...
async fn test_handle_certificate_response() {
    ...
    let (sender, sender_kp): (_, AccountKeyPair) = get_key_pair();
    let gas_object = random_object_ref();
    let tx = VerifiedTransaction::new_unchecked(make_transfer_mgo_transaction(
        gas_object,
        MgoAddress::default(),
        None,
        sender,
    ...
    assert_resp_err(&agg, tx.clone().into(), |e| matches!(e, AggregatorProcessTransactionError::RetryableTransaction { .. }),
        |e| matches!(e, MgoError::WrongEpoch { expected_epoch, actual_epoch } if *expected_epoch == 1 && *actual_epoch == 0)
    ).await;
    ...
    assert_matches!(
        ...
        AggregatorProcessCertificateError::RetryableExecuteCertificate {
            retryable_errors, ..
        } if retryable_errors.iter().any(|(error, _, _)| matches!(error, MgoError::WrongEpoch {
...
async fn test_handle_transaction_response() {
    ...
    let tx2 = VerifiedTransaction::new_unchecked(make_transfer_mgo_transaction(
        gas_object,
        MgoAddress::default(),
        Some(1),
        sender,
    ...
    let package_not_found_error = MgoError::UserInputError {
        error: UserInputError::DependentPackageNotFound {
            package_id: gas_object.0,
        },
    };
    let object_not_found_error = MgoError::UserInputError {
    ...
    assert_resp_err(
        ...
        |e| matches!(e, MgoError::MissingCommitteeAtEpoch(e) if *e == 1),
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MgoError::QuorumFailedToGetEffectsQuorumWhenProcessingTransaction { .. }
                    | MgoError::RpcError(..)
            )
        },
    )
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                ...
                MgoError::QuorumFailedToGetEffectsQuorumWhenProcessingTransaction { .. }
                    | MgoError::RpcError(..)
                    | MgoError::ByzantineAuthoritySuspicion { .. }
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| matches!(e, MgoError::WrongEpoch { expected_epoch, actual_epoch } if *expected_epoch == 0 && *actual_epoch == 1)
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| matches!(e, MgoError::UserInputError { .. } | MgoError::RpcError(..)),
    )
    .await;
...
async fn test_handle_conflicting_transaction_response() {
    ...
    let tx1 = VerifiedTransaction::new_unchecked(make_transfer_mgo_transaction(
        conflicting_object,
        MgoAddress::default(),
        Some(1),
        sender,
    ...
    let conflicting_tx2 = VerifiedTransaction::new_unchecked(make_transfer_mgo_transaction(
        conflicting_object,
        MgoAddress::default(),
        Some(2),
        sender,
    ...
    let conflicting_error = MgoError::ObjectLockConflict {
        obj_ref: conflicting_object,
        pending_transaction: *conflicting_tx2.digest(),
    };
    let retryable_error = MgoError::RpcError("RPC".into(), "Error".into());
    let non_retryable_error = MgoError::ByzantineAuthoritySuspicion {
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MgoError::ObjectLockConflict { .. } | MgoError::RpcError(..)
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| matches!(e, MgoError::ObjectLockConflict { .. }),
    )
    .await;
    ...
    let conflicting_tx3 = make_transfer_mgo_transaction(
        conflicting_object,
        MgoAddress::default(),
        Some(3),
        sender,
    ...
    let conflicting_error_2 = MgoError::ObjectLockConflict {
        obj_ref: conflicting_object,
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MgoError::ObjectLockConflict { .. } | MgoError::ByzantineAuthoritySuspicion { .. }
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MgoError::ObjectLockConflict { .. } | MgoError::UserInputError { .. }
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MgoError::MissingCommitteeAtEpoch(..) | MgoError::ObjectLockConflict { .. }
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MgoError::WrongEpoch { .. } | MgoError::ObjectLockConflict { .. }
            )
        },
    )
    .await;
...
async fn test_handle_overload_response() {
    ...
    let (sender, sender_kp): (_, AccountKeyPair) = get_key_pair();
    let gas_object = random_object_ref();
    let txn = make_transfer_mgo_transaction(
        gas_object,
        MgoAddress::default(),
        None,
        sender,
    ...
    let overload_error = MgoError::TooManyTransactionsPendingExecution {
        queue_len: 100,
        threshold: 100,
    };
    let rpc_error = MgoError::RpcError("RPC".into(), "Error".into());
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                MgoError::TooManyTransactionsPendingExecution { .. } | MgoError::RpcError(..)
            )
        },
    )
    .await;
...
async fn test_early_exit_with_too_many_conflicts() {
    ...
    set_tx_info_response_with_error(
        ...
        MgoError::TooManyTransactionsPendingExecution {
            queue_len: 100,
            threshold: 100,
        },
    );
    ...
    for (name, secret) in &authority_keys {
        let auth_signature = if byzantines.contains(name) {
            ...
            AuthoritySignInfo::new(
                0,
                tx.clone().data(),
                Intent::mgo_app(IntentScope::ProofOfPossession), // bad intent
                *name,
                secret,
            )
        } else {
    ...
    match agg.process_transaction(tx).await {
        Err(received_agg_err) if agg_err_checker(&received_agg_err) => match received_agg_err {
            AggregatorProcessTransactionError::RetryableConflictingTransaction {
                ...
                assert!(errors.iter().map(|e| &e.0).all(mgo_err_checker));
            }
...
fn set_retryable_tx_info_response_error(
    ...
    let error = MgoError::RpcError("RPC".into(), "Error".into());
