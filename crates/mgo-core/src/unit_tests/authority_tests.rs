...
use mgo_json_rpc_types::{
    MgoArgument, MgoExecutionResult, MgoExecutionStatus, MgoTransactionBlockEffectsAPI, MgoTypeTag,
};
use mgo_macros::sim_test;
use mgo_protocol_config::{ProtocolConfig, SupportedProtocolVersions};
use mgo_types::digests::ConsensusCommitDigest;
use mgo_types::dynamic_field::DynamicFieldType;
use mgo_types::effects::TransactionEffects;
use mgo_types::epoch_data::EpochData;
use mgo_types::error::UserInputError;
use mgo_types::execution_status::{ExecutionFailureStatus, ExecutionStatus};
use mgo_types::gas_coin::GasCoin;
use mgo_types::messages_consensus::{ConsensusCommitPrologue, ConsensusCommitPrologueV2};
use mgo_types::object::Data;
use mgo_types::programmable_transaction_builder::ProgrammableTransactionBuilder;
use mgo_types::randomness_state::get_randomness_state_obj_initial_shared_version;
use mgo_types::storage::GetSharedLocks;
use mgo_types::mgo_system_state::MgoSystemStateWrapper;
use mgo_types::utils::{
...
use mgo_types::{
    ...
    MOVE_STDLIB_PACKAGE_ID, MGO_AUTHENTICATOR_STATE_OBJECT_ID, MGO_CLOCK_OBJECT_ID,
    MGO_FRAMEWORK_PACKAGE_ID, MGO_RANDOMNESS_STATE_OBJECT_ID, MGO_SYSTEM_STATE_OBJECT_ID,
};
...
async fn test_dry_run_transaction_block() {
    ...
    let (response, _, _, _) = fullnode
        ...
        .await
        .unwrap();
    assert_eq!(*response.effects.status(), MgoExecutionStatus::Success);
...
async fn test_dry_run_no_gas_big_transfer() {
    ...
    builder.transfer_mgo(recipient, Some(amount));
    ...
    let (dry_run_res, _, _, _) = fullnode
        ...
        .await
        .unwrap();
    assert_eq!(*dry_run_res.effects.status(), MgoExecutionStatus::Success);
}
...
async fn test_dev_inspect_gas_coin_argument() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_mgo(vec![recipient], vec![amount]).unwrap();
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    ...
    assert_eq!(arg, &MgoArgument::GasCoin);
    check_coin_value(
        arg_value,
        arg_type,
...
fn check_coin_value(actual_value: &[u8], actual_type: &MgoTypeTag, expected_value: u64) {
...
async fn test_dry_run_dev_inspect_max_gas_version() {
    ...
    let DevInspectResults { effects, .. } = fullnode
        ...
        .await
        .unwrap();
    assert_eq!(effects.status(), &MgoExecutionStatus::Success);
...
async fn test_transfer_package() {
    ...
    let package_object_ref = authority_state
        .get_mgo_system_package_object_ref()
        .await
        .unwrap();
...
async fn test_objected_owned_gas() {
    ...
    authority_state
        ...
        .await;
    let rgp = authority_state.reference_gas_price_for_testing().unwrap();
    let data = TransactionData::new_transfer_mgo(
        recipient,
        sender,
        None,
...
async fn test_conflicting_transactions() {
    ...
    for _ in 0..100 {
        ...
        assert!(matches!(err, MgoError::ObjectLockConflict { .. }));
...
#[tokio::test]
async fn test_handle_transfer_mgo_with_amount_insufficient_gas() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let object_id = ObjectID::random();
    let authority_state = init_state_with_ids(vec![(sender, object_id)]).await;
...
async fn test_refusal_to_sign_consensus_commit_prologue() {
    ...
    assert!(matches!(
        authority_state
            .handle_transaction(&epoch_store, transaction)
            .await,
        Err(MgoError::InvalidSystemTransaction),
    ));
}
...
async fn test_invalid_authenticator_state_parameter() {
    ...
    assert_eq!(
        ...
        UserInputError::InaccessibleSystemObject {
            object_id: MGO_AUTHENTICATOR_STATE_OBJECT_ID
        }
    );
}
...
async fn test_invalid_randomness_parameter() {
    ...
    let random_mut = CallArg::Object(ObjectArg::SharedObject {
        id: MGO_RANDOMNESS_STATE_OBJECT_ID,
    ...
    assert_eq!(
        UserInputError::try_from(e).unwrap(),
        UserInputError::ImmutableParameterExpectedError {
            object_id: MGO_RANDOMNESS_STATE_OBJECT_ID
        }
    );
}
...
#[tokio::test]
async fn test_genesis_mgo_system_state_object() {
    // This test verifies that we can read the genesis MgoSystemState object.
    ...
    let _mgo_system_state =
        bcs::from_bytes::<MgoSystemStateWrapper>(move_object.contents()).unwrap();
    assert!(move_object.type_().is(&MgoSystemStateWrapper::type_()));
    let mgo_system_state = authority_state
        .get_mgo_system_state_object_for_testing()
        .unwrap();
    assert_eq!(
        &mgo_system_state.get_current_epoch_committee().committee,
        authority_state
...
#[tokio::test]
async fn test_transfer_mgo_no_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
    let init_balance = mgo_types::gas::get_gas_balance(&gas_object).unwrap();
    let authority_state = init_state_with_objects(vec![gas_object.clone()]).await;
    ...
    let gas_ref = gas_object.compute_object_reference();
    let tx_data = TransactionData::new_transfer_mgo(
        recipient,
        sender,
        None,
        gas_ref,
    ...
    let new_balance = mgo_types::gas::get_gas_balance(
        &authority_state
            .get_object(&gas_object_id)
            .await
            .unwrap()
...
#[tokio::test]
async fn test_transfer_mgo_with_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
    ...
    let new_gas = authority_state
        ...
        .unwrap()
        .unwrap();
    assert_eq!(mgo_types::gas::get_gas_balance(&new_gas).unwrap(), 500);
    assert!(gas_ref.1 < effects.gas_object().0 .1);
...
#[tokio::test]
async fn test_store_revert_transfer_mgo() {
    // This test checks the correctness of revert_state_update in MgoDataStore.
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
...
async fn create_and_retrieve_df_info(function: &IdentStr) -> (MgoAddress, Vec<DynamicFieldInfo>) {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let gas_object_id = ObjectID::random();
    let (authority_state, object_basics) =
        init_state_with_ids_and_object_basics(vec![(sender, gas_object_id)]).await;
...
pub async fn init_state_with_ids_and_object_basics<
    I: IntoIterator<Item = (MgoAddress, ObjectID)>,
>(
    objects: I,
) -> (Arc<AuthorityState>, ObjectRef) {
    let state = TestAuthorityBuilder::new().build().await;
...
async fn publish_object_basics(state: Arc<AuthorityState>) -> (Arc<AuthorityState>, ObjectRef) {
    use mgo_move_build::BuildConfig;
...
async fn test_consensus_message_processed() {
    ...
    let network_config = mgo_swarm_config::network_config_builder::ConfigBuilder::new(&dir)
...
async fn test_gas_smashing() {
    ...
    fn make_gas_coins(owner: MgoAddress, gas_amount: u64, coin_num: u64) -> Vec<Object> {
        let mut objects = vec![];
    ...
    async fn run_and_check(
        ...
        let balance = mgo_types::gas::get_gas_balance(
...
async fn test_publish_missing_dependency() {
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.publish_immutable(modules, vec![MGO_FRAMEWORK_PACKAGE_ID]);
    let kind = TransactionKind::programmable(builder.finish());
...
async fn test_publish_not_a_package_dependency() {
    ...
    deps.push(MGO_SYSTEM_STATE_OBJECT_ID);
    builder.publish_immutable(modules, deps);
    let kind = TransactionKind::programmable(builder.finish());
    ...
    assert_eq!(
        MgoError::UserInputError {
            error: UserInputError::MoveObjectAsPackage {
                object_id: MGO_SYSTEM_STATE_OBJECT_ID
            }
        },
        failure,
    )
