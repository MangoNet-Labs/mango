...
use mgo_types::error::{ExecutionError, MgoError};
use mgo_types::execution_status::ExecutionFailureStatus::{
    InputObjectDeleted, SharedObjectOperationNotAllowed,
};
use mgo_types::transaction::{ObjectArg, VerifiedCertificate};
...
async fn test_object_lock_conflict() {
    ...
    assert!(matches!(
        mutate_cert_res.err(),
        Some(MgoError::ObjectLockConflict { .. })
    ));
}
