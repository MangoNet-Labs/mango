...
use std::sync::Arc;
use mgo_config::certificate_deny_config::CertificateDenyConfigBuilder;
use mgo_config::transaction_deny_config::{TransactionDenyConfig, TransactionDenyConfigBuilder};
...
use mgo_types::messages_grpc::HandleTransactionResponse;
...
use mgo_types::utils::get_zklogin_user_address;
use mgo_types::utils::{
...
async fn setup_test(deny_config: TransactionDenyConfig) -> (NetworkConfig, Arc<AuthorityState>) {
    let network_config =
        mgo_swarm_config::network_config_builder::ConfigBuilder::new_with_temp_dir()
            .with_accounts(vec![
                AccountConfig {
                    address: None,
...
type Account = (MgoAddress, Ed25519KeyPair, Vec<ObjectRef>);
...
fn get_accounts_and_coins(
    ...
    let accounts: Vec<_> = network_config
        ...
        .iter()
        .map(|account| {
            let address: MgoAddress = account.public().into();
...
async fn transfer_with_account(
    ...
    let data = TransactionData::new_transfer_mgo_allow_sponsor(
        sender_account.0,
        sender_account.0,
        None,
        sponsor_account.2[0],
...
fn assert_denied<T: std::fmt::Debug>(result: &MgoResult<T>) {
    assert!(matches!(
...
async fn test_shared_object_transaction_disabled() {
    ...
    let tx = TestTransactionBuilder::new(account.0, account.2[0], gas_price)
        .call_staking(account.2[1], MgoAddress::default())
