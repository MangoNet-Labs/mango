...
use mgo_types::move_package::UpgradeCap;
...
make_vec_tests_for_type!(
    ...
    ObjectID,
    TypeTag::Struct(Box::new(mgo_types::id::ID::type_())),
    ObjectID::ZERO
);
...
async fn test_object_no_id_error() {
    ...
    matches!(res.err(), Some(MgoError::ExecutionError(err_str)) if
                 err_str.contains("MgoMoveVerificationError")
