...
use mgo_types::{
    ...
    base_types::dbg_addr,
    crypto::{get_key_pair, AccountKeyPair, PublicKey, Signature, MgoKeyPair},
...
async fn test_handle_transfer_transaction_bad_signature() {
    do_transaction_test(
        ...
        |err| {
            assert_matches!(err, MgoError::SignerSignatureAbsent { .. });
        },
    )
    .await;
}
...
async fn zklogin_test_cached_proof_wrong_key() {
    ...
    */
    ...
    let sender = MgoAddress::try_from_padded(zklogin).unwrap();
    let recipient = dbg_addr(2);
...
async fn setup_zklogin_network(
    ...
    mgo_types::message_envelope::Envelope<SenderSignedData, mgo_types::crypto::EmptySignInfo>,
    ...
    let sender = MgoAddress::try_from_unpadded(zklogin).unwrap();
...
async fn zk_multisig_test() {
    ...
    .unwrap();
    let victim_addr = MgoAddress::from(&multisig_pk);
    ...
    let file = std::fs::File::open("../mgo-types/src/unit_tests/zklogin_test_vectors.json")
...
async fn test_very_large_certificate() {
    ...
    let quorum_signature = mgo_types::crypto::AuthorityQuorumSignInfo {
        epoch: 0,
        signature: mgo_types::crypto::AggregateAuthoritySignature::aggregate(&sigs)
    ...
    let cert = mgo_types::message_envelope::Envelope::new_from_data_and_sig(
