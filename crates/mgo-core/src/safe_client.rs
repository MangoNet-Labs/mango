...
use mango_metrics::histogram::{Histogram, HistogramVec};
...
use mgo_types::{base_types::*, committee::*, fp_ensure};
use mgo_types::{
...
impl<C: Clone> SafeClient<C> {
    ...
    fn get_committee(&self, epoch_id: &EpochId) -> MgoResult<Arc<Committee>> {
        self.committee_store
            .get_committee(epoch_id)?
            .ok_or(MgoError::MissingCommitteeAtEpoch(*epoch_id))
    }
    ...
    ) -> MgoResult<SignedTransactionEffects> {
                ...
                match cert_opt {
                    Some(cert) => {
                        ...
                        ct.verify_committee_sigs_only(&committee).map_err(|e| {
                            MgoError::FailedToVerifyTxCertWithExecutedEffects {
    ...
    ) -> MgoResult<VerifiedObjectInfoResponse> {
    ...
    C: AuthorityAPI + Send + Sync + Clone + 'static,
{
    ...
    pub async fn handle_transaction(
        &self,
        transaction: Transaction,
    ) -> Result<PlainTransactionInfoResponse, MgoError> {
    ...
    ) -> MgoResult<HandleCertificateResponseV2> {
        let signed_effects =
    ...
    pub async fn handle_object_info_request(
        &self,
        request: ObjectInfoRequest,
    ) -> Result<VerifiedObjectInfoResponse, MgoError> {
    ...
    fn verify_checkpoint_sequence(
        ...
        if let (Some(e), Some(o)) = (expected_seq, observed_seq) {
            fp_ensure!(
                e == o,
                MgoError::from("Expected checkpoint number doesn't match with returned")
            );
        }
        Ok(())
    }
    ...
    fn verify_contents_exist<T, O>(
        ...
        match (request_content, checkpoint, contents) {
            ...
            (true, Some(_), None) | (false, _, Some(_)) | (_, None, Some(_)) => Err(
                MgoError::from("Checkpoint contents inconsistent with request"),
            ),
            _ => Ok(()),
        }
    }
    ...
    pub async fn handle_system_state_object(&self) -> Result<MgoSystemState, MgoError> {
        self.authority_client
