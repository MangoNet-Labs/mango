...
use mgo_types::messages_consensus::{ConsensusTransaction, ConsensusTransactionKind};
use tap::TapFallible;
use tracing::{info, warn};

/// Allows verifying the validity of transactions
#[derive(Clone)]
pub struct MgoTxValidator {
    epoch_store: Arc<AuthorityPerEpochStore>,
    checkpoint_service: Arc<dyn CheckpointServiceNotify + Send + Sync>,
    _transaction_manager: Arc<TransactionManager>,
    metrics: Arc<MgoTxValidatorMetrics>,
}
...
impl MgoTxValidator {
    pub fn new(
        ...
        info!(
            "MgoTxValidator constructed for epoch {}",
            epoch_store.epoch()
        );
        Self {
            epoch_store,
...
#[async_trait]
impl TransactionValidator for MgoTxValidator {
    type Error = eyre::Report;

    fn validate(&self, _tx: &[u8]) -> Result<(), Self::Error> {
        // We only accept transactions from local mgo instance so no need to re-verify it
        Ok(())
    }
...
#[async_trait]
impl BlockVerifier for MgoTxValidator {
    type Error = eyre::Report;
...
mod tests {
    use crate::{
        ...
        consensus_validator::{MgoTxValidator, MgoTxValidatorMetrics},
    };
    ...
    async fn accept_valid_transaction() {
        ...
        let metrics = MgoTxValidatorMetrics::new(&Default::default());
        let validator = MgoTxValidator::new(
        ...
        let bogus_transaction_bytes: Vec<_> = certificates
            ...
            .map(|mut cert| {
                ...
                cert.tx_signatures_mut_for_testing()[0] =
                    GenericSignature::Signature(mgo_types::crypto::Signature::Ed25519MgoSignature(
                        Ed25519MgoSignature::default(),
                    ));
                bcs::to_bytes(&ConsensusTransaction::new_certificate_message(&name1, cert)).unwrap()
            })
            .collect();
