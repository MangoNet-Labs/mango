...
use mgo_config::node::OverloadThresholdConfig;
use mgo_framework::BuiltInFramework;
use mgo_genesis_builder::validator_info::ValidatorInfo;
...
use mgo_types::base_types::{random_object_ref, ObjectID};
use mgo_types::crypto::{
    generate_proof_of_possession, get_key_pair, AccountKeyPair, AuthorityPublicKeyBytes,
    NetworkKeyPair, MgoKeyPair,
};
use mgo_types::crypto::{AuthorityKeyPair, Signer};
use mgo_types::effects::{SignedTransactionEffects, TransactionEffects};
...
use mgo_types::{
    base_types::{AuthorityName, ExecutionDigests, ObjectRef, MgoAddress, TransactionDigest},
    committee::Committee,
...
) -> Result<(CertifiedTransaction, SignedTransactionEffects), MgoError> {
...
pub fn compile_basics_package() -> CompiledPackage {
    compile_example_package("../../mgo_programmability/examples/basics")
}

pub fn compile_managed_coin_package() -> CompiledPackage {
    compile_example_package("../../crates/mgo-core/src/unit_tests/data/managed_coin")
}
...
async fn init_genesis(
    ...
    let mut builder = mgo_genesis_builder::Builder::new().add_objects(genesis_objects);
    ...
    for i in 0..committee_size {
        ...
        let account_key_pair: MgoKeyPair = get_key_pair::<AccountKeyPair>().1.into();
        let network_key_pair: NetworkKeyPair = get_key_pair().1;
        let validator_info = ValidatorInfo {
            name: format!("validator-{i}"),
            protocol_key: authority_name,
            worker_key: worker_name,
            account_address: MgoAddress::from(&account_key_pair.public()),
...
pub fn make_transfer_mgo_transaction(
...
pub fn make_pay_mgo_transaction(
    gas_object: ObjectRef,
    coins: Vec<ObjectRef>,
    ...
    let data = TransactionData::new_pay_mgo(
