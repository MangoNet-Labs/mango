...
use std::hash::Hash;
use std::sync::Arc;
use mgo_types::digests::SenderSignedDataDigest;
use mgo_types::digests::ZKLoginInputsDigest;
...
type Sender = oneshot::Sender<MgoResult<VerifiedCertificate>>;
...
impl SignatureVerifier {
    ...
    pub async fn verify_cert(&self, cert: CertifiedTransaction) -> MgoResult<VerifiedCertificate> {
    ...
    pub fn verify_tx(&self, signed_tx: &SenderSignedData) -> MgoResult {
...
fn batch_verify(
    ...
    for cert in certs {
        let idx = obligation.add_message(cert.data(), cert.epoch(), Intent::mgo_app(cert.scope()));
        cert.auth_sig()
            .add_to_verification_obligation(committee, &mut obligation, idx)?;
    }

    for ckpt in checkpoints {
        let idx = obligation.add_message(ckpt.data(), ckpt.epoch(), Intent::mgo_app(ckpt.scope()));
        ckpt.auth_sig()
            .add_to_verification_obligation(committee, &mut obligation, idx)?;
    }
...
impl<D: Hash + Eq + Copy> VerifiedDigestCache<D> {
    ...
    pub fn is_verified<F, G>(&self, digest: D, verify_callback: F, uncached_checks: G) -> MgoResult
    where
        F: FnOnce() -> MgoResult,
        G: FnOnce() -> MgoResult,
    {
