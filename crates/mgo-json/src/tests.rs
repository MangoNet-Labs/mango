...
use mgo_types::base_types::{
    ObjectID, MgoAddress, TransactionDigest, STD_ASCII_MODULE_NAME, STD_ASCII_STRUCT_NAME,
    STD_OPTION_MODULE_NAME, STD_OPTION_STRUCT_NAME,
};
use mgo_types::dynamic_field::derive_dynamic_field_id;
use mgo_types::gas_coin::GasCoin;
use mgo_types::object::Object;
use mgo_types::{parse_mgo_type_tag, MOVE_STDLIB_ADDRESS};

use crate::ResolvedCallArg;

use super::{check_valid_homogeneous, HEX_PREFIX};
use super::{resolve_move_function_args, MgoJsonValue};
...
fn test_json_struct_homogeneous() {
    ...
    assert!(MgoJsonValue::new(positive).is_ok());

    let negative = json!({"inner_vec":[1, 2, 3, true, 5, 6, 7]});
    assert!(MgoJsonValue::new(negative).is_err());
}

#[test]
fn test_json_is_not_valid_mgo_json() {
    let checks = vec![
        // Not homogeneous
        (json!([1, 2, 3, true, 5, 6, 7])),
        // Not homogeneous
    ...
    // Driver
    for arg in checks {
        assert!(MgoJsonValue::new(arg).is_err());
    }
}

#[test]
fn test_json_is_valid_mgo_json() {
    let checks = vec![
        // Homogeneous
        (json!([1, 2, 3, 4, 5, 6, 7])),
        // String allowed
    ...
    // Driver
    for arg in checks {
        assert!(MgoJsonValue::new(arg).is_ok());
    }
}
...
fn test_basic_args_linter_top_level() {
    let path =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../mgo_programmability/examples/nfts");
    ...
    */
    ...
    .iter()
    .map(|q| MgoJsonValue::new(q.clone()).unwrap())
    .collect();
    ...
    let path =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../mgo_programmability/examples/basics");
    let compiled_modules = BuildConfig::new_for_testing()
        .build(path)
        .unwrap()
        .into_modules();
    ...
    */
    let value_raw = "29897";
    let address = MgoAddress::random_for_testing_only();
    ...
    */
    ...
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../mgo-core/src/unit_tests/data/entry_point_vector");
    let compiled_modules = BuildConfig::new_for_testing()
        .build(path)
        .unwrap()
        .into_modules();
    ...
    let args = vec![MgoJsonValue::new(Value::Array(vec![object_id1, object_id2])).unwrap()];
...
fn test_convert_address_from_bcs() {
    ...
    let value = MgoJsonValue::from_bcs_bytes(Some(&MoveTypeLayout::Signer), &bcs_bytes).unwrap();
...
fn test_convert_number_from_bcs() {
    ...
    let value = MgoJsonValue::from_bcs_bytes(Some(&MoveTypeLayout::U32), &bcs_bytes).unwrap();
...
fn test_no_address_zero_trimming() {
    ...
    .unwrap();
    let value = MgoJsonValue::from_bcs_bytes(Some(&MoveTypeLayout::Address), &bcs_bytes).unwrap();
    assert_eq!(
        "0x0000000000000000000000000000011111111111111111111111111111111111",
        value.0.as_str().unwrap()
    );
...
fn test_convert_number_array_from_bcs() {
    ...
    let value = MgoJsonValue::from_bcs_bytes(
...
fn test_from_str() {
    // test number
    let test = MgoJsonValue::from_str("10000").unwrap();
    assert!(test.0.is_number());
    // Test array
    let test = MgoJsonValue::from_str("[10,10,10,10]").unwrap();
    ...
    // test bool
    let test = MgoJsonValue::from_str("true").unwrap();
    assert!(test.0.is_boolean());

    // test id without quotes
    let object_id = ObjectID::random().to_hex_uncompressed();
    let test = MgoJsonValue::from_str(&object_id).unwrap();
    assert!(test.0.is_string());
    assert_eq!(object_id, test.0.as_str().unwrap());

    // test id with quotes
    let test = MgoJsonValue::from_str(&format!("\"{}\"", &object_id)).unwrap();
    assert!(test.0.is_string());
    assert_eq!(object_id, test.0.as_str().unwrap());

    // test string without quotes
    let test = MgoJsonValue::from_str("Some string").unwrap();
    assert!(test.0.is_string());
    ...
    let test = MgoJsonValue::from_object_id(
...
fn test_mgo_call_arg_string_type() {
    ...
    let v = MgoJsonValue::from_bcs_bytes(string_layout.as_ref(), &arg1).unwrap();
...
fn test_mgo_call_arg_string_type() {
    ...
    let v = MgoJsonValue::from_bcs_bytes(Some(option_layout).as_ref(), &arg1).unwrap();
    ...
    let s = MgoJsonValue::from_str("[test, test2]").unwrap();
    println!("{s:?}");
}
...
fn test_convert_struct() {
    ...
    let mgo_json = MgoJsonValue::new(value).unwrap();

    println!("JS: {:#?}", mgo_json);

    let bcs = mgo_json.to_bcs_bytes(&layout).unwrap();
...
fn test_convert_string_vec() {
    ...
    let bcs2 = mgo_json.to_bcs_bytes(&layout).unwrap();
...
fn test_string_vec_df_name_child_id_eq() {
    ...
    let mgo_json = MgoJsonValue::new(name).unwrap();
    let bcs2 = mgo_json.to_bcs_bytes(&layout).unwrap();
