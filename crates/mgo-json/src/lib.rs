...
use mgo_types::base_types::{
    ObjectID, MgoAddress, TxContext, TxContextKind, RESOLVED_ASCII_STR, RESOLVED_STD_OPTION,
...
use mgo_types::id::{ID, RESOLVED_MGO_ID};
use mgo_types::move_package::MovePackage;
use mgo_types::transfer::RESOLVED_RECEIVING_STRUCT;
use mgo_types::MOVE_STDLIB_ADDRESS;
...
pub enum MgoJsonValueErrorKind {
...
#[derive(Debug)]
pub struct MgoJsonValueError {
    kind: MgoJsonValueErrorKind,
    val: JsonValue,
}

impl MgoJsonValueError {
    pub fn new(val: &JsonValue, kind: MgoJsonValueErrorKind) -> Self {
        Self {
            kind,
            val: val.clone(),
        }
...
impl std::error::Error for MgoJsonValueError {}

impl fmt::Display for MgoJsonValueError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let err_str = match self.kind {
            MgoJsonValueErrorKind::ValueTypeNotAllowed => {
                format!("JSON value type {} not allowed.", self.val)
            }
            MgoJsonValueErrorKind::ArrayNotHomogeneous => {
...
pub struct MgoJsonValue(JsonValue);
impl MgoJsonValue {
    pub fn new(json_value: JsonValue) -> Result<MgoJsonValue, anyhow::Error> {
    ...
    pub fn from_object_id(id: ObjectID) -> MgoJsonValue {
    ...
    pub fn to_mgo_address(&self) -> anyhow::Result<MgoAddress> {
        json_value_to_mgo_address(&self.0)
    }
    ...
    fn to_move_value(val: &JsonValue, ty: &MoveTypeLayout) -> Result<R::MoveValue, anyhow::Error> {
        Ok(match (val, ty) {
                ...
                if type_ == &ID::type_() =>
            {
                ...
                let addr = MgoAddress::from_str(s)?;
            ...
            (v, MoveTypeLayout::Address) => {
                let addr = json_value_to_mgo_address(v)?;
                R::MoveValue::Address(addr.into())
            }
...
impl Debug for MgoJsonValue {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", self.0)
    }
}

fn json_value_to_mgo_address(value: &JsonValue) -> anyhow::Result<MgoAddress> {
    match value {
        JsonValue::String(s) => {
        ...
        JsonValue::Array(bytes) => {
            ...
            let bytes = value_to_byte_array(bytes)
                .ok_or_else(|| anyhow!("Invalid input: Cannot parse input into MgoAddress."))?;
            Ok(MgoAddress::try_from(bytes)?)
        }
...
fn move_value_to_json(move_value: &MoveValue) -> Option<JsonValue> {
    Some(match move_value {
        ...
        MoveValue::Signer(v) | MoveValue::Address(v) => json!(MgoAddress::from(*v).to_string()),
...
impl FromStr for MgoJsonValue {
    ...
    fn from_str(s: &str) -> Result<Self, anyhow::Error> {
        ...
        MgoJsonValue::new(serde_json::from_str(s).unwrap_or_else(|_| try_escape_array(s)))
    }
}
...
pub fn check_valid_homogeneous(val: &JsonValue) -> Result<(), MgoJsonValueError> {
...
fn check_valid_homogeneous_rec(curr_q: &mut VecDeque<&JsonValue>) -> Result<(), MgoJsonValueError> {
    if curr_q.is_empty() {
        // Nothing to do
        return Ok(());
    }
    ...
    while let Some(v) = curr_q.pop_front() {
        let curr = match v {
            ...
            // Not valid
            _ => {
                return Err(MgoJsonValueError::new(
                    v,
                    MgoJsonValueErrorKind::ValueTypeNotAllowed,
                ))
            }
...
fn is_primitive_type_tag(t: &TypeTag) -> bool {
    match t {
        ...
        TypeTag::Struct(st) => {
            ...
            // is id or..
            if resolved_struct == RESOLVED_MGO_ID {
                return true;
            }
...
pub fn primitive_type(
    ...
    match param {
        ...
        SignatureToken::Struct(struct_handle_idx) => {
            ...
            } else if resolved_struct == RESOLVED_MGO_ID {
                (
                    true,
                    Some(MoveTypeLayout::Struct(MoveStructLayout {
                        type_: resolved_to_struct(RESOLVED_MGO_ID),
                        fields: vec![MoveFieldLayout::new(
                            ident_str!("bytes").into(),
...
fn resolve_object_vec_arg(idx: usize, arg: &MgoJsonValue) -> Result<Vec<ObjectID>, anyhow::Error> {
    // Every elem has to be a string convertible to a ObjectID
...
pub fn resolve_move_function_args(
    ...
    function: Identifier,
    type_args: &[TypeTag],
    combined_args_json: Vec<MgoJsonValue>,
...
macro_rules! call_args {
        ($($value:expr),*) => {
        Ok::<_, anyhow::Error>(vec![$(mgo_json::call_arg!($value)?,)*])
    };
    }
...
macro_rules! call_arg {
    ($value:expr) => {{
        ...
        trait MgoJsonArg {
            fn to_mgo_json(&self) -> anyhow::Result<MgoJsonValue>;
        }
        // TODO: anyway to condense this?
        impl MgoJsonArg for &str {
            fn to_mgo_json(&self) -> anyhow::Result<MgoJsonValue> {
                MgoJsonValue::from_str(self)
            }
        }
        impl MgoJsonArg for String {
            fn to_mgo_json(&self) -> anyhow::Result<MgoJsonValue> {
                MgoJsonValue::from_str(&self)
            }
        }
        impl MgoJsonArg for mgo_types::base_types::ObjectID {
            fn to_mgo_json(&self) -> anyhow::Result<MgoJsonValue> {
                MgoJsonValue::from_str(&self.to_string())
            }
        }
        impl MgoJsonArg for mgo_types::base_types::MgoAddress {
            fn to_mgo_json(&self) -> anyhow::Result<MgoJsonValue> {
                MgoJsonValue::from_str(&self.to_string())
            }
        }
        impl MgoJsonArg for u64 {
            fn to_mgo_json(&self) -> anyhow::Result<MgoJsonValue> {
                MgoJsonValue::from_bcs_bytes(
                    Some(&mgo_json::MoveTypeLayout::U64),
                    &bcs::to_bytes(self)?,
                )
            }
        }
        impl MgoJsonArg for Vec<u8> {
            fn to_mgo_json(&self) -> anyhow::Result<MgoJsonValue> {
                MgoJsonValue::from_bcs_bytes(None, &self)
            }
        }
        impl MgoJsonArg for &[u8] {
            fn to_mgo_json(&self) -> anyhow::Result<MgoJsonValue> {
                MgoJsonValue::from_bcs_bytes(None, self)
            }
        }
...
macro_rules! type_args {
    ($($value:expr), *) => {{
        use mgo_json_rpc_types::MgoTypeTag;
        use mgo_types::TypeTag;
        trait MgoJsonTypeArg {
            fn to_mgo_json(&self) -> anyhow::Result<MgoTypeTag>;
        }
        impl <T: core::fmt::Display> MgoJsonTypeArg for T {
            fn to_mgo_json(&self) -> anyhow::Result<MgoTypeTag> {
                Ok(mgo_types::parse_mgo_type_tag(&self.to_string())?.into())
            }
        }
        Ok::<_, anyhow::Error>(vec![$($value.to_mgo_json()?,)*])
    }};
    }
