...
trait IntoMgoResult<T> {
    fn into_mgo_result(self) -> MgoResult<T>;
}

impl<T, E> IntoMgoResult<T> for Result<T, E>
where
    E: std::error::Error,
{
    fn into_mgo_result(self) -> MgoResult<T> {
        self.map_err(|e| MgoError::Storage(e.to_string()))
    }
}
...
fn key_to_path_elements(key: &Key) -> MgoResult<(String, &'static str)> {
    match key {
...
impl HttpKVStore {
    ...
    pub fn new(base_url: &str) -> MgoResult<Self> {
        ...
        let base_url = Url::parse(&base_url).into_mgo_result()?;
    ...
    fn get_url(&self, key: &Key) -> MgoResult<Uri> {
        let (digest, item_type) = key_to_path_elements(key)?;
        let joined = self
            .base_url
            .join(&format!("{}/{}", digest, item_type))
            .into_mgo_result()?;
        Uri::from_str(joined.as_str()).into_mgo_result()
    }

    async fn multi_fetch(&self, uris: Vec<Key>) -> Vec<MgoResult<Option<Bytes>>> {
    ...
    async fn fetch(&self, key: Key) -> MgoResult<Option<Bytes>> {
        let uri = self.get_url(&key)?;
        trace!("fetching uri: {}", uri);
        let resp = self.client.get(uri.clone()).await.into_mgo_result()?;
        trace!(
...
fn map_fetch<'a, K>(fetch: (&'a MgoResult<Option<Bytes>>, &'a K)) -> Option<(&'a Bytes, &'a K)>
where
    K: std::fmt::Debug,
{
    let (fetch, key) = fetch;
