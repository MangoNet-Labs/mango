...
impl TransactionKeyValueStore {
    ...
    ) -> MgoResult<Vec<Option<CertifiedCheckpointSummary>>> {
    ...
    pub async fn multi_get_checkpoints_contents(
        &self,
        keys: &[CheckpointSequenceNumber],
    ) -> MgoResult<Vec<Option<CheckpointContents>>> {
        self.multi_get_checkpoints(&[], keys, &[], &[])
            .await
    ...
    ) -> MgoResult<Vec<Option<Transaction>>> {
        self.multi_get(keys, &[], &[])
            .await
            .map(|(txns, _, _)| txns)
    }
    ...
    pub async fn get_tx(&self, digest: TransactionDigest) -> MgoResult<Transaction> {
        self.multi_get_tx(&[digest])
            .await?
            .into_iter()
            .next()
            .flatten()
            .ok_or(MgoError::TransactionNotFound { digest })
    }
    ...
    ) -> MgoResult<CertifiedCheckpointSummary> {
