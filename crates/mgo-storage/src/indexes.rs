...
//! IndexStore supports creation of various ancillary indexes of state in MgoDataStore.
...
use mgo_types::execution::DynamicallyLoadedObjectMetadata;
use tokio::sync::OwnedMutexGuard;
use typed_store::TypedStoreError;

use crate::mutex_table::MutexTable;
use crate::sharded_lru::ShardedLruCache;
use mgo_json_rpc_types::{MgoObjectDataFilter, TransactionFilter};
use mgo_types::base_types::{
    ObjectDigest, ObjectID, SequenceNumber, MgoAddress, TransactionDigest, TxSequenceNumber,
};
use mgo_types::base_types::{ObjectInfo, ObjectRef};
use mgo_types::digests::TransactionEventsDigest;
use mgo_types::dynamic_field::{self, DynamicFieldInfo};
use mgo_types::effects::TransactionEvents;
use mgo_types::error::{MgoError, MgoResult, UserInputError};
use mgo_types::inner_temporary_store::TxCoins;
...
type OwnerIndexKey = (MgoAddress, ObjectID);
type CoinIndexKey = (MgoAddress, String, ObjectID);
...
pub struct IndexStoreCaches {
    per_coin_type_balance: ShardedLruCache<(MgoAddress, TypeTag), MgoResult<TotalBalance>>,
    all_balances: ShardedLruCache<MgoAddress, MgoResult<Arc<HashMap<TypeTag, TotalBalance>>>>,
    locks: MutexTable<MgoAddress>,
}

#[derive(Default)]
pub struct IndexStoreCacheUpdates {
    _locks: Vec<OwnedMutexGuard<()>>,
    per_coin_type_balance_changes: Vec<((MgoAddress, TypeTag), MgoResult<TotalBalance>)>,
    all_balance_changes: Vec<(MgoAddress, MgoResult<Arc<AllBalance>>)>,
}

#[derive(DBMapUtils)]
pub struct IndexStoreTables {
    /// Index from mgo address to transactions initiated by that address.
    #[default_options_override_fn = "transactions_from_addr_table_default_config"]
    transactions_from_addr: DBMap<(MgoAddress, TxSequenceNumber), TransactionDigest>,

    /// Index from mgo address to transactions that were sent to that address.
    #[default_options_override_fn = "transactions_to_addr_table_default_config"]
    transactions_to_addr: DBMap<(MgoAddress, TxSequenceNumber), TransactionDigest>,
    ...
    /// composite key of the MgoAddress of their owner and the object ID of the object.
    ...
    #[default_options_override_fn = "index_table_default_config"]
    event_by_sender: DBMap<(MgoAddress, EventId), EventIndex>,
    #[default_options_override_fn = "index_table_default_config"]
...
impl IndexStore {
    ...
    pub async fn index_coin(
    ...
    ) -> MgoResult<IndexStoreCacheUpdates> {
        ...
        let mut addresses: HashSet<MgoAddress> = HashSet::new();
        addresses.extend(
        ...
        let mut balance_changes: HashMap<MgoAddress, HashMap<TypeTag, TotalBalance>> =
            HashMap::new();
        // Index coin info
            ...
            .flat_map(|(address, balance_map)| {
                balance_map.iter().map(|(type_tag, balance)| {
                    (
                        ...
                        Ok::<TotalBalance, MgoError>(*balance),
                    )
                })
            })
            .collect();
        let all_balance_changes: Vec<_> = balance_changes
            ...
            .map(|(address, balance_map)| {
                (
                    address,
                    Ok::<Arc<HashMap<TypeTag, TotalBalance>>, MgoError>(Arc::new(balance_map)),
                )
            })
            .collect();
    ...
    pub fn get_transactions(
        ...
        let cursor = if let Some(cursor) = cursor {
            Some(
                self.get_transaction_seq(&cursor)?
                    .ok_or(MgoError::TransactionNotFound { digest: cursor })?,
            )
        } else {
            None
        };
        match filter {
            ...
            Some(_) => Err(MgoError::UserInputError {
    ...
    pub fn get_transaction_seq(
        &self,
        digest: &TransactionDigest,
    ) -> MgoResult<Option<TxSequenceNumber>> {
    ...
    pub fn all_events(
        ...
        limit: usize,
        descending: bool,
    ) -> MgoResult<Vec<(TransactionEventsDigest, TransactionDigest, usize, u64)>> {
        Ok(if descending {
            self.tables
                .event_order
                .unbounded_iter()
    ...
    pub fn events_by_transaction(
        ...
        let seq = self
            ...
            .ok_or(MgoError::TransactionNotFound { digest: *digest })?;
        Ok(if descending {
            self.tables
                .event_order
                .unbounded_iter()
    ...
    pub fn insert_genesis_objects(&self, object_index_changes: ObjectIndexChanges) -> MgoResult {
    ...
    pub async fn get_balance(
        ...
        if force_disable_cache {
            ...
            .unwrap()
            .map_err(|e| {
                MgoError::ExecutionError(format!("Failed to read balance frm DB: {:?}", e))
            });
        }
    ...
    pub async fn get_all_balance(
        &self,
        owner: MgoAddress,
    ) -> MgoResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        let force_disable_cache = read_size_from_env(ENV_VAR_DISABLE_INDEX_CACHE).unwrap_or(0) > 0;
        let metrics_cloned = self.metrics.clone();
        let coin_index_cloned = self.tables.coin_index.clone();
        if force_disable_cache {
            ...
            .unwrap()
            .map_err(|e| {
                MgoError::ExecutionError(format!("Failed to read all balance from DB: {:?}", e))
            });
        }
    ...
    /// Read balance for a `MgoAddress` and `CoinType` from the backend database
    ...
    /// Read all balances for a `MgoAddress` from the backend database
    pub fn get_all_balances_from_db(
        ...
        for (coin_type, coins) in &coins {
            ...
            let coin_type =
                TypeTag::Struct(Box::new(parse_mgo_struct_tag(&coin_type).map_err(|e| {
                    MgoError::ExecutionError(format!(
    ...
    async fn invalidate_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = (MgoAddress, TypeTag)>,
    ) -> MgoResult {
        self.caches
            .per_coin_type_balance
    ...
    async fn invalidate_all_balance_cache(
        &self,
        addresses: impl IntoIterator<Item = MgoAddress>,
    ) -> MgoResult {
    ...
    async fn update_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = ((MgoAddress, TypeTag), MgoResult<TotalBalance>)>,
    ) -> MgoResult {
        self.caches
            .per_coin_type_balance
    ...
    fn merge_balance(
        old_balance: &MgoResult<TotalBalance>,
        balance_delta: &MgoResult<TotalBalance>,
    ) -> MgoResult<TotalBalance> {
    ...
    async fn update_all_balance_cache(
        &self,
        keys: impl IntoIterator<Item = (MgoAddress, MgoResult<Arc<HashMap<TypeTag, TotalBalance>>>)>,
    ) -> MgoResult {
        self.caches
            .all_balances
    ...
    fn merge_all_balance(
        old_balance: &MgoResult<Arc<HashMap<TypeTag, TotalBalance>>>,
        balance_delta: &MgoResult<Arc<HashMap<TypeTag, TotalBalance>>>,
    ) -> MgoResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        if let Ok(old_balance) = old_balance {
            if let Ok(balance_delta) = balance_delta {
...
mod tests {
    ...
    use mgo_types::base_types::{ObjectInfo, ObjectType, MgoAddress};
    use mgo_types::digests::TransactionDigest;
    use mgo_types::effects::TransactionEvents;
    use mgo_types::gas_coin::GAS;
    use mgo_types::object;
    ...
    async fn test_index_cache() -> anyhow::Result<()> {
        ...
        let address: MgoAddress = AccountAddress::random().into();
