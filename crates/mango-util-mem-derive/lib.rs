...
fn malloc_size_of_derive(s: synstructure::Structure) -> proc_macro2::TokenStream {
        ...
        if ignore {
            ...
            Some(quote! {
                for item in #binding.iter() {
                    sum += mango_util_mem::MallocSizeOf::size_of(item, ops);
                }
            })
        } else {
            Some(quote! {
                sum += mango_util_mem::MallocSizeOf::size_of(#binding, ops);
            })
        }
    });
    ...
    for param in ast.generics.type_params() {
        ...
        where_clause
            .predicates
            .push(parse_quote!(#ident: mango_util_mem::MallocSizeOf));
    }

    let tokens = quote! {
        impl #impl_generics mango_util_mem::MallocSizeOf for #name #ty_generics #where_clause {
            #[inline]
            #[allow(unused_variables, unused_mut, unreachable_code)]
            fn size_of(&self, ops: &mut mango_util_mem::MallocSizeOfOps) -> usize {
                let mut sum = 0;
                match *self {
                    #match_body
                }
