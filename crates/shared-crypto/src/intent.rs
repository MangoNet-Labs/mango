...
/// (i.e., Narwhal, Mgo, Ethereum etc) should never collide, so that even when a signing
...
/// be any type that implements [trait Serialize]. *ALL* signatures in Mgo must commits
...
/// A 1-byte domain separator for hashing Object ID in Mgo. It is starting from 0xf0
/// to ensure no hashing collision for any ObjectID vs MgoAddress which is derived
/// as the hash of `flag || pubkey`. See `mgo_types::crypto::SignatureScheme::flag()`.
#[derive(Serialize_repr, Deserialize_repr, Copy, Clone, PartialEq, Eq, Debug, Hash)]
#[repr(u8)]
