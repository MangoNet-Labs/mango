...
use mgo_sdk::rpc_types::{MgoTransactionBlockEffects, TransactionFilter};
...
pub const MGO_SOURCE_VALIDATION_VERSION_HEADER: &str = "x-mgo-source-validation-version";
pub const MGO_SOURCE_VALIDATION_VERSION: &str = "0.1";

pub const MAINNET_URL: &str = "https://fullnode.mainnet.mangonetwork.io:443";
pub const TESTNET_URL: &str = "https://fullnode.testnet.mangonetwork.io:443";
pub const DEVNET_URL: &str = "https://fullnode.devnet.mangonetwork.io:443";
pub const LOCALNET_URL: &str = "http://127.0.0.1:9000";

pub const MAINNET_WS_URL: &str = "wss://rpc.mainnet.mangonetwork.io:443";
pub const TESTNET_WS_URL: &str = "wss://rpc.testnet.mangonetwork.io:443";
pub const DEVNET_WS_URL: &str = "wss://rpc.devnet.mangonetwork.io:443";
...
pub async fn verify_package(
    ...
    let client = MgoClientBuilder::default().build(network_url).await?;
...
pub async fn watch_for_upgrades(
    ...
    network: Network,
    channel: Option<Sender<MgoTransactionBlockEffects>>,
) -> anyhow::Result<()> {
    ...
    let client: WsClient = WsClientBuilder::default()
        ...
        .build(websocket_url)
        .await?;
    let mut subscription: Subscription<MgoTransactionBlockEffects> = client
        .subscribe(
    ...
    loop {
        let result: Option<Result<MgoTransactionBlockEffects, _>> = subscription.next().await;
        match result {
            Some(Ok(result)) => {
...
async fn check_version_header<B>(
    ...
    let version = headers
        .get(MGO_SOURCE_VALIDATION_VERSION_HEADER)
        .as_ref()
        .and_then(|h| h.to_str().ok())
        .map(|s| s.to_string());

    match version {
        Some(v) if v != MGO_SOURCE_VALIDATION_VERSION => {
            let error = format!(
                "Unsupported version '{v}' specified in header \
		 {MGO_SOURCE_VALIDATION_VERSION_HEADER}"
            );
            let mut headers = HeaderMap::new();
            headers.insert(
                HeaderName::from_static(MGO_SOURCE_VALIDATION_VERSION_HEADER),
                HeaderValue::from_static(MGO_SOURCE_VALIDATION_VERSION),
            );
            return (
                StatusCode::BAD_REQUEST,
                headers,
...
pub fn start_prometheus_server(addr: TcpListener) -> RegistryService {
    ...
    let app = Router::new()
        .route(METRICS_ROUTE, get(mango_metrics::metrics))
