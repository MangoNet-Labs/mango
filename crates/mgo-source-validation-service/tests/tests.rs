...
use mgo_sdk::rpc_types::{
    OwnedObjectRef, MgoObjectDataOptions, MgoObjectResponseQuery, MgoTransactionBlockEffectsV1,
};
use mgo_sdk::types::base_types::ObjectID;
use mgo_sdk::types::object::Owner;
use mgo_sdk::types::transaction::TEST_ONLY_GAS_UNIT_FOR_PUBLISH;
use mgo_sdk::wallet_context::WalletContext;
use tokio::sync::oneshot;

use move_core_types::account_address::AccountAddress;
use move_symbol_pool::Symbol;
use mgo_source_validation_service::{
    ...
    SourceResponse, SourceServiceMetrics, METRICS_HOST_PORT, MGO_SOURCE_VALIDATION_VERSION_HEADER,
};
...
async fn test_end_to_end() -> anyhow::Result<()> {
    ...
    let Ok(MgoTransactionBlockEffects::V1(effects)) = rx.await else {
    ...
    let config = Config {
        packages: vec![PackageSource::Repository(RepositorySource {
            repository: "https://github.com/MangoNetworkOs/Mango".into(),
            branch: "main".into(),
            packages: vec![Package {
    ...
    fs_extra::dir::copy(
        PathBuf::from(TEST_FIXTURES_DIR).join("mgo"),
...
async fn run_publish(
    ...
    gas_obj_id: ObjectID,
    rgp: u64,
) -> anyhow::Result<MgoTransactionBlockEffectsV1> {
...
async fn test_api_route() -> anyhow::Result<()> {
    ...
    let source_path = fixtures
        .into_path()
        .join("mgo/move-stdlib/sources/address.move");
    ...
    let json = client
        ...
        .header(MGO_SOURCE_VALIDATION_VERSION_HEADER, "bogus")
        .send()
        .await
        .expect("Request failed.")
        .json::<ErrorResponse>()
    ...
    let expected =
        expect!["Unsupported version 'bogus' specified in header x-mgo-source-validation-version"];
...
fn test_parse_package_config() -> anyhow::Result<()> {
    ...
    source = "Repository"
    [packages.values]
    repository = "https://github.com/MangoNetworkOs/Mango"
    branch = "main"
    packages = [
        { path = "crates/mgo-framework/packages/mgo-framework", watch = "0x2" },
        { path = "immutable" },
    ]
            ...
            packages: [
                Repository(
                    RepositorySource {
                        ...
                        packages: [
                            Package {
                                path: "crates/mgo-framework/packages/mgo-framework",
                                watch: Some(
