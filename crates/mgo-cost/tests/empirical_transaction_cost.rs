...
use mgo_test_transaction_builder::publish_basics_package_and_make_counter;
use mgo_test_transaction_builder::TestTransactionBuilder;
use mgo_types::base_types::{ObjectRef, MgoAddress};
use mgo_types::coin::PAY_JOIN_FUNC_NAME;
use mgo_types::coin::PAY_MODULE_NAME;
use mgo_types::coin::PAY_SPLIT_VEC_FUNC_NAME;
...
pub enum CommonTransactionCosts {
    ...
    SplitCoin(usize),
    TransferWholeCoin,
    TransferWholeMgoCoin,
    TransferPortionMgoCoin,
    SharedCounterCreate,
...
// 1. Run `cargo insta test --review` under `./mgo-cost`.
...
async fn create_txes(
    ...
    //
    // Transfer Whole Mgo Coin and Transfer Portion of Mgo Coin
    //
    let whole_mgo_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_mgo(None, MgoAddress::default())
            .build();
    let partial_mgo_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_mgo(Some(10), MgoAddress::default())
            .build();
    ret.insert(
        CommonTransactionCosts::TransferWholeMgoCoin,
        whole_mgo_coin_tx,
    );
    ret.insert(
        CommonTransactionCosts::TransferPortionMgoCoin,
        partial_mgo_coin_tx,
    );
    ...
    let whole_coin_tx = TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
        .transfer(gas_objects.pop().unwrap(), MgoAddress::default())
        .build();
