...
use std::sync::Arc;
use mgo_types::base_types::{ObjectID, ObjectRef, MgoAddress, MGO_ADDRESS_LENGTH};
use mgo_types::crypto::{get_account_key_pair, AccountKeyPair};
...
) -> (BTreeMap<MgoAddress, Account>, Vec<Object>) {
...
fn new_gas_object(idx: u64, owner: MgoAddress) -> Object {
    // Predictable and cheaper way of generating object IDs for benchmarking.
    let mut id_bytes = [0u8; MGO_ADDRESS_LENGTH];
