...
use clap::Parser;
use mgo_graphql_rpc::commands::Command;
use mgo_graphql_rpc::config::{ConnectionConfig, ServerConfig, ServiceConfig};
use mgo_graphql_rpc::config::{Ide, TxExecFullNodeConfig};
use mgo_graphql_rpc::server::builder::export_schema;
use mgo_graphql_rpc::server::graphiql_server::{
...
async fn main() {
    ...
    match cmd {
        ...
        Command::GenerateDocsExamples => {
            ...
            // mgo/docs/content/references/mgo-graphql/examples.mdx
            let filename = "docs/content/references/mgo-graphql/examples.mdx";
            buf.pop();
            buf.pop();
            buf.push(filename);
            let content = mgo_graphql_rpc::examples::generate_examples_for_docs()
        ...
        Command::GenerateExamples { file } => {
            let new_content: String = mgo_graphql_rpc::examples::generate_markdown()
