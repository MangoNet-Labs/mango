...
use async_graphql::*;
use mgo_types::mgo_system_state::mgo_system_state_summary::MgoValidatorSummary as NativeMgoValidatorSummary;

#[derive(Clone, Debug)]
pub(crate) struct Validator {
    pub validator_summary: NativeMgoValidatorSummary,
...
impl Validator {
    ...
    async fn address(&self) -> Address {
        Address {
            address: MgoAddress::from(self.validator_summary.mgo_address),
    ...
    /// the amount of MGO tokens that each past MGO staker can withdraw in the future.
    ...
    async fn staking_pool_mgo_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.staking_pool_mgo_balance,
        ))
    }
    ...
    async fn pending_total_mgo_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.pending_total_mgo_withdraw,
        ))
    }
    ...
    /// The total number of MGO tokens in this pool plus
    ...
    async fn apy(&self, ctx: &Context<'_>) -> Result<Option<u64>, Error> {
        Ok(ctx
            ...
            .fetch_validator_apys(&self.validator_summary.mgo_address)
            .await?
...
impl Validator {
    pub fn operation_cap_id(&self) -> MgoAddress {
        MgoAddress::from_array(**self.validator_summary.operation_cap_id)
    }
    pub fn staking_pool_id(&self) -> MgoAddress {
        MgoAddress::from_array(**self.validator_summary.staking_pool_id)
    }
    pub fn exchange_rates_id(&self) -> MgoAddress {
        MgoAddress::from_array(**self.validator_summary.exchange_rates_id)
    }
}
