...
use mgo_json_rpc_types::MgoArgument;
...
impl MoveCallTransaction {
    ...
    async fn package(&self) -> MgoAddress {
    ...
    fn from(pt: NativeProgrammableTransaction, checkpoint_viewed_at: u64) -> Self {
        ...
        match pt {
            ...
            N::Publish(modules, dependencies) => P::Publish(PublishTransaction {
                ...
                dependencies: dependencies.into_iter().map(MgoAddress::from).collect(),
            }),
...
impl From<MgoArgument> for TransactionArgument {
    fn from(argument: MgoArgument) -> Self {
        use MgoArgument as S;
        use TransactionArgument as A;
        match argument {
