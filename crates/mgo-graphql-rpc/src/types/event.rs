...
use super::{
    ...
    move_value::MoveValue, mgo_address::MgoAddress,
};
...
use mgo_indexer::models_v2::{events::StoredEvent, transactions::StoredTransaction};
use mgo_indexer::schema_v2::{events, transactions, tx_senders};
use mgo_types::base_types::ObjectID;
use mgo_types::Identifier;
use mgo_types::{
    base_types::MgoAddress as NativeMgoAddress, event::Event as NativeEvent, parse_mgo_struct_tag,
};

/// A Mgo node emits one of the following events:
/// Move event
/// Publish event
...
impl Event {
    ...
    async fn sender(&self) -> Result<Option<Address>> {
        if self.native.sender == NativeMgoAddress::ZERO {
            return Ok(None);
        }
...
impl Event {
    ...
    fn try_from_stored_event(
        ...
        let sender = NativeMgoAddress::from_bytes(sender_bytes)
            .map_err(|e| Error::Internal(e.to_string()))?;

        let package_id =
            ObjectID::from_bytes(&stored.package).map_err(|e| Error::Internal(e.to_string()))?;
        let type_ =
            parse_mgo_struct_tag(&stored.event_type).map_err(|e| Error::Internal(e.to_string()))?;
