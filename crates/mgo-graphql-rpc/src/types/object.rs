...
use super::{owner::Owner, mgo_address::MgoAddress, transaction_block::TransactionBlock};
...
use mgo_indexer::models_v2::objects::{
    StoredDeletedHistoryObject, StoredHistoryObject, StoredObject,
};
use mgo_indexer::schema_v2::{objects, objects_history, objects_snapshot};
use mgo_indexer::types_v2::ObjectStatus as NativeObjectStatus;
use mgo_indexer::types_v2::OwnerType;
use mgo_package_resolver::Resolver;
...
pub(crate) struct ObjectFilter {
    ...
    /// the full type name, such as `0x2::coin::Coin<0x2::mgo::MGO>`.
    ...
    pub object_ids: Option<Vec<MgoAddress>>,
...
/// An object in Mgo is a package (set of Move bytecode modules) or object (typed data structure
    ...
    pub(crate) async fn owner(&self, ctx: &Context<'_>) -> Option<ObjectOwner> {
        ...
        match native.owner {
            O::AddressOwner(address) => {
                let address = MgoAddress::from(address);
...
impl ObjectFilter {
    ...
    /// it).
    fn keys(&self) -> Option<BTreeMap<MgoAddress, Option<u64>>> {
...
/// Parse a `MgoAddress` from its stored representation.  Failure is an internal error: the
/// database should never contain a malformed address (containing the wrong number of bytes).
fn addr(bytes: impl AsRef<[u8]>) -> Result<MgoAddress, Error> {
    MgoAddress::from_bytes(bytes.as_ref()).map_err(|e| {
...
mod tests {
    ...
    fn test_owner_filter_intersection() {
        let f0 = ObjectFilter {
            owner: Some(MgoAddress::from_str("0x1").unwrap()),
            ..Default::default()
        };

        let f1 = ObjectFilter {
            owner: Some(MgoAddress::from_str("0x2").unwrap()),
            ..Default::default()
        };
    ...
    fn test_key_filter_intersection() {
        let i1 = MgoAddress::from_str("0x1").unwrap();
        let i2 = MgoAddress::from_str("0x2").unwrap();
        let i3 = MgoAddress::from_str("0x3").unwrap();
        let i4 = MgoAddress::from_str("0x4").unwrap();
