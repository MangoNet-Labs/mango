...
use mgo_indexer::{models_v2::display::StoredDisplay, schema_v2::display};
...
pub(crate) fn get_value_from_move_struct(
    ...
    // todo: 10 is a carry-over from the mgo-json-rpc implementation
    ...
    // TODO: move off dependency on MgoMoveValue
    let mgo_move_value: MgoMoveValue = result.clone().into();

    match mgo_move_value {
        MgoMoveValue::Option(move_option) => match move_option.as_ref() {
            Some(move_value) => Ok(move_value.to_string()),
            None => Ok("".to_string()),
        },
        MgoMoveValue::Vector(_) => Err(DisplayRenderError::Vector(var_name.to_string())),
        _ => Ok(mgo_move_value.to_string()),
    }
}
