...
use async_graphql::*;
use mgo_package_resolver::{error::Error as PackageCacheError, Package as ParsedMovePackage};
use mgo_types::{move_package::MovePackage as NativeMovePackage, object::Data};
...
impl MovePackage {
    ...
    /// The `0x3::staking_pool::StakedMgo` objects owned by this package.
    ///
    /// Note that objects owned by a package are inaccessible, because packages are immutable and
    /// cannot be owned by an address.
    ...
    /// The MgonsRegistration NFTs owned by this package. These grant the owner the capability to
