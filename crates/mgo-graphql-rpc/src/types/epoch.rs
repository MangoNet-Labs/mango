...
use mgo_indexer::models_v2::epoch::QueryableEpochInfo;
use mgo_indexer::schema_v2::epochs;
use mgo_types::messages_checkpoint::CheckpointCommitment as EpochCommitment;
...
/// Operation of the Mgo network is temporally partitioned into non-overlapping epochs,
...
impl Epoch {
    ...
    async fn validator_set(&self, ctx: &Context<'_>) -> Result<Option<ValidatorSet>> {
        let system_state = ctx
            ...
            .fetch_mgo_system_state(Some(self.stored.epoch as u64))
            .await?;
