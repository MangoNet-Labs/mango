...
use mgo_types::base_types::{ObjectID, MgoAddress as NativeMgoAddress};
use thiserror::Error;

const MGO_ADDRESS_LENGTH: usize = 32;

#[derive(Serialize, Deserialize, Clone, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Copy)]
pub(crate) struct MgoAddress([u8; MGO_ADDRESS_LENGTH]);

#[derive(Error, Debug, Eq, PartialEq)]
pub(crate) enum FromStrError {
    #[error("Invalid MgoAddress. Missing 0x prefix.")]
    NoPrefix,

    #[error(
        "Expected MgoAddress string with between 1 and {} digits ({} bytes), received {0}",
        MGO_ADDRESS_LENGTH * 2,
        MGO_ADDRESS_LENGTH,
    )]
    WrongLength(usize),
...
pub(crate) enum FromVecError {
    #[error("Expected MgoAddress with {} bytes, received {0}", MGO_ADDRESS_LENGTH)]
    WrongLength(usize),
}

impl MgoAddress {
    pub fn from_array(arr: [u8; MGO_ADDRESS_LENGTH]) -> Self {
        MgoAddress(arr)
    }
...
impl ScalarType for MgoAddress {
...
impl Description for MgoAddress {
    fn description() -> &'static str {
        "String containing 32B hex-encoded address, with a leading \"0x\". Leading zeroes can be \
         omitted on input but will always appear in outputs (MgoAddress in output is guaranteed \
...
impl From<AccountAddress> for MgoAddress {
    fn from(value: AccountAddress) -> Self {
        MgoAddress(value.into_bytes())
    }
}

impl From<MgoAddress> for AccountAddress {
    fn from(value: MgoAddress) -> Self {
...
impl From<NativeMgoAddress> for MgoAddress {
    fn from(value: NativeMgoAddress) -> Self {
        MgoAddress(value.to_inner())
    }
}

impl From<MgoAddress> for NativeMgoAddress {
    fn from(value: MgoAddress) -> Self {
...
impl FromStr for MgoAddress {
    ...
    fn from_str(s: &str) -> Result<Self, FromStrError> {
        ...
        if s.is_empty() || s.len() > MGO_ADDRESS_LENGTH * 2 {
            return Err(FromStrError::WrongLength(s.len()));
        }

        let mut arr = [0u8; MGO_ADDRESS_LENGTH];
        hex::decode_to_slice(
            // Left pad with `0`-s up to MGO_ADDRESS_LENGTH * 2 characters long.
            format!("{:0>width$}", s, width = MGO_ADDRESS_LENGTH * 2),
            &mut arr[..],
        )
        .map_err(|e| match e {
...
impl std::fmt::Display for MgoAddress {
...
mod tests {
    ...
    const ARR_ADDRESS: [u8; MGO_ADDRESS_LENGTH] = [
        1, 35, 69, 103, 137, 171, 205, 239, 1, 35, 69, 103, 137, 171, 205, 239, 1, 35, 69, 103,
        137, 171, 205, 239, 1, 35, 69, 103, 137, 171, 205, 239,
    ];
    const MGO_ADDRESS: MgoAddress = MgoAddress(ARR_ADDRESS);

    #[test]
    fn test_parse_valid_mgoaddress() {
        let parsed = MgoAddress::from_str(STR_ADDRESS).unwrap();
        assert_eq!(parsed.0, ARR_ADDRESS);
    }

    #[test]
    fn test_to_value() {
        let value = ScalarType::to_value(&MGO_ADDRESS);
        assert_eq!(value, Value::String(STR_ADDRESS.to_string()));
    }

    #[test]
    fn test_from_array() {
        let addr = MgoAddress::from_array(ARR_ADDRESS);
        assert_eq!(addr, MGO_ADDRESS);
    }

    #[test]
    fn test_as_slice() {
        assert_eq!(MGO_ADDRESS.as_slice(), &ARR_ADDRESS);
    }
    ...
    fn test_round_trip() {
        ...
        assert_eq!(MGO_ADDRESS, parsed_back);
    }

    #[test]
    fn test_parse_no_prefix() {
        let err = MgoAddress::from_str(&STR_ADDRESS[2..]).unwrap_err();
    ...
    fn test_parse_invalid_prefix() {
        ...
        let err = MgoAddress::from_str(&input).unwrap_err();
        assert_eq!(FromStrError::NoPrefix, err)
    }
    ...
    fn test_unicode_gibberish() {
        let parsed = MgoAddress::from_str("aAௗ0㌀0");
    ...
    fn bad_scalar_type() {
        ...
        let parsed = <MgoAddress as ScalarType>::parse(input);
        assert!(parsed.is_err());
    }
}
