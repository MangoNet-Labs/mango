...
use mgo_types::transaction::{TransactionData, TransactionKind};
use mgo_types::{gas_coin::GAS, transaction::TransactionDataAPI, TypeTag};
...
pub(crate) type MgoGraphQLSchema = async_graphql::Schema<Query, Mutation, EmptySubscription>;
...
impl Query {
    ...
    async fn owner(&self, address: MgoAddress) -> Option<Owner> {
        Some(Owner {
            address,
    ...
    /// Look-up an Account by its MgoAddress.
    async fn address(&self, address: MgoAddress) -> Option<Address> {
        Some(Address {
            address,
            checkpoint_viewed_at: None,
        })
    ...
    /// `0x2::mgo::MGO`). If no type is provided, it will default to `0x2::mgo::MGO`.
    async fn coins(
        &self,
        ctx: &Context<'_>,
        first: Option<u64>,
    ...
    /// Resolves a MgoNS `domain` name to an address, if it has been bound.
    async fn resolve_mgons_address(
        &self,
        ctx: &Context<'_>,
        domain: Domain,
