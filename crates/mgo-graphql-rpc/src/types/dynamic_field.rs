...
use mgo_indexer::models_v2::objects::StoredHistoryObject;
use mgo_indexer::types_v2::{ObjectStatus, OwnerType};
use mgo_package_resolver::Resolver;
use mgo_types::dynamic_field::{derive_dynamic_field_id, DynamicFieldInfo, DynamicFieldType};

use super::cursor::{Page, Target};
use super::object::{self, deserialize_move_struct, Object, ObjectKind, ObjectLookupKey};
use super::type_filter::ExactTypeFilter;
use super::{
    base64::Base64, move_object::MoveObject, move_value::MoveValue, mgo_address::MgoAddress,
};
...
/// 2) Dynamic Object Fields values must be Mgo objects (have the `key` and `store`
...
impl DynamicField {
    ...
    pub(crate) async fn query(
        ...
        let super_ = MoveObject::query(db, MgoAddress::from(field_id), key).await?;
...
impl TryFrom<MoveObject> for DynamicField {
    ...
    fn try_from(stored: MoveObject) -> Result<Self, Error> {
        ...
        let df_object_id = MgoAddress::from_bytes(df_object_id).map_err(|e| {
...
fn apply_filter(query: RawQuery, parent: MgoAddress, parent_version: Option<u64>) -> RawQuery {
    let query = filter!(
        query,
        format!(
