...
use super::{address::Address, big_int::BigInt, object::ObjectLookupKey, mgo_address::MgoAddress};
use super::{
    cursor::Page,
...
#[Object]
impl GasCostSummary {
    /// Gas paid for executing this transaction (in MANGO).
    async fn computation_cost(&self) -> Option<BigInt> {
        Some(BigInt::from(self.computation_cost))
    }

    /// Gas paid for the data stored on-chain by this transaction (in MANGO).
