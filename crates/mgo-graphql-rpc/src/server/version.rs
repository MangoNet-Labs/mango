...
pub(crate) static VERSION_HEADER: HeaderName = HeaderName::from_static("x-mgo-rpc-version");

pub(crate) struct MgoRpcVersion(Vec<u8>, Vec<Vec<u8>>);

impl headers::Header for MgoRpcVersion {
        ...
        I: Iterator<Item = &'i HeaderValue>,
    {
        ...
        Ok(MgoRpcVersion(value, values.collect()))
    }
...
pub(crate) async fn check_version_middleware<B>(
    version: Option<TypedHeader<MgoRpcVersion>>,
    request: Request<B>,
    next: Next<B>,
) -> Response {
    if let Some(TypedHeader(MgoRpcVersion(req_version, rest))) = version {
        if !rest.is_empty() {
            return (
    ...
    async fn case_insensitive() {
        ...
        let response = service
            .oneshot(header_request(&[(
                &HeaderName::try_from("x-mGo-RpC-vERSION").unwrap(),
                version.as_bytes(),
            )]))
            .await
            .unwrap();
        ...
        let expect = expect![[r#"
            {
              ...
              "errors": [
                {
                  "message": "Failed to parse x-mgo-rpc-version: Multiple possible versions found.",
                  "extensions": {
                    "code": "BAD_REQUEST"
                  }
                }
        ...
        let expect = expect![[r#"
            {
              ...
              "errors": [
                {
                  "message": "Failed to parse x-mgo-rpc-version: 'not-a-version' not a valid <YEAR>.<MONTH> version.",
                  "extensions": {
                    "code": "BAD_REQUEST"
                  }
                }
        ...
        let expect = expect![[r#"
            {
              ...
              "errors": [
                {
                  "message": "Failed to parse x-mgo-rpc-version: Not a UTF8 string.",
                  "extensions": {
                    "code": "BAD_REQUEST"
                  }
                }
