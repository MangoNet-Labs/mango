...
use crate::{
    ...
    types::query::{Query, MgoGraphQLSchema},
};
...
use mgo_package_resolver::{PackageStoreWithLruCache, Resolver};
...
impl ServerBuilder {
    ...
    pub async fn from_config(config: &ServerConfig) -> Result<Self, Error> {
        ...
        let registry_service = mango_metrics::start_prometheus_server(prom_addr);
        ...
        let mgo_sdk_client = if let Some(url) = &config.tx_exec_full_node.node_rpc_url {
            Some(
                MgoClientBuilder::default()
                    ...
                    .build(url)
                    .await
                    .map_err(|e| Error::Internal(format!("Failed to create MgoClient: {}", e)))?,
            )
        } else {
...
async fn graphql_handler(
    ...
    schema: axum::Extension<MgoGraphQLSchema>,
    headers: HeaderMap,
    req: GraphQLRequest,
) -> GraphQLResponse {
...
pub mod tests {
    ...
    fn metrics() -> Metrics {
        ...
        let registry = mango_metrics::start_prometheus_server(binding_address).default_registry();
