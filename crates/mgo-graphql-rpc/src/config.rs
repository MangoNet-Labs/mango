...
use crate::{error::Error as MgoGraphQLError, types::big_int::BigInt};
use async_graphql::*;
...
const DEFAULT_IDE_TITLE: &str = "Mgo GraphQL IDE";
...
pub(crate) const DEFAULT_SERVER_DB_URL: &str =
    "postgres://postgres:postgrespw@localhost:5432/mgo_indexer_v2";
...
impl Limits {
    ...
    pub fn package_resolver_limits(&self) -> mgo_package_resolver::Limits {
        mgo_package_resolver::Limits {
...
impl ConnectionConfig {
    ...
    pub fn ci_integration_test_cfg() -> Self {
        Self {
            db_url: "postgres://postgres:postgrespw@localhost:5432/mgo_indexer_v2".to_string(),
            ..Default::default()
        }
    }
...
impl ServerConfig {
    pub fn from_yaml(path: &str) -> Result<Self, MgoGraphQLError> {
        let contents = std::fs::read_to_string(path).map_err(|e| {
            MgoGraphQLError::Internal(format!(
    ...
    pub fn to_yaml(&self) -> Result<String, MgoGraphQLError> {
        serde_yaml::to_string(&self).map_err(|e| {
            MgoGraphQLError::Internal(format!("Failed to create yaml from cfg: {}", e))
        })
    }

    pub fn to_yaml_file(&self, path: PathBuf) -> Result<(), MgoGraphQLError> {
