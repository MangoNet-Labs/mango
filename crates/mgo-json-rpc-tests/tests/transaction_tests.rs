...
use mgo_json_rpc_types::MgoTransactionBlockResponseQuery;
use mgo_json_rpc_types::TransactionFilter;
use mgo_json_rpc_types::{
    MgoObjectDataOptions, MgoObjectResponseQuery, MgoTransactionBlockResponse,
    MgoTransactionBlockResponseOptions, TransactionBlockBytes,
};
...
use mgo_json_rpc_api::{IndexerApiClient, TransactionBuilderClient, WriteApiClient};
...
async fn test_get_transaction_block() -> Result<(), anyhow::Error> {
    ...
    let mut tx_responses: Vec<MgoTransactionBlockResponse> = Vec::new();
    ...
    // let batch_responses: Vec<MgoTransactionBlockResponse> = http_client
    //     .multi_get_transaction_blocks(tx, Some(MgoTransactionBlockResponseOptions::new()))
    //     .await?;
    ...
    //     assert!(tx_responses
    //         .iter()
    //         .any(|resp| matches!(resp, MgoTransactionBlockResponse {digest, ..} if *digest == r.digest)))
    // }

    // // test get_transaction
    // for tx_digest in tx {
    //     let response: MgoTransactionBlockResponse = http_client
    //         .get_transaction_block(
    //             tx_digest,
    //             Some(MgoTransactionBlockResponseOptions::new().with_raw_input()),
    //         )
    //         .await?;
    //     assert!(tx_responses.iter().any(
    //         |resp| matches!(resp, MgoTransactionBlockResponse {digest, ..} if *digest == response.digest)
    //     ));
...
async fn test_get_fullnode_transaction() -> Result<(), anyhow::Error> {
    ...
    let query = MgoTransactionBlockResponseQuery {
        options: Some(MgoTransactionBlockResponseOptions {
            show_input: true,
            show_effects: true,
            show_events: true,
            ..Default::default()
    ...
    let first_page = client
        ...
        .query_transaction_blocks(
            MgoTransactionBlockResponseQuery::default(),
            None,
            Some(5),
            false,
        )
    ...
    let address_txs_asc = client
        .read_api()
        .query_transaction_blocks(
            MgoTransactionBlockResponseQuery::new_with_filter(TransactionFilter::FromAddress(
    ...
    for tx_resp in tx.data {
        let response: MgoTransactionBlockResponse = client
            .read_api()
            .get_transaction_with_options(tx_resp.digest, MgoTransactionBlockResponseOptions::new())
            .await
            .unwrap();
