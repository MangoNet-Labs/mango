...
use mgo_json::{call_args, type_args};
...
use mgo_json_rpc_types::ObjectChange;
use mgo_json_rpc_types::ObjectsPage;
use mgo_json_rpc_types::{
    Balance, CoinPage, DelegatedStake, StakeStatus, MgoCoinMetadata, MgoExecutionStatus,
    MgoObjectDataOptions, MgoObjectResponse, MgoObjectResponseQuery, MgoTransactionBlockEffectsAPI,
    MgoTransactionBlockResponse, MgoTransactionBlockResponseOptions, TransactionBlockBytes,
};
...
use mgo_types::coin::{TreasuryCap, COIN_MODULE_NAME};
...
use mgo_types::{parse_mgo_struct_tag, MGO_FRAMEWORK_ADDRESS};
...
async fn test_get_package_with_display_should_not_fail() -> Result<(), anyhow::Error> {
    ...
    let response = http_client
        .get_object(
            ObjectID::from(MGO_FRAMEWORK_ADDRESS),
            Some(MgoObjectDataOptions::new().with_display()),
        )
        .await;
    assert!(response.is_ok());
    let response: MgoObjectResponse = response?;
    assert!(response
        .into_object()
        .unwrap()
        .display
...
async fn test_public_transfer_object() -> Result<(), anyhow::Error> {
    ...
    let tx_response: MgoTransactionBlockResponse = http_client
...
async fn test_publish() -> Result<(), anyhow::Error> {
    ...
    let compiled_package = BuildConfig::new_for_testing()
        .build(Path::new("../../mgo_programmability/examples/fungible_tokens").to_path_buf())?;
    ...
    let tx_response = http_client
        .execute_transaction_block(
            tx_bytes,
            signatures,
            Some(MgoTransactionBlockResponseOptions::new().with_effects()),
            Some(ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await?;
    matches!(tx_response, MgoTransactionBlockResponse {effects, ..} if effects.as_ref().unwrap().created().len() == 6);
    Ok(())
}
...
async fn test_move_call() -> Result<(), anyhow::Error> {
    ...
    // now do the call
    let package_id = ObjectID::new(MGO_FRAMEWORK_ADDRESS.into_bytes());
    ...
    let tx_response = http_client
        ...
        .await?;
    matches!(tx_response, MgoTransactionBlockResponse {effects, ..} if effects.as_ref().unwrap().created().len() == 1);
    Ok(())
}
...
async fn test_get_object_info() -> Result<(), anyhow::Error> {
    ...
    for obj in objects {
        ...
        let result = http_client
            .get_object(
                oref.object_id,
                Some(MgoObjectDataOptions::new().with_owner()),
            )
            .await?;
        assert!(
            matches!(result, MgoObjectResponse { data: Some(object), .. } if oref.object_id == object.object_id && object.owner.unwrap().get_owner_address()? == address)
        );
    }
    Ok(())
}
...
async fn test_get_object_data_with_content() -> Result<(), anyhow::Error> {
    ...
    let objects = http_client
        .get_owned_objects(
            address,
            Some(MgoObjectResponseQuery::new_with_options(
                MgoObjectDataOptions::new().with_content().with_owner(),
            )),
            None,
            None,
        )
    ...
    for obj in objects {
        ...
        let result = http_client
            .get_object(
                oref.object_id,
                Some(MgoObjectDataOptions::new().with_content().with_owner()),
            )
            .await?;
        assert!(
            matches!(result, MgoObjectResponse { data: Some(object), .. } if oref.object_id == object.object_id && object.owner.unwrap().get_owner_address()? == address)
...
async fn test_get_coins() -> Result<(), anyhow::Error> {
    ...
    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::mgo::TestCoin".into()), None, None)
        .await?;
    assert_eq!(0, result.data.len());

    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::mgo::MGO".into()), None, None)
        .await?;
    assert_eq!(5, result.data.len());
    assert!(!result.has_next_page);
    ...
    // Test paging
    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::mgo::MGO".into()), None, Some(3))
        .await?;
...
async fn test_get_balance() -> Result<(), anyhow::Error> {
    ...
    assert_eq!("0x2::mgo::MGO", result.coin_type);
    assert_eq!(
...
async fn test_get_metadata() -> Result<(), anyhow::Error> {
    ...
    let result: MgoCoinMetadata = http_client
...
async fn test_get_total_supply() -> Result<(), anyhow::Error> {
    ...
    let treasury_cap = object_changes
        ...
        .find_map(|e| {
            ...
            {
                if &TreasuryCap::type_(parse_mgo_struct_tag(&coin_name).unwrap()) == object_type {
                    Some(object_id)
                } else {
                    None
                }
    ...
    let MgoTransactionBlockResponse { effects, .. } = tx_response;

    assert_eq!(MgoExecutionStatus::Success, *effects.unwrap().status());
...
async fn test_staking() -> Result<(), anyhow::Error> {
    ...
    // Check StakedMgo object before test
    let staked_mgo: Vec<DelegatedStake> = http_client.get_stakes(address).await?;
    assert!(staked_mgo.is_empty());
    ...
    let staked_mgo: Vec<DelegatedStake> = http_client.get_stakes(address).await?;
    assert_eq!(1, staked_mgo.len());
    assert_eq!(1000000000, staked_mgo[0].stakes[0].principal);
    assert!(matches!(
        staked_mgo[0].stakes[0].status,
        StakeStatus::Pending
    ));
    let staked_mgo_copy = http_client
        .get_stakes_by_ids(vec![staked_mgo[0].stakes[0].staked_mgo_id])
        .await?;
    assert_eq!(
        staked_mgo[0].stakes[0].staked_mgo_id,
        staked_mgo_copy[0].stakes[0].staked_mgo_id
    );
    Ok(())
}
...
async fn test_unstaking() -> Result<(), anyhow::Error> {
    ...
    let staked_mgo_copy = http_client
        .get_stakes_by_ids(vec![
            staked_mgo[0].stakes[0].staked_mgo_id,
            staked_mgo[0].stakes[1].staked_mgo_id,
            staked_mgo[0].stakes[2].staked_mgo_id,
        ])
        .await?;

    assert!(matches!(
        &staked_mgo_copy[0].stakes[0].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    assert!(matches!(
        &staked_mgo_copy[0].stakes[1].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    assert!(matches!(
        &staked_mgo_copy[0].stakes[2].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    ...
    let transaction_bytes: TransactionBlockBytes = http_client
        .request_withdraw_stake(
            address,
            staked_mgo_copy[0].stakes[2].staked_mgo_id,
            None,
            1_000_000.into(),
        )
        .await?;
