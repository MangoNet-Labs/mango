...
use mgo_genesis_builder::validator_info::GenesisValidatorMetadata;
use mgo_move_build::{BuildConfig, CompiledPackage};
use mgo_sdk::rpc_types::{
    get_new_package_obj_from_response, MgoTransactionBlockEffectsAPI, MgoTransactionBlockResponse,
};
use mgo_sdk::wallet_context::WalletContext;
use mgo_types::base_types::{ObjectID, ObjectRef, SequenceNumber, MgoAddress};
use mgo_types::crypto::{get_key_pair, AccountKeyPair, Signature, Signer};
...
use mgo_types::{TypeTag, MGO_SYSTEM_PACKAGE_ID};
...
impl TestTransactionBuilder {
    pub fn new(sender: MgoAddress, gas_object: ObjectRef, gas_price: u64) -> Self {
        Self {
    ...
    pub fn call_nft_create(self, package_id: ObjectID) -> Self {
        self.move_call(
            ...
            vec![
                ...
                CallArg::Pure(
                    bcs::to_bytes("https://mangonetwork.io/_nuxt/img/mgo-logo.8d3c44e.svg").unwrap(),
                ),
            ],
        )
    }
    ...
    pub fn call_staking(self, stake_coin: ObjectRef, validator: MgoAddress) -> Self {
        self.move_call(
            MGO_SYSTEM_PACKAGE_ID,
            MGO_SYSTEM_MODULE_NAME.as_str(),
            "request_add_stake",
            vec![
    ...
    pub fn transfer(mut self, object: ObjectRef, recipient: MgoAddress) -> Self {
        self.test_data = TestTransactionData::Transfer(TransferData { object, recipient });
        self
    }

    pub fn transfer_mgo(mut self, amount: Option<u64>, recipient: MgoAddress) -> Self {
        self.test_data = TestTransactionData::TransferMgo(TransferMgoData { amount, recipient });
        self
    }
    ...
    pub fn publish_examples(self, subpath: &'static str) -> Self {
        let path = if let Ok(p) = std::env::var("MOVE_EXAMPLES_DIR") {
            ...
            path.extend(["..", "..", "mgo_programmability", "examples", subpath]);
            path
        };
        self.publish(path)
    }
    ...
    pub fn build(self) -> TransactionData {
        match self.test_data {
            ...
            TestTransactionData::TransferMgo(data) => TransactionData::new_transfer_mgo(
                data.recipient,
                self.sender,
                data.amount,
                self.gas_object,
    ...
    pub fn build_and_sign_multisig(
        ...
        let data = self.build();
        let intent_msg = IntentMessage::new(Intent::mgo_transaction(), data.clone());
...
pub async fn make_transfer_mgo_transaction(
    context: &WalletContext,
    recipient: Option<MgoAddress>,
    amount: Option<u64>,
) -> Transaction {
    ...
    context.sign_transaction(
        &TestTransactionBuilder::new(sender, gas_object, gas_price)
            .transfer_mgo(amount, recipient.unwrap_or(sender))
            .build(),
    )
}
...
pub async fn create_devnet_nft(
    context: &WalletContext,
    package_id: ObjectID,
) -> (MgoAddress, ObjectID, TransactionDigest) {
    let (sender, gas_object) = context.get_one_gas_object().await.unwrap().unwrap();
    let rgp = context.get_reference_gas_price().await.unwrap();
