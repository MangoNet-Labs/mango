...
use mgo_config::{PersistedConfig, MGO_KEYSTORE_FILENAME, MGO_NETWORK_CONFIG};
use mgo_graphql_rpc::config::ConnectionConfig;
use mgo_graphql_rpc::test_infra::cluster::start_graphql_server_with_fn_rpc;
use mgo_indexer::test_utils::{start_test_indexer, start_test_indexer_v2, ReaderWriterConfig};
use mgo_indexer::IndexerConfig;
...
use mgo_swarm_config::genesis_config::GenesisConfig;
...
const DEVNET_FAUCET_ADDR: &str = "https://faucet.devnet.mangonetwork.io:443";
const STAGING_FAUCET_ADDR: &str = "https://faucet.staging.mangonetwork.io:443";
const CONTINUOUS_FAUCET_ADDR: &str = "https://faucet.ci.mangonetwork.io:443";
const CONTINUOUS_NOMAD_FAUCET_ADDR: &str = "https://faucet.nomad.ci.mangonetwork.io:443";
const TESTNET_FAUCET_ADDR: &str = "https://faucet.testnet.mangonetwork.io:443";
const DEVNET_FULLNODE_ADDR: &str = "https://rpc.devnet.mangonetwork.io:443";
const STAGING_FULLNODE_ADDR: &str = "https://fullnode.staging.mangonetwork.io:443";
const CONTINUOUS_FULLNODE_ADDR: &str = "https://fullnode.ci.mangonetwork.io:443";
const CONTINUOUS_NOMAD_FULLNODE_ADDR: &str = "https://fullnode.nomad.ci.mangonetwork.io:443";
const TESTNET_FULLNODE_ADDR: &str = "https://fullnode.testnet.mangonetwork.io:443";
...
impl Cluster for LocalNewCluster {
    async fn start(options: &ClusterTestOpt) -> Result<Self, anyhow::Error> {
        ...
        if let Some(config_dir) = options.config_dir.clone() {
            ...
            let network_config_path = config_dir.join(MGO_NETWORK_CONFIG);
        ...
        let faucet_address = MgoAddress::from(faucet_key.public());
...
pub async fn new_wallet_context_from_cluster(
    ...
    let keystore_path = config_dir.join(MGO_KEYSTORE_FILENAME);
    let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
    let address: MgoAddress = key_pair.public().into();
    keystore
        .add_key(None, MgoKeyPair::Ed25519(key_pair))
        .unwrap();
    MgoClientConfig {
        keystore,
        envs: vec![MgoEnv {
