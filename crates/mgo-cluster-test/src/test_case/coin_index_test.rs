...
use mgo_core::test_utils::compile_managed_coin_package;
use mgo_json::MgoJsonValue;
use mgo_json_rpc_types::ObjectChange;
use mgo_json_rpc_types::MgoTransactionBlockResponse;
use mgo_json_rpc_types::{Balance, MgoTransactionBlockResponseOptions};
...
impl TestCaseImpl for CoinIndexTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        ctx.get_mgo_from_faucet(None).await;
        ...
        assert_eq!(
            ...
            "total_balance: {}, old_total_balance: {}, mgo_balance_change.amount: {}",
            total_balance,
            old_total_balance,
        ...
        let args = vec![
            MgoJsonValue::from_object_id(cap.0),
            MgoJsonValue::new(json!("10000"))?,
            MgoJsonValue::new(json!(account))?,
        ];
        let txn = client
            .transaction_builder()
            .move_call(
        ...
        let mgo_balance_change = balance_changes
            .iter()
            .find(|b| b.coin_type.to_string().contains("MGO"))
            .unwrap();
        ...
        assert_eq!(mgo_balance_change.owner, Owner::AddressOwner(account));
        ...
        assert_eq!(
            total_balance,
            (old_total_balance as i128 + mgo_balance_change.amount) as u128,
            "total_balance: {}, old_total_balance: {}, mgo_balance_change.amount: {}",
            total_balance,
            old_total_balance,
            mgo_balance_change.amount
        );
        old_coin_object_count = coin_object_count;
        ...
        let args = vec![MgoJsonValue::from_object_id(envelope.0)];
        let txn = client
            .transaction_builder()
            .move_call(
                account,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    MgoJsonValue::from_object_id(cap.0),
                    MgoJsonValue::from_object_id(envelope.0),
                ],
                None,
                rgp * 2_000_000,
            )
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    MgoJsonValue::from_object_id(cap.0),
                    MgoJsonValue::from_object_id(managed_coin_id_10k),
                ],
                None,
                rgp * 2_000_000,
            )
        ...
        let mgo_coins = client
            .coin_read_api()
            .get_coins(account, Some(mgo_type_str.into()), None, None)
            .await
            .unwrap()
            .data;
        ...
        assert_eq!(
            // this is only MGO coins at the moment
            mgo_coins,
            client
                .coin_read_api()
        ...
        let mgo_balance = client
            .coin_read_api()
            .get_balance(account, None)
            .await
            .unwrap();
        assert_eq!(
            mgo_balance.total_balance,
            mgo_coins.iter().map(|c| c.balance as u128).sum::<u128>()
        );
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    MgoJsonValue::from_object_id(cap.0),
                    MgoJsonValue::new(json!("5"))?,  // balance = 5
                    MgoJsonValue::new(json!("40"))?, // num = 40
                    MgoJsonValue::new(json!(account))?,
                ],
                None,
                rgp * 2_000_000,
        ...
        assert_eq!(
            ...
            client
                .coin_read_api()
                .get_coins(account, None, None, Some(mgo_coins.len() + 1))
                .await
                .unwrap()
                .data,
        );
        ...
        assert_eq!(mgo_coins.len() + managed_coins.len(), total_coins,);

        let mgo_coins_with_managed_coin_1 = client
            .coin_read_api()
            .get_all_coins(account, None, Some(mgo_coins.len() + 1))
            .await
            .unwrap();
        assert_eq!(
            mgo_coins_with_managed_coin_1.data.len(),
            mgo_coins.len() + 1
        );
        assert_eq!(
            mgo_coins_with_managed_coin_1.next_cursor,
            Some(first_managed_coin)
        );
        assert!(mgo_coins_with_managed_coin_1.has_next_page);
        let cursor = mgo_coins_with_managed_coin_1.next_cursor;
...
async fn add_to_envelope(
    ...
    let txn = client
        ...
        .move_call(
            ...
            vec![],
            vec![
                MgoJsonValue::from_object_id(envelope),
                MgoJsonValue::from_object_id(coin),
            ],
            None,
            rgp * 2_000_000,
        )
