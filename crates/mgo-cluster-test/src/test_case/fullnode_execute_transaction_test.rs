...
use mgo_json_rpc_types::{
    MgoExecutionStatus, MgoTransactionBlockEffectsAPI, MgoTransactionBlockResponseOptions,
};
...
impl FullNodeExecuteTransactionTest {
    async fn verify_transaction(fullnode: &MgoClient, tx_digest: TransactionDigest) {
        fullnode
            .read_api()
            .get_transaction_with_options(tx_digest, MgoTransactionBlockResponseOptions::new())
            .await
            .unwrap_or_else(|e| {
                panic!(
...
impl TestCaseImpl for FullNodeExecuteTransactionTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        let txn_count = 4;
        ctx.get_mgo_from_faucet(Some(1)).await;
        ...
        if !matches!(effects.status(), MgoExecutionStatus::Success { .. }) {
            panic!(
