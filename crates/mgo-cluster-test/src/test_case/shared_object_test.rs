...
use mgo_json_rpc_types::{MgoExecutionStatus, MgoTransactionBlockEffectsAPI};
use mgo_sdk::wallet_context::WalletContext;
use mgo_test_transaction_builder::{increment_counter, publish_basics_package_and_make_counter};
...
impl TestCaseImpl for SharedCounterTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        let mgo_objs = ctx.get_mgo_from_faucet(Some(1)).await;
        assert!(!mgo_objs.is_empty());
