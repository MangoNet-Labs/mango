...
use std::sync::Arc;
use mgo_faucet::CoinInfo;
use mgo_json_rpc_types::{
    MgoExecutionStatus, MgoTransactionBlockEffectsAPI, MgoTransactionBlockResponse,
...
impl TestContext {
    async fn get_mgo_from_faucet(&self, minimum_coins: Option<usize>) -> Vec<GasCoin> {
        let addr = self.get_wallet_address();
        let faucet_response = self.faucet.request_mgo_coins(addr).await;
        ...
        if gas_coins.len() < minimum_coins {
            panic!(
                "Expect to get at least {minimum_coins} Mgo Coins for address {addr}, but only got {}",
                gas_coins.len()
            )
        }
    ...
    fn get_fullnode_client(&self) -> &MgoClient {
        self.client.get_fullnode_client()
    }

    fn clone_fullnode_client(&self) -> MgoClient {
    ...
    async fn get_latest_mgo_system_state(&self) -> MgoSystemStateSummary {
        self.client
            .get_fullnode_client()
            .governance_api()
    ...
    fn get_wallet_address(&self) -> MgoAddress {
    ...
    async fn get_tx_with_retry_times(
        ...
        match self
            ...
            .get_fullnode_client()
            .read_api()
            .get_transaction_with_options(digest, MgoTransactionBlockResponseOptions::new())
            .await
        {
