...
use mgo_json_rpc_types::{BalanceChange, MgoData, MgoObjectData, MgoObjectDataOptions};
use mgo_sdk::MgoClient;
use mgo_types::error::MgoObjectResponseError;
use mgo_types::gas_coin::GasCoin;
use mgo_types::{base_types::ObjectID, object::Owner, parse_mgo_type_tag};
use tracing::{debug, trace};

/// A util struct that helps verify Mgo Object.
...
/// `check_into_gas_coin` expect to get a `MgoObjectData` and `GasCoin`
/// respectfully.
#[derive(Debug)]
...
impl ObjectChecker {
    ...
    pub fn is_mgo_coin(mut self, is_mgo_coin: bool) -> Self {
        self.is_mgo_coin = Some(is_mgo_coin);
        self
    }

    pub async fn check_into_gas_coin(self, client: &MgoClient) -> GasCoin {
        if self.is_mgo_coin == Some(false) {
            panic!("'check_into_gas_coin' shouldn't be called with 'is_mgo_coin' set as false");
        }
        self.is_mgo_coin(true)
            .check(client)
            .await
            .unwrap()
            .into_gas_coin()
    ...
    pub async fn check_into_object(self, client: &MgoClient) -> MgoObjectData {
        self.check(client).await.unwrap().into_object()
    }

    pub async fn check(self, client: &MgoClient) -> Result<CheckerResultObject, anyhow::Error> {
        debug!(?self);
        ...
        match (object_info.data, object_info.error) {
            (None, Some(MgoObjectResponseError::NotExists { object_id })) => {
                panic!(
            ...
            (
                None,
                Some(MgoObjectResponseError::DynamicFieldNotFound {
            ...
            (None, Some(MgoObjectResponseError::DisplayError { error })) => {
                panic!("Display Error: {error:?}");
            }
            (None, None) | (None, Some(MgoObjectResponseError::Unknown)) => {
...
pub struct CheckerResultObject {
    ...
    object: Option<MgoObjectData>,
}

impl CheckerResultObject {
    pub fn new(gas_coin: Option<GasCoin>, object: Option<MgoObjectData>) -> Self {
        Self { gas_coin, object }
    }
    pub fn into_gas_coin(self) -> GasCoin {
        self.gas_coin.unwrap()
    }
    pub fn into_object(self) -> MgoObjectData {
        self.object.unwrap()
    }
}
...
impl BalanceChangeChecker {
    ...
    pub fn coin_type(mut self, coin_type: &str) -> Self {
        self.coin_type = Some(parse_mgo_type_tag(coin_type).unwrap());
        self
    }
