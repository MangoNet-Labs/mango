...
use mgo_cluster_test::{config::ClusterTestOpt, ClusterTest};
...
#[tokio::test]
async fn test_mgo_cluster() {
    use reqwest::StatusCode;
    use mgo_cluster_test::cluster::Cluster;
    use mgo_cluster_test::cluster::LocalNewCluster;
    use mgo_cluster_test::config::Env;
    use mgo_graphql_rpc::client::simple_client::SimpleClient;
    use tokio::time::sleep;
    let fullnode_rpc_port: u16 = 9020;
    let indexer_rpc_port: u16 = 9124;
    let pg_address = "postgres://postgres:postgrespw@localhost:5432/mgo_indexer_v2".to_string();
