...
use mgo_types::{
    ...
    crypto::{SignatureScheme, MgoKeyPair},
    error::MgoError,
};
...
) -> Result<(MgoAddress, MgoKeyPair), MgoError> {
    ...
    match key_scheme {
        SignatureScheme::ED25519 => {
            ...
            let sk = Ed25519PrivateKey::from_bytes(&derived)
                .map_err(|e| MgoError::SignatureKeyGenError(e.to_string()))?;
            let kp: Ed25519KeyPair = sk.into();
            Ok((kp.public().into(), MgoKeyPair::Ed25519(kp)))
        }
        SignatureScheme::Secp256k1 => {
            ...
            Ok((kp.public().into(), MgoKeyPair::Secp256k1(kp)))
        }
        SignatureScheme::Secp256r1 => {
            ...
            Ok((kp.public().into(), MgoKeyPair::Secp256r1(kp)))
        }
        SignatureScheme::BLS12381
        | SignatureScheme::MultiSig
        | SignatureScheme::ZkLoginAuthenticator => Err(MgoError::UnsupportedFeatureError {
...
) -> Result<DerivationPath, MgoError> {
    match key_scheme {
        SignatureScheme::ED25519 => {
            match path {
                Some(p) => {
                    ...
                    if let &[purpose, coin_type, account, change, address] = p.as_ref() {
                            ...
                            && address.is_hardened()
                        {
                            ...
                            Err(MgoError::SignatureKeyGenError("Invalid path".to_string()))
                        }
                    } else {
                        Err(MgoError::SignatureKeyGenError("Invalid path".to_string()))
                    }
                ...
                .parse()
                .map_err(|_| MgoError::SignatureKeyGenError("Cannot parse path".to_string()))?),
            }
        }
        SignatureScheme::Secp256k1 => {
            match path {
...
) -> Result<(MgoAddress, MgoKeyPair, SignatureScheme, String), anyhow::Error> {
