...
use mgo_types::crypto::{
    enum_dispatch, EncodeDecodeBase64, PublicKey, Signature, SignatureScheme, MgoKeyPair,
};
...
pub trait AccountKeystore: Send + Sync {
    fn add_key(&mut self, alias: Option<String>, keypair: MgoKeyPair) -> Result<(), anyhow::Error>;
    fn keys(&self) -> Vec<PublicKey>;
    fn get_key(&self, address: &MgoAddress) -> Result<&MgoKeyPair, anyhow::Error>;

    fn sign_hashed(&self, address: &MgoAddress, msg: &[u8]) -> Result<Signature, signature::Error>;
    ...
    where
        T: Serialize;
    fn addresses(&self) -> Vec<MgoAddress> {
        self.keys().iter().map(|k| k.into()).collect()
    }
    fn addresses_with_alias(&self) -> Vec<(&MgoAddress, &Alias)>;
    ...
    fn get_alias_by_address(&self, address: &MgoAddress) -> Result<String, anyhow::Error>;
    fn get_address_by_alias(&self, alias: String) -> Result<&MgoAddress, anyhow::Error>;
    ...
    ) -> Result<(MgoAddress, String, SignatureScheme), anyhow::Error> {
    ...
    fn import_from_mnemonic(
        ...
        key_scheme: SignatureScheme,
        derivation_path: Option<DerivationPath>,
    ) -> Result<MgoAddress, anyhow::Error> {
...
pub struct FileBasedKeystore {
    keys: BTreeMap<MgoAddress, MgoKeyPair>,
    aliases: BTreeMap<MgoAddress, Alias>,
    path: Option<PathBuf>,
}
...
impl AccountKeystore for FileBasedKeystore {
    ...
    fn add_key(&mut self, alias: Option<String>, keypair: MgoKeyPair) -> Result<(), anyhow::Error> {
        let address: MgoAddress = (&keypair.public()).into();
...
impl FileBasedKeystore {
        ...
        let aliases = if aliases_path.exists() {
            ...
            aliases
                ...
                .map(|alias| {
                    ...
                    key.map(|k| (Into::<MgoAddress>::into(&k), alias))
                })
                .collect::<Result<BTreeMap<_, _>, _>>()
                .map_err(|e| {
                    anyhow!(
    ...
    pub fn key_pairs(&self) -> Vec<&MgoKeyPair> {
...
impl InMemKeystore {
    pub fn new_insecure_for_tests(initial_key_number: usize) -> Self {
        ...
        let keys = (0..initial_key_number)
            ...
            .map(|(ad, k)| (ad, MgoKeyPair::Ed25519(k)))
            .collect::<BTreeMap<MgoAddress, MgoKeyPair>>();
...
mod tests {
    ...
    fn validate_alias_test() {
        ...
        assert!(validate_alias("abc_123.mgo").is_ok());
        // Not allowed
