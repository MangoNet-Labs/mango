...
use mgo_types::crypto::{DefaultHash, SignatureScheme, MgoSignatureInner};
use mgo_types::{
    base_types::{MgoAddress, MGO_ADDRESS_LENGTH},
    crypto::Ed25519MgoSignature,
};
...
fn create_alias_keystore_file_test() {
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("mgo.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
    keystore
...
fn check_reading_aliases_file_correctly() {
    ...
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("mgo.keystore");
    let keystore_path_keep = temp_dir.path().join("mgo.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
    let kp = keystore
        .generate_and_add_new_key(SignatureScheme::ED25519, None, None, None)
        .unwrap();
...
fn keystore_no_aliases() {
    ...
    // mgo.aliases file with the existing address in the mgo.keystore,
...
/// This test confirms rust's implementation of mnemonic is the same with the Mgo Wallet
#[test]
fn mgo_wallet_address_mnemonic_test() -> Result<(), anyhow::Error> {
    let phrase = "result crisp session latin must fruit genuine question prevent start coconut brave speak student dismiss";
    let expected_address =
        MgoAddress::from_str("0x936accb491f0facaac668baaedcf4d0cfc6da1120b66f77fa6a43af718669973")?;
    ...
    assert_eq!(pubkey.flag(), Ed25519MgoSignature::SCHEME.flag());
    ...
    res.copy_from_slice(&AsRef::<[u8]>::as_ref(&g_arr)[..MGO_ADDRESS_LENGTH]);
    let address = MgoAddress::try_from(res.as_slice())?;
