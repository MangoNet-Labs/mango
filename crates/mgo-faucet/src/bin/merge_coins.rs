...
use mgo_config::{mgo_config_dir, MGO_CLIENT_CONFIG};
use mgo_faucet::FaucetError;
...
use mgo_types::{base_types::ObjectID, gas_coin::GasCoin, transaction::Transaction};
use tracing::info;
...
async fn _split_coins_equally(
    ...
    let signature = wallet
        .config
        .keystore
        .sign_secure(&active_address, &tx_data, Intent::mgo_transaction())
        .unwrap();
...
async fn _merge_coins(gas_coin: &str, mut wallet: WalletContext) -> Result<(), anyhow::Error> {
    ...
    for chunk in small_coins.chunks(254) {
        ...
        let tx_data = client
            .transaction_builder()
            .pay_mgo(active_address, coin_vector, target, target_amount, 1000000)
            .await?;
        let signature = wallet
            .config
            .keystore
