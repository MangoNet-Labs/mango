...
use mgo_json_rpc_types::{
    OwnedObjectRef, MgoObjectDataOptions, MgoTransactionBlockEffectsAPI,
...
use mgo_types::{
    base_types::{ObjectID, MgoAddress, TransactionDigest},
    gas_coin::GasCoin,
...
pub struct SimpleFaucet {
    ...
    request_producer: Sender<(Uuid, MgoAddress, Vec<u64>)>,
...
impl SimpleFaucet {
    pub async fn new(
        ...
        let (sender, mut receiver) =
            mpsc::channel::<(Uuid, MgoAddress, Vec<u64>)>(config.max_request_queue_length as usize);
    ...
    async fn sign_and_execute_txn(
        ...
        for_batch: bool,
    ) -> Result<MgoTransactionBlockResponse, FaucetError> {
        let signature = self
            .wallet
            .config
            .keystore
            .sign_secure(&self.active_address, &tx_data, Intent::mgo_transaction())
            .map_err(FaucetError::internal)?;
        ...
        info!(
            ...
            ?coin_id,
            ?uuid,
            "PayMgo transaction in faucet."
        );

        match timeout(
            Duration::from_secs(300),
            self.execute_pay_mgo_txn_with_retries(&tx, coin_id, recipient, uuid),
        )
        .await
        {
            Err(elapsed) => {
                warn!(
                    ...
                    ?coin_id,
                    ?uuid,
                    "Failed to execute PayMgo transactions in faucet after {elapsed}. Coin will \
                     not be reused."
                );
    ...
    async fn transfer_gases(
        ...
        match gas_coin_response {
            GasCoinResponse::ValidGasCoin(coin_id) => {
                let tx_data = self
                    .build_pay_mgo_txn(coin_id, self.active_address, recipient, amounts, gas_cost)
                    .await
                    .map_err(FaucetError::internal)?;
    ...
    async fn execute_pay_mgo_txn_with_retries(
        &self,
        tx: &Transaction,
        coin_id: ObjectID,
        recipient: MgoAddress,
        ...
        loop {
            let res = self.execute_pay_mgo_txn(tx, coin_id, recipient, uuid).await;
            ...
            info!(
                ...
                ?uuid,
                ?retry_delay,
                "PayMgo transaction in faucet failed, previous error: {:?}",
                &res,
            );
    ...
    async fn build_pay_mgo_txn(
        ...
        client
            .transaction_builder()
            .pay_mgo(signer, vec![coin_id], recipients, amounts.to_vec(), budget)
            .await
            .map_err(|e| {
                anyhow::anyhow!(
                    "Failed to build PayMgo transaction for coin {:?}, with err {:?}",
                    coin_id,
                    e
                )
            })
    ...
    async fn check_and_map_transfer_gas_result(
        &self,
        res: MgoTransactionBlockResponse,
        ...
        if created.len() != number_of_coins {
            return Err(FaucetError::CoinAmountTransferredIncorrect(format!(
                "PayMgo Transaction should create exact {:?} new coins, but got {:?}",
    ...
    async fn build_batch_pay_mgo_txn(
        &self,
        coin_id: ObjectID,
        batch_requests: Vec<(Uuid, MgoAddress, Vec<u64>)>,
        signer: MgoAddress,
        budget: u64,
    ) -> Result<TransactionData, anyhow::Error> {
        ...
        let pt = {
            ...
            for (_uuid, recipient, amounts) in batch_requests {
                let recipients = vec![recipient; amounts.len()];
                builder.pay_mgo(recipients, amounts)?;
            }
            builder.finish()
        };
    ...
    async fn check_and_map_batch_transfer_gas_result(
        &self,
        res: MgoTransactionBlockResponse,
        requests: Vec<(Uuid, MgoAddress, Vec<u64>)>,
    ) -> Result<(), FaucetError> {
        // Grab the list of created coins and turn it into a map of destination MgoAddress to Vec<Coins>
        let created = res
            .effects
            .ok_or_else(|| {
                FaucetError::ParseTransactionResponseError(format!(
        ...
        let mut address_coins_map: HashMap<MgoAddress, Vec<OwnedObjectRef>> = HashMap::new();
        ...
        // Assert that the number of times a mgo_address occurs is the number of times the coins
        // come up in the vector.
        let mut request_count: HashMap<MgoAddress, u64> = HashMap::new();
        ...
        for (uuid, addy, amounts) in requests {
            let number_of_coins = amounts.len();
            // Get or insert mgo_address into request count
...
impl Faucet for SimpleFaucet {
    async fn send(
        ...
        info!(uuid = ?id, ?recipient, ?digest, "PayMgo txn succeeded");
...
pub async fn batch_gather(
    request_consumer: &mut Receiver<(Uuid, MgoAddress, Vec<u64>)>,
    requests: &mut Vec<(Uuid, MgoAddress, Vec<u64>)>,
    batch_request_size: u64,
) -> Result<(), FaucetError> {
...
pub async fn batch_transfer_gases(
    ...
    let total_mgo_needed: u64 = requests.iter().flat_map(|(_, _, amounts)| amounts).sum();
    // This loop is utilized to grab a coin that is large enough for the request
    loop {
        let gas_coin_response = faucet
            .prepare_gas_coin(total_mgo_needed + gas_cost, uuid, true)
            .await;

        match gas_coin_response {
            GasCoinResponse::ValidGasCoin(coin_id) => {
                let tx_data = faucet
                    .build_batch_pay_mgo_txn(
                        coin_id,
                        requests.clone(),
                ...
                // mgo address, and instead just fill it with the ZERO address.
                let recipient = MgoAddress::ZERO;
                {
                    // Register the intention to send this transaction before we send it, so that if
                    // faucet fails or we give up before we get a definite response, we have a
                    // chance to retry later.
...
mod tests {
    ...
    use mgo_json_rpc_types::MgoExecutionStatus;
    use mgo_sdk::wallet_context::WalletContext;
    ...
    async fn test_batch_transfer_interface() {
        ...
        let target_addresses: Vec<MgoAddress> = (0..5)
            .map(|_| MgoAddress::random_for_testing_only())
            .collect();
    ...
    async fn test_discard_invalid_gas() {
        ...
        let res = MgoClientCommands::PayAllMgo {
            input_coins: vec![*bad_gas.id()],
            recipient: KeyIdentity::Address(MgoAddress::random_for_testing_only()),
        ...
        if let MgoClientCommandResult::PayAllMgo(response) = res {
            assert!(matches!(
                response.effects.unwrap().status(),
                MgoExecutionStatus::Success
            ));
        } else {
            panic!("PayAllMgo command did not return MgoClientCommandResult::PayAllMgo");
        };
    ...
    async fn test_clear_wal() {
        ...
        let tx_data = faucet
            .build_pay_mgo_txn(coin_id, faucet_address, recipient, &[100], 200_000_000)
            .await
            .map_err(FaucetError::internal)
            .unwrap();
    ...
    async fn test_discard_smaller_amount_gas() {
        ...
        let res = MgoClientCommands::SplitCoin {
            coin_id: *gases[0].id(),
        ...
        let tiny_coin_id = if let MgoClientCommandResult::SplitCoin(resp) = res.unwrap() {
            resp.effects.as_ref().unwrap().created()[0]
                .reference
                .object_id
        } else {
            panic!("split command did not return MgoClientCommandResult::SplitCoin");
        };
    ...
    async fn test_insufficient_balance_will_retry_success() {
        ...
        let destination_address = MgoAddress::random_for_testing_only();
    ...
    async fn test_faucet_no_loop_forever() {
        ...
        let tiny_value = (config.num_coins as u64 * config.amount) + 1;
        let _res = MgoClientCommands::SplitCoin {
            coin_id: *gases[0].id(),
            amounts: Some(vec![tiny_value]),
            gas_budget: 50000000,
            gas: None,
    ...
    async fn test_amounts_transferred_on_batch() {
        ...
        let target_addresses: Vec<MgoAddress> = (0..2)
            .map(|_| MgoAddress::random_for_testing_only())
            .collect();

        // Send 2 coins of 1 mgo each. We
        let coins_sent = 2;
    ...
    async fn get_current_gases(address: MgoAddress, context: &mut WalletContext) -> Vec<GasCoin> {
        // Get the latest list of gas
        let results = MgoClientCommands::Gas {
        ...
        .unwrap();
        match results {
            MgoClientCommandResult::Gas(gases) => gases,
            other => panic!("Expect MgoClientCommandResult::Gas, but got {:?}", other),
        }
    }
}
