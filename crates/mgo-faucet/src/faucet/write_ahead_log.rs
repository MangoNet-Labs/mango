...
use mgo_types::{base_types::ObjectID, transaction::TransactionData};
...
/// Persistent log of transactions paying out mgo from the faucet, keyed by the coin serving the
...
mod tests {
    ...
    fn random_request(coin: ObjectRef) -> (MgoAddress, TransactionData) {
        let gas_price = 1;
        let send = MgoAddress::random_for_testing_only();
        let recv = MgoAddress::random_for_testing_only();
        (
            recv,
