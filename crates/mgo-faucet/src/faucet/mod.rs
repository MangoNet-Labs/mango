...
#[async_trait]
pub trait Faucet {
    /// Send `Coin<MGO>` of the specified amount to the recipient
    async fn send(
        &self,
        id: Uuid,
    ...
    /// Send `Coin<MGO>` of the specified amount to the recipient in a batch request
    async fn batch_send(
        &self,
        id: Uuid,
        recipient: MgoAddress,
...
#[clap(
    name = "Mgo Faucet",
    about = "Faucet for requesting test tokens on Mgo",
