...
impl AuthenticatorTrait for MultiSig {
    fn verify_user_authenticator_epoch(&self, epoch_id: EpochId) -> Result<(), MgoError> {
        ...
        T: Serialize,
    {
        ...
        if MgoAddress::from(&self.multisig_pk) != multisig_address {
        ...
        for (sig, i) in self.sigs.iter().zip(as_indices(self.bitmap)?) {
            let (subsig_pubkey, weight) =
                self.multisig_pk
                    .pk_map
                    .get(i as usize)
                    .ok_or(MgoError::InvalidSignature {
            ...
            let res =
                match sig {
                    CompressedSignature::Ed25519(s) => {
                        ...
                        pk.verify(
                            &digest,
                            &s.try_into().map_err(|_| MgoError::InvalidSignature {
                    ...
                    CompressedSignature::ZkLogin(z) => {
                        let authenticator = ZkLoginAuthenticator::from_bytes(&z.0)
                            .map_err(|_| MgoError::InvalidAuthenticator)?;
                        authenticator
                            .verify_claims(value, MgoAddress::from(subsig_pubkey), verify_params)
            ...
            if res.is_ok() {
                ...
                return res.map_err(|e| MgoError::InvalidSignature {
                    error: format!(
                        "Invalid signature for pk={:?} address={:?} error={:?}",
                        subsig_pubkey.encode_base64(),
                        MgoAddress::from(subsig_pubkey),
                        e.to_string()
                    ),
                });
            }
...
pub fn as_indices(bitmap: u16) -> Result<Vec<u8>, MgoError> {
...
impl MultiSig {
    ...
    pub fn combine(
        ...
        for s in full_sigs {
            ...
            let index = multisig_pk
                .get_index(&pk)
                .ok_or(MgoError::IncorrectSigner {
    ...
    pub fn get_zklogin_sigs(&self) -> Result<Vec<ZkLoginAuthenticator>, MgoError> {
        ...
        authenticator_as_bytes
            .iter()
            .map(|z| {
                ZkLoginAuthenticator::from_bytes(&z.0).map_err(|_| MgoError::InvalidAuthenticator)
            })
            .collect()
    }

    pub fn get_indices(&self) -> Result<Vec<u8>, MgoError> {
...
impl FromStr for MultiSig {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let bytes = Base64::decode(s).map_err(|_| MgoError::InvalidSignature {
            error: "Invalid base64 string".to_string(),
        })?;
        let sig = MultiSig::from_bytes(&bytes).map_err(|_| MgoError::InvalidSignature {
