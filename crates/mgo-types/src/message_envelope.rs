...
impl<T: Message + AuthenticatedMessage> Envelope<T, EmptySignInfo> {
    pub fn verify_signature(&self, verify_params: &VerifyParams) -> MgoResult {
    ...
    ) -> MgoResult<VerifiedEnvelope<T, EmptySignInfo>> {
    ...
    T: Message + Serialize,
{
    ...
    pub fn sign(
        ...
        AuthoritySignInfo::new(epoch, &data, Intent::mgo_app(T::SCOPE), authority, secret)
    }
    ...
    pub fn verify_committee_sigs_only(&self, committee: &Committee) -> MgoResult
    where
        <T as Message>::DigestType: PartialEq,
    {
        self.data.verify_epoch(self.auth_sig().epoch)?;
        self.auth_signature
            .verify_secure(self.data(), Intent::mgo_app(T::SCOPE), committee)
    }
}
    ...
    T: Message + AuthenticatedMessage + Serialize,
{
    ...
    pub fn verify_authenticated(
        ...
        committee: &Committee,
        verify_params: &VerifyParams,
    ) -> MgoResult<VerifiedEnvelope<T, AuthoritySignInfo>> {
...
{
    pub fn verify_authority_signatures(&self, committee: &Committee) -> MgoResult {
        self.data.verify_epoch(self.auth_sig().epoch)?;
        self.auth_signature
            .verify_secure(self.data(), Intent::mgo_app(T::SCOPE), committee)
    }
    ...
    T: Message + AuthenticatedMessage + Serialize,
{
    ...
    pub fn verify_authenticated(
        ...
        committee: &Committee,
        verify_params: &VerifyParams,
    ) -> MgoResult<VerifiedEnvelope<T, AuthorityQuorumSignInfo<S>>> {
        self.verify_signatures_authenticated(committee, verify_params)?;
