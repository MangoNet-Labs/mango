...
use crate::base_types::{AuthorityName, ConciseableName, MgoAddress};
use crate::committee::{Committee, EpochId, StakeUnit};
use crate::error::{MgoError, MgoResult};
use crate::signature::GenericSignature;
use crate::mgo_serde::{Readable, MgoBitmap};
...
pub fn generate_proof_of_possession(
    ...
    AuthoritySignature::new_secure(
        &IntentMessage::new(Intent::mgo_app(IntentScope::ProofOfPossession), msg),
        &DEFAULT_EPOCH_ID,
        keypair,
    )
}
...
/// * accounts to interact with Mgo.
/// * Currently we support eddsa and ecdsa on Mgo.
///
...
impl MgoKeyPair {
    pub fn public(&self) -> PublicKey {
        match self {
            MgoKeyPair::Ed25519(kp) => PublicKey::Ed25519(kp.public().into()),
            MgoKeyPair::Secp256k1(kp) => PublicKey::Secp256k1(kp.public().into()),
            MgoKeyPair::Secp256r1(kp) => PublicKey::Secp256r1(kp.public().into()),
        }
    }
}
...
impl Signer<Signature> for MgoKeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        match self {
            ...
            MgoKeyPair::Secp256r1(kp) => kp.sign(msg),
        }
    }
}
...
impl MgoKeyPair {
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        match self {
            MgoKeyPair::Ed25519(kp) => {
                bytes.extend_from_slice(kp.as_bytes());
            }
            MgoKeyPair::Secp256k1(kp) => {
                bytes.extend_from_slice(kp.as_bytes());
            }
            MgoKeyPair::Secp256r1(kp) => {
                bytes.extend_from_slice(kp.as_bytes());
            }
        }
        bytes
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, eyre::Report> {
        ...
        {
            Ok(x) => match x {
                SignatureScheme::ED25519 => Ok(MgoKeyPair::Ed25519(Ed25519KeyPair::from_bytes(
                    bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                )?)),
                SignatureScheme::Secp256k1 => {
                    Ok(MgoKeyPair::Secp256k1(Secp256k1KeyPair::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?))
                }
                SignatureScheme::Secp256r1 => {
                    Ok(MgoKeyPair::Secp256r1(Secp256r1KeyPair::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?))
                }
    ...
    /// Encode a MgoKeyPair as `flag || privkey` in Bech32 starting with "mgoprivkey" to a string. Note that the pubkey is not encoded.
    pub fn encode(&self) -> Result<String, eyre::Report> {
        Bech32::encode(self.to_bytes(), MGO_PRIV_KEY_PREFIX)
    }

    /// Decode a MgoKeyPair from `flag || privkey` in Bech32 starting with "mgoprivkey" to MgoKeyPair. The public key is computed directly from the private key bytes.
    pub fn decode(value: &str) -> Result<Self, eyre::Report> {
        let bytes = Bech32::decode(value, MGO_PRIV_KEY_PREFIX)?;
        Self::from_bytes(&bytes)
    }
}
...
impl<'de> Deserialize<'de> for MgoKeyPair {
        ...
        D: Deserializer<'de>,
    {
        ...
        MgoKeyPair::decode_base64(&s).map_err(|e| Error::custom(e.to_string()))
    }
}
...
impl ZkLoginPublicIdentifier {
    ...
    pub fn new(iss: &str, address_seed: &str) -> MgoResult<Self> {
        ...
        let address_seed_bytes =
            big_int_str_to_bytes(address_seed).map_err(|_| MgoError::InvalidAddress)?;
...
impl PublicKey {
    ...
    pub fn scheme(&self) -> SignatureScheme {
        match self {
            ...
            PublicKey::Secp256r1(_) => Secp256r1MgoSignature::SCHEME,
            PublicKey::ZkLogin(_) => SignatureScheme::ZkLoginAuthenticator,
        }
    }

    pub fn from_zklogin_inputs(inputs: &ZkLoginInputs) -> MgoResult<Self> {
...
pub trait MgoAuthoritySignature {
    fn verify_secure<T>(
        &self,
...
impl MgoAuthoritySignature for AuthoritySignature {
...
pub fn get_key_pair<KP: KeypairTraits>() -> (MgoAddress, KP)
where
    <KP as KeypairTraits>::PubKey: MgoPublicKey,
{
...
pub fn random_committee_key_pairs_of_size(size: usize) -> Vec<AuthorityKeyPair> {
    ...
    (0..size)
        .map(|_| {
            ...
            // keys in ConfigBuilder::build (mgo-config/src/network_config_builder). This is because
...
pub fn deterministic_random_account_key() -> (MgoAddress, AccountKeyPair) {
    let mut rng = StdRng::from_seed([0; 32]);
    get_key_pair_from_rng(&mut rng)
}

pub fn get_account_key_pair() -> (MgoAddress, AccountKeyPair) {
    get_key_pair()
}

pub fn get_authority_key_pair() -> (MgoAddress, AuthorityKeyPair) {
    get_key_pair()
}

/// Generate a keypair from the specified RNG (useful for testing with seedable rngs).
pub fn get_key_pair_from_rng<KP: KeypairTraits, R>(csprng: &mut R) -> (MgoAddress, KP)
where
...
// TODO: C-GETTER
pub fn get_key_pair_from_bytes<KP: KeypairTraits>(bytes: &[u8]) -> MgoResult<(MgoAddress, KP)>
where
    <KP as KeypairTraits>::PubKey: MgoPublicKey,
{
    ...
    if bytes.len() != priv_length + pub_key_length {
        return Err(MgoError::KeyConversionError(format!(
    ...
    let sk = <KP as KeypairTraits>::PrivKey::from_bytes(
        bytes
            .get(..priv_length)
            .ok_or(MgoError::InvalidPrivateKey)?,
    )
...
impl AsRef<[u8]> for Signature {
    fn as_ref(&self) -> &[u8] {
        match self {
            ...
            Signature::Secp256r1MgoSignature(sig) => sig.as_ref(),
        }
    }
}
impl AsMut<[u8]> for Signature {
    fn as_mut(&mut self) -> &mut [u8] {
        match self {
            Signature::Ed25519MgoSignature(sig) => sig.as_mut(),
            Signature::Secp256k1MgoSignature(sig) => sig.as_mut(),
            Signature::Secp256r1MgoSignature(sig) => sig.as_mut(),
        }
    }
}
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        match bytes.first() {
            Some(x) => {
                ...
                } else if x == &Secp256r1MgoSignature::SCHEME.flag() {
                    Ok(<Secp256r1MgoSignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else {
...
impl MgoPublicKey for BLS12381PublicKey {
...
impl Default for Ed25519MgoSignature {
    fn default() -> Self {
...
//
// Secp256r1 Mgo Signature port
//
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash, AsRef, AsMut)]
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Secp256r1MgoSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
    [u8; Secp256r1PublicKey::LENGTH + Secp256r1Signature::LENGTH + 1],
);

impl MgoSignatureInner for Secp256r1MgoSignature {
...
impl MgoPublicKey for Secp256r1PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::Secp256r1;
}

impl ToFromBytes for Secp256r1MgoSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        if bytes.len() != Self::LENGTH {
            return Err(FastCryptoError::InputLengthWrong(Self::LENGTH));
        }
...
impl Signer<Signature> for Secp256r1KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Secp256r1MgoSignature::new(self, msg).into()
    }
}
...
pub trait MgoSignatureInner: Sized + ToFromBytes + PartialEq + Eq + Hash {
    ...
    fn get_verification_inputs(&self) -> MgoResult<(Self::Sig, Self::PubKey)> {
...
impl<S: MgoSignatureInner + Sized> MgoSignature for S {
        ...
        T: Serialize,
    {
        ...
        match scheme {
            ...
            _ => {
                let address = MgoAddress::from(pk);
                if author != address {
...
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema)]
pub struct MgoAuthorityStrongQuorumSignInfo {
    pub epoch: EpochId,
...
impl From<&AuthorityStrongQuorumSignInfo> for MgoAuthorityStrongQuorumSignInfo {
...
impl TryFrom<&MgoAuthorityStrongQuorumSignInfo> for AuthorityStrongQuorumSignInfo {
    type Error = FastCryptoError;

    fn try_from(info: &MgoAuthorityStrongQuorumSignInfo) -> Result<Self, Self::Error> {
        Ok(Self {
            epoch: info.epoch,
...
//
// see also https://github.com/MangoNetworkOs/Mango/issues/266
    ...
    for AuthorityQuorumSignInfo<STRONG_THRESHOLD>
{
    ...
    fn add_to_verification_obligation<'a>(
        ...
        obligation
            .signatures
            .get_mut(message_index)
            .ok_or(MgoError::InvalidAuthenticator)?
        ...
        for authority_index in self.signers_map.iter() {
            let authority = committee
                ...
                .ok_or_else(|| MgoError::UnknownSigner {
                    signer: None,
        ...
        fp_ensure!(
            ...
            MgoError::CertificateRequiresQuorum
        );
...
impl<'a> VerificationObligation<'a> {
    ...
    pub fn verify_all(self) -> MgoResult<()> {
...
pub mod bcs_signable_test {
        ...
        T: super::bcs_signable::BcsSignable,
    {
        ...
        let idx = obligation.add_message(
            value,
            0,
            Intent::mgo_app(IntentScope::SenderSignedTransaction),
        );
        (obligation, idx)
    }
}
...
pub enum SignatureScheme {
    ...
    Secp256k1,
    Secp256r1,
    BLS12381, // This is currently not supported for user Mgo Address.
    MultiSig,
...
impl SignatureScheme {
    pub fn flag(&self) -> u8 {
        match self {
            ...
            SignatureScheme::BLS12381 => 0x04, // This is currently not supported for user Mgo Address.
            SignatureScheme::ZkLoginAuthenticator => 0x05,
        }
    }

    pub fn from_flag(flag: &str) -> Result<SignatureScheme, MgoError> {
        let byte_int = flag
            .parse::<u8>()
            .map_err(|_| MgoError::KeyConversionError("Invalid key scheme".to_string()))?;
        Self::from_flag_byte(&byte_int)
    }

    pub fn from_flag_byte(byte_int: &u8) -> Result<SignatureScheme, MgoError> {
        match byte_int {
            ...
            _ => Err(MgoError::KeyConversionError(
