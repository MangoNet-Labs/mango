...
use crate::{error::MgoError, mgo_serde::Readable};
...
impl TryFrom<Vec<u8>> for Digest {
    ...
    fn try_from(bytes: Vec<u8>) -> Result<Self, MgoError> {
        let bytes: [u8; 32] =
            <[u8; 32]>::try_from(&bytes[..]).map_err(|_| MgoError::InvalidDigestLength {
                expected: 32,
                actual: bytes.len(),
            })?;
...
const MGO_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME: &str = "MGO_PROTOCOL_CONFIG_CHAIN_OVERRIDE";

static MGO_PROTOCOL_CONFIG_CHAIN_OVERRIDE: Lazy<Option<Chain>> = Lazy::new(|| {
    if let Ok(s) = env::var(MGO_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME) {
        info!("MGO_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {:?}", s);
        match s.as_str() {
            "mainnet" => Some(Chain::Mainnet),
            "testnet" => Some(Chain::Testnet),
            "" => None,
            _ => panic!("unrecognized MGO_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {s:?}"),
        }
    } else {
        None
    }
...
impl ChainIdentifier {
    pub fn chain(&self) -> Chain {
        ...
        if let Some(override_chain) = *MGO_PROTOCOL_CONFIG_CHAIN_OVERRIDE {
...
pub fn get_mainnet_chain_identifier() -> ChainIdentifier {
    let digest = MAINNET_CHAIN_IDENTIFIER.get_or_init(|| {
        let digest = CheckpointDigest::new(
            Base58::decode("GULjaLVeBQ9dtAj5gVe4hH2qWxamzGBuT72vsDtge6ad")
...
pub fn get_testnet_chain_identifier() -> ChainIdentifier {
    let digest = TESTNET_CHAIN_IDENTIFIER.get_or_init(|| {
        let digest = CheckpointDigest::new(
            Base58::decode("ACaDn8ue58z3q7ujnBsDo8SPQTe1SooUH3nsNQXniW1M")
...
impl TransactionDigest {
    ...
    // TODO(https://github.com/MangoNetworkOs/Mango/issues/65): we can pick anything here
...
impl TryFrom<&[u8]> for TransactionDigest {
    type Error = crate::error::MgoError;

    fn try_from(bytes: &[u8]) -> Result<Self, crate::error::MgoError> {
        let arr: [u8; 32] = bytes
            .try_into()
            .map_err(|_| crate::error::MgoError::InvalidTransactionDigest)?;
        Ok(Self::new(arr))
    }
}
