...
pub struct MgoBitmap;

impl SerializeAs<roaring::RoaringBitmap> for MgoBitmap {
...
impl<'de> DeserializeAs<'de, roaring::RoaringBitmap> for MgoBitmap {
...
pub struct MgoStructTag;

impl SerializeAs<StructTag> for MgoStructTag {
    fn serialize_as<S>(value: &StructTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let f = to_mgo_struct_tag_string(value).map_err(S::Error::custom)?;
...
pub fn to_mgo_struct_tag_string(value: &StructTag) -> Result<String, fmt::Error> {
    let mut f = String::new();
    // trim leading zeros if address is in MGO_ADDRESSES
    let address = if MGO_ADDRESSES.contains(&value.address) {
    ...
    if let Some(first_ty) = value.type_params.first() {
        write!(f, "<")?;
        write!(f, "{}", to_mgo_type_tag_string(first_ty)?)?;
        for ty in value.type_params.iter().skip(1) {
            write!(f, ", {}", to_mgo_type_tag_string(ty)?)?;
        }
        write!(f, ">")?;
    }
    Ok(f)
...
fn to_mgo_type_tag_string(value: &TypeTag) -> Result<String, fmt::Error> {
    match value {
        TypeTag::Vector(t) => Ok(format!("vector<{}>", to_mgo_type_tag_string(t)?)),
        TypeTag::Struct(s) => to_mgo_struct_tag_string(s),
        _ => Ok(value.to_string()),
    }
}

impl<'de> DeserializeAs<'de, StructTag> for MgoStructTag {
    fn deserialize_as<D>(deserializer: D) -> Result<StructTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_mgo_struct_tag(&s).map_err(D::Error::custom)
    }
}

pub struct MgoTypeTag;

impl SerializeAs<TypeTag> for MgoTypeTag {
    fn serialize_as<S>(value: &TypeTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s = to_mgo_type_tag_string(value).map_err(S::Error::custom)?;
        s.serialize(serializer)
    }
}

impl<'de> DeserializeAs<'de, TypeTag> for MgoTypeTag {
    fn deserialize_as<D>(deserializer: D) -> Result<TypeTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_mgo_type_tag(&s).map_err(D::Error::custom)
    }
}
