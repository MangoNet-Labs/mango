...
use crate::crypto::{
    ...
    DefaultHash, Ed25519MgoSignature, EmptySignInfo, Signature, Signer, MgoSignatureInner,
    ToFromBytes,
};
...
use crate::{
    MGO_AUTHENTICATOR_STATE_OBJECT_ID, MGO_AUTHENTICATOR_STATE_OBJECT_SHARED_VERSION,
    MGO_CLOCK_OBJECT_ID, MGO_CLOCK_OBJECT_SHARED_VERSION, MGO_FRAMEWORK_PACKAGE_ID,
    MGO_RANDOMNESS_STATE_OBJECT_ID, MGO_SYSTEM_STATE_OBJECT_ID,
    MGO_SYSTEM_STATE_OBJECT_SHARED_VERSION,
};
...
impl CallArg {
    pub const MGO_SYSTEM_MUT: Self = Self::Object(ObjectArg::MGO_SYSTEM_MUT);
    pub const CLOCK_IMM: Self = Self::Object(ObjectArg::SharedObject {
        id: MGO_CLOCK_OBJECT_ID,
        initial_shared_version: MGO_CLOCK_OBJECT_SHARED_VERSION,
        mutable: false,
    });
    ...
    pub const AUTHENTICATOR_MUT: Self = Self::Object(ObjectArg::SharedObject {
        id: MGO_AUTHENTICATOR_STATE_OBJECT_ID,
        initial_shared_version: MGO_AUTHENTICATOR_STATE_OBJECT_SHARED_VERSION,
        mutable: true,
    });
}
...
impl EndOfEpochTransactionKind {
        ...
        match self {
            Self::ChangeEpoch(_) => {
                vec![InputObjectKind::SharedMoveObject {
                    id: MGO_SYSTEM_STATE_OBJECT_ID,
                    initial_shared_version: MGO_SYSTEM_STATE_OBJECT_SHARED_VERSION,
                    mutable: true,
                }]
            }
    ...
    fn shared_input_objects(&self) -> impl Iterator<Item = SharedInputObject> + '_ {
        match self {
            Self::ChangeEpoch(_) => Either::Left(iter::once(SharedInputObject::MGO_SYSTEM_OBJ)),
...
impl VersionedProtocolMessage for TransactionKind {
    fn check_version_supported(&self, protocol_config: &ProtocolConfig) -> MgoResult {
        ...
        match &self {
            ...
            TransactionKind::AuthenticatorStateUpdate(_) => {
                if protocol_config.enable_jwk_consensus_updates() {
                    ...
                    Err(MgoError::UnsupportedFeatureError {
...
impl ObjectArg {
    pub const MGO_SYSTEM_MUT: Self = Self::SharedObject {
        id: MGO_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: MGO_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutable: true,
    };
...
impl SharedInputObject {
    pub const MGO_SYSTEM_OBJ: Self = Self {
        id: MGO_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: MGO_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutable: true,
    };
...
impl TransactionKind {
    ...
    pub fn shared_input_objects(&self) -> impl Iterator<Item = SharedInputObject> + '_ {
        match &self {
            Self::ChangeEpoch(_) => {
                Either::Left(Either::Left(iter::once(SharedInputObject::MGO_SYSTEM_OBJ)))
            }
...
impl VersionedProtocolMessage for TransactionData {
    ...
    fn check_version_supported(&self, protocol_config: &ProtocolConfig) -> MgoResult {
        ...
        if !supported.is_version_supported(protocol_config.version) {
            return Err(MgoError::WrongMessageVersion {
                error: format!(
...
impl TransactionData {
    ...
    pub fn new_with_gas_data(kind: TransactionKind, sender: MgoAddress, gas_data: GasData) -> Self {
        TransactionData::V1(TransactionDataV1 {
            kind,
            sender,
            gas_data,
    ...
    pub fn new_transfer_mgo(
        ...
        Self::new_transfer_mgo_allow_sponsor(
            recipient,
            sender,
            amount,
            gas_payment,
    ...
    pub fn new_transfer_mgo_allow_sponsor(
        recipient: MgoAddress,
        sender: MgoAddress,
        amount: Option<u64>,
        gas_payment: ObjectRef,
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.transfer_mgo(recipient, amount);
            builder.finish()
        };
    ...
    pub fn execution_parts(&self) -> (TransactionKind, MgoAddress, Vec<ObjectRef>) {
        (
            self.kind().clone(),
            self.sender(),
...
pub trait TransactionDataAPI {
    ...
    fn signers(&self) -> NonEmpty<MgoAddress>;

    fn gas_data(&self) -> &GasData;

    fn gas_owner(&self) -> MgoAddress;
...
impl SenderSignedData {
    ...
    ) -> MgoResult<BTreeMap<MgoAddress, &GenericSignature>> {
        ...
        for sig in &self.inner().tx_signatures {
            if verify_legacy_zklogin_address {
                ...
                if let GenericSignature::ZkLoginAuthenticator(z) = sig {
                    mapping.insert(MgoAddress::try_from_padded(&z.inputs)?, sig);
                };
            }
    ...
    pub fn serialized_size(&self) -> MgoResult<usize> {
        bcs::serialized_size(self).map_err(|e| MgoError::TransactionSerializationError {
            error: e.to_string(),
        })
    }
    ...
    pub fn validity_check(&self, config: &ProtocolConfig) -> MgoResult {
...
impl Message for SenderSignedData {
    ...
    fn verify_user_input(&self) -> MgoResult {
        ...
        fp_ensure!(
            ...
            MgoError::SignerSignatureNumberMismatch {
        ...
        for s in signers {
            if !present_sigs.contains_key(&s) {
                return Err(MgoError::SignerSignatureAbsent {
    ...
    fn verify_epoch(&self, epoch: EpochId) -> MgoResult {
        for sig in &self.inner().tx_signatures {
...
impl AuthenticatedMessage for SenderSignedData {
    ...
    fn verify_uncached_checks(&self, verify_params: &VerifyParams) -> MgoResult {
    ...
    fn verify_message_signature(&self, verify_params: &VerifyParams) -> MgoResult {
        fp_ensure!(
            self.0.len() == 1,
            MgoError::UserInputError {
                error: UserInputError::Unsupported(
...
impl<S> Envelope<SenderSignedData, S> {
    pub fn sender_address(&self) -> MgoAddress {
...
impl Transaction {
    pub fn from_data_and_signer(
        ...
        let signatures = {
            let intent_msg = IntentMessage::new(Intent::mgo_transaction(), &data);
            signers
                .into_iter()
        ...
        system_transaction
            ...
            .pipe(|data| {
                SenderSignedData::new_from_sender_signature(
                    data,
                    Intent::mgo_transaction(),
                    Ed25519MgoSignature::from_bytes(&[0; Ed25519MgoSignature::LENGTH])
                        .unwrap()
                        .into(),
                )
            })
...
// An InputObjects new-type that has been verified by mgo-transaction-checks, and can be
// safely passed to execution.
pub struct CheckedInputObjects(InputObjects);

// DO NOT CALL outside of mgo-transaction-checks, genesis, or replay.
//
// CheckedInputObjects should really be defined in mgo-transaction-checks so that we can
// make public construction impossible. But we can't do that because it would result in circular
// dependencies.
impl CheckedInputObjects {
    // Only called by mgo-transaction-checks.
