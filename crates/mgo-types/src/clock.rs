...
use crate::{id::UID, MGO_FRAMEWORK_ADDRESS};

pub const CLOCK_MODULE_NAME: &IdentStr = ident_str!("clock");
pub const CLOCK_STRUCT_NAME: &IdentStr = ident_str!("Clock");
pub const RESOLVED_MGO_CLOCK: (&AccountAddress, &IdentStr, &IdentStr) =
    (&MGO_FRAMEWORK_ADDRESS, CLOCK_MODULE_NAME, CLOCK_STRUCT_NAME);
...
impl Clock {
    ...
    /// Detects a `&mut mgo::clock::Clock` or `mgo::clock::Clock` in the signature.
    pub fn is_mutable(view: &BinaryIndexedView<'_>, s: &SignatureToken) -> bool {
        use SignatureToken as S;
        match s {
            S::MutableReference(inner) => Self::is_mutable(view, inner),
            S::Struct(idx) => resolve_struct(view, *idx) == RESOLVED_MGO_CLOCK,
            _ => false,
        }
    }
}
