...
use crate::{
    ...
    crypto::DefaultHash,
    error::{ExecutionError, ExecutionErrorKind, MgoError, MgoResult},
    id::{ID, UID},
...
/// Rust representation of upgrade policy constants in `mgo::package`.
#[repr(u8)]
...
/// Rust representation of `mgo::package::UpgradeCap`.
...
/// Rust representation of `mgo::package::UpgradeTicket`.
#[derive(Debug, Serialize, Deserialize)]
...
/// Rust representation of `mgo::package::UpgradeReceipt`.
#[derive(Debug, Serialize, Deserialize)]
...
impl MovePackage {
    ...
    pub fn deserialize_module(
    ...
    ) -> MgoResult<CompiledModule> {
        // TODO use the session's cache
        let bytes = self
            .serialized_module_map()
            .get(module.as_str())
            .ok_or_else(|| MgoError::ModuleNotFound {
        ...
        .map_err(|error| MgoError::ModuleDeserializationFailure {
            error: error.to_string(),
        })
    }

    pub fn disassemble(&self) -> MgoResult<BTreeMap<String, Value>> {
    ...
    pub fn normalize(
        ...
        max_binary_format_version: u32,
        check_no_bytes_remaining: bool,
    ) -> MgoResult<BTreeMap<String, normalized::Module>> {
        normalize_modules(
            self.module_map.values(),
            max_binary_format_version,
            check_no_bytes_remaining,
...
pub fn disassemble_modules<'a, I>(modules: I) -> MgoResult<BTreeMap<String, Value>>
where
    I: Iterator<Item = &'a Vec<u8>>,
{
    ...
    for bytecode in modules {
        ...
        let module = CompiledModule::deserialize_with_defaults(bytecode).map_err(|error| {
            MgoError::ModuleDeserializationFailure {
                error: error.to_string(),
            }
        })?;
        ...
        let bytecode_str = d
            .disassemble()
            .map_err(|e| MgoError::ObjectSerializationError {
                error: e.to_string(),
            })?;
