...
use crate::base_types::{SequenceNumber, MgoAddress};
...
use crate::MGO_DENY_LIST_OBJECT_ID;
...
impl DenyList {
    ...
    fn check_deny_list(
        ...
        let Ok(count) = get_dynamic_field_from_store::<MgoAddress, u64>(
            object_store,
        ...
        for coin_type in coin_types {
            let Ok(denied_addresses) = get_dynamic_field_from_store::<Vec<u8>, VecSet<MgoAddress>>(
                object_store,
...
pub fn get_deny_list_root_object(object_store: &dyn ObjectStore) -> Option<Object> {
    match object_store.get_object(&MGO_DENY_LIST_OBJECT_ID) {
