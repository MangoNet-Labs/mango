...
pub use mgo_protocol_config::ProtocolVersion;
...
impl Committee {
    ...
    pub fn public_key(&self, authority: &AuthorityName) -> MgoResult<&AuthorityPublicKey> {
        debug_assert_eq!(self.expanded_keys.len(), self.voting_rights.len());
        match self.expanded_keys.get(authority) {
            Some(v) => Ok(v),
            None => Err(MgoError::InvalidCommittee(format!(
