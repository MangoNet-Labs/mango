...
use crate::crypto::{
    CompressedSignature, PublicKey, SignatureScheme, MgoSignature, ZkLoginAuthenticatorAsBytes,
};
use crate::error::MgoError;
use crate::multisig_legacy::MultiSigLegacy;
use crate::zk_login_authenticator::ZkLoginAuthenticator;
use crate::{base_types::MgoAddress, crypto::Signature, error::MgoResult, multisig::MultiSig};
...
pub trait AuthenticatorTrait {
    fn verify_user_authenticator_epoch(&self, epoch: EpochId) -> MgoResult;
...
impl GenericSignature {
    ...
    /// Parse [enum CompressedSignature] from trait MgoSignature `flag || sig || pk`.
    /// This is useful for the MultiSig to combine partial signature into a MultiSig public key.
    pub fn to_compressed(&self) -> Result<CompressedSignature, MgoError> {
        match self {
            GenericSignature::Signature(s) => {
                ...
                match s.scheme() {
                    ...
                    _ => Err(MgoError::UnsupportedFeatureError {
    ...
    /// Parse [struct PublicKey] from trait MgoSignature `flag || sig || pk`.
    /// This is useful for the MultiSig to construct the bitmap in [struct MultiPublicKey].
    pub fn to_public_key(&self) -> Result<PublicKey, MgoError> {
        match self {
            GenericSignature::Signature(s) => {
                ...
                match s.scheme() {
                    SignatureScheme::ED25519 => Ok(PublicKey::Ed25519(
                        (&Ed25519PublicKey::from_bytes(bytes).map_err(|_| {
                            MgoError::KeyConversionError("Cannot parse pk".to_string())
                        })?)
                            .into(),
                    )),
...
impl AuthenticatorTrait for Signature {
    fn verify_user_authenticator_epoch(&self, _: EpochId) -> MgoResult {
        Ok(())
    }
    fn verify_uncached_checks<T>(
        &self,
