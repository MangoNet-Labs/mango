...
pub enum UserInputError {
    ...
    #[error("Gas price cannot exceed {:?} mango", max_gas_price)]
    ...
    #[error("MGO payment transactions use first input coin for gas payment, but found a different gas object.")]
...
pub enum MgoObjectResponseError {
    ...
    // TODO: also integrate MgoPastObjectResponse (VersionNotFound,  VersionTooHigh)
}

/// Custom error type for Mgo.
#[derive(
    Eq, PartialEq, Clone, Debug, Serialize, Deserialize, Error, Hash, AsRefStr, IntoStaticStr,
)]
pub enum MgoError {
    ...
    MgoObjectResponseError { error: MgoObjectResponseError },
    ...
    TooManyIncorrectAuthorities {
        errors: Vec<(AuthorityName, MgoError)>,
        action: String,
    },
    ...
    #[error("Failed to execute handle_consensus_transaction on Mgo: {0}")]
    ...
    MgoSystemStateReadError(String),
...
impl From<mgo_protocol_config::Error> for MgoError {
    fn from(error: mgo_protocol_config::Error) -> Self {
        MgoError::WrongMessageVersion { error: error.0 }
    }
}

impl From<ExecutionError> for MgoError {
    fn from(error: ExecutionError) -> Self {
        MgoError::ExecutionError(error.to_string())
    }
}

impl From<Status> for MgoError {
    fn from(status: Status) -> Self {
        let result = bcs::from_bytes::<MgoError>(status.details());
        if let Ok(mgo_error) = result {
            mgo_error
        } else {
            Self::RpcError(
...
impl From<TypedStoreError> for MgoError {
    fn from(e: TypedStoreError) -> Self {
        Self::Storage(e.to_string())
    }
}

impl From<crate::storage::error::Error> for MgoError {
    fn from(e: crate::storage::error::Error) -> Self {
        Self::Storage(e.to_string())
    }
}

impl From<MgoError> for Status {
    fn from(error: MgoError) -> Self {
        let bytes = bcs::to_bytes(&error).unwrap();
        Status::with_details(tonic::Code::Internal, error.to_string(), bytes.into())
    }
}

impl From<ExecutionErrorKind> for MgoError {
    fn from(kind: ExecutionErrorKind) -> Self {
        ExecutionError::from_kind(kind).into()
    }
}

impl From<&str> for MgoError {
    fn from(error: &str) -> Self {
        MgoError::GenericAuthorityError {
            error: error.to_string(),
        }
    }
}

// impl From<FastCryptoError> for MgoError {
//     fn from(kind: FastCryptoError) -> Self {
//         match kind {
//             FastCryptoError::InvalidSignature => MgoError::InvalidSignature {
//                 error: "Invalid signature".to_string(),
//             },
//             _ => MgoError::Unknown("Unknown cryptography error".to_string()),
//         }
//     }
// }

impl TryFrom<MgoError> for UserInputError {
    type Error = anyhow::Error;

    fn try_from(err: MgoError) -> Result<Self, Self::Error> {
        match err {
            MgoError::UserInputError { error } => Ok(error),
            other => anyhow::bail!("error {:?} is not UserInputError", other),
        }
    }
}

impl From<UserInputError> for MgoError {
    fn from(error: UserInputError) -> Self {
        MgoError::UserInputError { error }
    }
}

impl From<MgoObjectResponseError> for MgoError {
    fn from(error: MgoObjectResponseError) -> Self {
        MgoError::MgoObjectResponseError { error }
    }
}
...
impl MgoError {
    pub fn individual_error_indicates_epoch_change(&self) -> bool {
        matches!(
            self,
            MgoError::ValidatorHaltedAtEpochEnd | MgoError::MissingCommitteeAtEpoch(_)
        )
    }
    ...
    pub fn is_retryable(&self) -> (bool, bool) {
        match self {
            // Network error
            MgoError::RpcError { .. } => (true, true),

            // Reconfig error
            MgoError::ValidatorHaltedAtEpochEnd => (true, true),
            MgoError::MissingCommitteeAtEpoch(..) => (true, true),
            MgoError::WrongEpoch { .. } => (true, true),
            ...
            MgoError::PotentiallyTemporarilyInvalidSignature { .. } => (true, true),

            // Overload errors
            MgoError::TooManyTransactionsPendingExecution { .. } => (true, true),
            MgoError::TooManyTransactionsPendingOnObject { .. } => (true, true),
            MgoError::TooOldTransactionPendingOnObject { .. } => (true, true),
            MgoError::TooManyTransactionsPendingConsensus => (true, true),

            // Non retryable error
            MgoError::ExecutionError(..) => (false, true),
            MgoError::ByzantineAuthoritySuspicion { .. } => (false, true),
            MgoError::QuorumFailedToGetEffectsQuorumWhenProcessingTransaction { .. } => {
                (false, true)
            }
            MgoError::TxAlreadyFinalizedWithDifferentUserSigs => (false, true),
            MgoError::FailedToVerifyTxCertWithExecutedEffects { .. } => (false, true),
            MgoError::ObjectLockConflict { .. } => (false, true),
    ...
    pub fn is_overload(&self) -> bool {
        matches!(
            ...
            MgoError::TooManyTransactionsPendingExecution { .. }
                | MgoError::TooManyTransactionsPendingOnObject { .. }
                | MgoError::TooOldTransactionPendingOnObject { .. }
                | MgoError::TooManyTransactionsPendingConsensus
        )
    }
}

impl Ord for MgoError {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        Ord::cmp(self.as_ref(), other.as_ref())
    }
}

impl PartialOrd for MgoError {
