...
use crate::crypto::{
    AuthorityPublicKeyBytes, DefaultHash, PublicKey, SignatureScheme, MgoPublicKey, MgoSignature,
};
...
use crate::error::{ExecutionError, MgoResult};
use crate::gas_coin::GasCoin;
use crate::gas_coin::GAS;
use crate::governance::StakedMgo;
use crate::governance::STAKED_MGO_STRUCT_NAME;
use crate::governance::STAKING_POOL_MODULE_NAME;
use crate::messages_checkpoint::CheckpointTimestamp;
use crate::multisig::MultiSigPublicKey;
use crate::object::{Object, Owner};
use crate::parse_mgo_struct_tag;
use crate::signature::GenericSignature;
use crate::mgo_serde::Readable;
use crate::mgo_serde::{to_mgo_struct_tag_string, HexAccountAddress};
use crate::transaction::Transaction;
use crate::transaction::VerifiedTransaction;
use crate::zk_login_authenticator::ZkLoginAuthenticator;
use crate::MOVE_STDLIB_ADDRESS;
use crate::MGO_CLOCK_OBJECT_ID;
use crate::MGO_FRAMEWORK_ADDRESS;
...
pub enum MoveObjectType_ {
    ...
    Other(StructTag),
    /// A MGO coin (i.e., `0x2::coin::Coin<0x2::mgo::MGO>`)
    GasCoin,
    /// A record of a staked MGO coin (i.e., `0x3::staking_pool::StakedMgo`)
    StakedMgo,
    /// A non-MGO coin type (i.e., `0x2::coin::Coin<T> where T != 0x2::mgo::MGO`)
    Coin(TypeTag),
...
impl MoveObjectType {
    ...
    pub fn staked_mgo() -> Self {
        Self(MoveObjectType_::StakedMgo)
    }

    pub fn address(&self) -> AccountAddress {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => MGO_FRAMEWORK_ADDRESS,
            MoveObjectType_::StakedMgo => MGO_SYSTEM_ADDRESS,
    ...
    pub fn module(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedMgo => STAKING_POOL_MODULE_NAME,
    ...
    pub fn name(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedMgo => STAKED_MGO_STRUCT_NAME,
    ...
    pub fn coin_type_maybe(&self) -> Option<TypeTag> {
        match &self.0 {
            ...
            MoveObjectType_::StakedMgo => None,
    ...
    pub fn size_for_gas_metering(&self) -> usize {
        ...
        match &self.0 {
            ...
            MoveObjectType_::StakedMgo => 1,
            MoveObjectType_::Coin(inner) => bcs::serialized_size(inner).unwrap() + 1,
            MoveObjectType_::Other(s) => bcs::serialized_size(s).unwrap() + 1,
        }
    }

    /// Return true if `self` is `0x2::coin::Coin<T>` for some T (note: T can be MGO)
    pub fn is_coin(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => true,
            MoveObjectType_::StakedMgo | MoveObjectType_::Other(_) => false,
        }
    }

    /// Return true if `self` is 0x2::coin::Coin<0x2::mgo::MGO>
    pub fn is_gas_coin(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin => true,
            MoveObjectType_::StakedMgo | MoveObjectType_::Coin(_) | MoveObjectType_::Other(_) => {
                false
            }
        }
    }
    ...
    pub fn is_staked_mgo(&self) -> bool {
        match &self.0 {
            MoveObjectType_::StakedMgo => true,
    ...
    pub fn is_coin_metadata(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedMgo | MoveObjectType_::Coin(_) => {
                false
            }
    ...
    pub fn is_upgrade_cap(&self) -> bool {
        self.address() == MGO_FRAMEWORK_ADDRESS
    ...
    pub fn try_extract_field_name(&self, type_: &DynamicFieldType) -> MgoResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedMgo | MoveObjectType_::Coin(_) => {
    ...
    pub fn try_extract_field_value(&self) -> MgoResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedMgo | MoveObjectType_::Coin(_) => {
                Err(MgoError::ObjectDeserializationError {
    ...
    pub fn is(&self, s: &StructTag) -> bool {
        match &self.0 {
            ...
            MoveObjectType_::StakedMgo => StakedMgo::is_staked_mgo(s),
...
impl From<StructTag> for MoveObjectType {
    fn from(mut s: StructTag) -> Self {
        ...
        } else if StakedMgo::is_staked_mgo(&s) {
            MoveObjectType_::StakedMgo
        } else {
...
impl From<MoveObjectType> for StructTag {
    fn from(t: MoveObjectType) -> Self {
        match t.0 {
            ...
            MoveObjectType_::StakedMgo => StakedMgo::type_(),
...
impl FromStr for ObjectType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.to_lowercase() == PACKAGE {
            ...
            let tag = parse_mgo_struct_tag(s)?;
...
pub const MGO_ADDRESS_LENGTH: usize = ObjectID::LENGTH;
...
#[cfg_attr(feature = "fuzzing", derive(proptest_derive::Arbitrary))]
pub struct MgoAddress(
    #[schemars(with = "Hex")]
    #[serde_as(as = "Readable<Hex, _>")]
    [u8; MGO_ADDRESS_LENGTH],
);
...
impl MgoAddress {
    ...
    /// Deserialize into an `Option<MgoAddress>`.
    pub fn optional_address_from_hex<'de, D>(
        deserializer: D,
    ) -> Result<Option<MgoAddress>, D::Error>
    where
    ...
    /// Return the underlying byte array of a MgoAddress.
    pub fn to_inner(self) -> [u8; MGO_ADDRESS_LENGTH] {
        self.0
    }

    /// Parse a MgoAddress from a byte array or buffer.
    pub fn from_bytes<T: AsRef<[u8]>>(bytes: T) -> Result<Self, MgoError> {
        <[u8; MGO_ADDRESS_LENGTH]>::try_from(bytes.as_ref())
            .map_err(|_| MgoError::InvalidAddress)
            .map(MgoAddress)
    }
    ...
    pub fn try_from_padded(inputs: &ZkLoginInputs) -> MgoResult<Self> {
        Ok((&PublicKey::from_zklogin_inputs(inputs)?).into())
    }

    /// Define as iss_bytes_len || iss_bytes || unpadded_32_byte_address_seed.
    pub fn try_from_unpadded(inputs: &ZkLoginInputs) -> MgoResult<Self> {
        ...
        Ok(MgoAddress(hasher.finalize().digest))
    }
}

impl From<ObjectID> for MgoAddress {
    fn from(object_id: ObjectID) -> MgoAddress {
        Self(object_id.into_bytes())
    }
}

impl From<AccountAddress> for MgoAddress {
    fn from(address: AccountAddress) -> MgoAddress {
...
impl TryFrom<&[u8]> for MgoAddress {
    ...
    /// Tries to convert the provided byte array into a MgoAddress.
    fn try_from(bytes: &[u8]) -> Result<Self, MgoError> {
...
impl TryFrom<Vec<u8>> for MgoAddress {
    ...
    /// Tries to convert the provided byte buffer into a MgoAddress.
    fn try_from(bytes: Vec<u8>) -> Result<Self, MgoError> {
        Self::from_bytes(bytes)
    }
}
...
impl From<&MultiSigPublicKey> for MgoAddress {
    /// Derive a MgoAddress from [struct MultiSigPublicKey]. A MultiSig address
    ...
    fn from(multisig_pk: &MultiSigPublicKey) -> Self {
        ...
        MgoAddress(hasher.finalize().digest)
    }
}

/// Mgo address for [struct ZkLoginAuthenticator] is defined as the black2b hash of
/// [zklogin_flag || iss_bytes_length || iss_bytes || unpadded_address_seed_in_bytes].
impl TryFrom<&ZkLoginAuthenticator> for MgoAddress {
    type Error = MgoError;
    fn try_from(authenticator: &ZkLoginAuthenticator) -> MgoResult<Self> {
        MgoAddress::try_from_unpadded(&authenticator.inputs)
    }
}

impl TryFrom<&GenericSignature> for MgoAddress {
    type Error = MgoError;
    /// Derive a MgoAddress from a serialized signature in Mgo [GenericSignature].
    fn try_from(sig: &GenericSignature) -> MgoResult<Self> {
        match sig {
            ...
            GenericSignature::ZkLoginAuthenticator(zklogin) => {
                MgoAddress::try_from_unpadded(&zklogin.inputs)
            }
        }
    }
}
...
/// Generate a fake MgoAddress with repeated one byte.
pub fn dbg_addr(name: u8) -> MgoAddress {
    let addr = [name; MGO_ADDRESS_LENGTH];
    MgoAddress(addr)
}
...
impl TxContext {
    pub fn new(sender: &MgoAddress, digest: &TransactionDigest, epoch_data: &EpochData) -> Self {
    ...
    pub fn kind(view: &BinaryIndexedView<'_>, s: &SignatureToken) -> TxContextKind {
        ...
        let is_tx_context_type = module_name == TX_CONTEXT_MODULE_NAME
            && module_addr == &MGO_FRAMEWORK_ADDRESS
    ...
    pub fn sender(&self) -> MgoAddress {
        MgoAddress::from(ObjectID(self.sender))
    }
    ...
    pub fn with_sender_for_testing_only(sender: &MgoAddress) -> Self {
...
impl ObjectID {
    ...
    pub fn is_clock(&self) -> bool {
        *self == MGO_CLOCK_OBJECT_ID
    }
}

impl From<MgoAddress> for ObjectID {
    fn from(address: MgoAddress) -> ObjectID {
...
impl From<MgoAddress> for AccountAddress {
...
impl fmt::Display for MoveObjectType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{}",
            to_mgo_struct_tag_string(&s).map_err(fmt::Error::custom)?
        )
    }
}
