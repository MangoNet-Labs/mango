...
use crate::base_types::{AuthorityName, ObjectID, MgoAddress};
...
use crate::mgo_serde::BigInt;
use crate::mgo_serde::Readable;
use crate::mgo_system_state::get_validator_from_table;
...
use super::{MgoSystemState, MgoSystemStateTrait};

/// This is the JSON-RPC type for the MGO system state object.
/// It flattens all fields to make them top-level fields such that it as minimum
/// dependencies to the internal data structures of the MGO system state type.

#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct MgoSystemStateSummary {
    ...
    pub active_validators: Vec<MgoValidatorSummary>,
    ...
    #[schemars(with = "Vec<(MgoAddress, BigInt<u64>)>")]
    #[serde_as(as = "Vec<(_, Readable<BigInt<u64>, _>)>")]
    pub at_risk_validators: Vec<(MgoAddress, u64)>,
    /// A map storing the records of validator reporting each other.
    pub validator_report_records: Vec<(MgoAddress, Vec<MgoAddress>)>,
}

impl MgoSystemStateSummary {
    pub fn get_mgo_committee_for_benchmarking(&self) -> CommitteeWithNetworkMetadata {
...
/// This is the JSON-RPC type for the MGO validator. It flattens all inner structures
/// to top-level fields so that they are decoupled from the internal definitions.
#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct MgoValidatorSummary {
    // Metadata
    ...
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "Readable<BigInt<u64>, _>")]
    pub staking_pool_mgo_balance: u64,
...
impl Default for MgoSystemStateSummary {
    fn default() -> Self {
        Self {
            epoch: 0,
            protocol_version: 1,
...
impl Default for MgoValidatorSummary {
    fn default() -> Self {
        Self {
            mgo_address: MgoAddress::default(),
    ...
    S: ObjectStore,
{
    ...
    let candidate_address: MgoAddress = get_dynamic_field_from_store(
        object_store,
