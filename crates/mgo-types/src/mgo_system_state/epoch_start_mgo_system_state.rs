...
use crate::base_types::{AuthorityName, EpochId, MgoAddress};
...
pub trait EpochStartSystemStateTrait {
    ...
    fn get_validator_addresses(&self) -> Vec<MgoAddress>;
    fn get_mgo_committee(&self) -> Committee;
    fn get_mgo_committee_with_network_metadata(&self) -> CommitteeWithNetworkMetadata;
...
/// to run the protocol. This allows us to decouple from the actual MgoSystemState type, and hence
/// do not need to evolve it when we upgrade the MgoSystemState type.
...
impl EpochStartSystemStateTrait for EpochStartSystemStateV1 {
    ...
    fn get_validator_addresses(&self) -> Vec<MgoAddress> {
        self.active_validators
            .iter()
            .map(|validator| validator.mgo_address)
            .collect()
    }
            ...
            .map(|validator| {
                (
                    ...
                    (
                        ...
                        NetworkMetadata {
                            network_address: validator.mgo_net_address.clone(),
