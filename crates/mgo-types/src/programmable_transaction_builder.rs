...
impl ProgrammableTransactionBuilder {
    ...
    pub fn publish_immutable(&mut self, modules: Vec<Vec<u8>>, dep_ids: Vec<ObjectID>) {
        ...
        self.commands
            .push(Command::MoveCall(Box::new(ProgrammableMoveCall {
                package: MGO_FRAMEWORK_PACKAGE_ID,
    ...
    pub fn transfer_arg(&mut self, recipient: MgoAddress, arg: Argument) {
        self.transfer_args(recipient, vec![arg])
    }

    pub fn transfer_args(&mut self, recipient: MgoAddress, args: Vec<Argument>) {
    ...
    pub fn transfer_mgo(&mut self, recipient: MgoAddress, amount: Option<u64>) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    pub fn pay_all_mgo(&mut self, recipient: MgoAddress) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    fn pay_impl(
        ...
        let mut recipient_map: IndexMap<MgoAddress, Vec<usize>> = IndexMap::new();
