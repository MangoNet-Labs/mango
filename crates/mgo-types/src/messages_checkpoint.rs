...
use crate::mgo_serde::AsProtocolVersion;
use crate::mgo_serde::BigInt;
use crate::mgo_serde::Readable;
use crate::transaction::{Transaction, TransactionData};
use crate::{base_types::AuthorityName, committee::Committee, error::MgoError};
use anyhow::Result;
    ...
    pub fn verify_with_contents(
        ...
        if let Some(contents) = contents {
            ...
            fp_ensure!(
                ...
                MgoError::GenericAuthorityError{error:format!("Checkpoint contents digest mismatch: summary={:?}, received content digest {:?}, received {} transactions", self.data(), content_digest, contents.size())}
            );
        }
...
impl CheckpointSignatureMessage {
    pub fn verify(&self, committee: &Committee) -> MgoResult {
