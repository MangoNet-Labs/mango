...
impl MoveObject {
    ...
    pub fn get_layout(&self, resolver: &impl GetModule) -> Result<MoveStructLayout, MgoError> {
    ...
    ) -> Result<MoveStructLayout, MgoError> {
    ...
    pub fn to_move_struct(&self, layout: &MoveStructLayout) -> Result<MoveStruct, MgoError> {
    ...
    /// Get the total amount of MGO embedded in `self`. Intended for testing purposes
    pub fn get_total_mgo(&self, layout_resolver: &mut dyn LayoutResolver) -> Result<u64, MgoError> {
...
impl MoveObject {
    ...
    ) -> Result<BTreeMap<TypeTag, u64>, MgoError> {
...
pub enum Owner {
    ...
    /// The object ID is converted to MgoAddress as MgoAddress is universal.
    ObjectOwner(MgoAddress),
...
impl Owner {
    ...
    pub fn get_address_owner_address(&self) -> MgoResult<MgoAddress> {
        match self {
            Self::AddressOwner(address) => Ok(*address),
            Self::Shared { .. } | Self::Immutable | Self::ObjectOwner(_) => {
                Err(MgoError::UnexpectedOwnerType)
            }
        }
    }

    // NOTE: this function will return address of both AddressOwner and ObjectOwner,
    // address of ObjectOwner is converted from object id, even though the type is MgoAddress.
    pub fn get_owner_address(&self) -> MgoResult<MgoAddress> {
        match self {
            Self::AddressOwner(address) | Self::ObjectOwner(address) => Ok(*address),
            Self::Shared { .. } | Self::Immutable => Err(MgoError::UnexpectedOwnerType),
        }
    }
...
impl PartialEq<MgoAddress> for Owner {
    fn eq(&self, other: &MgoAddress) -> bool {
        match self {
...
impl PartialEq<ObjectID> for Owner {
    fn eq(&self, other: &ObjectID) -> bool {
        let other_id: MgoAddress = (*other).into();
        match self {
...
impl ObjectInner {
    ...
    pub fn get_single_owner(&self) -> Option<MgoAddress> {
    ...
    // context: https://github.com/MangoNetworkOs/Mango/pull/10679#discussion_r1165877816
    ...
    pub fn transfer(&mut self, new_owner: MgoAddress) {
    ...
    pub fn get_layout(
        &self,
        resolver: &impl GetModule,
    ) -> Result<Option<MoveStructLayout>, MgoError> {
        match &self.data {
    ...
    pub fn get_move_template_type(&self) -> MgoResult<TypeTag> {
        let move_struct = self.data.struct_tag().ok_or_else(|| MgoError::TypeError {
...
impl Object {
    /// Get the total amount of MGO embedded in `self`, including both Move objects and the storage rebate
    pub fn get_total_mgo(&self, layout_resolver: &mut dyn LayoutResolver) -> Result<u64, MgoError> {
        Ok(self.storage_rebate
            + match &self.data {
                Data::Move(m) => m.get_total_mgo(layout_resolver)?,
                Data::Package(_) => 0,
            })
    }
    ...
    pub fn with_id_owner_gas_for_testing(id: ObjectID, owner: MgoAddress, gas: u64) -> Self {
        let data = Data::Move(MoveObject {
            type_: GasCoin::type_().into(),
            has_public_transfer: true,
            version: OBJECT_START_VERSION,
    ...
    pub fn with_id_owner_for_testing(id: ObjectID, owner: MgoAddress) -> Self {
    ...
    pub fn with_owner_for_testing(owner: MgoAddress) -> Self {
    ...
    pub fn new_gas_with_balance_and_owner_for_testing(value: u64, owner: MgoAddress) -> Self {
...
fn test_object_digest_and_serialized_format() {
    ...
    let o = Object::new_move(
        g,
        Owner::AddressOwner(MgoAddress::ZERO),
