...
use crate::crypto::{PublicKey, Signature, SignatureScheme, MgoKeyPair, ZkLoginPublicIdentifier};
...
use crate::{
    base_types::MgoAddress, signature::GenericSignature, zk_login_util::DEFAULT_JWK_BYTES,
};
...
fn zklogin_authenticator_jwk() {
    ...
    // Create mgo address derived with a legacy way.
...
fn test_serde_zk_login_signature() {
    ...
    let addr: MgoAddress = (&authenticator).try_into().unwrap();
...
fn test_serde_zk_public_identifier() {
    ...
    assert_eq!(
        MgoAddress::try_from_unpadded(&modified_inputs).unwrap(),
        MgoAddress::from(&pk1)
    );
    ...
    assert_eq!(
        MgoAddress::try_from_padded(&modified_inputs).unwrap(),
        MgoAddress::from(&pk2)
    );
