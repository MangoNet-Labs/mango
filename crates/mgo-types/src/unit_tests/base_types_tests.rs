...
use crate::crypto::{
    ...
    Signature, MgoAuthoritySignature, MgoSignature,
};
use crate::digests::Digest;
use crate::id::{ID, UID};
use crate::{gas_coin::GasCoin, object::Object, MGO_FRAMEWORK_ADDRESS};
...
fn test_signatures() {
    ...
    let foo = IntentMessage::new(Intent::mgo_transaction(), Foo("hello".into()));
    let foox = IntentMessage::new(Intent::mgo_transaction(), Foo("hellox".into()));
    let bar = IntentMessage::new(Intent::mgo_transaction(), Bar("hello".into()));
...
fn test_signatures_serde() {
    ...
    let s = Signature::new_secure(&IntentMessage::new(Intent::mgo_transaction(), foo), &sec1);
...
fn test_object_id_zero_padding() {
    ...
    assert_eq!(MGO_FRAMEWORK_ADDRESS, obj_id_1.0);
    assert_eq!(MGO_FRAMEWORK_ADDRESS, obj_id_2.0);
    assert_eq!(MGO_FRAMEWORK_ADDRESS, obj_id_3.0);
    assert_eq!(MGO_FRAMEWORK_ADDRESS, obj_id_4.0);
    assert_eq!(MGO_FRAMEWORK_ADDRESS, obj_id_5.0);
    assert_eq!(MGO_FRAMEWORK_ADDRESS, obj_id_6.0);
}

#[test]
fn test_address_display() {
    let hex = SAMPLE_ADDRESS;
    let id = MgoAddress::from_str(hex).unwrap();
    assert_eq!(format!("{:?}", id), format!("0x{hex}"));
}
...
fn test_address_serde_not_human_readable() {
    ...
    let deserialized: MgoAddress = bincode::deserialize(&serialized).unwrap();
...
fn test_address_serde_human_readable() {
    ...
    let deserialized: MgoAddress = serde_json::from_str(&serialized).unwrap();
    assert_eq!(deserialized, address);
}

#[test]
fn test_address_serde_with_expected_value() {
    let address = MgoAddress::try_from(SAMPLE_ADDRESS_VEC.to_vec()).unwrap();
...
fn test_authority_signature_serde_not_human_readable() {
    ...
    let sig = AuthoritySignature::new_secure(
        &IntentMessage::new(Intent::mgo_transaction(), Foo("some data".to_string())),
        &0,
        &key,
    );
...
fn derive_sample_address() -> (MgoAddress, AccountKeyPair) {
...
fn move_object_type_consistency() {
    ...
    let ty = assert_consistent(&StakedMgo::type_());
    assert!(ty.is_staked_mgo());
