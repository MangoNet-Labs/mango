...
#[test]
fn serde_keypair() {
    let skp = MgoKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([0; 32])));
    let encoded = skp.encode().unwrap();
    assert_eq!(
        encoded,
        "mgoprivkey1qzdlfxn2qa2lj5uprl8pyhexs02sg2wrhdy7qaq50cqgnffw4c2477kg9h3"
    );
    let decoded = MgoKeyPair::decode(&encoded).unwrap();
    assert_eq!(skp, decoded);
}

#[test]
fn serde_pubkey() {
    let skp = MgoKeyPair::Ed25519(get_key_pair().1);
...
fn public_key_equality() {
    let ed_kp1: MgoKeyPair = MgoKeyPair::Ed25519(get_key_pair().1);
    let ed_kp2: MgoKeyPair = MgoKeyPair::Ed25519(get_key_pair().1);
    let k1_kp1: MgoKeyPair = MgoKeyPair::Secp256k1(get_key_pair().1);
    let k1_kp2: MgoKeyPair = MgoKeyPair::Secp256k1(get_key_pair().1);
...
fn test_proof_of_possession() {
    let address =
        MgoAddress::from_str("0x1a4623343cd42be47d67314fce0ad042f3c82685544bc91d8c11d24e74ba7357")
            .unwrap();
...
proptest! {
    ...
    fn test_authority_pk_bytes(
        ...
        let _mgosig = Ed25519MgoSignature::from_bytes(&bytes);
        let _mgosig = Secp256k1MgoSignature::from_bytes(&bytes);
    ...
    fn test_deserialize_keypair(
        ...
        let _skp: Result<MgoKeyPair, _> = bcs::from_bytes(&bytes);
