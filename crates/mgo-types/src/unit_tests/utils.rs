...
use crate::crypto::{Signer, MgoKeyPair};
...
pub fn make_transaction_data(sender: MgoAddress) -> TransactionData {
    let object = Object::immutable_with_id_for_testing(ObjectID::random_from_rng(
        &mut StdRng::from_seed([0; 32]),
    ));
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_mgo(dbg_addr(2), None);
        builder.finish()
    };
...
pub fn make_transaction(sender: MgoAddress, kp: &MgoKeyPair) -> Transaction {
...
mod zk_login {
    ...
    pub fn load_test_vectors() -> Vec<(MgoKeyPair, PublicKey, ZkLoginInputs)> {
        ...
        for test in test_datum {
            let kp = MgoKeyPair::decode(&test.kp).unwrap();
            let inputs =
    ...
    pub fn get_zklogin_user_address() -> MgoAddress {
        thread_local! {
            static USER_ADDRESS: MgoAddress = {
                ...
                MgoAddress::from_bytes(hasher.finalize().digest).unwrap()
            };
        }
        USER_ADDRESS.with(|a| *a)
    }

    fn get_zklogin_user_key() -> MgoKeyPair {
    ...
    pub fn get_legacy_zklogin_user_address() -> MgoAddress {
        thread_local! {
            static USER_ADDRESS: MgoAddress = {
                let inputs = get_inputs_with_bad_address_seed();
                MgoAddress::from(&PublicKey::from_zklogin_inputs(&inputs).unwrap())
            };
        }
        USER_ADDRESS.with(|a| *a)
    }

    pub fn sign_zklogin_personal_msg(data: PersonalMessage) -> (MgoAddress, GenericSignature) {
        let inputs = get_zklogin_inputs();
    ...
    pub fn sign_zklogin_tx_with_default_proof(
        data: TransactionData,
        legacy: bool,
    ) -> (MgoAddress, Transaction, GenericSignature) {
    ...
    pub fn keys() -> Vec<MgoKeyPair> {
        let mut seed = StdRng::from_seed([0; 32]);
        let kp1: MgoKeyPair = MgoKeyPair::Ed25519(get_key_pair_from_rng(&mut seed).1);
        let kp2: MgoKeyPair = MgoKeyPair::Secp256k1(get_key_pair_from_rng(&mut seed).1);
        let kp3: MgoKeyPair = MgoKeyPair::Secp256r1(get_key_pair_from_rng(&mut seed).1);
        vec![kp1, kp2, kp3]
    }
    ...
    pub fn make_upgraded_multisig_tx() -> Transaction {
        ...
        let msg = IntentMessage::new(Intent::mgo_transaction(), tx.transaction_data().clone());
