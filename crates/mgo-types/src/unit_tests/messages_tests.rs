...
use crate::crypto::Secp256k1MgoSignature;
use crate::crypto::MgoKeyPair;
use crate::crypto::MgoSignature;
use crate::crypto::MgoSignatureInner;
use crate::crypto::VerificationObligation;
use crate::crypto::{
    get_key_pair, AccountKeyPair, AuthorityKeyPair, AuthorityPublicKeyBytes,
    AuthoritySignInfoTrait, MgoAuthoritySignature,
};
...
fn test_new_with_signatures() {
    ...
    assert_eq!(
        quorum
            ...
            .collect::<MgoResult<Vec<&AuthorityName>>>()
            .unwrap(),
...
fn test_handle_reject_malicious_signature() {
        ...
        AuthorityStrongQuorumSignInfo::new_from_auth_sign_infos(signatures, &committee).unwrap();
    {
        ...
        let sig = AuthoritySignature::new_secure(
            &IntentMessage::new(Intent::mgo_transaction(), message.clone()),
            &committee.epoch,
            &sec,
        );
...
fn test_sponsored_transaction_message() {
    let sender_kp = MgoKeyPair::Ed25519(get_key_pair().1);
    let sender = (&sender_kp.public()).into();
    let sponsor_kp = MgoKeyPair::Ed25519(get_key_pair().1);
    ...
    let third_party_kp = MgoKeyPair::Ed25519(get_key_pair().1);
    ...
    assert!(matches!(
        Transaction::from_generic_sig_data(tx_data, vec![sponsor_sig, third_party_sig],)
            .verify(&Default::default())
            .unwrap_err(),
        MgoError::SignerSignatureAbsent { .. }
    ));
...
fn test_sponsored_transaction_validity_check() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_mgo(MgoAddress::random_for_testing_only(), Some(50000));
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    TransactionData::new_with_gas_data(kind, sender, gas_data.clone())
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_mgo(vec![], vec![]).unwrap();
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    TransactionData::new_with_gas_data(kind, sender, gas_data.clone())
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_all_mgo(MgoAddress::random_for_testing_only());
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
...
fn verify_sender_signature_correctly_with_flag() {
    ...
    let receiver_kp = MgoKeyPair::Secp256k1(get_key_pair().1);
    let receiver_address = (&receiver_kp.public()).into();

    // create a sender keypair with Secp256k1
    let sender_kp = MgoKeyPair::Secp256k1(get_key_pair().1);
    ...
    let sender_kp_2 = MgoKeyPair::Ed25519(get_key_pair().1);
    let mut tx_data_2 = tx_data.clone();
    *tx_data_2.sender_mut_for_testing() = (&sender_kp_2.public()).into();
    ...
    let sender_kp_3 = MgoKeyPair::Secp256r1(get_key_pair().1);
    ...
    *tx_data_3.sender_mut_for_testing() = (&sender_kp_3.public()).into();
    ...
    assert_eq!(s.scheme().flag(), Secp256k1MgoSignature::SCHEME.flag());
    ...
    assert_eq!(s.scheme().flag(), Ed25519MgoSignature::SCHEME.flag());
...
fn test_change_epoch_transaction() {
    ...
    assert_eq!(
        ...
        SharedInputObject::MGO_SYSTEM_OBJ
    );
