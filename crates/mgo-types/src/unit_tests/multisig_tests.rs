...
use crate::{
    ...
    crypto::{
        get_key_pair, get_key_pair_from_rng, CompressedSignature, DefaultHash, Ed25519MgoSignature,
        PublicKey, Signature, MgoKeyPair, MgoSignatureInner, ZkLoginAuthenticatorAsBytes,
...
fn multisig_scenarios() {
    ...
    .unwrap();
    let addr = MgoAddress::from(&multisig_pk);
    ...
    let kp4: MgoKeyPair = MgoKeyPair::Secp256r1(get_key_pair().1);
    let pk4 = kp4.public();
    ...
    .unwrap();
    let addr_2 = MgoAddress::from(&multisig_pk_2);
...
fn test_combine_sigs() {
    let kp1: MgoKeyPair = MgoKeyPair::Ed25519(get_key_pair().1);
    let kp2: MgoKeyPair = MgoKeyPair::Secp256k1(get_key_pair().1);
    let kp3: MgoKeyPair = MgoKeyPair::Secp256r1(get_key_pair().1);
...
fn test_serde_roundtrip() {
    ...
    let sig = Ed25519MgoSignature::default();
    ...
    assert_eq!(single_sig_bytes.len(), Ed25519MgoSignature::LENGTH);
    assert_eq!(
...
fn single_sig_port_works() {
    let kp: MgoKeyPair = MgoKeyPair::Ed25519(get_key_pair().1);
    let addr = MgoAddress::from(&kp.public());
    let msg = IntentMessage::new(
        Intent::mgo_transaction(),
        PersonalMessage {
            message: "Hello".as_bytes().to_vec(),
...
fn test_multisig_address() {
    ...
    assert_eq!(
        MgoAddress::from_str("0xe35c69eb504de34afdbd9f307fb3ca152646c92d549fea00065d26fc422109ea")
            .unwrap(),
        address
    );
}
...
fn test_max_sig() {
    ...
    for _ in 0..11 {
        let k = MgoKeyPair::Ed25519(get_key_pair_from_rng(&mut seed).1);
    ...
    .unwrap();
    let address: MgoAddress = (&high_threshold_pk).into();
    ...
    .unwrap();
    let address: MgoAddress = (&low_threshold_pk).into();
...
fn multisig_serde_test() {
    let k1 = MgoKeyPair::Ed25519(Ed25519KeyPair::from(
    ...
    let k2 = MgoKeyPair::Secp256k1(Secp256k1KeyPair::from(
    ...
    let k3 = MgoKeyPair::Ed25519(Ed25519KeyPair::from(
    ...
    assert_eq!(
        addr,
        MgoAddress::from_str("0x37b048598ca569756146f4e8ea41666c657406db154a31f11bb5c1cbaf0b98d7")
            .unwrap()
    );
}
...
fn multisig_legacy_serde_test() {
    ...
    .unwrap();
    let multisig_pk = MultiSigPublicKey::new(vec![pk1, pk2, pk3], vec![1, 2, 3], 3).unwrap();
    let addr: MgoAddress = (&multisig_pk).into();
...
fn multisig_invalid_instance() {
    ...
    let addr = MgoAddress::from(&invalid_multisig_pk);
...
fn multisig_zklogin_scenarios() {
    ...
    let skp: MgoKeyPair = MgoKeyPair::Ed25519(kp);
    let pk1 = skp.public();
    ...
    assert_eq!(
        multisig_addr,
        MgoAddress::from_str("0xb9c0780a3943cde13a2409bf1a6f06ae60b0dff2b2f373260cf627aa4f43a588")
            .unwrap()
    );
    ...
    let intent_msg = &IntentMessage::new(Intent::mgo_transaction(), tx.clone());
    ...
    let zklogin_addr = MgoAddress::try_from(&zklogin_sig).unwrap();
    let multisig = MultiSig::combine(vec![zklogin_sig.clone()], multisig_pk.clone()).unwrap();
    assert!(multisig
    ...
    .unwrap();
    let bad_addr = MgoAddress::from(&bad_multisig_pk);
    ...
    let multisig_address = MgoAddress::from(&multisig_pk);
    let tx_data = make_transaction_data(multisig_address);
    let msg = IntentMessage::new(Intent::mgo_transaction(), tx_data);
...
fn zklogin_in_multisig_works_with_both_addresses() {
    ...
    assert!(
        matches!(res, Err(crate::error::MgoError::InvalidSignature { error }) if error.contains("General cryptographic error: Groth16 proof verify failed"))
    );
    ...
    let multisig_address_padded = MgoAddress::from(&multisig_pk_padded);
...
fn test_derive_multisig_address() {
    ...
    assert_eq!(
        multisig_addr,
        MgoAddress::from_str("0x77a9fbf3c695d78dd83449a81a9e70aa79a77dbfd6fb72037bf09201c12052cd")
            .unwrap()
    );
}
