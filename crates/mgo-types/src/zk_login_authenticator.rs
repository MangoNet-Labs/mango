...
use crate::{
    ...
    crypto::{DefaultHash, Signature, SignatureScheme, MgoSignature},
...
impl ZkLoginAuthenticator {
    ...
    pub fn get_pk(&self) -> MgoResult<PublicKey> {
...
impl AuthenticatorTrait for ZkLoginAuthenticator {
        ...
        T: Serialize,
    {
        ...
        if author != MgoAddress::try_from_unpadded(&self.inputs)? {
            ...
            if !aux_verify_data.verify_legacy_zklogin_address
                || author != MgoAddress::try_from_padded(&self.inputs)?
            {
