...
use crate::{id::UID, MGO_AUTHENTICATOR_STATE_OBJECT_ID, MGO_FRAMEWORK_ADDRESS};
...
pub const RESOLVED_MGO_AUTHENTICATOR_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
) -> MgoResult<Option<AuthenticatorStateInner>> {
    let outer = object_store.get_object(&MGO_AUTHENTICATOR_STATE_OBJECT_ID)?;
...
pub fn get_authenticator_state_obj_initial_shared_version(
    ...
    Ok(object_store
        .get_object(&MGO_AUTHENTICATOR_STATE_OBJECT_ID)?
