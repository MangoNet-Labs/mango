...
use crate::base_types::{ObjectDigest, MgoAddress};
...
use crate::mgo_serde::MgoTypeTag;
use crate::{MoveTypeTagTrait, ObjectID, SequenceNumber, MGO_FRAMEWORK_ADDRESS};
...
/// Rust version of the Move mgo::dynamic_field::Field type
...
pub struct DynamicFieldName {
    ...
    #[serde_as(as = "Readable<MgoTypeTag, _>")]
    pub type_: TypeTag,
...
impl DynamicFieldInfo {
    pub fn is_dynamic_field(tag: &StructTag) -> bool {
        tag.address == MGO_FRAMEWORK_ADDRESS
    ...
    pub fn try_extract_field_name(tag: &StructTag, type_: &DynamicFieldType) -> MgoResult<TypeTag> {
        match (type_, tag.type_params.first()) {
            ...
            (DynamicFieldType::DynamicObject, Some(TypeTag::Struct(s))) => Ok(s
                .type_params
                .first()
                .ok_or_else(|| MgoError::ObjectDeserializationError {
                    error: format!("Error extracting dynamic object name from object: {tag}"),
                })?
                .clone()),
            _ => Err(MgoError::ObjectDeserializationError {
                error: format!("Error extracting dynamic object name from object: {tag}"),
            }),
        }
    }

    pub fn try_extract_field_value(tag: &StructTag) -> MgoResult<TypeTag> {
        match tag.type_params.last() {
            Some(value_type) => Ok(value_type.clone()),
            None => Err(MgoError::ObjectDeserializationError {
    ...
    pub fn parse_move_object(
    ...
    ) -> MgoResult<(MoveValue, DynamicFieldType, ObjectID)> {
        let name = extract_field_from_move_struct(move_struct, "name").ok_or_else(|| {
            MgoError::ObjectDeserializationError {
                error: "Cannot extract [name] field from mgo::dynamic_field::Field".to_string(),
            }
        })?;

        let value = extract_field_from_move_struct(move_struct, "value").ok_or_else(|| {
            MgoError::ObjectDeserializationError {
                error: "Cannot extract [value] field from mgo::dynamic_field::Field".to_string(),
            }
        })?;
        ...
        Ok(if is_dynamic_object(move_struct) {
            ...
            .ok_or_else(|| MgoError::ObjectDeserializationError {
                error: "Cannot extract [name] field from mgo::dynamic_object_field::Wrapper."
                    .to_string(),
            })?;
            // ID extracted from the wrapper object
            let object_id =
                extract_id_value(value).ok_or_else(|| MgoError::ObjectDeserializationError {
                    error: format!(
                        "Cannot extract dynamic object's object id from \
                        mgo::dynamic_field::Field, {value:?}"
                    ),
                })?;
            ...
            let object_id = extract_object_id(move_struct).ok_or_else(|| {
                MgoError::ObjectDeserializationError {
                    error: format!(
                        "Cannot extract dynamic object's object id from \
                        mgo::dynamic_field::Field, {move_struct:?}",
                    ),
                }
            })?;
...
where
    T: Into<MgoAddress>,
{
    let parent: MgoAddress = parent.into();
    ...
    K: MoveTypeTagTrait + Serialize + DeserializeOwned + fmt::Debug,
{
    let id = derive_dynamic_field_id(parent_id, &K::get_type_tag(), &bcs::to_bytes(key).unwrap())
        .map_err(|err| MgoError::DynamicFieldReadError(err.to_string()))?;
