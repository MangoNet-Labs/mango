...
use crate::{
    ...
    error::{ExecutionError, ExecutionErrorKind, MgoError},
    event::Event,
...
pub trait MgoResolver: ResourceResolver<Error = MgoError> + BackingPackageStore {
...
impl<T> MgoResolver for T
where
    T: ResourceResolver<Error = MgoError>,
...
pub trait ExecutionState: StorageView + MgoResolver {
    fn as_mgo_resolver(&self) -> &dyn MgoResolver;
...
/// Used by mgo-execution v1 and above, to capture the execution results from Move.
...
impl TryFromValue for MgoAddress {
    fn try_from_value(value: Value) -> Result<Self, CommandArgumentError> {
