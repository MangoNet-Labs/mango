...
mod checked {
    ...
    /// Rust version of the Move mgo::coin::Coin<Mgo::mgo::MGO> type
    ...
    impl GasCoin {
        ...
        /// Return `true` if `s` is the type of a gas coin (i.e., 0x2::coin::Coin<0x2::mgo::MGO>)
        pub fn is_gas_coin(s: &StructTag) -> bool {
            Coin::is_coin(s) && s.type_params.len() == 1 && GAS::is_gas_type(&s.type_params[0])
        }

        /// Return `true` if `s` is the type of a gas balance (i.e., 0x2::balance::Balance<0x2::mgo::MGO>)
