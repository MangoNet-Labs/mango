...
use mgo_types::authenticator_state::{get_authenticator_state, AuthenticatorStateInner};
use mgo_types::base_types::{ObjectID, MgoAddress};
use mgo_types::clock::Clock;
...
use mgo_types::mgo_system_state::{
    get_mgo_system_state, get_mgo_system_state_wrapper, MgoSystemState, MgoSystemStateTrait,
    MgoSystemStateWrapper, MgoValidatorGenesis,
};
...
impl Genesis {
    ...
    pub fn validator_set_for_tooling(&self) -> Vec<MgoValidatorGenesis> {
        self.mgo_system_object()
    ...
    pub fn committee_with_network(&self) -> CommitteeWithNetworkMetadata {
        self.mgo_system_object().get_current_epoch_committee()
    }

    pub fn reference_gas_price(&self) -> u64 {
        self.mgo_system_object().reference_gas_price()
    }

    // TODO: No need to return MgoResult.
    pub fn committee(&self) -> MgoResult<Committee> {
        Ok(self.committee_with_network().committee)
    }

    pub fn mgo_system_wrapper_object(&self) -> MgoSystemStateWrapper {
        get_mgo_system_state_wrapper(&self.objects())
            .expect("Mgo System State Wrapper object must always exist")
    }

    pub fn mgo_system_object(&self) -> MgoSystemState {
        get_mgo_system_state(&self.objects()).expect("Mgo System State object must always exist")
    }
    ...
    pub fn clock(&self) -> Clock {
        let clock = self
            .objects()
            .iter()
            .find(|o| o.id() == mgo_types::MGO_CLOCK_OBJECT_ID)
...
impl TokenDistributionSchedule {
    ...
    pub fn check_all_stake_operations_are_for_valid_validators<
        I: IntoIterator<Item = MgoAddress>,
    >(
        &self,
        validators: I,
    ) {
        ...
        let mut validators: HashMap<MgoAddress, u64> =
    ...
    ///
    /// All entries in a token distribution schedule must add up to 10B Mgo.
...
impl TokenDistributionScheduleBuilder {
    ...
    pub fn default_allocation_for_validators<I: IntoIterator<Item = MgoAddress>>(
        &mut self,
        validators: I,
    ) {
        let default_allocation = mgo_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_MIST;
