...
use std::usize;
use mgo_keys::keypair_file::{read_authority_keypair_from_file, read_keypair_from_file};
use mgo_protocol_config::{Chain, SupportedProtocolVersions};
...
use mgo_types::crypto::NetworkKeyPair;
...
pub struct NodeConfig {
    ...
    /// In a `mgo-node` binary, this is set to SupportedProtocolVersions::SYSTEM_DEFAULT
    /// in mgo-node/src/main.rs. It is present in the config so that it can be changed by tests in
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name_service_package_address: Option<MgoAddress>,
...
fn default_transaction_kv_store_config() -> TransactionKeyValueStoreReadConfig {
    TransactionKeyValueStoreReadConfig {
        base_url: "https://transactions.mangonetwork.io/".to_string(),
    }
}
...
pub struct ExpensiveSafetyCheckConfig {
    ...
    /// rebate of all live objects, and that the total MGO in the network remains
    /// the same.
    #[serde(default)]
    enable_epoch_mgo_conservation_check: bool,

    /// If enabled, we will check that the total MGO in all input objects of a tx
    /// (both the Move part and the storage rebate) matches the total MGO in all
    /// output objects of the tx + gas fees
    #[serde(default)]
    enable_deep_per_tx_mgo_conservation_check: bool,

    /// Disable epoch MGO conservation check even when we are running in debug mode.
    #[serde(default)]
    force_disable_epoch_mgo_conservation_check: bool,
...
impl ExpensiveSafetyCheckConfig {
    pub fn new_enable_all() -> Self {
        Self {
            enable_epoch_mgo_conservation_check: true,
            enable_deep_per_tx_mgo_conservation_check: true,
            force_disable_epoch_mgo_conservation_check: false,
    ...
    pub fn new_disable_all() -> Self {
        Self {
            enable_epoch_mgo_conservation_check: false,
            enable_deep_per_tx_mgo_conservation_check: false,
            force_disable_epoch_mgo_conservation_check: true,
    ...
    pub fn force_disable_epoch_mgo_conservation_check(&mut self) {
        self.force_disable_epoch_mgo_conservation_check = true;
    }

    pub fn enable_epoch_mgo_conservation_check(&self) -> bool {
        (self.enable_epoch_mgo_conservation_check || cfg!(debug_assertions))
            && !self.force_disable_epoch_mgo_conservation_check
    }
    ...
    pub fn enable_deep_per_tx_mgo_conservation_check(&self) -> bool {
        self.enable_deep_per_tx_mgo_conservation_check || cfg!(debug_assertions)
    }
...
/// Wrapper struct for MgoKeyPair that can be deserialized from a file path. Used by network, worker, and account keypair.
...
pub struct KeyPairWithPath {
    ...
    #[serde(skip)]
    keypair: OnceCell<Arc<MgoKeyPair>>,
}
...
impl KeyPairWithPath {
    pub fn new(kp: MgoKeyPair) -> Self {
        let cell: OnceCell<Arc<MgoKeyPair>> = OnceCell::new();
    ...
    pub fn keypair(&self) -> &MgoKeyPair {
        self.keypair
...
mod tests {
    ...
    use mgo_keys::keypair_file::{write_authority_keypair_to_file, write_keypair_to_file};
    use mgo_types::crypto::{get_key_pair_from_rng, AuthorityKeyPair, NetworkKeyPair, MgoKeyPair};
    ...
    fn load_key_pairs_to_node_config() {
        ...
        write_keypair_to_file(
            &MgoKeyPair::Ed25519(worker_key_pair.copy()),
            PathBuf::from("worker.key"),
        )
        .unwrap();
        write_keypair_to_file(
            &MgoKeyPair::Ed25519(network_key_pair.copy()),
