...
mod compatibility_tests {
    ...
    use mgo_framework::{compare_system_package, BuiltInFramework};
    use mgo_framework_snapshot::{load_bytecode_snapshot, load_bytecode_snapshot_manifest};
    ...
    async fn test_framework_compatibility() {
        ...
        for (version, _snapshots) in load_bytecode_snapshot_manifest() {
            ...
            for cur_package in BuiltInFramework::iter_system_packages() {
                ...
                {
                    panic!(
                        "The current Mgo framework {:?} is not compatible with version {:?}",
                        cur_package.id(),
                        version
                    );
                }
