...
module test::m1 {
    ...
    public entry fun test_dof(parent: &mut Object, ctx: &mut TxContext) {
        ...
        let C1 { id } = mgo::dynamic_object_field::remove(&mut parent.id, 0);
        object::delete(id);
    ...
    public entry fun test_df(parent: &mut Object) {
        mgo::dynamic_field::add(&mut parent.id, 0, b"true");
        let _: vector<u8> = mgo::dynamic_field::remove(&mut parent.id, 0);
        mgo::dynamic_field::add(&mut parent.id, 0, true);
        let _: bool = mgo::dynamic_field::remove(&mut parent.id, 0);
    }
}
