...
pub struct Registry {
    ...
    reverse_registry: Table<MgoAddress, Domain>,
}

/// Rust version of the Move mgo::table::Table type.
...
impl Domain {
    pub fn type_(package_address: MgoAddress) -> StructTag {
        StructTag {
...
pub struct NameServiceConfig {
    pub package_address: MgoAddress,
...
impl NameServiceConfig {
    ...
    pub fn record_field_id(&self, domain: &Domain) -> ObjectID {
        ...
        mgo_types::dynamic_field::derive_dynamic_field_id(
            self.registry_id,
...
impl Default for NameServiceConfig {
    fn default() -> Self {
        let package_address = MgoAddress::from_str(NAME_SERVICE_DEFAULT_PACKAGE_ADDRESS).unwrap();
...
pub struct NameRecord {
    ...
    pub target_address: Option<MgoAddress>,
