...
use mgo_json_rpc_api::{TransactionBuilderOpenRpc, TransactionBuilderServer};
use mgo_json_rpc_types::{RPCTransactionRequestParams, MgoObjectDataFilter};
use mgo_json_rpc_types::{
    MgoObjectDataOptions, MgoObjectResponse, MgoTransactionBlockBuilderMode, MgoTypeTag,
    TransactionBlockBytes,
};
use mgo_open_rpc::Module;
use mgo_transaction_builder::{DataReader, TransactionBuilder};
use mgo_types::base_types::ObjectInfo;
use mgo_types::base_types::{ObjectID, MgoAddress};
use mgo_types::mgo_serde::BigInt;

use crate::authority_state::StateRead;
use crate::MgoRpcModule;
    ...
    async fn get_owned_objects(
        ...
        Ok(self
            ...
            .get_owner_objects(
                address,
                None,
                Some(MgoObjectDataFilter::StructType(object_type)),
            )?)
    }
...
impl TransactionBuilderServer for TransactionBuilderApi {
    ...
    async fn move_call(
        ...
        gas: Option<ObjectID>,
        gas_budget: BigInt<u64>,
        _txn_builder_mode: Option<MgoTransactionBlockBuilderMode>,
    ) -> RpcResult<TransactionBlockBytes> {
