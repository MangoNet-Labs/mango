...
use mgo_json_rpc_api::{TRANSACTION_EXECUTION_CLIENT_ERROR_CODE, TRANSIENT_ERROR_CODE};
use mgo_types::error::{MgoError, MgoObjectResponseError, UserInputError};
use mgo_types::quorum_driver_types::QuorumDriverError;
use thiserror::Error;
...
pub enum Error {
    ...
    #[error(transparent)]
    MgoObjectResponseError(#[from] MgoObjectResponseError),

    #[error(transparent)]
    MgoRpcInputError(#[from] MgoRpcInputError),
...
impl From<MgoError> for Error {
    fn from(e: MgoError) -> Self {
        match e {
            MgoError::UserInputError { error } => Self::UserInputError(error),
            MgoError::MgoObjectResponseError { error } => Self::MgoObjectResponseError(error),
            MgoError::UnsupportedFeatureError { error } => Self::UnsupportedFeature(error),
            MgoError::IndexStoreNotAvailable => Self::UnsupportedFeature(
                "Required indexes are not available on this node".to_string(),
            ),
            other => Self::MgoError(other),
        }
    }
}
...
impl From<Error> for RpcError {
    ...
    fn from(e: Error) -> RpcError {
        match e {
            ...
            Error::MgoObjectResponseError(err) => match err {
                MgoObjectResponseError::NotExists { .. }
                | MgoObjectResponseError::DynamicFieldNotFound { .. }
                | MgoObjectResponseError::Deleted { .. }
                | MgoObjectResponseError::DisplayError { .. } => {
                    RpcError::Call(CallError::InvalidParams(err.into()))
                }
                _ => RpcError::Call(CallError::Failed(err.into())),
            },
            Error::MgoRpcInputError(err) => RpcError::Call(CallError::InvalidParams(err.into())),
            Error::MgoError(mgo_error) => match mgo_error {
                MgoError::TransactionNotFound { .. }
                | MgoError::TransactionsNotFound { .. }
                | MgoError::TransactionEventsNotFound { .. } => {
                    RpcError::Call(CallError::InvalidParams(mgo_error.into()))
                }
                _ => RpcError::Call(CallError::Failed(mgo_error.into())),
            },
                ...
                match err {
                    QuorumDriverError::InvalidUserSignature(err) => {
                        let inner_error_str = match err {
                            // TODO(wlmyng): update MgoError display trait to render UserInputError with display
                            MgoError::UserInputError { error } => error.to_string(),
                            _ => err.to_string(),
                        };
                        ...
                        let new_errors: Vec<String> = errors
                            ...
                            .filter_map(|(err, _, _)| {
                                match &err {
                                    ...
                                    MgoError::UserInputError { error } => Some(error.to_string()),
                                    _ => {
...
pub enum MgoRpcInputError {
    ...
    #[error("{0}")]
    CannotParseMgoStructTag(String),
...
impl From<MgoRpcInputError> for RpcError {
    fn from(e: MgoRpcInputError) -> Self {
...
mod tests {
    ...
    use mgo_types::crypto::AuthorityPublicKey;
    ...
    mod match_quorum_driver_error_tests {
        ...
        fn test_invalid_user_signature() {
            let quorum_driver_error =
                QuorumDriverError::InvalidUserSignature(MgoError::InvalidSignature {
        ...
        fn test_objects_double_used() {
            use mgo_types::crypto::VerifyingKey;
        ...
        fn test_non_recoverable_transaction_error_with_transient_errors() {
            let quorum_driver_error = QuorumDriverError::NonRecoverableTransactionError {
                errors: vec![
                    ...
                    (
                        MgoError::RpcError("Hello".to_string(), "Testing".to_string()),
                        0,
                        vec![],
                    ),
                ],
        ...
        fn test_quorum_driver_internal_error() {
            let quorum_driver_error =
                QuorumDriverError::QuorumDriverInternalError(MgoError::UnexpectedMessage);
        ...
        fn test_system_overload() {
            let quorum_driver_error = QuorumDriverError::SystemOverload {
                overloaded_stake: 10,
                errors: vec![(MgoError::UnexpectedMessage, 0, vec![])],
            };
