...
use mgo_json_rpc_api::{MoveUtilsOpenRpc, MoveUtilsServer};
use mgo_json_rpc_types::{
    MoveFunctionArgType, ObjectValueKind, MgoMoveNormalizedFunction, MgoMoveNormalizedModule,
...
use mgo_types::object::{Data, ObjectRead};
use tap::TapFallible;
...
impl MoveUtilsInternalTrait for MoveUtilsInternal {
    ...
    async fn get_move_module(
        ...
        Ok(match normalized.get(&module_name) {
            ...
            None => Err(MgoRpcInputError::GenericNotFound(format!(
        ...
        match object_read {
            ObjectRead::Exists(_obj_ref, object, _layout) => {
                match object.into_inner().data {
                    ...
                    _ => Err(MgoRpcInputError::GenericInvalid(format!(
                        "Object is not a package with ID {}",
                        package
                    )))?,
                }
            }
            _ => Err(MgoRpcInputError::GenericNotFound(format!(
...
impl MgoRpcModule for MoveUtils {
    ...
    async fn get_normalized_move_struct(
        ...
        with_tracing!(async move {
            ...
            let identifier = Identifier::new(struct_name.as_str())
                .map_err(|e| MgoRpcInputError::GenericInvalid(format!("{e}")))?;
...
mod tests {
    ...
    mod get_normalized_move_module_tests {
        ...
        async fn test_success_response() {
            ...
            let expected_module: MgoMoveNormalizedModule = normalized_module.clone().into();
        ...
        async fn test_no_module_found() {
            ...
            let expected_error =
                Error::MgoRpcInputError(MgoRpcInputError::GenericNotFound(error_string.clone()));
            mock_internal
                .expect_get_move_module()
