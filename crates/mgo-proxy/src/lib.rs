...
mod tests {
    ...
    use crate::{admin::CertKeyPair, config::RemoteWriteConfig, peers::MgoNodeProvider};
    ...
    use mgo_tls::{CertVerifier, TlsAcceptor};
    ...
    /// mgo-node client that posts data to the proxy using the protobuf format.  The server processes this
    ...
    async fn axum_acceptor() {
        ...
        let CertKeyPair(client_priv_cert, client_pub_key) = admin::generate_self_cert("mgo".into());
        ...
        let mut allower = MgoNodeProvider::new("".into(), Duration::from_secs(30));
