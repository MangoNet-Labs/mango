...
use crate::{consumer::ProtobufDecoder, peers::MgoNodeProvider};
use axum::{
    async_trait,
    body::Bytes,
...
/// we expect mgo-node to send us an http header content-length encoding.
...
/// we expect mgo-node to send us an http header content-type encoding.
pub async fn expect_mango_proxy_header<B>(
    ...
    match format!("{content_type}").as_str() {
        ...
        ct => {
            ...
            MIDDLEWARE_OPS
                .with_label_values(&["expect_mango_proxy_header", "invalid-content-type"])
                .inc();
...
/// we expect that calling mgo-nodes are known on the blockchain and we enforce
/// their pub key tls creds here
pub async fn expect_valid_public_key<B>(
    Extension(allower): Extension<Arc<MgoNodeProvider>>,
