...
use crate::middleware::{
    expect_content_length, expect_mango_proxy_header, expect_valid_public_key,
};
use crate::peers::MgoNodeProvider;
use crate::var;
use anyhow::Result;
...
use mgo_tls::{rustls::ServerConfig, AllowAll, CertVerifier, SelfSignedCertificate, TlsAcceptor};
use tokio::signal;
...
pub fn app(
    ...
    allower: Option<MgoNodeProvider>,
) -> Router {
    ...
    let mut router = Router::new()
        ...
        .route_layer(middleware::from_fn(expect_mango_proxy_header))
...
pub fn create_server_cert_default_allow(
    hostname: String,
) -> Result<ServerConfig, mgo_tls::rustls::Error> {
...
/// Verify clients against mgo blockchain, clients that are not found in mgo_getValidators
/// will be rejected
pub fn create_server_cert_enforce_peer(
    peer_config: PeerValidationConfig,
) -> Result<(ServerConfig, Option<MgoNodeProvider>), mgo_tls::rustls::Error> {
    let (Some(certificate_path), Some(private_key_path)) =
        (peer_config.certificate_file, peer_config.private_key)
    else {
        return Err(mgo_tls::rustls::Error::General(
            "missing certs to initialize server".into(),
        ));
    };
    let allower = MgoNodeProvider::new(peer_config.url, peer_config.interval);
