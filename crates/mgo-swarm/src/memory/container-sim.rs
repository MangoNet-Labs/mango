...
use mgo_node::{MgoNode, MgoNodeHandle};
...
pub(crate) struct Container {
    ...
    node_watch: watch::Receiver<Weak<MgoNode>>,
}
...
impl Drop for Container {
    fn drop(&mut self) {
        if let Some(handle) = self.handle.take() {
            ...
            mgo_simulator::runtime::Handle::try_current().map(|h| h.delete_node(handle.node_id));
        }
    }
}
...
impl Container {
    ...
    pub async fn spawn(config: NodeConfig, _runtime: RuntimeType) -> Self {
        ...
        let handle = mgo_simulator::runtime::Handle::current();
        ...
        let node = builder
            ...
            .init(move || {
                ...
                async move {
                    let registry_service = mango_metrics::RegistryService::new(Registry::new());
                    let server = MgoNode::start(&config, registry_service, None)
                        .await
                        .unwrap();
    ...
    /// Get a MgoNodeHandle to the node owned by the container.
    pub fn get_node_handle(&self) -> Option<MgoNodeHandle> {
        Some(MgoNodeHandle::new(self.node_watch.borrow().upgrade()?))
    }
