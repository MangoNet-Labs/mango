...
use serde_json::json;
use mgo::key_identity::{get_identity_address, KeyIdentity};
use mgo_test_transaction_builder::batch_make_transfer_transactions;
...
use mgo::client_commands::SwitchResponse;
use mgo::{
    client_commands::{MgoClientCommandResult, MgoClientCommands},
    mgo_commands::MgoCommand,
};
use mgo_config::{
    PersistedConfig, MGO_CLIENT_CONFIG, MGO_FULLNODE_CONFIG, MGO_GENESIS_FILENAME,
    MGO_KEYSTORE_ALIASES_FILENAME, MGO_KEYSTORE_FILENAME, MGO_NETWORK_CONFIG,
};
use mgo_json::MgoJsonValue;
use mgo_json_rpc_types::{
    OwnedObjectRef, MgoObjectData, MgoObjectDataFilter, MgoObjectDataOptions, MgoObjectResponse,
    MgoObjectResponseQuery, MgoTransactionBlockEffects, MgoTransactionBlockEffectsAPI,
};
use mgo_keys::keystore::AccountKeystore;
use mgo_macros::sim_test;
use mgo_move_build::{BuildConfig, MgoPackageHooks};
use mgo_sdk::mgo_client_config::MgoClientConfig;
use mgo_sdk::wallet_context::WalletContext;
use mgo_swarm_config::genesis_config::{AccountConfig, GenesisConfig};
use mgo_swarm_config::network_config::NetworkConfig;
use mgo_types::base_types::MgoAddress;
use mgo_types::crypto::{
    Ed25519MgoSignature, Secp256k1MgoSignature, SignatureScheme, MgoKeyPair, MgoSignatureInner,
};
use mgo_types::error::MgoObjectResponseError;
use mgo_types::{base_types::ObjectID, crypto::get_key_pair, gas_coin::GasCoin};
...
async fn test_genesis() -> Result<(), anyhow::Error> {
    ...
    let config = working_dir.join(MGO_NETWORK_CONFIG);

    // Start network without authorities
    let start = MgoCommand::Start {
        config: Some(config),
        no_full_node: false,
    }
    .execute()
    ...
    assert!(files.contains(&MGO_CLIENT_CONFIG.to_string()));
    assert!(files.contains(&MGO_NETWORK_CONFIG.to_string()));
    assert!(files.contains(&MGO_FULLNODE_CONFIG.to_string()));
    assert!(files.contains(&MGO_GENESIS_FILENAME.to_string()));
    assert!(files.contains(&MGO_KEYSTORE_FILENAME.to_string()));
    assert!(files.contains(&MGO_KEYSTORE_ALIASES_FILENAME.to_string()));

    // Check network config
    let network_conf =
        PersistedConfig::<NetworkConfig>::read(&working_dir.join(MGO_NETWORK_CONFIG))?;
    assert_eq!(4, network_conf.validator_configs().len());

    // Check wallet config
    let wallet_conf =
        PersistedConfig::<MgoClientConfig>::read(&working_dir.join(MGO_CLIENT_CONFIG))?;
...
async fn test_addresses_command() -> Result<(), anyhow::Error> {
    ...
    for _ in 0..3 {
        context
            .config
            .keystore
            .add_key(None, MgoKeyPair::Ed25519(get_key_pair().1))?;
    }

    // Print all addresses
    MgoClientCommands::Addresses {
        sort_by_alias: true,
    }
    .execute(&mut context)
    .await
...
// fixing issue https://github.com/MangoNetworkOs/Mango/issues/6546
#[tokio::test]
async fn test_regression_6546() -> Result<(), anyhow::Error> {
    ...
    let MgoClientCommandResult::Objects(coins) = MgoClientCommands::Objects {
        address: Some(KeyIdentity::Address(address)),
    }
    .execute(context)
    .await?
    ...
    let config_path = test_cluster.swarm.dir().join(MGO_CLIENT_CONFIG);

    test_with_mgo_binary(&[
        "client",
        "--client.config",
...
async fn test_gas_command() -> Result<(), anyhow::Error> {
    ...
    MgoClientCommands::Transfer {
        to: KeyIdentity::Address(MgoAddress::random_for_testing_only()),
...
async fn test_move_call_args_linter_command() -> Result<(), anyhow::Error> {
    ...
    let address2 = MgoAddress::random_for_testing_only();
    ...
    let resp = MgoClientCommands::Publish {
        package_path,
        build_config,
        gas: Some(gas_obj_id),
    ...
    let package = if let MgoClientCommandResult::Publish(response) = resp {
        assert!(
    ...
    // Create the args
    let args = vec![
        MgoJsonValue::new(json!("123"))?,
        MgoJsonValue::new(json!(address1))?,
    ];

    // Test case with no gas specified
    let resp = MgoClientCommands::Call {
        package,
    ...
    let created_obj: ObjectID = if let MgoClientCommandResult::Call(resp) = resp {
        resp.effects
            .unwrap()
            .created()
            .first()
    ...
    assert!(MgoJsonValue::new(args_json.as_array().unwrap().first().unwrap().clone()).is_err());

    // Try a bad argument: too few args
    let args_json = json!([300usize]);
    let mut args = vec![];
    for a in args_json.as_array().unwrap() {
        args.push(MgoJsonValue::new(a.clone()).unwrap());
    }
    ...
    let args = vec![
        MgoJsonValue::new(json!(obj))?,
        MgoJsonValue::new(json!(address2))?,
    ];
    ...
    // let err_string = format!("{} ", resp.err().unwrap());
    // let framework_addr = MGO_FRAMEWORK_ADDRESS.to_hex_literal();
    // let package_addr = package.to_hex_literal();
    // assert!(err_string.contains(&format!("Expected argument of type {package_addr}::object_basics::Object, but found type {framework_addr}::coin::Coin<{framework_addr}::mgo::MGO>")));

    // Try a proper transfer
    let args = vec![
        MgoJsonValue::new(json!(created_obj))?,
        MgoJsonValue::new(json!(address2))?,
    ];
...
async fn test_package_publish_command() -> Result<(), anyhow::Error> {
    ...
    let obj_ids = if let MgoClientCommandResult::Publish(response) = resp {
        response
            .effects
            .as_ref()
            .unwrap()
...
async fn test_delete_shared_object() -> Result<(), anyhow::Error> {
    ...
    let owned_obj_ids = if let MgoClientCommandResult::Publish(response) = resp {
    ...
    let start_call_result = MgoClientCommands::Call {
    ...
    let shared_id = if let MgoClientCommandResult::Call(response) = start_call_result {
    ...
    let delete_result = MgoClientCommands::Call {
        package: (*package_id.object_id).into(),
        module: "sod".to_string(),
        ...
        gas: None,
        gas_budget: rgp * TEST_ONLY_GAS_UNIT_FOR_PUBLISH,
        args: vec![MgoJsonValue::from_str(&shared_id.to_string()).unwrap()],
        serialize_unsigned_transaction: false,
        serialize_signed_transaction: false,
    }
    .execute(context)
    ...
    if let MgoClientCommandResult::Call(response) = delete_result {
...
async fn test_receive_argument() -> Result<(), anyhow::Error> {
    ...
    let (parent, child) = if let MgoClientCommandResult::Call(response) = start_call_result {
    ...
    let receive_result = MgoClientCommands::Call {
        package: (*package_id.object_id).into(),
        module: "tto".to_string(),
        ...
        gas_budget: rgp * TEST_ONLY_GAS_UNIT_FOR_PUBLISH,
        args: vec![
            MgoJsonValue::from_str(&parent.object_id.to_string()).unwrap(),
            MgoJsonValue::from_str(&child.object_id.to_string()).unwrap(),
        ],
        serialize_unsigned_transaction: false,
        serialize_signed_transaction: false,
    }
    ...
    if let MgoClientCommandResult::Call(response) = receive_result {
        assert!(response.effects.unwrap().into_status().is_ok());
    } else {
        unreachable!("Invalid response");
    };
...
async fn test_package_publish_command_with_unpublished_dependency_fails(
    ...
    package_path.push("module_publish_with_unpublished_dependency");
    let build_config = BuildConfig::new_for_testing().config;
    let result = MgoClientCommands::Publish {
        package_path,
        build_config,
        gas: Some(gas_obj_id),
        gas_budget: rgp * TEST_ONLY_GAS_UNIT_FOR_PUBLISH,
...
async fn test_package_publish_test_flag() -> Result<(), anyhow::Error> {
    ...
    // this would have been the result of calling `mgo client publish --test`
...
async fn test_package_upgrade_command() -> Result<(), anyhow::Error> {
    ...
    let MgoClientCommandResult::Publish(response) = resp else {
        unreachable!("Invalid response");
    };

    let MgoTransactionBlockEffects::V1(effects) = response.effects.unwrap();
    ...
    // Now run the upgrade
    let build_config = BuildConfig::new_for_testing().config;
    let resp = MgoClientCommands::Upgrade {
    ...
    let MgoClientCommandResult::Upgrade(response) = resp else {
...
async fn test_native_transfer() -> Result<(), anyhow::Error> {
    ...
    let resp = MgoClientCommands::Transfer {
        gas: Some(gas_obj_id),
    ...
    let (mut_obj1, mut_obj2) = if let MgoClientCommandResult::Transfer(response) = resp {
        assert!(
            response.status_ok().unwrap(),
            "Command failed: {:?}",
            response
    ...
    // Check the objects
    let resp = MgoClientCommands::Object {
        id: mut_obj1,
        bcs: false,
    }
    .execute(context)
    .await?;
    let mut_obj1 = if let MgoClientCommandResult::Object(resp) = resp {
    ...
    let resp2 = MgoClientCommands::Object {
        id: mut_obj2,
        bcs: false,
    }
    .execute(context)
    .await?;
    let mut_obj2 = if let MgoClientCommandResult::Object(resp2) = resp2 {
    ...
    // Get the mutated objects
    let (_mut_obj1, _mut_obj2) = if let MgoClientCommandResult::Transfer(response) = resp {
        (
            response
                .effects
                .as_ref()
...
#[test]
// Test for issue https://github.com/MangoNetworkOs/Mango/issues/1078
fn test_bug_1078() {
    let read = MgoClientCommandResult::Object(MgoObjectResponse::new_with_error(
        MgoObjectResponseError::NotExists {
...
async fn test_switch_command() -> Result<(), anyhow::Error> {
    ...
    let os = MgoClientCommands::Objects { address: None }
        .execute(context)
        .await?;

    let mut cmd_objs = if let MgoClientCommandResult::Objects(v) = os {
        v
    } else {
    ...
    // Switch the address
    let resp = MgoClientCommands::Switch {
    ...
    assert_eq!(
        ...
        format!(
            "{}",
            MgoClientCommandResult::Switch(SwitchResponse {
    ...
    let os = MgoClientCommands::NewAddress {
    ...
    .execute(context)
    .await?;
    let new_addr = if let MgoClientCommandResult::NewAddress(x) = os {
        x.address
    } else {
        panic!("Command failed")
    };
...
async fn test_new_address_command_by_flag() -> Result<(), anyhow::Error> {
    ...
    assert_eq!(
        context
            ...
            .keys()
            .iter()
            .filter(|k| k.flag() == Ed25519MgoSignature::SCHEME.flag())
            .count(),
        5
    );
    ...
    assert_eq!(
        context
            ...
            .keys()
            .iter()
            .filter(|k| k.flag() == Secp256k1MgoSignature::SCHEME.flag())
            .count(),
        1
    );
...
async fn test_active_address_command() -> Result<(), anyhow::Error> {
    ...
    // Run a command with address omitted
    let os = MgoClientCommands::ActiveAddress {}.execute(context).await?;

    let a = if let MgoClientCommandResult::ActiveAddress(Some(v)) = os {
        v
    } else {
        panic!("Command failed")
    };
...
fn get_gas_value(o: &MgoObjectData) -> u64 {
    GasCoin::try_from(o).unwrap().value()
}

async fn get_object(id: ObjectID, context: &WalletContext) -> Option<MgoObjectData> {
...
async fn test_merge_coin() -> Result<(), anyhow::Error> {
    ...
    let resp = MgoClientCommands::MergeCoin {
        primary_coin,
        coin_to_merge,
        gas: Some(gas),
    ...
    .execute(context)
    .await?;
    let g = if let MgoClientCommandResult::MergeCoin(r) = resp {
...
async fn test_split_coin() -> Result<(), anyhow::Error> {
    ...
    // Test with gas specified
    let resp = MgoClientCommands::SplitCoin {
        gas: Some(gas),
    ...
    let (updated_coin, new_coins) = if let MgoClientCommandResult::SplitCoin(r) = resp {
        assert!(r.status_ok().unwrap(), "Command failed: {:?}", r);
...
async fn test_with_mgo_binary(args: &[&str]) -> Result<(), anyhow::Error> {
    let mut cmd = assert_cmd::Command::cargo_bin("mgo").unwrap();
...
async fn test_get_owned_objects_owned_by_address_and_check_pagination() -> Result<(), anyhow::Error>
{
    ...
    let mut cursor = None;
    let mut response_data: Vec<MgoObjectResponse> = Vec::new();
    while has_next {
        let object_responses = client
            .read_api()
            .get_owned_objects(
...
async fn test_linter_suppression_stats() -> Result<(), anyhow::Error> {
    ...
    let output = cmd
        .args(&args)
        .output()
        .expect("failed to run 'mgo move test'");
