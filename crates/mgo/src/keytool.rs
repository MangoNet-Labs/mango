...
use mgo_keys::key_derive::generate_new_key;
use mgo_keys::keypair_file::{
...
use mgo_types::crypto::{
    get_authority_key_pair, EncodeDecodeBase64, Signature, SignatureScheme, MgoKeyPair,
};
use mgo_types::crypto::{DefaultHash, PublicKey};
use mgo_types::error::MgoResult;
use mgo_types::multisig::{MultiSig, MultiSigPublicKey, ThresholdUnit, WeightUnit};
use mgo_types::multisig_legacy::{MultiSigLegacy, MultiSigPublicKeyLegacy};
use mgo_types::signature::{AuthenticatorTrait, GenericSignature, VerifyParams};
use mgo_types::transaction::TransactionData;
use mgo_types::zk_login_authenticator::ZkLoginAuthenticator;
use mgo_types::zk_login_util::get_zklogin_inputs;
...
pub enum KeyToolCommand {
    ...
    /// encoded 33 byte flag || private key starting with "mgoprivkey").
    /// Hex private key format import and export are both deprecated in
    /// Mgo Wallet and Mgo CLI Keystore. Use `mgo keytool import` if you
    /// wish to import a key to Mgo Keystore.
    ...
    ///
    /// Use `mgo client new-address` if you want to generate and save the key into mgo.keystore.
    Generate {
    ...
    /// Output the private key of the given key identity in Mgo CLI Keystore as Bech32
    /// encoded string starting with `mgoprivkey`.
    Export {
        #[clap(long)]
        key_identity: KeyIdentity,
    },
    /// List all keys by its Mgo address, Base64 encoded public key, key scheme name in
    /// mgo.keystore.
    List {
        /// Sort by alias
        #[clap(long, short = 's')]
    ...
    /// [enum MgoKeyPair] (Base64 encoded of 33-byte `flag || privkey`) or `type AuthorityKeyPair`
    ...
    /// To MultiSig Mgo Address. Pass in a list of all public keys `flag || pk` in Base64.
    ...
    /// result can be used as signature field for `mgo client execute-signed-tx`. The sum
    ...
    Show { file: PathBuf },
    /// Create signature using the private key for for the given address (or its alias) in mgo keystore.
    ...
    /// Generate PubKey from pem using MangoNetLabs/base64pemkey
    /// Any signature commits to a [struct IntentMessage] consisting of the Base64 encoded
    /// of the BCS serialized transaction bytes itself and its intent. If intent is absent,
    /// default will be used.
    SignKMS {
    ...
    /// This takes [enum MgoKeyPair] of Base64 encoded of 33-byte `flag || privkey`). It
    /// outputs the keypair into a file at the current directory where the address is the filename,
    /// and prints out its Mgo address, Base64 encoded public key, the key scheme, and the key scheme flag.
    ...
    /// Example request: mgo keytool zk-login-sig-verify --sig $SERIALIZED_ZKLOGIN_SIG --bytes $BYTES --intent-scope 0 --network devnet --curr-epoch 10
    ZkLoginSigVerify {
...
pub struct ConvertOutput {
    bech32_with_flag: String, // latest Mgo Keystore and Mgo Wallet import/export format
...
pub struct SignData {
    ...
    // serialized Mgo signature to be send for executing the transaction.
    mgo_signature: String,
}
    ...
    pub async fn execute(self, keystore: &mut Keystore) -> Result<CommandOutput, anyhow::Error> {
        let cmd_result = Ok(match self {
            ...
            KeyToolCommand::DecodeMultiSig { multisig, tx_bytes } => {
                ...
                let address = MgoAddress::from(multisig.get_pk());
            ...
            KeyToolCommand::Generate {
                ...
                SignatureScheme::BLS12381 => {
                    let (mgo_address, kp) = get_authority_key_pair();
                    let file_name = format!("bls-{mgo_address}.key");
                ...
                _ => {
                    let (mgo_address, skp, _scheme, phrase) =
                        generate_new_key(key_scheme, derivation_path, word_length)?;
                    let file = format!("{mgo_address}.key");
            ...
            KeyToolCommand::Import {
                ...
                match MgoKeyPair::decode(&input_string) {
                    ...
                    Err(_) => {
                        ...
                        let skp = keystore.get_key(&mgo_address)?;
                        let key = Key::from(skp);
                        CommandOutput::Import(key)
                    }
                }
            ...
            KeyToolCommand::List { sort_by_alias } => {
                let mut keys = keystore
                    ...
                    .map(|pk| {
                        ...
                        key.alias = keystore.get_alias_by_address(&key.mgo_address).ok();
                        key
                    })
                    .collect::<Vec<Key>>();
                if sort_by_alias {
                ...
                let output = match read_keypair_from_file(&file) {
                    Ok(keypair) => {
                        ...
                        let network_worker_keypair = match &keypair {
                            MgoKeyPair::Ed25519(kp) => kp.encode_base64(),
                            MgoKeyPair::Secp256k1(kp) => kp.encode_base64(),
                            MgoKeyPair::Secp256r1(kp) => kp.encode_base64(),
                        };
                        KeypairData {
            ...
            KeyToolCommand::MultiSigAddress {
                ...
                let address: MgoAddress = (&multisig_pk).into();
                ...
                for (pk, w) in pks.into_iter().zip(weights.into_iter()) {
                    output.multisig.push(MultiSigOutput {
                        address: Into::<MgoAddress>::into(&pk),
                        public_base64_key: pk.encode_base64(),
                        weight: w,
                    });
                }
                    ...
                    Err(_) => match read_authority_keypair_from_file(&file) {
                        Ok(keypair) => {
                            ...
                            CommandOutput::Show(Key {
                                ...
                                mgo_address: (keypair.public()).into(),
                                public_base64_key,
                                key_scheme: SignatureScheme::BLS12381.to_string(),
                                flag: SignatureScheme::BLS12381.flag(),
                                peer_id: None,
            ...
            KeyToolCommand::Sign {
                ...
                let intent = intent.unwrap_or_else(Intent::mgo_transaction);
                ...
                CommandOutput::Sign(SignData {
                    ...
                    mgo_signature: mgo_signature.encode_base64(),
                })
            }
            ...
            KeyToolCommand::SignKMS {
                ...
                let pk_owner = PublicKey::decode_base64(&base64pk)
                    .map_err(|e| anyhow!("Invalid base64 key: {:?}", e))?;
                let address_owner = MgoAddress::from(&pk_owner);
            ...
            KeyToolCommand::Unpack { keypair } => {
                let keypair = MgoKeyPair::decode_base64(&keypair)
                    .map_err(|_| anyhow!("Invalid Base64 encode keypair"))?;

                let key = Key::from(&keypair);
                let path_str = format!("{}.key", key.mgo_address).to_lowercase();
            ...
            KeyToolCommand::ZkLoginSignAndExecuteTx {
                ...
                let pk = skp.public();
                let ephemeral_key_identifier: MgoAddress = (&skp.public()).into();
                ...
                println!("Finish login and paste the entire URL here (e.g. https://mangonetwork.io/#id_token=...):");
                            ...
                            .map_err(|_| anyhow!("Invalid scope"))?
                        {
                            IntentScope::TransactionData => {
                                ...
                                let res = zk.verify_authenticator(
                                    &IntentMessage::new(Intent::mgo_transaction(), tx_data.clone()),
...
impl From<&MgoKeyPair> for Key {
    fn from(skp: &MgoKeyPair) -> Self {
        Key::from(skp.public())
    }
}
...
impl From<PublicKey> for Key {
    fn from(pk: PublicKey) -> Self {
        Key {
            ...
            mgo_address: MgoAddress::from(&pk),
            public_base64_key: pk.encode_base64(),
...
fn convert_private_key_to_bech32(value: String) -> Result<ConvertOutput, anyhow::Error> {
    let skp = match MgoKeyPair::decode(&value) {
        Ok(s) => s,
        Err(_) => match Hex::decode(&value) {
            Ok(decoded) => {
                ...
                MgoKeyPair::Ed25519(Ed25519KeyPair::from_bytes(&decoded)?)
            }
            Err(_) => match MgoKeyPair::decode_base64(&value) {
                Ok(skp) => skp,
                Err(_) => match Ed25519KeyPair::decode_base64(&value) {
                    Ok(kp) => MgoKeyPair::Ed25519(kp),
