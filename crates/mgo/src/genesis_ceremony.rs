...
use mgo_config::{genesis::UnsignedGenesis, MGO_GENESIS_FILENAME};
use mgo_genesis_builder::Builder;
use mgo_types::multiaddr::Multiaddr;
use mgo_types::{
    base_types::MgoAddress,
    committee::ProtocolVersion,
    crypto::{
        generate_proof_of_possession, AuthorityKeyPair, KeypairTraits, NetworkKeyPair, MgoKeyPair,
    },
...
pub fn run(cmd: Ceremony) -> Result<()> {
    ...
    match cmd.command {
        ...
        CeremonyCommand::AddValidator {
            ...
            let account_keypair: MgoKeyPair = read_keypair_from_file(account_key_file)?;
            ...
            builder = builder.add_validator(
                mgo_genesis_builder::validator_info::ValidatorInfo {
                    name,
                    protocol_key: keypair.public().into(),
                    worker_key: worker_keypair.public().clone(),
                    account_address: MgoAddress::from(&account_keypair.public()),
                    network_key: network_keypair.public().clone(),
                    gas_price: mgo_config::node::DEFAULT_VALIDATOR_GAS_PRICE,
        ...
        CeremonyCommand::Finalize => {
            ...
            genesis.save(dir.join(MGO_GENESIS_FILENAME))?;

            println!("Successfully built {MGO_GENESIS_FILENAME}");
            println!(
                "{MGO_GENESIS_FILENAME} blake2b-256: {}",
...
fn check_protocol_version(builder: &Builder, protocol_version: ProtocolVersion) -> Result<()> {
    ...
    // (e.g. using a `mgo` binary built at the wrong commit).
...
mod test {
    ...
    use mgo_types::crypto::{get_key_pair_from_rng, AccountKeyPair, AuthorityKeyPair, MgoKeyPair};
    ...
    fn ceremony() -> Result<()> {
        ...
        let validators = (0..10)
            .map(|i| {
                ...
                write_keypair_to_file(&MgoKeyPair::Ed25519(worker_keypair), &worker_key_file)
                    .unwrap();

                let network_key_file = dir.path().join(format!("{}-1.key", info.name));
                write_keypair_to_file(&MgoKeyPair::Ed25519(network_keypair), &network_key_file)
                    .unwrap();

                let account_key_file = dir.path().join(format!("{}-2.key", info.name));
                write_keypair_to_file(&MgoKeyPair::Ed25519(account_keypair), &account_key_file)
                    .unwrap();
