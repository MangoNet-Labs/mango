...
use mgo_config::genesis::UnsignedGenesis;
use mgo_types::mgo_system_state::MgoValidatorGenesis;
use mgo_types::{
    base_types::ObjectID,
    coin::CoinMetadata,
...
const STR_MGO: &str = "Mgo";
const STR_STAKED_MGO: &str = "StakedMgo";
const STR_PACKAGE: &str = "Package";
const STR_COIN_METADATA: &str = "CoinMetadata";
const STR_OTHER: &str = "Other";
const STR_MGO_DISTRIBUTION: &str = "Mgo Distribution";
...
pub(crate) fn examine_genesis_checkpoint(genesis: UnsignedGenesis) {
    ...
    // Prepare Mgo distribution info
    let mut mgo_distribution = BTreeMap::new();
    let entry = mgo_distribution
        .entry("Mgo System".to_string())
    ...
    entry.insert(
        ...
        (STR_MGO, system_object.stake_subsidy.balance.value()),
    );

    // Prepare Object Info
    let mut owner_map = BTreeMap::new();
    let mut package_map = BTreeMap::new();
    let mut mgo_map = BTreeMap::new();
    let mut staked_mgo_map = BTreeMap::new();
            ...
            mgo_types::object::Data::Move(move_object) => {
                if let Ok(gas) = GasCoin::try_from(object) {
                    let entry = mgo_distribution
                        ...
                        .or_default();
                    entry.insert(object_id_str.clone(), (STR_MGO, gas.value()));
                    mgo_map.insert(object.id(), gas);
                } else if let Ok(coin_metadata) = CoinMetadata::try_from(object) {
                    coin_metadata_map.insert(object.id(), coin_metadata);
                } else if let Ok(staked_mgo) = StakedMgo::try_from(object) {
                    let entry = mgo_distribution
                        .entry(object.owner.to_string())
                        .or_default();
                    entry.insert(object_id_str, (STR_STAKED_MGO, staked_mgo.principal()));
                    // Assert pool id is associated with a knonw validator.
                    let validator = validator_pool_id_map.get(&staked_mgo.pool_id()).unwrap();
                    assert_eq!(validator.staking_pool.id, staked_mgo.pool_id());

                    staked_mgo_map.insert(object.id(), staked_mgo);
                } else {
                    other_object_map.insert(object.id(), move_object);
                }
            }
            mgo_types::object::Data::Package(p) => {
    ...
    examine_total_supply(&mgo_distribution, false);

    // Main loop for inspection
    let main_options: Vec<&str> = vec![STR_MGO_DISTRIBUTION, STR_VALIDATORS, STR_OBJECTS, STR_EXIT];
    loop {
        let ans = Select::new(
        ...
        .prompt();
        match ans {
            Ok(name) if name == STR_MGO_DISTRIBUTION => {
                examine_total_supply(&mgo_distribution, true)
            }
...
fn examine_validators(
    ...
    validator_map: &BTreeMap<&str, &MgoValidatorGenesis>,
) {
    loop {
...
fn examine_object(
    ...
    validator_pool_id_map: &BTreeMap<ObjectID, &MgoValidatorGenesis>,
    package_map: &BTreeMap<ObjectID, &MovePackage>,
    mgo_map: &BTreeMap<ObjectID, GasCoin>,
    staked_mgo_map: &BTreeMap<ObjectID, StakedMgo>,
    ...
    loop {
        ...
        match ans {
            Ok(name) if name == STR_EXIT => break,
            Ok(name) if name == STR_MGO => {
                for gas_coin in mgo_map.values() {
                    display_mgo(gas_coin, owner_map);
                }
                print_divider("Mgo");
            }
            Ok(name) if name == STR_STAKED_MGO => {
                for staked_mgo_coin in staked_mgo_map.values() {
                    display_staked_mgo(staked_mgo_coin, validator_pool_id_map, owner_map);
                }
                print_divider(STR_STAKED_MGO);
            }
...
fn examine_total_supply(
    mgo_distribution: &BTreeMap<String, BTreeMap<String, (&str, u64)>>,
    print: bool,
) {
    let mut total_mgo = 0;
    let mut total_staked_mgo = 0;
    for (owner, coins) in mgo_distribution {
        let mut amount_sum = 0;
        for (owner, value) in coins.values() {
            amount_sum += value;
            if *owner == STR_STAKED_MGO {
                total_staked_mgo += value;
            }
        }
        total_mgo += amount_sum;
        if print {
            println!("Owner {:?}", owner);
            println!(
...
fn display_validator(validator: &MgoValidatorGenesis) {
    ...
    println!(
        "Staking Pool Mgo Balance: {:?}",
        validator.staking_pool.mgo_balance
    );
    println!(
        "Rewards Pool: {}",
    ...
    println!(
        "Pending Total Mgo Withdraw: {}",
        validator.staking_pool.pending_total_mgo_withdraw
    );
    println!(
...
fn display_mgo(gas_coin: &GasCoin, owner_map: &BTreeMap<ObjectID, Owner>) {
    println!("ID: {}", gas_coin.id());
    println!("Balance: {}", gas_coin.value());
    println!("Owner: {}\n", owner_map.get(gas_coin.id()).unwrap());
}

fn display_staked_mgo(
    ...
    println!("Owner: {}\n", owner_map.get(&staked_mgo.id()).unwrap());
}
