...
use crate::client_commands::{MgoClientCommandResult, MgoClientCommands};
use crate::shell::{
...
pub async fn start_console(
    ...
    let app: Command = MgoClientCommands::command();
    writeln!(out, "{}", MGO.cyan().bold())?;
    let mut version = env!("CARGO_PKG_VERSION").to_owned();
    if let Some(git_rev) = std::option_env!("GIT_REVISION") {
        version.push('-');
        version.push_str(git_rev);
    }
    writeln!(out, "--- Mgo Console {version} ---")?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "Connecting to Mgo full node. API version {}",
        client.api_version()
    )?;
    ...
    writeln!(out)?;
    writeln!(out, "Welcome to the Mgo interactive console.")?;
    writeln!(out)?;
...
async fn handle_command(
    ...
    if let Ok(mut cache) = completion_cache.write() {
        match result {
            MgoClientCommandResult::Addresses(ref addresses) => {
            ...
            MgoClientCommandResult::Objects(ref objects) => {
                let objects = objects
                    .iter()
    ...
    if matches!(
        result,
        MgoClientCommandResult::Switch(SwitchResponse { env: Some(_), .. })
    ) {
        println!("Mgo environment switch completed, please restart Mgo console.");
        return Ok(true);
    }
    Ok(false)
}
