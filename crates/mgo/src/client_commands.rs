...
use mgo_move::build::resolve_lock_file_path;
use mgo_protocol_config::ProtocolConfig;
use mgo_source_validation::{BytecodeSourceVerifier, SourceMode};

use shared_crypto::intent::Intent;
use mgo_execution::verifier::VerifierOverrides;
use mgo_json::MgoJsonValue;
use mgo_json_rpc_types::{
    DynamicFieldPage, MgoData, MgoObjectData, MgoObjectResponse, MgoObjectResponseQuery,
    MgoParsedData, MgoRawData, MgoTransactionBlockEffectsAPI, MgoTransactionBlockResponse,
    MgoTransactionBlockResponseOptions,
};
use mgo_json_rpc_types::{MgoExecutionStatus, MgoObjectDataOptions};
...
use mgo_replay::ReplayToolCommand;
use mgo_sdk::mgo_client_config::{MgoClientConfig, MgoEnv};
use mgo_sdk::MgoClient;
use mgo_sdk::{
    wallet_context::WalletContext, MGO_DEVNET_URL, MGO_LOCAL_NETWORK_URL, MGO_TESTNET_URL,
};
use mgo_types::{
    base_types::{ObjectID, SequenceNumber, MgoAddress},
...
macro_rules! serialize_or_execute {
    ($tx_data:expr, $serialize_unsigned:expr, $serialize_signed:expr, $context:expr, $result_variant:ident) => {{
        ...
        if $serialize_unsigned {
            MgoClientCommandResult::SerializedUnsignedTransaction($tx_data)
        } else {
            ...
            if $serialize_signed {
                MgoClientCommandResult::SerializedSignedTransaction(sender_signed_data)
            } else {
                let transaction = Transaction::new(sender_signed_data);
                let response = $context.execute_transaction_may_fail(transaction).await?;
                let effects = response.effects.as_ref().ok_or_else(|| {
                    anyhow!("Effects from MgoTransactionBlockResult should not be empty")
                })?;
                if matches!(effects.status(), MgoExecutionStatus::Failure { .. }) {
                    return Err(anyhow!(
                        "Error executing transaction: {:#?}",
                        effects.status()
                    ));
                }
                MgoClientCommandResult::$result_variant(response)
            }
        }
    }};
}
...
pub enum MgoClientCommands {
    ...
    Call {
        ...
        #[clap(
            long,
            value_parser = parse_mgo_type_tag,
            num_args(1..),
        )]
        ...
        /// be used to execute transaction with `mgo client execute-signed-tx --tx-bytes <TX_BYTES>`.
        ...
        /// can be used to execute transaction with `mgo client execute-combined-signed-tx --signed-tx-bytes
        /// <SIGNED_TX_BYTES>`.
        #[clap(long, required = false)]
    ...
    ExecuteSignedTx {
        /// BCS serialized transaction data bytes without its type tag, as base64 encoded string. This is the output of mgo client command using --serialize-unsigned-transaction.
        #[clap(long)]
        tx_bytes: String,
    ...
    ExecuteCombinedSignedTx {
        /// BCS serialized sender signed data, as base64 encoded string. This is the output of mgo client command using --serialize-signed-transaction.
        #[clap(long)]
    ...
    Objects {
        ...
        /// objects owned by `mgo client active-address`.
        #[clap(name = "owner_address")]
        address: Option<KeyIdentity>,
    },
    ...
    /// Pay all residual MGO coins to the recipient with input coins, after deducting the gas cost.
    ...
    /// Pay MGO coins to recipients following following specified amounts, with input coins.
    /// Length of recipients must be the same as that of amounts.
    /// The input coins also include the coin for gas payment, so no extra gas coin is required.
    PayMgo {
        /// The input coins to be used for pay recipients, including the gas coin.
    ...
    /// Transfer MGO, and pay gas with the same MGO coin object.
    /// If amount is specified, only the amount is transferred; otherwise the entire object
    /// is transferred.
    #[clap(name = "transfer-mgo")]
    TransferMgo {
        /// Recipient address (or its alias if it's an address in the keystore)
        #[clap(long)]
        to: KeyIdentity,

        /// Mgo coin object to transfer, ID in 20 bytes Hex string. This is also the gas object.
        #[clap(long)]
        mgo_coin_object_id: ObjectID,
...
impl MgoClientCommands {
    pub async fn execute(
        self,
        context: &mut WalletContext,
    ) -> Result<MgoClientCommandResult, anyhow::Error> {
        let ret = Ok(match self {
            MgoClientCommands::ProfileTransaction {
                tx_digest,
                profile_output,
            } => {
                ...
                let _command_result =
                    mgo_replay::execute_replay_command(Some(rpc), false, false, None, cmd).await?;
                // this will be displayed via trace info, so no output is needed here
                MgoClientCommandResult::NoOutput
            }
            MgoClientCommands::ReplayTransaction {
                tx_digest,
                gas_info: _,
                ptb_info: _,
            } => {
            ...
            MgoClientCommands::ReplayCheckpoints {
                start,
                end,
                terminate_early,
            } => {
            ...
            MgoClientCommands::Addresses { sort_by_alias } => {
                let active_address = context.active_address()?;
                let mut addresses: Vec<(String, MgoAddress)> = context
                    .config
                    .keystore
                ...
                MgoClientCommandResult::Addresses(output)
            }
            MgoClientCommands::DynamicFieldQuery { id, cursor, limit } => {
                let client = context.get_client().await?;
                let df_read = client
                    .read_api()
                    .get_dynamic_fields(id, cursor, Some(limit))
                    .await?;
                MgoClientCommandResult::DynamicFieldQuery(df_read)
            }

            MgoClientCommands::Upgrade {
                package_path,
                upgrade_capability,
                build_config,
                gas,
                ...
                let resp = context
                    ...
                    .get_object_with_options(
                        upgrade_capability,
                        MgoObjectDataOptions::default().with_bcs().with_owner(),
                    )
                    .await?;
            ...
            MgoClientCommands::Publish {
                ...
                if build_config.test_mode {
                    return Err(MgoError::ModulePublishFailure {
                        error:
                            ...
                            compiling the package normally with `mgo move build`."
                                .to_string(),
                    }
                    .into());
                }
            ...
            MgoClientCommands::VerifyBytecodeMeter {
                package_path,
                build_config,
            } => {
                ...
                let mut verifier =
                    mgo_execution::verifier(&protocol_config, true, &bytecode_verifier_metrics);
                ...
                MgoClientCommandResult::VerifyBytecodeMeter {
            ...
            MgoClientCommands::Object { id, bcs } => {
                // Fetch the object ref
                let client = context.get_client().await?;
                if !bcs {
                    let object_read = client
                        .read_api()
                        .get_object_with_options(id, MgoObjectDataOptions::full_content())
                        .await?;
                    MgoClientCommandResult::Object(object_read)
                } else {
                    let raw_object_read = client
                        .read_api()
                        .get_object_with_options(id, MgoObjectDataOptions::bcs_lossless())
                        .await?;
                    MgoClientCommandResult::RawObject(raw_object_read)
                }
            }

            MgoClientCommands::TransactionBlock { digest } => {
                let client = context.get_client().await?;
                let tx_read = client
                    .read_api()
                    ...
                    .await?;
                MgoClientCommandResult::TransactionBlock(tx_read)
            }

            MgoClientCommands::Call {
                package,
                module,
                function,
                type_args,
            ...
            MgoClientCommands::TransferMgo {
                to,
                mgo_coin_object_id: object_id,
                gas_budget,
                amount,
                serialize_unsigned_transaction,
                serialize_signed_transaction,
                ...
                let data = client
                    .transaction_builder()
                    .transfer_mgo(from, object_id, gas_budget, to, amount)
                    .await?;
                serialize_or_execute!(
                    data,
                    serialize_unsigned_transaction,
            ...
            MgoClientCommands::Pay {
                ...
                let recipients = recipients
                    ...
                    .collect::<Result<Vec<MgoAddress>, anyhow::Error>>()
                    .map_err(|e| anyhow!("{e}"))?;
            ...
            MgoClientCommands::PayMgo {
                ...
                serialize_signed_transaction,
                    ...
                    !input_coins.is_empty(),
                    "PayMgo transaction requires a non-empty list of input coins"
                );
                ensure!(
                    !recipients.is_empty(),
                    "PayMgo transaction requires a non-empty list of recipient addresses"
                );
                ensure!(
                    recipients.len() == amounts.len(),
                    format!(
            ...
            MgoClientCommands::PayAllMgo {
                ...
                serialize_signed_transaction,
                    ...
                    !input_coins.is_empty(),
                    "PayAllMgo transaction requires a non-empty list of input coins"
                );
            ...
            MgoClientCommands::Objects { address } => {
                    ...
                    let response = client
                        ...
                        .get_owned_objects(
                            ...
                            Some(MgoObjectResponseQuery::new_with_options(
                                MgoObjectDataOptions::full_content(),
                            )),
                            cursor,
                            None,
                        )
                ...
                MgoClientCommandResult::Objects(objects)
            }

            MgoClientCommands::NewAddress {
                key_scheme,
                alias,
                derivation_path,
                word_length,
                ...
                MgoClientCommandResult::NewAddress(NewAddressOutput {
                    alias,
                    address,
                    key_scheme: scheme,
            ...
            MgoClientCommands::Gas { address } => {
                ...
                let coins = context
                    ...
                    .collect();
                MgoClientCommandResult::Gas(coins)
            }
            MgoClientCommands::Faucet { address, url } => {
                let address = get_identity_address(address, context)?;
                let url = if let Some(url) = url {
                    ...
                    if let Ok(env) = active_env {
                        let network = match env.rpc.as_str() {
                            MGO_DEVNET_URL => "https://faucet.devnet.mangonetwork.io/v1/gas",
                            MGO_TESTNET_URL => "https://faucet.testnet.mangonetwork.io/v1/gas",
                            // TODO when using mgo-test-validator, and 5003 when using mgo start
                            MGO_LOCAL_NETWORK_URL => "http://127.0.0.1:9123/gas",
            ...
            MgoClientCommands::ChainIdentifier => {
                let ci = context
                    .get_client()
                    .await?
                    .read_api()
                    ...
                    .await?;
                MgoClientCommandResult::ChainIdentifier(ci)
            }
            MgoClientCommands::SplitCoin {
                coin_id,
                amounts,
                count,
                gas,
            ...
            MgoClientCommands::Switch { address, env } => {
                let mut addr = None;
                ...
                context.config.save()?;
                MgoClientCommandResult::Switch(SwitchResponse { address: addr, env })
            }
            MgoClientCommands::ActiveAddress => {
                MgoClientCommandResult::ActiveAddress(context.active_address().ok())
            }

            MgoClientCommands::ExecuteSignedTx {
                tx_bytes,
                signatures,
            } => {
                let data = bcs::from_bytes(
                    ...
                    .to_vec()
                    .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                ).map_err(|_| anyhow!("Failed to parse tx bytes, check if it matches the output of mgo client commands with --serialize-unsigned-transaction"))?;
                ...
                let response = context.execute_transaction_may_fail(transaction).await?;
                MgoClientCommandResult::ExecuteSignedTx(response)
            }
            MgoClientCommands::ExecuteCombinedSignedTx { signed_tx_bytes } => {
                let data: SenderSignedData = bcs::from_bytes(
                    &Base64::try_from(signed_tx_bytes)
                        .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                        .to_vec()
                        .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                ).map_err(|_| anyhow!("Failed to parse SenderSignedData bytes, check if it matches the output of mgo client commands with --serialize-signed-transaction"))?;
                let transaction = Envelope::<SenderSignedData, EmptySignInfo>::new(data);
                let response = context.execute_transaction_may_fail(transaction).await?;
                MgoClientCommandResult::ExecuteSignedTx(response)
            }
            MgoClientCommands::NewEnv { alias, rpc, ws } => {
                if context.config.envs.iter().any(|env| env.alias == alias) {
                    return Err(anyhow!(
                        "Environment config with name [{alias}] already exists."
                    ));
                }
                let env = MgoEnv { alias, rpc, ws };

                // Check urls are valid and server is reachable
                env.create_rpc_client(None, None).await?;
                context.config.envs.push(env.clone());
                context.config.save()?;
                MgoClientCommandResult::NewEnv(env)
            }
            MgoClientCommands::ActiveEnv => {
                MgoClientCommandResult::ActiveEnv(context.config.active_env.clone())
            }
            MgoClientCommands::Envs => MgoClientCommandResult::Envs(
                context.config.envs.clone(),
                context.config.active_env.clone(),
            ),
            MgoClientCommands::VerifySource {
                package_path,
                build_config,
                verify_deps,
                skip_source,
                ...
                MgoClientCommandResult::VerifySource
            }
        });
        ret
    }

    pub fn switch_env(config: &mut MgoClientConfig, env: &str) -> Result<(), anyhow::Error> {
        let env = Some(env.into());
        ensure!(config.get_env(&env).is_some(), "Environment config not found for [{env:?}], add new environment config using the `mgo client new-env` command.");
...
impl Display for MgoClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let mut writer = String::new();
        match self {
            MgoClientCommandResult::Addresses(addresses) => {
            ...
            MgoClientCommandResult::DynamicFieldQuery(df_refs) => {
            ...
            MgoClientCommandResult::Gas(gas_coins) => {
            ...
            MgoClientCommandResult::NewAddress(new_address) => {
                let mut builder = TableBuilder::default();
            ...
            MgoClientCommandResult::Object(object_read) => match object_read.object() {
                Ok(obj) => {
            ...
            MgoClientCommandResult::Objects(object_refs) => {
            ...
            MgoClientCommandResult::Upgrade(response)
            | MgoClientCommandResult::Publish(response) => {
                write!(writer, "{}", response)?;
            }
            MgoClientCommandResult::TransactionBlock(response) => {
                write!(writer, "{}", response)?;
            }
            MgoClientCommandResult::RawObject(raw_object_read) => {
                let raw_object = match raw_object_read.object() {
                    Ok(v) => match &v.bcs {
                        Some(MgoRawData::MoveObject(o)) => {
                            format!("{:?}\nNumber of bytes: {}", o.bcs_bytes, o.bcs_bytes.len())
                        }
                        Some(MgoRawData::Package(p)) => {
            ...
            MgoClientCommandResult::Call(response) => {
                write!(writer, "{}", response)?;
            }
            MgoClientCommandResult::SerializedUnsignedTransaction(tx_data) => {
                writeln!(
            ...
            MgoClientCommandResult::SerializedSignedTransaction(sender_signed_tx) => {
                writeln!(
                    writer,
                    "{}",
            ...
            MgoClientCommandResult::Transfer(response) => {
                write!(writer, "{}", response)?;
            }
            MgoClientCommandResult::TransferMgo(response) => {
                write!(writer, "{}", response)?;
            }
            MgoClientCommandResult::Pay(response) => {
                write!(writer, "{}", response)?;
            }
            MgoClientCommandResult::PayMgo(response) => {
                write!(writer, "{}", response)?;
            }
            MgoClientCommandResult::PayAllMgo(response) => {
                write!(writer, "{}", response)?;
            }
            MgoClientCommandResult::SyncClientState => {
                writeln!(writer, "Client state sync complete.")?;
            }
            MgoClientCommandResult::ChainIdentifier(ci) => {
                writeln!(writer, "{}", ci)?;
            }
            MgoClientCommandResult::SplitCoin(response) => {
                write!(writer, "{}", response)?;
            }
            MgoClientCommandResult::MergeCoin(response) => {
                write!(writer, "{}", response)?;
            }
            MgoClientCommandResult::Switch(response) => {
                write!(writer, "{}", response)?;
            }
            MgoClientCommandResult::ActiveAddress(response) => {
                match response {
            ...
            MgoClientCommandResult::ActiveEnv(env) => {
                write!(writer, "{}", env.as_deref().unwrap_or("None"))?;
            }
            MgoClientCommandResult::NewEnv(env) => {
                writeln!(writer, "Added new Mgo env [{}] to config.", env.alias)?;
            }
            MgoClientCommandResult::Envs(envs, active) => {
                let mut builder = TableBuilder::default();
...
async fn construct_move_call_transaction(
    ...
    // Convert all numeric input to String, this will allow number input from the CLI without failing MgoJSON's checks.
    let args = args
        .into_iter()
        .map(|value| MgoJsonValue::new(convert_number_to_string(value.to_json_value())))
        .collect::<Result<_, _>>()?;
...
impl Debug for MgoClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
...
impl MgoClientCommandResult {
    pub fn objects_response(&self) -> Option<Vec<MgoObjectResponse>> {
        use MgoClientCommandResult::*;
        match self {
    ...
    pub fn tx_block_response(&self) -> Option<&MgoTransactionBlockResponse> {
        use MgoClientCommandResult::*;
        match self {
            Upgrade(b) | Publish(b) | TransactionBlock(b) | Call(b) | Transfer(b)
            | TransferMgo(b) | Pay(b) | PayMgo(b) | PayAllMgo(b) | SplitCoin(b) | MergeCoin(b)
...
pub struct AddressesOutput {
    pub active_address: MgoAddress,
    pub addresses: Vec<(String, MgoAddress)>,
}
...
impl From<&MgoObjectData> for ObjectOutput {
    fn from(obj: &MgoObjectData) -> Self {
...
impl ObjectsOutput {
    fn from(obj: MgoObjectResponse) -> Result<Self, anyhow::Error> {
        let obj = obj.into_object()?;
        // this replicates the object type display as in the mgo explorer
        let object_type = match obj.type_ {
            Some(mgo_types::base_types::ObjectType::Struct(x)) => {
            ...
            Some(mgo_types::base_types::ObjectType::Package) => "Package".to_string(),
            None => "unknown".to_string(),
        };
        Ok(Self {
            object_id: obj.object_id,
    ...
    fn from_vec(objs: Vec<MgoObjectResponse>) -> Result<Vec<Self>, anyhow::Error> {
        objs.into_iter()
...
#[derive(Serialize)]
#[serde(untagged)]
pub enum MgoClientCommandResult {
    ActiveAddress(Option<MgoAddress>),
    ActiveEnv(Option<String>),
    Addresses(AddressesOutput),
    Call(MgoTransactionBlockResponse),
    ChainIdentifier(String),
    DynamicFieldQuery(DynamicFieldPage),
    Envs(Vec<MgoEnv>, Option<String>),
    ExecuteSignedTx(MgoTransactionBlockResponse),
    Gas(Vec<GasCoin>),
    MergeCoin(MgoTransactionBlockResponse),
    NewAddress(NewAddressOutput),
    NewEnv(MgoEnv),
    NoOutput,
    Object(MgoObjectResponse),
    Objects(Vec<MgoObjectResponse>),
    Pay(MgoTransactionBlockResponse),
    PayAllMgo(MgoTransactionBlockResponse),
    PayMgo(MgoTransactionBlockResponse),
    Publish(MgoTransactionBlockResponse),
    RawObject(MgoObjectResponse),
    SerializedSignedTransaction(SenderSignedData),
    SerializedUnsignedTransaction(TransactionData),
    SplitCoin(MgoTransactionBlockResponse),
    Switch(SwitchResponse),
    SyncClientState,
    TransactionBlock(MgoTransactionBlockResponse),
    Transfer(MgoTransactionBlockResponse),
    TransferMgo(MgoTransactionBlockResponse),
    Upgrade(MgoTransactionBlockResponse),
    VerifyBytecodeMeter {
...
pub async fn request_tokens_from_faucet(
    ...
    if let Some(err) = faucet_resp.error {
        ...
        println!("Request successful. It can take up to 1 minute to get the coin. Run mgo client gas to check your gas coins.");
    }
    Ok(())
}
