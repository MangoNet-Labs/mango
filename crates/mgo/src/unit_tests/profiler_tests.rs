...
/// `mgo = { path = ".", features = ["gas-profiler"] }` to our dev-dependencies.
...
async fn test_profiler() {
    ...
    let testnet_url = "https://fullnode.testnet.mangonetwork.io:443".to_string();
    ...
    let command_result =
        mgo_replay::execute_replay_command(Some(testnet_url), false, false, None, cmd).await;
