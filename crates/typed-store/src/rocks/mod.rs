...
use mgo_macros::{fail_point, nondeterministic};
use tap::TapFallible;
...
impl Default for ReadWriteOptions {
    fn default() -> Self {
        Self {
            ...
            sync_to_disk: std::env::var("MGO_DB_SYNC_TO_DISK").map_or(false, |v| v != "0"),
        }
    }
}
...
pub fn default_db_options() -> DBOptions {
    ...
    // Mgo uses multiple RocksDB in a node, so total sizes of write buffers and WAL can be higher
