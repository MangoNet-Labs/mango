...
/// Start a Mgo validator and fullnode for easy testing.
#[derive(Parser, Debug)]
#[clap(author, version, about, long_about = None)]
struct Args {
    /// Config directory that will be used to store network config, node db, keystore
    /// mgo genesis -f --with-faucet generates a genesis config that can be used to start this process.
    /// Example: mgo-test-validator --config-dir ~/.mgo/mgo_config
    /// We can use any config dir that is generated by the mgo genesis.
    #[clap(short, long)]
    ...
    /// Port to start the Mgo faucet on
    ...
    #[clap(long, default_value = "mgo_indexer")]
    pg_db_name: String,
...
async fn main() -> Result<()> {
    ...
    println!("Starting Mgo validator with config: {:#?}", cluster_config);
...
async fn faucet_request(
    ...
    let result = match payload {
        FaucetRequest::FixedAmountRequest(FixedAmountRequest { recipient }) => {
            state.faucet.request_mgo_coins(recipient).await
        }
        _ => {
            return (
...
async fn faucet_batch_request(
    ...
    let result = match payload {
        FaucetRequest::FixedAmountRequest(FixedAmountRequest { recipient }) => {
            state.faucet.batch_request_mgo_coins(recipient).await
        }
        _ => {
            return (
                StatusCode::BAD_REQUEST,
