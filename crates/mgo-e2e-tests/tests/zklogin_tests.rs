...
use mgo_types::MGO_AUTHENTICATOR_STATE_OBJECT_ID;
use test_cluster::{TestCluster, TestClusterBuilder};

use mgo_core::authority_client::AuthorityAPI;
use mgo_macros::sim_test;

async fn do_zklogin_test(address: MgoAddress, legacy: bool) -> MgoResult {
...
async fn test_zklogin_feature_legacy_address_deny() {
    ...
    let err = do_zklogin_test(get_legacy_zklogin_user_address(), true)
        .await
        .unwrap_err();
    assert!(matches!(err, MgoError::SignerSignatureAbsent { .. }));
}
...
async fn test_legacy_zklogin_address_accept() {
    ...
    assert!(matches!(err, MgoError::InvalidSignature { .. }));
}
    ...
    for h in &handles {
        h.with(|node| {
            assert!(node
                .state()
                .database
                .get_latest_object_ref_or_tombstone(MGO_AUTHENTICATOR_STATE_OBJECT_ID)
                .unwrap()
                .is_none());
        });
    }
