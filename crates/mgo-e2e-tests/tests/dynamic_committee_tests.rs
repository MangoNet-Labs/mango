...
use mgo_macros::*;
use mgo_swarm_config::genesis_config::{AccountConfig, DEFAULT_GAS_AMOUNT};
...
struct StressTestRunner {
    ...
    pub accounts: Vec<MgoAddress>,
    pub active_validators: BTreeSet<MgoAddress>,
    pub preactive_validators: BTreeMap<MgoAddress, u64>,
    pub removed_validators: BTreeSet<MgoAddress>,
    pub delegation_requests_this_epoch: BTreeMap<ObjectID, MgoAddress>,
    pub delegation_withdraws_this_epoch: u64,
    pub delegations: BTreeMap<ObjectID, MgoAddress>,
    pub reports: BTreeMap<MgoAddress, BTreeSet<MgoAddress>>,
    pub rng: StdRng,
}
...
impl StressTestRunner {
    ...
    pub fn pick_random_sender(&mut self) -> MgoAddress {
        self.accounts[self.rng.gen_range(0..self.accounts.len())]
    }

    pub fn system_state(&self) -> MgoSystemStateSummary {
        self.state()
    ...
    pub fn pick_random_active_validator(&mut self) -> MgoValidatorSummary {
    ...
    pub async fn run(&self, sender: MgoAddress, pt: ProgrammableTransaction) -> TransactionEffects {
    ...
    pub fn display_effects(&self, effects: &TransactionEffects) {
        ...
        for (obj_ref, _) in effects.created() {
            ...
            println!(">> {struct_tag} TOTAL_MGO: {total_mgo}");
        }
    ...
    pub fn state(&self) -> Arc<AuthorityState> {
        self.test_cluster.fullnode_handle.mgo_node.state()
    }
...
mod add_stake {
    ...
    impl GenStateChange for RequestAddStakeGen {
        ...
        fn create(&self, runner: &mut StressTestRunner) -> Self::StateChange {
            ...
            let staked_with = runner.pick_random_active_validator().mgo_address;
    ...
    impl StatePredicate for RequestAddStake {
        async fn run(&mut self, runner: &mut StressTestRunner) -> Result<TransactionEffects> {
            let pt = {
                ...
                builder.obj(ObjectArg::MGO_SYSTEM_MUT).unwrap();
                builder.pure(self.staked_with).unwrap();
                let coin = StressTestRunner::split_off(&mut builder, self.stake_amount);
                move_call! {
                    builder,
                    (MGO_SYSTEM_PACKAGE_ID)::mgo_system::request_add_stake(Argument::Input(0), coin, Argument::Input(1))
                };
                builder.finish()
            };
        ...
        async fn pre_epoch_post_condition(
            ...
            // Assert that a `StakedMgo` object matching the amount delegated is created.
            // Assert that this staked mgo
            let object = runner
                .get_created_object_of_type_name(effects, "StakedMgo")
                .await
                .unwrap();
