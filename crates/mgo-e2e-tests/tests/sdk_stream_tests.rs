...
use std::future;
use mgo_sdk::{MgoClientBuilder, MGO_COIN_TYPE};
...
//     let client = MgoClientBuilder::default().build(rpc_url).await?;
//     let txs = client
//         .read_api()
//         .get_transactions_stream(MgoTransactionBlockResponseQuery::default(), None, true)
//         .collect::<Vec<_>>()
//         .await;
...
//     MgoClientCommands::CreateExampleNFT {
//         name: None,
//         description: None,
//         url: None,
//         gas: None,
...
async fn test_coins_stream() -> Result<(), anyhow::Error> {
    ...
    let coins = client
        .coin_read_api()
        .get_coins_stream(address, Some(MGO_COIN_TYPE.to_string()))
        .collect::<Vec<_>>()
        .await;
    ...
    let page = client
        .coin_read_api()
        .get_coins(address, Some(MGO_COIN_TYPE.to_string()), None, None)
        .await?;
