...
mod sim_only_tests {
    ...
    use mgo_test_transaction_builder::publish_package;
    use mgo_types::messages_checkpoint::CheckpointSequenceNumber;
    use mgo_types::{base_types::ObjectID, digests::TransactionDigest};
    ...
    async fn object_pruning_test() {
        ...
        let fullnode = &test_cluster.fullnode_handle.mgo_node;
    ...
    async fn publish_package_and_create_parent_object(
        ...
        let package_id = publish_package(
            ...
            PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("../mgo-surfer/tests/move_building_blocks"),
        )
        .await
        .0;
    ...
    fn count_wrapped_tombstone(node: &MgoNode) -> usize {
        let db = node.state().db();
    ...
    async fn wait_until_checkpoint_pruned(node: &MgoNode, checkpoint: CheckpointSequenceNumber) {
        loop {
