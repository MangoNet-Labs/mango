...
use mgo_protocol_config::{ProtocolConfig, ProtocolVersion, SupportedProtocolVersions};
...
mod sim_only_tests {
    ...
    use std::sync::Arc;
    use mgo_core::authority::framework_injection;
    use mgo_framework::BuiltInFramework;
    use mgo_json_rpc_api::WriteApiClient;
    use mgo_json_rpc_types::{MgoTransactionBlockEffects, MgoTransactionBlockEffectsAPI};
    use mgo_macros::*;
    use mgo_move_build::{BuildConfig, CompiledPackage};
    use mgo_protocol_config::SupportedProtocolVersions;
    use mgo_types::base_types::ConciseableName;
    use mgo_types::base_types::{ObjectID, ObjectRef};
    use mgo_types::effects::{TransactionEffects, TransactionEffectsAPI};
    use mgo_types::id::ID;
    use mgo_types::object::Owner;
    use mgo_types::mgo_system_state::{
        epoch_start_mgo_system_state::EpochStartSystemStateTrait, get_validator_from_table,
        MgoSystemState, MgoSystemStateTrait, MGO_SYSTEM_STATE_SIM_TEST_DEEP_V2,
        MGO_SYSTEM_STATE_SIM_TEST_SHALLOW_V2, MGO_SYSTEM_STATE_SIM_TEST_V1,
    };
    ...
    use mgo_types::{
        base_types::{SequenceNumber, MgoAddress},
    ...
    use mgo_types::{
        MGO_AUTHENTICATOR_STATE_OBJECT_ID, MGO_CLOCK_OBJECT_ID, MGO_RANDOMNESS_STATE_OBJECT_ID,
    ...
    async fn test_framework_add_struct_ability() {
        ...
        transfer_obj(&cluster, MgoAddress::ZERO, to_transfer0).await;
        wrap_obj(&cluster, to_wrap1).await;
        transfer_obj(&cluster, MgoAddress::ZERO, to_transfer1).await;
    }
    ...
    async fn test_new_framework_package() {
        ...
        let mgo_extra = ObjectID::from_single_byte(0x42);
        framework_injection::set_override(mgo_extra, fixture_modules("extra_package"));
        ...
        // initializer
        let effects = get_framework_upgrade_effects(&cluster, &mgo_extra).await;
            ...
            .find_map(|(obj, owner)| {
                if let Owner::Shared { .. } = owner {
                    let is_framework_obj = [
                        MGO_SYSTEM_STATE_OBJECT_ID,
                        MGO_CLOCK_OBJECT_ID,
                        MGO_AUTHENTICATOR_STATE_OBJECT_ID,
                        MGO_RANDOMNESS_STATE_OBJECT_ID,
                    ]
                    .contains(&obj.0);
    ...
    async fn run_framework_upgrade(from: &str, to: &str) -> TestCluster {
        ...
        override_mgo_system_modules(to);
        TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_objects([mgo_system_package_object(from)])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, FINISH,
            ))
            .build()
    ...
    async fn get_framework_upgrade_versions(
        ...
        let effects = get_framework_upgrade_effects(cluster, &MGO_SYSTEM_PACKAGE_ID).await;

        let modified_at = effects
            .modified_at_versions()
            .iter()
            .find_map(|(id, v)| (id == &MGO_SYSTEM_PACKAGE_ID).then_some(*v));

        let mutated_to = effects
            .mutated()
            .iter()
            .find_map(|((id, v, _), _)| (id == &MGO_SYSTEM_PACKAGE_ID).then_some(*v));
    ...
    async fn get_framework_upgrade_effects(
        ...
        let node_handle = &cluster.fullnode_handle.mgo_node;
    ...
    async fn test_framework_compatible_upgrade_no_protocol_version() {
        ...
        override_mgo_system_modules("compatible");
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_objects([mgo_system_package_object("base")])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, START,
            ))
            .build()
    ...
    async fn test_framework_upgrade_conflicting_versions() {
        ...
        override_mgo_system_modules_cb(Box::new(move |name| {
            if name == first_name {
                info!("node {:?} using compatible packages", name.concise());
                Some(mgo_system_modules("base"))
            } else {
                Some(mgo_system_modules("compatible"))
            }
        }));
    ...
    async fn test_safe_mode_recovery() {
        override_mgo_system_modules("mock_mgo_systems/base");
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            // Overrides with a mgo system package that would abort during epoch change txn
            .with_objects([mgo_system_package_object("mock_mgo_systems/safe_mode")])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, FINISH,
            ))
            .build()
    ...
    #[sim_test]
    async fn mgo_system_mock_smoke_test() {
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, START,
            ))
            .with_objects([mgo_system_package_object("mock_mgo_systems/base")])
            .build()
            .await;
    ...
    #[sim_test]
    async fn mgo_system_state_shallow_upgrade_test() {
        override_mgo_system_modules("mock_mgo_systems/shallow_upgrade");
        ...
        assert!(matches!(system_state, MgoSystemState::SimTestV1(_)));
        ...
        assert!(matches!(system_state, MgoSystemState::SimTestShallowV2(_)));
    }

    #[sim_test]
    async fn mgo_system_state_deep_upgrade_test() {
        override_mgo_system_modules("mock_mgo_systems/deep_upgrade");
        ...
        if let MgoSystemState::SimTestV1(inner) = system_state {
            ...
            get_validator_from_table(
                test_cluster.fullnode_handle.mgo_node.state().db().as_ref(),
        ...
        if let MgoSystemState::SimTestDeepV2(inner) = system_state {
            // Make sure we have 1 inactive validator for latter testing.
            assert_eq!(inner.validators.inactive_validators.size, 1);
            get_validator_from_table(
                test_cluster.fullnode_handle.mgo_node.state().db().as_ref(),
    ...
    #[sim_test]
    async fn mgo_system_state_production_upgrade_test() {
        // Use this test to test a real mgo system state upgrade. To make this test work,
        // put the new mgo system in a new path and point to it in the override.
        ...
        override_mgo_system_modules("../../../mgo-framework/packages/mgo-system");
        // Wait for the upgrade to finish. After the upgrade, the new framework will be installed,
        // but the system state object hasn't been upgraded yet.
        let system_state = test_cluster.wait_for_epoch(Some(1)).await;
        assert_eq!(system_state.protocol_version(), FINISH);
        ...
        // at epoch boundary.
        let system_state = test_cluster.wait_for_epoch(Some(2)).await;
        if let MgoSystemState::V2(inner) = system_state {
            assert_eq!(inner.parameters.min_validator_count, 4);
        } else {
            unreachable!("Unexpected mgo system state version");
        }
    }
    ...
    fn override_mgo_system_modules(path: &str) {
        framework_injection::set_override(MGO_SYSTEM_PACKAGE_ID, mgo_system_modules(path));
    }

    fn override_mgo_system_modules_cb(f: framework_injection::PackageUpgradeCallback) {
        framework_injection::set_override_cb(MGO_SYSTEM_PACKAGE_ID, f)
    }

    /// Get compiled modules for Mgo System, built from fixture `fixture` in the
    /// `framework_upgrades` directory.
    fn mgo_system_modules(fixture: &str) -> Vec<CompiledModule> {
        fixture_package(fixture)
            .get_mgo_system_modules()
            .cloned()
            .collect()
    }

    /// Like `mgo_system_modules`, but package the modules in an `Object`.
    fn mgo_system_package_object(fixture: &str) -> Object {
        Object::new_package(
            &mgo_system_modules(fixture),
