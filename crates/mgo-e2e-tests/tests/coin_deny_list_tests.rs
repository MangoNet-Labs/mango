...
use mgo_json_rpc_types::MgoTransactionBlockKind;
use mgo_json_rpc_types::{MgoTransactionBlockDataAPI, MgoTransactionBlockResponseOptions};
use mgo_macros::sim_test;
use mgo_types::deny_list::RegulatedCoinMetadata;
use mgo_types::deny_list::{
...
async fn test_coin_deny_list_creation() {
    ...
    for handle in test_cluster.all_node_handles() {
        handle.with(|node| {
            ...
            assert_eq!(deny_list.id, UID::new(MGO_DENY_LIST_OBJECT_ID));
    ...
    let tx = test_cluster
        ...
        .mgo_client
        .read_api()
        .get_transaction_with_options(prev_tx, MgoTransactionBlockResponseOptions::full_content())
        .await
        .unwrap()
        .transaction
        .unwrap();
    ...
    for handle in test_cluster.all_node_handles() {
        handle.with(|node| {
            assert_eq!(
                node.state()
                    .database
                    .get_object(&MGO_DENY_LIST_OBJECT_ID)
                    .unwrap()
                    .unwrap()
