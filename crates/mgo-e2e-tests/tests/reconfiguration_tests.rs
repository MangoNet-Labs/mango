...
use mgo_core::consensus_adapter::position_submit_certificate;
...
use mgo_swarm_config::genesis_config::{ValidatorGenesisConfig, ValidatorGenesisConfigBuilder};
use mgo_test_transaction_builder::{make_transfer_mgo_transaction, TestTransactionBuilder};
...
use mgo_types::transaction::{TransactionDataAPI, TransactionExpiration};
...
async fn test_transaction_expiration() {
    ...
    *expired_data.expiration_mut_for_testing() = TransactionExpiration::Epoch(0);
    ...
    let result = authority
        ...
        .await;
    assert!(matches!(result.unwrap_err(), MgoError::TransactionExpired));
...
async fn test_create_advance_epoch_tx_race() {
    use std::sync::Arc;
    use mgo_macros::{register_fail_point, register_fail_point_async};
    use tokio::sync::broadcast;
    use tracing::info;

    telemetry_subscribers::init_for_testing();
    mgo_protocol_config::ProtocolConfig::poison_get_for_min_version();
    ...
    let register_wait = |failpoint, node_id, tx: Arc<broadcast::Sender<()>>| {
        let node = mgo_simulator::task::NodeId(node_id);
        register_fail_point_async(failpoint, move || {
            let cur_node = mgo_simulator::current_simnode_id();
            let tx = tx.clone();
            async move {
                if cur_node == node {
...
async fn test_validator_resign_effects() {
    ...
    // in the new epoch.
    let test_cluster = TestClusterBuilder::new().build().await;
    let tx = make_transfer_mgo_transaction(&test_cluster.wallet, None, None).await;
...
async fn test_validator_candidate_pool_read() {
    ...
    let address: MgoAddress = (&new_validator.account_key_pair.public()).into();
    let test_cluster = TestClusterBuilder::new()
        .with_validator_candidates([address])
        .build()
        .await;
    add_validator_candidate(&test_cluster, &new_validator).await;
    test_cluster.fullnode_handle.mgo_node.with(|node| {
        let system_state = node
            .state()
            .get_mgo_system_state_object_for_testing()
            .unwrap();
        let system_state_summary = system_state.clone().into_mgo_system_state_summary();
        ...
        .unwrap();
        assert_eq!(validator.mgo_address, address);
    });
}
...
async fn test_inactive_validator_pool_read() {
    ...
    let address = validator.with(|node| node.get_config().mgo_address());
    let staking_pool_id = test_cluster.fullnode_handle.mgo_node.with(|node| {
        node.state()
            .get_mgo_system_state_object_for_testing()
            .unwrap()
...
async fn safe_mode_reconfig_test() {
    use mgo_test_transaction_builder::make_staking_transaction;
    use mgo_types::mgo_system_state::advance_epoch_result_injection;
...
async fn add_validator_candidate(
    ...
    let cur_validator_candidate_count = test_cluster.fullnode_handle.mgo_node.with(|node| {
        node.state()
            .get_mgo_system_state_object_for_testing()
            .unwrap()
            .into_mgo_system_state_summary()
    ...
    test_cluster.fullnode_handle.mgo_node.with(|node| {
        let system_state = node
            ...
            .get_mgo_system_state_object_for_testing()
            .unwrap();
        let system_state_summary = system_state.into_mgo_system_state_summary();
        assert_eq!(
...
async fn execute_remove_validator_tx(test_cluster: &TestCluster, handle: &MgoNodeHandle) {
    let address = handle.with(|node| node.get_config().mgo_address());
    let gas = test_cluster
        .wallet
        .get_one_gas_object_owned_by_address(address)
        .await
...
async fn execute_add_validator_transactions(
    ...
    let pending_active_count = test_cluster.fullnode_handle.mgo_node.with(|node| {
        let system_state = node
            .state()
            .get_mgo_system_state_object_for_testing()
            .unwrap();
    ...
    test_cluster.fullnode_handle.mgo_node.with(|node| {
        ...
        assert_eq!(
            pending_active_validators[pending_active_validators.len() - 1].mgo_address,
            address
        );
    });
}
