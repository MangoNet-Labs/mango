...
module mgo_system::mgo_system {
    ...
    fun load_inner_maybe_upgrade(self: &mut MgoSystemState): &mut MgoSystemStateInner {
        let version = self.version;
        // TODO: This is where we check the version and perform upgrade if necessary.

        let inner: &mut MgoSystemStateInner = dynamic_field::borrow_mut(&mut self.id, version);
