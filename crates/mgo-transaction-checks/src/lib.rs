...
mod checked {
    ...
    use mgo_types::error::{UserInputError, UserInputResult};
    use mgo_types::executable_transaction::VerifiedExecutableTransaction;
    use mgo_types::metrics::BytecodeVerifierMetrics;
    ...
    use mgo_types::{
        MGO_AUTHENTICATOR_STATE_OBJECT_ID, MGO_CLOCK_OBJECT_ID, MGO_CLOCK_OBJECT_SHARED_VERSION,
    ...
    ) -> MgoResult<(MgoGasStatus, CheckedInputObjects)> {
    ...
    pub fn check_dev_inspect_input(
        ...
        let mut used_objects: HashSet<MgoAddress> = HashSet::new();
    ...
    fn check_gas(
        ...
        if tx_kind.is_system_tx() {
            Ok(MgoGasStatus::new_unmetered())
        } else {
            let gas_status =
                MgoGasStatus::new(gas_budget, gas_price, reference_gas_price, protocol_config)?;
    ...
    pub fn check_non_system_packages_to_be_published(
        ...
        let mut verifier = mgo_execution::verifier(protocol_config, is_metered, metrics);
