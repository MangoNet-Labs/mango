...
use mgo_move_build::CompiledPackage;
...
use mgo_sdk::apis::ReadApi;
use mgo_sdk::error::Error;

use mgo_sdk::rpc_types::{MgoObjectDataOptions, MgoRawData, MgoRawMoveObject, MgoRawMovePackage};
...
const CANONICAL_BINARY_NAME: &str = "mgo";
...
pub enum SourceVerificationError {
    ...
    MgoObjectRefFailure(MgoObjectResponseError),

    #[error("Dependency ID contains a Mgo object, not a Move package: {0}")]
    ObjectFoundWhenPackageExpected(ObjectID, MgoRawMoveObject),
...
impl<'a> BytecodeSourceVerifier<'a> {
    ...
    /// the bytecode at the address specified on the Mgo network we are publishing to.
    ...
    /// at the address specified on the Mgo network we are publishing to.  If `verify_deps` is true,
    ...
    async fn pkg_for_address(
        &self,
        addr: AccountAddress,
    ) -> Result<MgoRawMovePackage, SourceVerificationError> {
        // Move packages are specified with an AccountAddress, but are
        // fetched from a mgo network via mgo_getObject, which takes an object ID
        let obj_id = ObjectID::from(addr);

        // fetch the Mgo object at the address specified for the package in the local resolution table
        ...
        let obj_read = self
            .rpc_client
            .get_object_with_options(obj_id, MgoObjectDataOptions::new().with_bcs())
            .await
            .map_err(SourceVerificationError::DependencyObjectReadFailure)?;

        let obj = obj_read
            .into_object()
            .map_err(SourceVerificationError::MgoObjectRefFailure)?
            .bcs
            .ok_or_else(|| {
        ...
        match obj {
            MgoRawData::Package(pkg) => Ok(pkg),
            MgoRawData::MoveObject(move_obj) => Err(
    ...
    async fn on_chain_modules(
        ...
        for (storage_id, pkg) in addresses.zip(resp) {
            let MgoRawMovePackage { module_map, .. } = pkg?;
...
fn current_toolchain() -> ToolchainVersion {
    ToolchainVersion {
        ...
        flavor: Flavor::Mgo,      /* does not matter, unused for current_toolchain */
    }
}
...
fn units_for_toolchain(
    ...
    if std::env::var("MGO_RUN_TOOLCHAIN_BUILD").is_err() {
    ...
    for (package, local_unit) in compiled_units {
        ...
        if mgo_types::is_system_package(local_unit.unit.address.into_inner()) {
...
fn download_and_compile(
    ...
    if !dest_canonical_binary.exists() {
        ...
        let mainnet_url = format!(
            "https://github.com/MangoNetworkOs/Mango/releases/download/mainnet-v{compiler_version}/mgo-mainnet-v{compiler_version}-{platform}.tgz",
        );
		...
		println!(
		    "{} mgo mainnet compiler {} not available, attempting to download testnet compiler release...",
		    "WARNING".bold().yellow(),
		    compiler_version.yellow()
		);
		println!(
		...
		let testnet_url = format!("https://github.com/MangoNetworkOs/Mango/releases/download/testnet-v{compiler_version}/mgo-testnet-v{compiler_version}-{platform}.tgz");
        ...
        dest_binary.extend(["target", "release", &format!("mgo-{platform}")]);
...
fn detect_platform(
    ...
    let s = match (std::env::consts::OS, std::env::consts::ARCH) {
        ...
        (os, arch) => bail!(
	    "The package {} needs to be built with mgo compiler version {compiler_version} but there \
	     is no binary release available to download for your platform:\n\
	     Operating System: {os}\n\
	     Architecture: {arch}\n\
	     You can manually put a `mgo` binary for your platform in {} and rerun your command to continue.",
