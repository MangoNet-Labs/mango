...
use rand::rngs::OsRng;
use mgo_config::genesis::{TokenAllocation, TokenDistributionScheduleBuilder};
...
use mgo_types::base_types::{AuthorityName, MgoAddress};
use mgo_types::committee::{Committee, ProtocolVersion};
use mgo_types::crypto::{get_key_pair_from_rng, AccountKeyPair, KeypairTraits, PublicKey};
...
impl<R: rand::RngCore + rand::CryptoRng> ConfigBuilder<R> {
    ...
    pub fn build(self) -> NetworkConfig {
        ...
        let genesis = {
            let mut builder = mgo_genesis_builder::Builder::new()
...
mod test {
    ...
    fn genesis_transaction() {
        ...
        let genesis = network_config.genesis;
        let protocol_version = ProtocolVersion::new(genesis.mgo_system_object().protocol_version());
