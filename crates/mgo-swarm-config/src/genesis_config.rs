...
use mgo_config::genesis::{GenesisCeremonyParameters, TokenAllocation};
use mgo_config::node::{DEFAULT_COMMISSION_RATE, DEFAULT_VALIDATOR_GAS_PRICE};
use mgo_config::{local_ip_utils, Config};
use mgo_genesis_builder::validator_info::{GenesisValidatorInfo, ValidatorInfo};
use mgo_types::base_types::MgoAddress;
use mgo_types::crypto::{
    generate_proof_of_possession, get_key_pair_from_rng, AccountKeyPair, AuthorityKeyPair,
    AuthorityPublicKeyBytes, NetworkKeyPair, NetworkPublicKey, PublicKey, MgoKeyPair,
};
...
pub struct ValidatorGenesisConfig {
    ...
    #[serde(default = "default_mgo_key_pair")]
    pub account_key_pair: MgoKeyPair,
    #[serde(default = "default_ed25519_key_pair")]
...
impl ValidatorGenesisConfig {
    pub fn to_validator_info(&self, name: String) -> GenesisValidatorInfo {
        ...
        let info = ValidatorInfo {
            ...
            worker_key,
            network_key,
            account_address: MgoAddress::from(&account_key),
...
fn default_mgo_key_pair() -> MgoKeyPair {
    MgoKeyPair::Ed25519(get_key_pair_from_rng(&mut rand::rngs::OsRng).1)
}

#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct AccountConfig {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub address: Option<MgoAddress>,
...
impl GenesisConfig {
    ...
    pub fn for_local_testing_with_addresses(addresses: Vec<MgoAddress>) -> Self {
    ...
    pub fn benchmark_gas_keys(n: usize) -> Vec<MgoKeyPair> {
        let mut rng = StdRng::seed_from_u64(Self::BENCHMARKS_RNG_SEED);
        (0..n)
            .map(|_| MgoKeyPair::Ed25519(NetworkKeyPair::generate(&mut rng)))
            .collect()
    }
