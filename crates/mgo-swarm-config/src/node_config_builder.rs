...
use mgo_config::node::{default_zklogin_oauth_providers, ConsensusProtocol, RunWithRange};
use mgo_config::p2p::{P2pConfig, SeedPeer};
use mgo_config::{
...
use mgo_types::crypto::{AuthorityKeyPair, AuthorityPublicKeyBytes, NetworkKeyPair, MgoKeyPair};
...
impl ValidatorConfigBuilder {
    ...
    pub fn build(
        ...
        genesis: mgo_config::genesis::Genesis,
    ) -> NodeConfig {
        ...
        NodeConfig {
            ...
            network_key_pair: KeyPairWithPath::new(MgoKeyPair::Ed25519(validator.network_key_pair)),
            account_key_pair: KeyPairWithPath::new(validator.account_key_pair),
            worker_key_pair: KeyPairWithPath::new(MgoKeyPair::Ed25519(validator.worker_key_pair)),
            db_path,
            network_address,
            ...
            genesis: mgo_config::node::Genesis::new(genesis),
            grpc_load_shed: None,
...
impl FullnodeConfigBuilder {
    ...
    pub fn with_network_key_pair(mut self, network_key_pair: Option<NetworkKeyPair>) -> Self {
        if let Some(network_key_pair) = network_key_pair {
            self.network_key_pair =
                Some(KeyPairWithPath::new(MgoKeyPair::Ed25519(network_key_pair)));
        }
        self
    }
    ...
    pub fn build<R: rand::RngCore + rand::CryptoRng>(
        ...
        NodeConfig {
            ...
            worker_key_pair: KeyPairWithPath::new(MgoKeyPair::Ed25519(
                validator_config.worker_key_pair,
            )),
            network_key_pair: self.network_key_pair.unwrap_or(KeyPairWithPath::new(
                MgoKeyPair::Ed25519(validator_config.network_key_pair),
            )),
            db_path: self
                .db_path
            ...
            enable_index_processing: default_enable_index_processing(),
            genesis: self.genesis.unwrap_or(mgo_config::node::Genesis::new(
