...
use mgo_json::{is_receiving_argument, resolve_move_function_args, ResolvedCallArg, MgoJsonValue};
use mgo_json_rpc_types::{
    RPCTransactionRequestParams, MgoData, MgoObjectDataOptions, MgoObjectResponse, MgoRawData,
    MgoTypeTag,
};
use mgo_protocol_config::ProtocolConfig;
use mgo_types::base_types::{ObjectID, ObjectInfo, ObjectRef, ObjectType, MgoAddress};
use mgo_types::error::UserInputError;
use mgo_types::gas_coin::GasCoin;
use mgo_types::governance::{ADD_STAKE_MUL_COIN_FUN_NAME, WITHDRAW_STAKE_FUN_NAME};
...
use mgo_types::{coin, fp_ensure, MGO_FRAMEWORK_PACKAGE_ID, MGO_SYSTEM_PACKAGE_ID};
...
impl TransactionBuilder {
    ...
    async fn select_gas(
        ...
        if let Some(gas) = input_gas {
            ...
            for obj in gas_objs {
                let response = self
                    .0
                    .get_object_with_options(obj.object_id, MgoObjectDataOptions::new().with_bcs())
                    .await?;
    ...
    pub async fn transfer_mgo(
        ...
        let object = self.get_object_ref(mgo_object_id).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_transfer_mgo(
    ...
    pub async fn pay(
        ...
        if let Some(gas) = gas {
            if input_coins.contains(&gas) {
                return Err(anyhow!("Gas coin is in input coins of Pay transaction, use PayMgo transaction instead!"));
            }
        }
    ...
    pub async fn pay_all_mgo(
        ...
        let gas_object_ref = coin_refs.remove(0);
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_pay_all_mgo(
            signer,
            coin_refs,
            recipient,
            gas_object_ref,
    ...
    pub async fn move_call(
        ...
        module: &str,
        function: &str,
        type_args: Vec<MgoTypeTag>,
        call_args: Vec<MgoJsonValue>,
        gas: Option<ObjectID>,
        gas_budget: u64,
    ) -> anyhow::Result<TransactionData> {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    async fn resolve_and_checks_json_args(
        ...
        function: &Identifier,
        type_args: &[TypeTag],
        json_args: Vec<MgoJsonValue>,
    ) -> Result<Vec<Argument>, anyhow::Error> {
        let object = self
            .0
            .get_object_with_options(package_id, MgoObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
        let Some(MgoRawData::Package(package)) = object.bcs else {
            bail!(
    ...
    pub async fn upgrade(
        ...
        let upgrade_cap = self
            .0
            .get_object_with_options(upgrade_capability, MgoObjectDataOptions::new().with_owner())
            .await?
            .into_object()?;
    ...
    pub async fn split_coin(
        ...
        let coin = self
            .0
            .get_object_with_options(coin_object_id, MgoObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
    ...
    pub async fn merge_coins(
        ...
        let coin = self
            .0
            .get_object_with_options(primary_coin, MgoObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
    ...
    pub async fn request_add_stake(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            let arguments = vec![
                builder.input(CallArg::MGO_SYSTEM_MUT).unwrap(),
    ...
    pub async fn request_withdraw_stake(
        ...
        let staked_mgo = self.get_object_ref(staked_mgo).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        let gas = self
        ...
        TransactionData::new_move_call(
            ...
            vec![
                ...
                CallArg::Object(ObjectArg::ImmOrOwnedObject(staked_mgo)),
            ],
            gas_budget,
            gas_price,
        )
    ...
    async fn get_object_ref_and_type(
        ...
        let object = self
            .0
            .get_object_with_options(object_id, MgoObjectDataOptions::new().with_type())
            .await?
            .into_object()?;
