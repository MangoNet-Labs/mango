    ...
    input.block = syn::parse2(quote! {
        {
            ...
            std::thread::spawn(|| {
                ...
                ::mgo_simulator::telemetry_subscribers::init_for_testing();
                ::mgo_simulator::mgo_types::execution::get_denied_certificates();
                ::mgo_simulator::mgo_framework::BuiltInFramework::all_package_ids();
                ::mgo_simulator::mgo_types::gas::MgoGasStatus::new_unmetered();
                ...
                let mut cache = ::mgo_simulator::lru::LruCache::new(1.try_into().unwrap());
                cache.put(1, 1);

                {
                    ...
                    use mgo_simulator::mgo_move_build::{BuildConfig, MgoPackageHooks};
                    use mgo_simulator::tempfile::TempDir;
                    use mgo_simulator::move_package::package_hooks::register_package_hooks;

                    register_package_hooks(Box::new(MgoPackageHooks {}));
                    let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
                    path.extend(["..", "..", "mgo_programmability", "examples", "basics"]);
                ...
                use ::mgo_simulator::anemo_tower::callback::CallbackLayer;
                use ::mgo_simulator::anemo_tower::trace::DefaultMakeSpan;
                use ::mgo_simulator::anemo_tower::trace::DefaultOnFailure;
                use ::mgo_simulator::anemo_tower::trace::TraceLayer;
                use ::mgo_simulator::narwhal_network::metrics::MetricsMakeCallbackHandler;
                use ::mgo_simulator::narwhal_network::metrics::NetworkMetrics;

                use std::sync::Arc;
                use ::mgo_simulator::fastcrypto::traits::KeyPair;
                use ::mgo_simulator::rand_crate::rngs::{StdRng, OsRng};
                use ::mgo_simulator::rand::SeedableRng;
                use ::mgo_simulator::tower::ServiceBuilder;
                ...
                let rt = ::mgo_simulator::runtime::Runtime::new();
                rt.block_on(async move {
                    use ::mgo_simulator::anemo::{Network, Request};

                    let make_network = |port: u16| {
                        let registry = prometheus::Registry::new();
                        let inbound_network_metrics =
                            NetworkMetrics::new("mgo", "inbound", &registry);
                        let outbound_network_metrics =
                            NetworkMetrics::new("mgo", "outbound", &registry);
                        ...
                        let service = ServiceBuilder::new()
                            ...
                            .service(::mgo_simulator::anemo::Router::new());
                        ...
                        Network::bind(format!("127.0.0.1:{}", port))
                            ...
                            .private_key(
                                ::mgo_simulator::fastcrypto::ed25519::Ed25519KeyPair::generate(&mut StdRng::from_rng(OsRng).unwrap())
                                    .private()
                                    .0
                                    .to_bytes(),
                            )
...
/// The mgo_test macro will invoke either `#[msim::test]` or `#[tokio::test]`,
...
#[proc_macro_attribute]
pub fn mgo_test(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let header = if cfg!(msim) {
        quote! {
            #[::mgo_simulator::sim_test(crate = "mgo_simulator", #(#args)* )]
        }
    } else {
        quote! {
            #[::tokio::test(#(#args)*)]
    ...
    let result = quote! {
        #header
        #[::mgo_macros::init_static_initializers]
        #input
    };
    ...
    let result = if cfg!(msim) {
        ...
        quote! {
            ...
            #sig {
                ...
                ::mgo_simulator::task::shutdown_all_nodes();
                ...
                assert_eq!(
                    mgo_simulator::NodeLeakDetector::get_current_node_count(),
                    0,
                    "MgoNode leak detected"
                );
        ...
        quote! {
            ...
            #ignore
            #sig {
                if std::env::var("MGO_SKIP_SIMTESTS").is_ok() {
                    println!("not running test {} in `cargo test`: MGO_SKIP_SIMTESTS is set", stringify!(#fn_name));
...
pub fn enum_variant_order_derive(input: TokenStream) -> TokenStream {
    ...
    if let Data::Enum(DataEnum { variants, .. }) = ast.data {
        ...
        let deriv = quote! {
            impl mgo_enum_compat_util::EnumOrderMap for #name {
