...
use mgo_core::authority::CHAIN_IDENTIFIER;
...
pub use handle::MgoNodeHandle;
use mango_metrics::{spawn_monitored_task, RegistryService};
use mango_network::server::ServerBuilder;
use narwhal_network::metrics::MetricsMakeCallbackHandler;
use narwhal_network::metrics::{NetworkConnectionMetrics, NetworkMetrics};
use mgo_archival::reader::ArchiveReaderBalancer;
use mgo_archival::writer::ArchiveWriter;
use mgo_config::node::{ConsensusProtocol, DBCheckpointConfig, RunWithRange};
...
use mgo_core::authority_aggregator::AuthorityAggregator;
use mgo_core::authority_server::{ValidatorService, ValidatorServiceMetrics};
use mgo_core::checkpoints::checkpoint_executor::{CheckpointExecutor, StopReason};
use mgo_core::checkpoints::{
    CheckpointMetrics, CheckpointService, CheckpointStore, SendCheckpointToStateSync,
    SubmitCheckpointToConsensus,
};
use mgo_core::consensus_adapter::{
    CheckConnection, ConnectionMonitorStatus, ConsensusAdapter, ConsensusAdapterMetrics,
};
use mgo_core::consensus_manager::{ConsensusManager, ConsensusManagerTrait};
use mgo_core::consensus_throughput_calculator::{
    ConsensusThroughputCalculator, ConsensusThroughputProfiler, ThroughputProfileRanges,
};
use mgo_core::consensus_validator::{MgoTxValidator, MgoTxValidatorMetrics};
use mgo_core::db_checkpoint_handler::DBCheckpointHandler;
use mgo_core::epoch::committee_store::CommitteeStore;
use mgo_core::epoch::data_removal::EpochDataRemover;
use mgo_core::epoch::epoch_metrics::EpochMetrics;
use mgo_core::epoch::reconfiguration::ReconfigurationInitiator;
use mgo_core::in_mem_execution_cache::ExecutionCache;
use mgo_core::module_cache_metrics::ResolverMetrics;
use mgo_core::signature_verifier::SignatureVerifierMetrics;
use mgo_core::state_accumulator::StateAccumulator;
use mgo_core::storage::RocksDbStore;
use mgo_core::transaction_orchestrator::TransactiondOrchestrator;
use mgo_core::{
    authority::{AuthorityState, AuthorityStore},
    authority_client::NetworkAuthorityClient,
};
use mgo_json_rpc::coin_api::CoinReadApi;
use mgo_json_rpc::governance_api::GovernanceReadApi;
use mgo_json_rpc::indexer_api::IndexerApi;
use mgo_json_rpc::move_utils::MoveUtils;
use mgo_json_rpc::read_api::ReadApi;
use mgo_json_rpc::transaction_builder_api::TransactionBuilderApi;
use mgo_json_rpc::transaction_execution_api::TransactionExecutionApi;
use mgo_json_rpc::JsonRpcServerBuilder;
use mgo_macros::{fail_point_async, replay_log};
use mgo_network::api::ValidatorServer;
use mgo_network::discovery;
use mgo_network::discovery::TrustedPeerChangeEvent;
use mgo_network::state_sync;
use mgo_protocol_config::{Chain, ProtocolConfig, SupportedProtocolVersions};
use mgo_snapshot::uploader::StateSnapshotUploader;
use mgo_storage::{
...
mod simulator {
    ...
    pub(super) struct SimState {
        pub sim_node: mgo_simulator::runtime::NodeHandle,
        pub sim_safe_mode_expected: AtomicBool,
        _leak_detector: mgo_simulator::NodeLeakDetector,
    }

    impl Default for SimState {
        fn default() -> Self {
            Self {
                sim_node: mgo_simulator::runtime::NodeHandle::current(),
                sim_safe_mode_expected: AtomicBool::new(false),
                _leak_detector: mgo_simulator::NodeLeakDetector::new(),
            }
        }
    }

    type JwkInjector = dyn Fn(AuthorityName, &OIDCProvider) -> MgoResult<Vec<(JwkId, JWK)>>
        + Send
        + Sync
        + 'static;
    ...
    ) -> MgoResult<Vec<(JwkId, JWK)>> {
        use fastcrypto_zkp::bn254::zk_login::parse_jwks;
        // Just load a default Twitch jwk for testing.
        parse_jwks(
            mgo_types::zk_login_util::DEFAULT_JWK_BYTES,
            &OIDCProvider::Twitch,
        )
        .map_err(|_| MgoError::JWKRetrievalError)
    }
...
use mgo_core::consensus_handler::ConsensusHandlerInitializer;
use mgo_core::mysticeti_adapter::LazyMysticetiClient;
...
pub struct MgoNode {
    ...
    end_of_epoch_channel: broadcast::Sender<MgoSystemState>,
...
impl MgoNode {
    ...
    pub async fn start_async(
        ...
        if is_genesis {
            ...
            // check MGO conservation is at genesis. Otherwise we may be in the middle of
            // an epoch and the MGO conservation check will fail. This also initialize
            // the expected_network_mgo_amount table.
            store
                .expensive_check_mgo_conservation(&epoch_store)
                .expect("MGO conservation check cannot fail at genesis");
        }
        ...
        if epoch_store.epoch() == 0 {
            ...
            let transaction =
                mgo_types::executable_transaction::VerifiedExecutableTransaction::new_unchecked(
                    mgo_types::executable_transaction::ExecutableTransaction::new_from_data_and_sig(
                        genesis.transaction().data().clone(),
                        mgo_types::executable_transaction::CertificateProof::Checkpoint(0, 0),
                    ),
                );
            state
    ...
    pub async fn close_epoch(&self, epoch_store: &Arc<AuthorityPerEpochStore>) -> MgoResult {
        info!("close_epoch (current epoch = {})", epoch_store.epoch());
        self.validator_components
            .lock()
            .await
            .as_ref()
            .ok_or_else(|| MgoError::from("Node is not a validator"))?
            .consensus_adapter
            .close_epoch(epoch_store);
        Ok(())
    }

    pub fn clear_override_protocol_upgrade_buffer_stake(&self, epoch: EpochId) -> MgoResult {
        self.state
    ...
    pub async fn close_epoch_for_testing(&self) -> MgoResult {
    ...
    fn create_p2p_network(
        ...
        let p2p_network = {
            ...
            let inbound_network_metrics =
                NetworkMetrics::new("mgo", "inbound", prometheus_registry);
            let outbound_network_metrics =
                NetworkMetrics::new("mgo", "outbound", prometheus_registry);
            ...
            let server_name = format!("mgo-{}", chain_identifier);
    ...
    async fn construct_validator_components(
        ...
        let mgo_tx_validator_metrics =
            MgoTxValidatorMetrics::new(&registry_service.default_registry());
        ...
        consensus_manager
            .start(
                ...
                MgoTxValidator::new(
                    ...
                    mgo_tx_validator_metrics.clone(),
                ),
            )
            .await;
    ...
    async fn start_grpc_validator_service(
        ...
        let mut server_conf = mango_network::config::Config::new();
    ...
    pub fn get_google_jwk_bytes(&self) -> Result<Vec<u8>, MgoError> {
    ...
    pub async fn monitor_reconfiguration(self: Arc<Self>) -> Result<()> {
        ...
        loop {
            ...
            let latest_system_state = self
                ...
                .expect("Read Mgo System State object cannot fail");
            ...
            let next_epoch_committee = new_epoch_start_state.get_mgo_committee();
            ...
            *self.validator_components.lock().await = new_validator_components;
            ...
            if !matches!(
                ...
                .prune_checkpoints_for_eligible_epochs(
                    self.config.clone(),
                    mgo_core::authority::authority_store_pruner::AuthorityStorePruningMetrics::new_for_test(),
                )
                .await?;
            }
...
#[cfg(msim)]
impl MgoNode {
    pub fn get_sim_node_id(&self) -> mgo_simulator::task::NodeId {
...
pub fn build_http_server(
    ...
    let json_rpc_router = {
        ...
        let name_service_config =
            if let (Some(package_address), Some(registry_id), Some(reverse_registry_id)) = (
                ...
                mgo_json_rpc::name_service::NameServiceConfig::new(
                    package_address,
                    registry_id,
                    reverse_registry_id,
                )
            } else {
                mgo_json_rpc::name_service::NameServiceConfig::default()
            };
    ...
    if config.enable_experimental_rest_api {
        let rest_router = mgo_rest_api::rest_router(store);
    ...
    info!(local_addr =? addr, "Mgo JSON-RPC server listening on {addr}");
