...
use mango_common::sync::async_once_cell::AsyncOnceCell;
...
fn main() {
    ...
    move_vm_profiler::gas_profiler_feature_enabled! {
        panic!("Cannot run the mgo-node binary with gas-profiler feature enabled");
    }
    ...
    // if it deadlocks.
    let node_once_cell = Arc::new(AsyncOnceCell::<Arc<mgo_node::MgoNode>>::new());
    ...
    runtimes.mgo_node.spawn(async move {
        ...
        // when we get a shutdown signal from mgo-node, forward it on to the runtime_shutdown_channel here in
        // main to signal runtimes to all shutdown.
        tokio::select! {
           _ = shutdown_rx.recv() => {
                runtime_shutdown_tx.send(()).expect("failed to forward shutdown signal from mgo-node to mgo-node main");
            }
        }
    ...
    runtimes.metrics.spawn(async move {
        ...
        info!("Mgo chain identifier: {chain_identifier}");
        prometheus_registry
            .register(mango_metrics::uptime_metric(
                if is_validator {
                    "validator"
                } else {
                    "fullnode"
