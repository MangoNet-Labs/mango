...
use mango_network::metrics::MetricsCallbackProvider;
use prometheus::{
...
use mgo_network::tonic::Code;

use mango_metrics::RegistryService;
use tracing::error;

pub struct MetricsPushClient {
    certificate: std::sync::Arc<mgo_tls::SelfSignedCertificate>,
    client: reqwest::Client,
}

impl MetricsPushClient {
    pub fn new(network_key: mgo_types::crypto::NetworkKeyPair) -> Self {
        use fastcrypto::traits::KeyPair;
        let certificate = std::sync::Arc::new(mgo_tls::SelfSignedCertificate::new(
            network_key.private(),
            mgo_tls::MGO_VALIDATOR_SERVER_NAME,
        ));
    ...
    pub fn certificate(&self) -> &mgo_tls::SelfSignedCertificate {
        &self.certificate
    }
...
/// is configured.
pub fn start_metrics_push_task(config: &mgo_config::NodeConfig, registry: RegistryService) {
    use fastcrypto::traits::KeyPair;
    use mgo_config::node::MetricsConfig;
...
mod tests {
    ...
    pub async fn test_metrics_endpoint_with_multiple_registries_add_remove() {
        ...
        let registry_2 = Registry::new_custom(Some("mgo".to_string()), None).unwrap();
        ...
        assert!(result.contains(
            "# HELP mgo_counter_2 a sample counter 2
# TYPE mgo_counter_2 counter
mgo_counter_2 0"
        ));
