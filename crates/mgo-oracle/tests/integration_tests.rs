...
use mgo_json_rpc_types::{ObjectChange, MgoExecutionStatus};
...
use mgo_sdk::types::base_types::{ObjectID, MgoAddress};
use mgo_sdk::types::programmable_transaction_builder::ProgrammableTransactionBuilder;
use mgo_sdk::types::quorum_driver_types::ExecuteTransactionRequestType;
use mgo_sdk::types::transaction::{CallArg, ObjectArg, Transaction, TransactionData};
use mgo_sdk::types::Identifier;
use mgo_sdk::{MgoClient, MgoClientBuilder};
use mgo_types::base_types::{ObjectRef, SequenceNumber};
use mgo_types::{parse_mgo_type_tag, TypeTag};

// Integration tests for MGO Oracle, these test can be run manually on local or remote testnet.
#[ignore]
#[tokio::test]
async fn test_publish_primitive() {
    ...
    let signature = keystore
        .sign_secure(&sender, &data, Intent::mgo_transaction())
        .unwrap();
...
async fn test_publish_complex_value() {
    ...
    for i in 1..200 {
        ...
        builder.programmable_move_call(
            ...
            module.clone(),
            submit_data.clone(),
            vec![parse_mgo_type_tag(&format!("{package}::decimal_value::DecimalValue")).unwrap()],
...
async fn test_consume_oracle_data() {
    ...
    for _ in 0..3 {
        ...
        let ticker = builder
            .input(CallArg::Pure(bcs::to_bytes("MGOUSD".as_bytes()).unwrap()))
            .unwrap();
        let identifier = builder
            .input(CallArg::Pure(
                bcs::to_bytes("identifier".as_bytes()).unwrap(),
...
async fn get_gas(client: &MgoClient, sender: MgoAddress) -> (ObjectRef, u64) {
    let gas = client
        .coin_read_api()
...
async fn init_test_client() -> (MgoClient, Keystore, MgoAddress) {
    let client = MgoClientBuilder::default()
        .build("https://rpc.devnet.mangonetwork.io:443")
        .await
        .unwrap();
    ...
    let keystore = Keystore::File(
        FileBasedKeystore::new(
            &dirs::home_dir()
                .unwrap()
                .join(".mgo/mgo_config/mgo.keystore"),
        )
        .unwrap(),
    );
    let sender: MgoAddress = keystore.addresses()[0];
    let gas = client
        .coin_read_api()
        .get_coins(sender, None, None, Some(1))
        .await
