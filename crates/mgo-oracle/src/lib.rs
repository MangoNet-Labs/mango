...
use mgo_json_rpc_types::{
    MgoObjectDataOptions, MgoTransactionBlockEffects, MgoTransactionBlockEffectsAPI,
    MgoTransactionBlockResponseOptions,
};
use mgo_sdk::apis::ReadApi;
use mgo_sdk::rpc_types::MgoObjectResponse;
...
use mgo_types::quorum_driver_types::NON_RECOVERABLE_ERROR_MSG;
use mgo_types::transaction::{Argument, Transaction};
use mgo_types::transaction::{Command, ObjectArg};
...
use mgo_types::base_types::{random_object_ref, ObjectID, ObjectRef};
...
impl DataProviderRunner {
    pub async fn new(
        ...
        client: Arc<MgoClient>,
        signer_address: MgoAddress,
        metrics: Arc<OracleMetrics>,
    ) -> Self {
...
async fn get_gas_obj_ref(
    read_api: &ReadApi,
    gas_obj_id: ObjectID,
    owner_address: MgoAddress,
) -> ObjectRef {
    loop {
        match read_api
            .get_object_with_options(gas_obj_id, MgoObjectDataOptions::default().with_owner())
            .await
            .map(|resp| resp.data)
        {
            Ok(Some(gas_obj)) => {
...
impl OnChainDataUploader {
    ...
    async fn upload(
    ...
    ) -> anyhow::Result<MgoTransactionBlockEffects> {
        ...
        for data_point in &data_points {
            ...
            builder.command(Command::move_call(
                ...
                vec![
                    parse_mgo_type_tag(&format!("{package_id}::decimal_value::DecimalValue"))
                        .unwrap(),
                ],
                arguments,
            ));
    ...
    async fn execute(&mut self, tx: Transaction) -> anyhow::Result<MgoTransactionBlockResponse> {
        ...
        loop {
            match self
                ...
                Err(mgo_sdk::error::Error::RpcError(err)) => {
                    // jsonrpsee translate every MgoError into jsonrpsee::core::Error, so we need to further distinguish
        ...
        loop {
            ...
            for (feed_name, object_id) in &self.read_configs {
                match self
                    .client
                    .read_api()
                    .get_object_with_options(*object_id, MgoObjectDataOptions::default())
                    .await
                {
                    Ok(MgoObjectResponse {
                        data: Some(_data), ..
                    }) => {
