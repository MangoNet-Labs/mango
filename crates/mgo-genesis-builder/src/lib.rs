...
use mgo_types::base_types::{
    ExecutionDigests, ObjectID, SequenceNumber, MgoAddress, TransactionDigest, TxContext,
};
...
use mgo_types::crypto::{
    ...
    AuthoritySignature, DefaultHash, MgoAuthoritySignature,
};
use mgo_types::deny_list::{DENY_LIST_CREATE_FUNC, DENY_LIST_MODULE};
...
use mgo_types::in_memory_storage::InMemoryStorage;
...
impl Builder {
        ...
        let token_distribution_schedule =
            if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
                ...
                TokenDistributionSchedule::new_for_validators_with_default_allocation(
                    validators.iter().map(|v| v.info.mgo_address()),
                )
            };
    ...
    fn committee(objects: &[Object]) -> Committee {
        let mgo_system_object =
            get_mgo_system_state(&objects).expect("Mgo System State object must always exist");
        mgo_system_object.get_current_epoch_committee().committee
    }
    ...
    fn validate_inputs(&self) -> anyhow::Result<(), anyhow::Error> {
        ...
        if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
            ...
            token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
                self.validators.values().map(|v| v.info.mgo_address()),
            );
        }
    ...
    fn validate_output(&self) {
        ...
        let system_state = match unsigned_genesis.mgo_system_object() {
            MgoSystemState::V1(inner) => inner,
            MgoSystemState::V2(_) => unreachable!(),
            #[cfg(msim)]
            _ => {
            ...
            .zip(system_state.validators.active_validators.iter())
        {
            ...
            assert!(address_to_pool_id
                .insert(metadata.mgo_address, onchain_validator.staking_pool.id)
                .is_none());
            assert_eq!(validator.info.mgo_address(), metadata.mgo_address);
            assert_eq!(validator.info.protocol_key(), metadata.mgo_pubkey_bytes());
        ...
        let mut gas_objects: BTreeMap<ObjectID, (&Object, GasCoin)> = unsigned_genesis
            ...
            .collect();
        let mut staked_mgo_objects: BTreeMap<ObjectID, (&Object, StakedMgo)> = unsigned_genesis
            .objects()
            .iter()
            .filter_map(|o| StakedMgo::try_from(o).ok().map(|s| (o.id(), (o, s))))
            .collect();
        ...
        for allocation in token_distribution_schedule.allocations {
            if let Some(staked_with_validator) = allocation.staked_with_validator {
                ...
                let staked_mgo_object_id = staked_mgo_objects
                    .iter()
                    .find(|(_k, (o, s))| {
                ...
                let staked_mgo_object = staked_mgo_objects.remove(&staked_mgo_object_id).unwrap();
                assert_eq!(
        ...
        if !self.parameters.allow_insertion_of_extra_objects {
            ...
            assert!(staked_mgo_objects.is_empty());
        }
...
fn get_genesis_protocol_config(version: ProtocolVersion) -> ProtocolConfig {
    ...
    // MGO_PROTOCOL_CONFIG_CHAIN_OVERRIDE if necessary
...
fn build_unsigned_genesis_data(
    ...
    token_distribution_schedule.validate();
    token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
        genesis_validators.iter().map(|v| v.mgo_address),
    );
    ...
    // framework.
    let system_packages =
        mgo_framework_snapshot::load_bytecode_snapshot(parameters.protocol_version.as_u64())
    ...
    let genesis_transaction = {
        let genesis_objects = objects
            ...
            .map(|mut object| {
                ...
                mgo_types::transaction::GenesisObject::RawObject {
                    data: object.data,
                    owner: object.owner,
                }
            })
        ...
        mgo_types::transaction::VerifiedTransaction::new_genesis_transaction(genesis_objects)
            .into_inner()
    };
...
pub fn generate_genesis_system_object(
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        // Step 1: Create the MgoSystemState UID
        let mgo_system_state_uid = builder.programmable_move_call(
            MGO_FRAMEWORK_ADDRESS.into(),
            ident_str!("object").to_owned(),
            ident_str!("mgo_system_state").to_owned(),
            vec![],
            vec![],
        );
        ...
        // Step 4: Mint the supply of MGO.
        let mgo_supply = builder.programmable_move_call(
            MGO_FRAMEWORK_ADDRESS.into(),
            ident_str!("mgo").to_owned(),
        ...
        // Step 5: Run genesis.
        // The first argument is the system state uid we got from step 1 and the second one is the MGO supply we
        // got from step 3.
        let mut arguments = vec![mgo_system_state_uid, mgo_supply];
    ...
    {
        let object = written.get_mut(&mgo_types::MGO_CLOCK_OBJECT_ID).unwrap();
        object
            .data
            .try_as_move_mut()
            .unwrap()
...
mod test {
    ...
    use fastcrypto::traits::KeyPair;
    use mgo_config::genesis::*;
    use mgo_config::local_ip_utils;
    use mgo_config::node::DEFAULT_COMMISSION_RATE;
    use mgo_config::node::DEFAULT_VALIDATOR_GAS_PRICE;
    ...
    fn ceremony() {
        ...
        let validator = ValidatorInfo {
            ...
            account_address: MgoAddress::from(account_key.public()),
