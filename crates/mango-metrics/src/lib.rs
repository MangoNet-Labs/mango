...
macro_rules! monitored_future {
    ...
    ($metric: ident, $fut: expr, $name: expr, $logging_level: ident, $logging_enabled: expr) => {{
        ...
        async move {
            let metrics = mango_metrics::get_metrics();

            let _metrics_guard = if let Some(m) = metrics {
                m.$metric.with_label_values(&[location]).inc();
                Some(mango_metrics::scopeguard::guard(m, |metrics| {
            ...
            let _logging_guard = if $logging_enabled {
                Some(mango_metrics::scopeguard::guard((), |_| {
                    tracing::event!(
...
macro_rules! spawn_monitored_task {
    ($fut: expr) => {
        tokio::task::spawn(mango_metrics::monitored_future!(
...
mod tests {
    ...
    fn registry_service() {
        ...
        assert_eq!(metric_2.get_name(), "mgo_counter_2");
        ...
        let metric_1 = metrics.remove(0);
        assert_eq!(metric_1.get_name(), "mgo_counter_2");
