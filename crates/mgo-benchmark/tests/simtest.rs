...
mod test {
    ...
    use mgo_benchmark::bank::BenchmarkBank;
    use mgo_benchmark::system_state_observer::SystemStateObserver;
    use mgo_benchmark::workloads::adversarial::AdversarialPayloadCfg;
    use mgo_benchmark::workloads::workload_configuration::WorkloadConfiguration;
    use mgo_benchmark::{
    ...
    use mgo_config::{AUTHORITIES_DB_NAME, MGO_KEYSTORE_FILENAME};
    ...
    use mgo_core::checkpoints::{CheckpointStore, CheckpointWatermark};
    ...
    use mgo_simulator::{configs::*, SimConfig};
    use mgo_storage::blob::Blob;
    use mgo_types::base_types::{ObjectRef, MgoAddress};
    use mgo_types::full_checkpoint_content::CheckpointData;
    ...
    struct DeadValidator {
        node_id: mgo_simulator::task::NodeId,
    ...
    fn get_keep_alive_nodes(cluster: &TestCluster) -> HashSet<mgo_simulator::task::NodeId> {
        ...
        keep_alive_nodes.insert(mgo_simulator::current_simnode_id());
        keep_alive_nodes
    }

    fn handle_failpoint(
        dead_validator: Arc<Mutex<Option<DeadValidator>>>,
        keep_alive_nodes: HashSet<mgo_simulator::task::NodeId>,
        probability: f64,
    ) {
        ...
        if rng.gen_range(0.0..1.0) < probability {
            ...
            *dead_validator = Some(DeadValidator {
            ...
            mgo_simulator::task::kill_current_node(Some(restart_after));
        }
    }
    ...
    async fn test_simulated_load_reconfig_with_prune_and_compact() {
        ...
        let node_state = test_cluster.fullnode_handle.mgo_node.clone().state();
    ...
    async fn test_protocol_upgrade_compatibility_impl() {
        ...
        let manifest = mgo_framework_snapshot::load_bytecode_snapshot_manifest();
        ...
        let init_framework =
            mgo_framework_snapshot::load_bytecode_snapshot(starting_version).unwrap();
        ...
        let _handle = tokio::task::spawn(async move {
            ...
            for version in starting_version..=max_ver {
                ...
                let new_framework = mgo_framework_snapshot::load_bytecode_snapshot(next_version);
    ...
    impl TestInitData {
        pub async fn new(test_cluster: &TestCluster) -> Self {
            ...
            Self {
                keystore_path: test_cluster.swarm.dir().join(MGO_KEYSTORE_FILENAME),
