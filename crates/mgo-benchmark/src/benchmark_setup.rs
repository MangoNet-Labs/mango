...
impl Env {
    ...
    async fn setup_local_env(
        ...
        let (primary_gas_owner, keypair): (MgoAddress, AccountKeyPair) =
            deterministic_random_account_key();
        let keypair = Arc::new(keypair);

        // spawn a thread to spin up mgo nodes on the multi-threaded server runtime.
        // running forever
    ...
    async fn setup_remote_env(
        ...
        let genesis = mgo_config::node::Genesis::new_from_file(genesis_blob_path);
