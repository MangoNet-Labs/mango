...
/// A Mgo account and all of the objects it owns
#[derive(Debug)]
pub struct MgoAccount {
...
pub struct InMemoryWallet {
    accounts: BTreeMap<MgoAddress, MgoAccount>, // TODO: track shared and immutable objects as well
}
...
impl InMemoryWallet {
    ...
    pub fn add_account(
        ...
        self.accounts.insert(addr, MgoAccount::new(key, gas, objs));
    }
    ...
    pub fn account_mut(&mut self, addr: &MgoAddress) -> Option<&mut MgoAccount> {
        self.accounts.get_mut(addr)
    }

    pub fn account(&self, addr: &MgoAddress) -> Option<&MgoAccount> {
        self.accounts.get(addr)
    }

    pub fn gas(&self, addr: &MgoAddress) -> Option<&ObjectRef> {
        self.accounts.get(addr).map(|a| &a.gas)
    }

    pub fn owned_object(&self, addr: &MgoAddress, id: &ObjectID) -> Option<&ObjectRef> {
        self.accounts.get(addr).and_then(|a| a.owned.get(id))
    }

    pub fn owned_objects(&self, addr: &MgoAddress) -> Option<impl Iterator<Item = &ObjectRef>> {
    ...
    pub fn keypair(&self, addr: &MgoAddress) -> Option<Arc<AccountKeyPair>> {
    ...
    pub fn addresses(&self) -> impl Iterator<Item = &MgoAddress> {
        self.accounts.keys()
    }
