...
use std::sync::Arc;
use mgo_core::test_utils::{make_pay_mgo_transaction, make_transfer_mgo_transaction};
...
impl BenchmarkBank {
    ...
    pub async fn generate(
        ...
        for chunk in chunked_coin_configs {
            let gas_coins = self.pay_mgo(chunk, &mut init_coin, gas_price).await?;
    ...
    async fn pay_mgo(
        ...
        let recipient_addresses: Vec<MgoAddress> = coin_configs.iter().map(|g| g.address).collect();
        ...
        let tx = make_pay_mgo_transaction(
            init_coin.0,
            vec![],
            recipient_addresses,
            amounts,
        ...
        let address_map: HashMap<MgoAddress, Arc<AccountKeyPair>> = coin_configs
            .iter()
