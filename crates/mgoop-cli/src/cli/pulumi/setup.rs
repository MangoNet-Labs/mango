...
fn ensure_gcloud_logged_in() -> Result<()> {
    ...
    for account in accounts {
        ...
        if account.status == "ACTIVE" {
            if str::ends_with(name, "@mangonetlabs.com") {
                return Ok(());
            } else {
                error!(
                    "Please select your @mangonetlabs.com profile: {}",
    ...
    error!(
        "No gcloud credentials found! Please log into your @mangonetlabs.com account via: {}",
...
fn ensure_gcloud_adc_logged_in() -> Result<()> {
    match run_cmd(
        ...
        Err(_) => {
            error!(
                "No gcloud ADC (Application Default Credentials) found! Please log into your @mangonetlabs.com account via: {}",
...
pub fn ensure_setup() -> Result<()> {
    ...
    let setup_marker = PathBuf::from(format!("{}/.mgoop/pulumi_setup", home));
