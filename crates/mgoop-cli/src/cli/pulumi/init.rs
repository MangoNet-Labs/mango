...
impl ProjectType {
    pub fn create_project(&self, use_kms: &bool) -> Result<()> {
        // make sure we're in mgoops
        ensure_in_mgoops_repo()?;
...
fn ensure_in_mgoops_repo() -> Result<()> {
    let remote_stdout = run_cmd(
        vec!["git", "config", "--get", "remote.origin.url"],
        Some(CommandOptions::new(false, false)),
    )
    .context("run this command within the mgo-operations repository")?
    .stdout;
    let in_mgoops = String::from_utf8_lossy(&remote_stdout)
        .trim()
        .contains("mgo-operations");
    if !in_mgoops {
        Err(anyhow!(
            "please run this command from within the mgo-operations repository"
        ))
    } else {
        Ok(())
    }
...
fn get_pulumi_dir() -> Result<PathBuf> {
    let mgoops_dir_stdout = run_cmd(
        vec!["git", "rev-parse", "--show-toplevel"],
        Some(CommandOptions::new(false, false)),
    )
    .context("run this command from within the mgo-operations repository")?
    .stdout;
    let mgoops_dir = PathBuf::from(String::from_utf8_lossy(&mgoops_dir_stdout).trim());
    Ok(mgoops_dir.join("pulumi"))
}
...
fn run_pulumi_new(
    ...
    run_cmd(
        vec![
            ...
            "-c",
            &format!(
                r#"pulumi new go --dir {0} -d "pulumi project for {1}" --name "{1}"  --stack mangonet/dev --yes {2}"#,
...
fn run_pulumi_new_from_template(
    ...
    info!("extra pulumi options added: {}", &opts.bright_purple());
    let cmd = &format!(
        r#"pulumi new {3}/templates/{2} --dir {0} -d "pulumi project for {1}" --name "{1}"  --stack mangonet/dev {4}"#,
        project_dir_str,
        project_name,
        template_dir,
        get_pulumi_dir()?
...
fn create_basic_project(
    ...
    run_pulumi_new(project_name, project_dir_str, project_opts).map_err(|e| {
        ...
        remove_stack(&backend, project_name, "mangonet/dev").unwrap();
        e
    })?;
