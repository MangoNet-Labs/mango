...
// and add 1 MGO to the active address.
// By default, the example will use the Mgo testnet network (fullnode.testnet.mangonetwork.io:443).
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // use `None` for the default `Coin<MGO>` which is represented as
    // "0x2::mgo::MGO"
    let coin_type = Some("0x2::mgo::MGO".to_string());
    let coins = mgo
        .coin_read_api()
    ...
    let coins_stream = mgo.coin_read_api().get_coins_stream(active_address, None);
    ...
    // Select coins based on the provided coin type (MGO in this example). Use `None` for the default Mgo coin
    let select_coins = mgo
        .coin_read_api()
    ...
    // or if None is passed, it will use Coin<MGO> as the coin type
    let balance = mgo
        .coin_read_api()
    ...
    let total_balance = mgo.coin_read_api().get_all_balances(active_address).await?;
    println!(" *** Balance + Total Balance *** ");
    println!("Balance: {:?}", balance);
    println!("Total Balance: {:?}", total_balance);
    println!(" *** Balance + Total Balance ***\n ");

    // Return the coin metadata for the Coin<MGO>
