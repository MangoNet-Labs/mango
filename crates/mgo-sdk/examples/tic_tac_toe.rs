...
use mgo_json_rpc_types::{MgoObjectDataOptions, MgoTransactionBlockResponseOptions};
use mgo_keys::keystore::{AccountKeystore, FileBasedKeystore, Keystore};
use mgo_sdk::{
    json::MgoJsonValue,
    rpc_types::{MgoData, MgoTransactionBlockEffectsAPI},
    types::{
        base_types::{ObjectID, MgoAddress},
        id::UID,
        transaction::Transaction,
    },
    MgoClient, MgoClientBuilder,
};
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let game = TicTacToe {
        ...
        client: MgoClientBuilder::default()
...
impl TicTacToe {
    async fn create_game(
        &self,
        player_x: Option<MgoAddress>,
        player_o: Option<MgoAddress>,
    ) -> Result<(), anyhow::Error> {
        ...
        let create_game_call = self
            ...
            .move_call(
                ...
                vec![],
                vec![
                    MgoJsonValue::from_str(&player_x.to_string())?,
                    MgoJsonValue::from_str(&player_o.to_string())?,
                ],
        ...
        let signature =
            self.keystore
                .sign_secure(&player_x, &create_game_call, Intent::mgo_transaction())?;
        ...
        if game_state.is_my_turn(my_identity) {
            ...
            let place_mark_call = self
                ...
                .move_call(
                    ...
                    vec![],
                    vec![
                        MgoJsonValue::from_str(&game_state.info.object_id().to_hex_literal())?,
                        MgoJsonValue::from_str(&row.to_string())?,
                        MgoJsonValue::from_str(&col.to_string())?,
                    ],
                    None,
                    1000,
                )
    ...
    async fn fetch_game_state(&self, game_id: ObjectID) -> Result<TicTacToeState, anyhow::Error> {
        ...
        let current_game = self
            .client
            .read_api()
            .get_object_with_options(game_id, MgoObjectDataOptions::new().with_bcs())
            .await?;
        current_game
            .object()?
            .bcs
...
struct TicTacToeOpts {
    ...
    #[clap(long, default_value = "https://fullnode.devnet.mangonetwork.io:443")]
...
fn default_keystore_path() -> PathBuf {
    match dirs::home_dir() {
        Some(v) => v.join(".mgo").join("mgo_config").join("mgo.keystore"),
...
struct TicTacToeState {
    ...
    cur_turn: u8,
    game_status: u8,
    x_address: MgoAddress,
    o_address: MgoAddress,
}
...
impl TicTacToeState {
    ...
    fn is_my_turn(&self, my_identity: MgoAddress) -> bool {
