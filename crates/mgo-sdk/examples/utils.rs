...
use mgo_config::{
    mgo_config_dir, Config, PersistedConfig, MGO_CLIENT_CONFIG, MGO_KEYSTORE_FILENAME,
};
use mgo_json_rpc_types::{Coin, MgoObjectDataOptions};
use mgo_keys::keystore::{AccountKeystore, FileBasedKeystore};
use mgo_sdk::{
    mgo_client_config::{MgoClientConfig, MgoEnv},
...
use mgo_sdk::{rpc_types::MgoTransactionBlockResponseOptions, MgoClient, MgoClientBuilder};
...
// const MGO_FAUCET: &str = "https://faucet.devnet.mangonetwork.io/gas"; // devnet faucet

pub const MGO_FAUCET: &str = "https://faucet.testnet.mangonetwork.io/v1/gas"; // testnet faucet

// if you use the mgo-test-validator and use the local network; if it does not work, try with port 5003.
// const MGO_FAUCET: &str = "http://127.0.0.1:9123/gas";

/// Return a mgo client to interact with the APIs,
...
/// Return a mgo client to interact with the APIs and an active address from the local wallet.
///
/// This function sets up a wallet in case there is no wallet locally,
/// and ensures that the active address of the wallet has MGO on it.
/// If there is no MGO owned by the active address, then it will request
/// MGO from the faucet.
pub async fn setup_for_read() -> Result<(MgoClient, MgoAddress), anyhow::Error> {
    let client = MgoClientBuilder::default().build_testnet().await?;
    println!("Mgo testnet version is: {}", client.api_version());
...
pub async fn request_tokens_from_faucet(
    ...
    loop {
        let resp = client
            .get("https://faucet.testnet.mangonetwork.io/v1/status")
            .header("Content-Type", "application/json")
            .json(&json_body)
            .send()
            .await?;
    ...
    loop {
        let owner = mgo_client
            .read_api()
            .get_object_with_options(
                ObjectID::from_str(&coin_id)?,
                MgoObjectDataOptions::new().with_owner(),
            )
            .await?;
...
pub async fn split_coin_digest(
    ...
    let coin = match fetch_coin(mgo, sender).await? {
        None => {
            request_tokens_from_faucet(*sender, mgo).await?;
            fetch_coin(mgo, sender)
                .await?
                .expect("Supposed to get a coin with MGO, but didn't. Aborting")
        }
        Some(c) => c,
    };
...
pub async fn retrieve_wallet() -> Result<WalletContext, anyhow::Error> {
    let wallet_conf = mgo_config_dir()?.join(MGO_CLIENT_CONFIG);
    let keystore_path = mgo_config_dir()?.join(MGO_KEYSTORE_FILENAME);

    // check if a wallet exists and if not, create a wallet and a mgo client config
    ...
    if !wallet_conf.exists() {
        let keystore = FileBasedKeystore::new(&keystore_path)?;
        let mut client_config = MgoClientConfig::new(keystore.into());

        client_config.add_env(MgoEnv::testnet());
        client_config.add_env(MgoEnv::devnet());
        client_config.add_env(MgoEnv::localnet());
    ...
    let mut client_config: MgoClientConfig = PersistedConfig::read(&wallet_conf)?;
...
async fn just_for_clippy() -> Result<(), anyhow::Error> {
    let (mgo, sender, _recipient) = setup_for_write().await?;
    let _digest = split_coin_digest(&mgo, &sender).await?;
    Ok(())
}
