...
module axelar::utils {
    ...
    /// Prefix for Mgo Messages.
    const PREFIX: vector<u8> = b"\x19Mgo Signed Message:\n";

    /// Normalize last byte of the signature. Have it 1 or 0.
    /// See https://tech.mangonetlabs.com/cryptography-in-mgo-cross-chain-signature-verification/
    ...
    public fun to_mgo_signed(bytes: vector<u8>): vector<u8> {
        let res = vector[];
