...
module axelar::channel {
    ...
    use mgo::linked_table::{Self, LinkedTable};
    ...
    ///
    /// This struct is required on the Mgo side to be the destination for the
    ...
    ///
    /// - Does not contain direct link to the state in Mgo, as some functions
    /// might not take any specific data (eg allow users to create new objects).
    /// If specific object on Mgo is targeted by this `Channel`, its reference
    /// should be implemented using the `data` field.
    ///
    /// - The funniest and extremely simple implementation would be a `Channel<ID>`
    /// since it actually contains the data required to point at the object in Mgo.
