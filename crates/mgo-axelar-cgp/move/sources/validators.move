...
module axelar::validators {
    ...
    #[test]
    /// Tests `ecrecover`, makes sure external signing process works with Mgo ecrecover.
    ...
    fun test_ecrecover() {
        ...
        normalize_signature(&mut signature);
        let pubkey = ecdsa::secp256k1_ecrecover(&signature, &to_mgo_signed(message), 0);

        assert!(pubkey == SIGNER, 0);
    }

    #[test]
    /// Tests "Mgo Signed Message" prefix addition ecrecover.
    ...
    fun test_to_signed() {
        ...
        let pub_key = ecdsa::secp256k1_ecrecover(&signature, &to_mgo_signed(message), 0);
