...
///
/// - When call approvals is sent to Mgo, it targets an object and not a module;
/// - To support cross-chain messaging, a Channel object has to be created;
/// - Channel can be either owned or shared but not frozen;
/// - Module developer on the Mgo side will have to implement a system to support messaging;
...
module axelar::gateway {
    ...
    use axelar::utils::to_mgo_signed;
    ...
    /// For when approval chainId is not MGO.
    ...
    /// Emitted when a new message is sent from the MGO network.
    ...
    entry fun process_commands(
        ...
        let allow_operatorship_transfer = validate_proof(validators, to_mgo_signed(*&data), proof);
    ...
    fun test_execute() {
        use mgo::test_scenario::{Self as ts, ctx};
