...
# This script attempts to update the Fastcrypto pointer in Mgo
...
function latest_fc_revision() {
	...
	cd "$FC_CHECKOUT"
	git clone --depth 1 https://github.com/MangoNetworkOs/fastcrypto
	cd fastcrypto
	git rev-parse HEAD
}
...
function current_fc_revision() {
	cd "$TOPLEVEL"
	readarray -t <<< "$(find ./ -iname '*.toml' -exec $GREP -oPe 'git = "https://github.com/MangoNetworkOs/fastcrypto", *rev *= *\"\K[0-9a-fA-F]+' '{}' \;)"
	watermark=${MAPFILE[0]}
	for i in "${MAPFILE[@]}"; do
	    if [[ "$watermark" != "$i" ]]; then
        	not_equal=true
	...
	[[ -n "$not_equal" ]] && echo "Different values found for the current Fastcrypto revision in Mgo, aborting" && exit 1
	echo "$watermark"
}
...
if [[ "$CURRENT_FC" != "$LATEST_FC" ]]; then
	echo "About to replace $CURRENT_FC with $LATEST_FC as the Narwhal pointer in Mgo"
else
	exit 0
fi
