...
case "$NETWORK" in
  devnet)
    URL="https://$API_USER:$API_KEY@gateway.mimir.mangonetwork.io/prometheus/api/v1/query"
    ;;
  testnet)
    URL="http://$API_USER:$API_KEY@metrics-gw.testnet.mangonetwork.io/prometheus/api/v1/query"
    ;;
  mainnet)
    URL="https://$API_USER:$API_KEY@metrics-gw.mainnet.mangonetwork.io/prometheus/api/v1/query"
    ;;
esac
...
# check out all files in crates/mgo-protocol-config/src/snapshots at origin commit
echo "Checking out $NETWORK snapshot files"
git checkout $ORIGIN_COMMIT -- crates/mgo-protocol-config/src/snapshots || exit 1
...
cargo test --package mgo-protocol-config snapshot_tests || exit 1
