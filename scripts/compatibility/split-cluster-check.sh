...
if [ -f "$WORKING_DIR/mgo-node-release" ] && [ -f "$WORKING_DIR/mgo-release" ] && [ -f "$WORKING_DIR/mgo-node-candidate" ]; then
  echo "Binaries already built, skipping build"
else
  echo "Building mgo-node and mgo at $RELEASE_COMMIT"

  # remember current commit
  CURRENT_COMMIT=$(git rev-parse HEAD)

  git checkout $RELEASE_COMMIT || exit 1
  cargo build --bin mgo-node --bin mgo || exit 1
  cp ./target/debug/mgo-node "$WORKING_DIR/mgo-node-release"
  cp ./target/debug/mgo "$WORKING_DIR/mgo-release"

  echo "Building mgo-node at $RELEASE_CANDIDATE_COMMIT"
  git checkout $RELEASE_CANDIDATE_COMMIT || exit 1
  cargo build --bin mgo-node || exit 1
  cp ./target/debug/mgo-node "$WORKING_DIR/mgo-node-candidate"

  echo "returning to $CURRENT_COMMIT"
  git checkout $CURRENT_COMMIT || exit 1
fi

export MGO_CONFIG_DIR="$WORKING_DIR/config"
rm -rf "$MGO_CONFIG_DIR"

"$WORKING_DIR/mgo-release" genesis --epoch-duration-ms 20000
...
while IFS= read -r -d '' file; do
  CONFIGS+=("$file")
done < <(find "$MGO_CONFIG_DIR" -name "127.0.0.1*.yaml" -print0)

export RUST_LOG=mgo=debug,info

# 2 release nodes
"$WORKING_DIR/mgo-node-release" --config-path "${CONFIGS[0]}" > "$LOG_DIR/node-0.log" 2>&1 &
"$WORKING_DIR/mgo-node-release" --config-path "${CONFIGS[1]}" > "$LOG_DIR/node-1.log" 2>&1 &

# 2 candidate nodes
"$WORKING_DIR/mgo-node-candidate" --config-path "${CONFIGS[2]}" > "$LOG_DIR/node-2.log" 2>&1 &
"$WORKING_DIR/mgo-node-candidate" --config-path "${CONFIGS[3]}" > "$LOG_DIR/node-3.log" 2>&1 &

# and a fullnode
"$WORKING_DIR/mgo-node-release" --config-path "$MGO_CONFIG_DIR/fullnode.yaml" > "$LOG_DIR/fullnode.log" 2>&1 &
