...
CLEAN=0
LOG_LEVEL="info"
MGO_RUN_PATH="/opt/mgo"
VERBOSE=""

function cleanup {
    echo "Performing exit cleanup..."
    [ ! -z $MGO_NODE_PID ] && kill $MGO_NODE_PID && echo "Shutdown mgonode process running on pid $MGO_NODE_PID"
}
...
while getopts "hvn:e:p:t:" OPT; do
    case $OPT in
        ...
        h)
            ...
            >&2 echo ""
            >&2 echo "Options:"
            >&2 echo " -p                 Path to mgo binary to run. If unspecified, will build from source."
...
[ ! -d "${MGO_RUN_PATH}/mgodb" ] && mkdir -p ${MGO_RUN_PATH}/mgodb
...
if [[ ! -f "${MGO_RUN_PATH}/genesis.blob" ]]; then
    echo "Copying genesis.blob for ${NETWORK}"
    curl -fLJO https://github.com/MangoNetworkOs/Mango-genesis/raw/main/${NETWORK}/genesis.blob
    mv ./genesis.blob ${MGO_RUN_PATH}/genesis.blob
    echo "Done"
fi

if [[ ! -f "${MGO_RUN_PATH}/fullnode.yaml" ]]; then
    echo "Generating fullnode.yaml at ${MGO_RUN_PATH}/fullnode.yaml"
    cp crates/mgo-config/data/fullnode-template.yaml ${MGO_RUN_PATH}/fullnode.yaml
    sed -i "s|genesis.blob|${MGO_RUN_PATH}/genesis.blob|g" ${MGO_RUN_PATH}/fullnode.yaml
    sed -i "s|mgodb|${MGO_RUN_PATH}/mgodb|g" ${MGO_RUN_PATH}/fullnode.yaml

    if [[ $NETWORK != "devnet" ]]; then
        cat >> "${MGO_RUN_PATH}/fullnode.yaml" <<- EOM

p2p-config:
  seed-peers:
    - address: /dns/ewr-tnt-ssfn-00.testnet.mangonetwork.io/udp/8084
      peer-id: df8a8d128051c249e224f95fcc463f518a0ebed8986bbdcc11ed751181fecd38
    - address: /dns/lax-tnt-ssfn-00.testnet.mangonetwork.io/udp/8084
      peer-id: f9a72a0a6c17eed09c27898eab389add704777c03e135846da2428f516a0c11d
    - address: /dns/lhr-tnt-ssfn-00.testnet.mangonetwork.io/udp/8084
      peer-id: 9393d6056bb9c9d8475a3cf3525c747257f17c6a698a7062cbbd1875bc6ef71e
    - address: /dns/mel-tnt-ssfn-00.testnet.mangonetwork.io/udp/8084
...
if [[ -z $MGO_BIN_PATH ]]; then
    echo "Building mgo..."
    cargo build --release --bin mgo-node
    MGO_BIN_PATH="target/release/mgo-node"
    echo "Done"
fi


echo "Starting mgonode..."
RUST_LOG=$LOG_LEVEL $MGO_BIN_PATH --config-path ${MGO_RUN_PATH}/fullnode.yaml &
MGO_NODE_PID=$!
