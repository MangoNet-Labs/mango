...
def parse_args():
    ...
    generate_lib = subparsers.add_parser(
        "generate-lib",
        help=(
            "Generate `mgo-execution/src/lib.rs` based on the current set of "
...
def do_cut(args):
    ...
    with open(Path() / "mgo-execution" / "src" / "lib.rs", mode="w") as lib:
        generate_lib(lib)
...
def do_generate_lib(args):
    if args.dry_run:
        ...
        lib_path = Path() / "mgo-execution" / "src" / "lib.rs"
...
def cut_command(f):
    ...
    return [
        ...
        *["-d", f"mgo-execution/latest:mgo-execution/{f}:-latest"],
        *["-d", f"external-crates/move:external-crates/move/move-execution/{f}"],
        *["-p", "mgo-adapter-latest"],
        *["-p", "mgo-move-natives-latest"],
        *["-p", "mgo-verifier-latest"],
...
def cut_directories(f):
    ...
    mgo_base = Path() / "mgo-execution"
    external = Path() / "external-crates"

    crates = [
        mgo_base / f / "mgo-adapter",
        mgo_base / f / "mgo-move-natives",
        mgo_base / f / "mgo-verifier",
    ]
...
def impl(feature):
    ...
    return Path() / "mgo-execution" / "src" / (feature.replace("-", "_") + ".rs")
...
def update_toml(feature):
    """Add dependencies for 'feature' to mgo-execution's manifest."""
    toml_path = Path() / "mgo-execution" / "Cargo.toml"
...
def generate_impls(feature, copy):
    ...
    orig = Path() / "mgo-execution" / "src" / "latest.rs"
...
def generate_lib(output_file: TextIO):
    ...
    Generates the contents of mgo-execution/src/lib.rs to assign a numeric
    ...
    template_path = Path() / "mgo-execution" / "src" / "lib.template.rs"
    cuts = discover_cuts()
...
# Modules in `mgo-execution` that don't count as "cuts" (they are
...
def discover_cuts():
    ...
    Finds all modules within the `mgo-execution` crate that count as
