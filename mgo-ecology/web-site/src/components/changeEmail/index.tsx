...
import { changeUserInfo, changeIsLogin, selectShowChangeEmail, changeShowChangeEmail } from '@/lib/storemodules/appSlice'
...
export default function index() {
  ...
  const showChangeEmail = useSelector(selectShowChangeEmail)
  ...
  const closeModal = () => {
    dispatch(changeShowChangeEmail(false))
  }
  ...
  const sendCode = async () => {
    if (!email) {
      messageApi.open({ type: 'warning', content: t('t189') });
      return
    }
  ...
  const changeEmail = async () => {
    ...
    try {
      ...
      const res = await $clientPost('/changeEmail', {
        language: getLangType(i18n.language),
        email: email,
        code: code
      })
  ...
  return (
    <>
      {contextHolder}
      <Modal open={showChangeEmail} width={screenWidth >= 768 ? 720 : '90%'} maskClosable={false} onCancel={closeModal}>
        <div className='login-style pic-style'>
          <div className={
            clsx('login-text1 mb-3', Chiq_Reduced_Bold.className)
          }>{t('t207')}</div>
          <div className='mb-4 md:mb-6'>
            <div className='add-inputbox'>
              <Input placeholder={t('t208')} value={email} onChange={(e: any) => setEmail(e.target.value)} />
            </div>
          </div>
          <div className='mb-4 md:mb-6'>
            <div className='add-inputbox'>
          ...
          <div className='pt-[15px] md:pt-[30px] end-center gap-[5px]'>
            <div className='end-center gap-[10px] md:gap-5'>
              ...
              <Button loading={forgetLoading} onClick={changeEmail}>{t('t194')}</Button>
            </div>
          </div>
        </div>
      </Modal>
