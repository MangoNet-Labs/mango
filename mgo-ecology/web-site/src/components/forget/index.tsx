...
import { selectShowForget, changeShowForget, changeUserInfo, changeIsLogin } from '@/lib/storemodules/appSlice'
...
export default function index() {
  ...
  const showForget = useSelector(selectShowForget)
  ...
  const closeModal = () => {
    dispatch(changeShowForget(false))
  }
  ...
  return (
    <>
      {contextHolder}
      <Modal open={showForget} width={screenWidth >= 768 ? 720 : '90%'} maskClosable={false} onCancel={closeModal}>
        <div className='login-style'>
          ...
          <div className='text-xs md:text-sm text-grayb2'></div>
          <div className='end-center gap-[10px] md:gap-5'>
            ...
            <Button loading={forgetLoading} onClick={forgetPassword}>{t('t194')}</Button>
          </div>
        </div>
      </Modal>
    </>
