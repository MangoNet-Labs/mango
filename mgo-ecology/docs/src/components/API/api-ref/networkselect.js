    ...
    <StyledEngineProvider injectFirst>
      <div className="w-11/12">
        <FormControl fullWidth>
          ...
          >{`RPC: https://fullnode.${selection.toLowerCase()}.mangonetwork.io`}</InputLabel>
          <Select
            label-id="network"
            id="network-select"
            value={selection}
            label={`RPC: https://fullnode.${selection.toLowerCase()}.mangonetwork.io`}
            onChange={handleChange}
            className="dark:text-white dark:bg-mgo-ghost-dark"
          >
