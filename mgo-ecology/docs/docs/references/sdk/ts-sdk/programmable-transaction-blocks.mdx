...
// Retain a copy of the coins as gas fee in the transaction and return the created new coins with a balance of 250
const [coin] = txb.splitCoins(txb.gas, [250]);

// Transfer split coins to a specific address
txb.transferObjects([coin], '0xSomeAddress');
...
```tsx
// Creates a new coin with a custom amount from the provided coin cut, returning the new coin so that it can be used by subsequent transactions.
txb.splitCoins(coin, amounts)
txb.splitCoins(txb.gas, [250, 500])
...
// Construct a vector as an input parameter of the contract (vector cannot be used directly as a contract parameter)
...
For the input parameters of a specific transaction (`amounts` of `splitCoins` and `address` of `transferObjects`), the expected types are known, so the raw JavaScript values are passed directly when calling the method.
...
If objects not on the chain are provided as input, these values must be serialized to bcs.

You can use txb.pure for serialization, for example: `txb.pure.u64(number)` or `txb.pure(bcs.vector(bcs.U8).serialize(bytes))`

```tsx
const [coin1] = txb.splitCoins(txb.gas, [txb.pure.u64(250)]);
const [coin2] = txb.splitCoins(txb.gas, [txb.pure(bcs.U64.serialize(250))]);

txb.transferObjects([coin1], txb.pure.address('0xSomeAddress'));
txb.transferObjects([coin2], txb.pure(bcs.Address.serialize('0xSomeAddress')));
```
...
```tsx
const [object1, object2] = txb.moveCall({ target: '${packageId}::${moduleName}::${methodName}' });
txb.transferObjects([object1, object2], address);

const objects = txb.moveCall({ target: '${packageId}::${moduleName}::${methodName}' });
