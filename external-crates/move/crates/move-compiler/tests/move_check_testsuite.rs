...
use move_compiler::{
    ...
    mgo_mode::linters::{known_filters, linter_visitors},
...
const MGO_MODE_DIR: &str = "mgo_mode";
...
fn default_testing_addresses(flavor: Flavor) -> BTreeMap<String, NumericalAddress> {
    ...
    if flavor == Flavor::Mgo {
        mapping.extend([("mgo", "0x2"), ("mgo_system", "0x3")]);
    }
    mapping
        .into_iter()
...
fn move_check_testsuite(path: &Path) -> datatest_stable::Result<()> {
    ...
    let flavor = if path.components().any(|c| c.as_os_str() == MGO_MODE_DIR) {
        Flavor::Mgo
    } else {
        Flavor::default()
    };
