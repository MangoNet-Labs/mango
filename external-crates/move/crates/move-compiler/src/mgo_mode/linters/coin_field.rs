...
//! it's preferable to use mgo::balance::Balance instead to save space.
...
use super::{
    ...
    LINT_WARNING_PREFIX, MGO_PKG_NAME,
};
...
const COIN_FIELD_DIAG: DiagnosticInfo = custom(
    ...
    "sub-optimal 'mgo::coin::Coin' field type",
);
    ...
    if let N::StructFields::Defined(sfields) = &sdef.fields {
        for (floc, fname, (_, ftype)) in sfields.iter() {
            if is_field_coin_type(ftype) {
                let msg = format!("The field '{fname}' of '{sname}' has type 'mgo::coin::Coin'");
                let uid_msg = "Storing 'mgo::balance::Balance' in this field will typically be more space-efficient";
...
fn is_field_coin_type(sp!(_, t): &N::Type) -> bool {
    ...
    match t {
        ...
        T::Apply(_, tname, _) => {
            ...
            tname.is(MGO_PKG_NAME, COIN_MOD_NAME, COIN_STRUCT_NAME)
        }
