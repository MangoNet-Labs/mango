...
use crate::{
    ...
    mgo_mode::{OBJECT_NEW, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT},
};
...
use super::{
    ...
    OBJECT_NEW_UID_FROM_HASH, RANDOMNESS_MODULE_NAME, RANDOMNESS_STATE_CREATE, MGO_ADDR_NAME,
    MGO_CLOCK_CREATE, MGO_SYSTEM_ADDR_NAME, MGO_SYSTEM_CREATE, MGO_SYSTEM_MODULE_NAME,
    UID_TYPE_NAME,
};

pub const FRESH_ID_FUNCTIONS: &[(Symbol, Symbol, Symbol)] = &[
    (MGO_ADDR_NAME, OBJECT_MODULE_NAME, OBJECT_NEW),
    (MGO_ADDR_NAME, OBJECT_MODULE_NAME, OBJECT_NEW_UID_FROM_HASH),
    (MGO_ADDR_NAME, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT),
];
pub const FUNCTIONS_TO_SKIP: &[(Symbol, Symbol, Symbol)] = &[
    (
        MGO_SYSTEM_ADDR_NAME,
        MGO_SYSTEM_MODULE_NAME,
        MGO_SYSTEM_CREATE,
    ),
    (MGO_ADDR_NAME, CLOCK_MODULE_NAME, MGO_CLOCK_CREATE),
    (
        MGO_ADDR_NAME,
    ...
    (MGO_ADDR_NAME, DENY_LIST_MODULE_NAME, DENY_LIST_CREATE),
];
...
impl<'a> SimpleAbsInt for IDLeakVerifierAI<'a> {
    ...
    fn exp_custom(
        ...
        if !matches!(first_value, Value::FreshID(_)) {
            ...
            let uid_msg = format!(
                "The UID must come directly from {mgo}::{object}::{new}. \
                Or for tests, it can come from {mgo}::{ts}::{ts_new}",
                mgo = MGO_ADDR_NAME,
...
fn value_for_ty(loc: &Loc, sp!(_, t): &SingleType) -> Value {
    if t.is_apply(MGO_ADDR_NAME, OBJECT_MODULE_NAME, UID_TYPE_NAME)
        .is_some()
    {
        Value::NotFresh(*loc)
    } else {
