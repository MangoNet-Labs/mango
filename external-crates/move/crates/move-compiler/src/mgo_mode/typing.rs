...
pub struct MgoTypeChecks;

impl TypingVisitorConstructor for MgoTypeChecks {
...
pub struct Context<'a> {
    ...
    mgo_transfer_ident: Option<ModuleIdent>,
...
impl<'a> Context<'a> {
    fn new(env: &'a mut CompilationEnv, info: &'a TypingProgramInfo) -> Self {
        let mgo_module_ident = info
            .modules
            .key_cloned_iter()
            .find(|(m, _)| m.value.is(MGO_ADDR_NAME, TRANSFER_MODULE_NAME))
            .map(|(m, _)| m);
        Context {
            env,
            info,
            mgo_transfer_ident: mgo_module_ident,
            current_module: None,
            otw_name: None,
...
fn struct_def(context: &mut Context, name: StructName, sdef: &N::StructDefinition) {
    ...
    if !id_field_type
        .value
        .is(MGO_ADDR_NAME, OBJECT_MODULE_NAME, UID_TYPE_NAME)
    {
        let actual = format!(
            "But found type: {}",
...
fn invalid_object_id_field_diag(key_loc: Loc, loc: Loc, name: StructName) -> Diagnostic {
    const KEY_MSG: &str = "The 'key' ability is used to declare objects in Mgo";
...
fn tx_context_kind(sp!(_, last_param_ty_): &Type) -> TxContextKind {
    ...
    if inner_name.is(MGO_ADDR_NAME, TX_CONTEXT_MODULE_NAME, TX_CONTEXT_TYPE_NAME) {
        if *is_mut {
...
fn is_mut_clock(param_ty: &Type) -> bool {
    match &param_ty.value {
        ...
        Type_::Apply(_, sp!(_, n_), _) => n_.is(MGO_ADDR_NAME, CLOCK_MODULE_NAME, CLOCK_TYPE_NAME),
        Type_::Unit
        | Type_::Param(_)
        | Type_::Var(_)
        | Type_::Anything
...
fn is_entry_receiving_ty(param_ty: &Type) -> bool {
    match &param_ty.value {
        ...
        Type_::Apply(_, sp!(_, n), targs)
            if n.is(MGO_ADDR_NAME, TRANSFER_MODULE_NAME, RECEIVING_TYPE_NAME) =>
        {
    ...
    match &param_ty.value {
        ...
        Type_::Apply(_, sp!(_, n), targs)
            if n.is(STD_ADDR_NAME, ASCII_MODULE_NAME, ASCII_TYPE_NAME)
                ...
                || n.is(MGO_ADDR_NAME, OBJECT_MODULE_NAME, ID_TYPE_NAME) =>
        {
...
fn exp(context: &mut Context, e: &T::Exp) {
    match &e.exp.value {
        T::UnannotatedExp_::ModuleCall(mcall) => {
            ...
            if module.value.is(MGO_ADDR_NAME, EVENT_MODULE_NAME)
                && name.value() == EVENT_FUNCTION_NAME
            {
                check_event_emit(context, e.exp.loc, mcall)
            }
            let is_transfer_module = module.value.is(MGO_ADDR_NAME, TRANSFER_MODULE_NAME);
        ...
        T::UnannotatedExp_::Pack(m, s, _, _) => {
            if !context.in_test
                && !context
                    .current_module()
                    .value
                    .is(MGO_ADDR_NAME, MGO_MODULE_NAME)
...
fn check_private_transfer(context: &mut Context, loc: Loc, mcall: &ModuleCall) {
    ...
    if current_module
        .value
        .is(MGO_ADDR_NAME, TRANSFER_FUNCTION_NAME)
    {
