...
fn unused_module_members(context: &mut Context, mident: &ModuleIdent_, mdef: &T::ModuleDefinition) {
    ...
    let is_mgo_mode = context.env.package_config(mdef.package_name).flavor == Flavor::Mgo;
    context
        .env
    ...
    for (loc, name, fun) in &mdef.functions {
        ...
        if is_mgo_mode && *name == mgo_mode::INIT_FUNCTION_NAME {
            // a Mgo-specific filter to avoid signaling that the init function is unused
            continue;
        }
        context
            .env
