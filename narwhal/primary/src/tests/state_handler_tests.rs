...
async fn resume_from_store() {
    ...
    // RandomnessState elements for each authority.
    let mut vss_key_outputs = Vec::new();
    let mut randomness_stores = Vec::new();
    let mut randomness_states = Vec::new();
    let mut rx_system_messages_channels = Vec::new();
    ...
    // Collect generated Confirmations before simulated crash.
    let mut dkg_confirmations = Vec::new();
    for (i, channel) in rx_system_messages_channels.iter_mut().enumerate() {
        let dkg_confirmation = channel.recv().await.unwrap();
        match dkg_confirmation {
    ...
    // Recreate RandomnessStates using existing stores, to simulate a crash.
    randomness_states.clear();
    rx_system_messages_channels.clear();
    for (i, authority) in fixture.authorities().enumerate() {
        ...
        let mut randomness_state = RandomnessState::try_new(
            &ChainIdentifier::unknown(),
            &protocol_config,
            committee.clone(),
            authority.id(),
