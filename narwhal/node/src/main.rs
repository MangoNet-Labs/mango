...
use mgo_types::{
    crypto::{
        get_key_pair_from_rng, AuthorityKeyPair, AuthorityPublicKey, NetworkPublicKey, MgoKeyPair,
    },
    multiaddr::Multiaddr,
};
...
async fn main() -> Result<(), eyre::Report> {
    ...
    match &app.command {
        ...
        Commands::GenerateNetworkKeys { filename } => {
            ...
            write_keypair_to_file(&MgoKeyPair::Ed25519(network_keypair), filename).unwrap();
        }
