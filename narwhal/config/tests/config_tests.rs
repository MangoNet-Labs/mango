...
// 3. Mgo configurations & snapshot tests when upgrading Narwhal in Mgo to include the change.

#[test]
fn parameters_snapshot_matches() {
    // This configuration is load-bearing in the NW benchmarks,
    // and in Mgo (prod config + shared object bench base). If this test breaks,
