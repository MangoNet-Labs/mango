...
module utils::safe_tests {
    ...
    use mgo::test_scenario::{Self as ts, Scenario, ctx};
    ...
    fun create_safe(scenario: &mut Scenario, owner: address, stored_amount: u64) {
        ...
        {
            let coin = coin::mint_for_testing<MGO>(stored_amount, ctx(scenario));
    ...
    fun delegate_safe(scenario: &mut Scenario, owner: address, delegate_to: address, delegate_amount: u64): ID {
        let id;
        ts::next_tx(scenario, owner);
        let safe = ts::take_shared<Safe<MGO>>(scenario);
        let cap = ts::take_from_sender<OwnerCapability<MGO>>(scenario);
    ...
    fun withdraw_as_delegatee(scenario: &mut Scenario, delegatee: address, withdraw_amount: u64) {
        ...
        let safe = ts::take_shared<Safe<MGO>>(scenario);
        let capability = ts::take_from_sender<TransferCapability<MGO>>(scenario);
    ...
    fun test_safe_create_and_withdraw_funds_as_owner() {
        ...
        ts::next_tx(scenario, owner);
        let withdrawn_coin = ts::take_from_sender<Coin<MGO>>(scenario);
