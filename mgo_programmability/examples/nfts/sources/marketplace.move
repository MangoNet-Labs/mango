...
///
/// Makes use of `mgo::dynamic_object_field` module by attaching `Listing`
...
module nfts::marketplaceTests {
    ...
    fun create_marketplace(scenario: &mut Scenario) {
        ...
        marketplace::create<MGO>(test_scenario::ctx(scenario));
    }

    #[allow(unused_function)]
    /// Mint MGO and send it to BUYER.
    fun mint_some_coin(scenario: &mut Scenario) {
        test_scenario::next_tx(scenario, ADMIN);
        let coin = coin::mint_for_testing<MGO>(1000, test_scenario::ctx(scenario));
    ...
    // // SELLER lists Kitty at the Marketplace for 100 MGO.
    ...
    //     marketplace::list<Kitty, MGO>(mkp, &mut bag, nft, 100, test_scenario::ctx(scenario));
    ...
    //         let mkp = &mut mkp_val;
    //         let bag = test_scenario::take_child_object<Marketplace, Bag>(scenario, mkp);
    //         let listing = test_scenario::take_child_object<Bag, bag::Item<Listing<Kitty, MGO>>>(scenario, &bag);

    //         // Do the delist operation on a Marketplace.
    //         let nft = marketplace::delist<Kitty, MGO>(mkp, &mut bag, listing, test_scenario::ctx(scenario));
    ...
    //     // BUYER takes 100 MGO from his wallet and purchases Kitty.
    //     test_scenario::next_tx(scenario, BUYER);
    //     {
    //         let coin = test_scenario::take_from_sender<Coin<MGO>>(scenario);
    //         let mkp_val = test_scenario::take_shared<Marketplace>(scenario);
    //         let mkp = &mut mkp_val;
    //         let bag = test_scenario::take_child_object<Marketplace, Bag>(scenario, mkp);
    //         let listing = test_scenario::take_child_object<Bag, bag::Item<Listing<Kitty, MGO>>>(scenario, &bag);
    ...
    //         let nft = marketplace::buy<Kitty, MGO>(&mut bag, listing, payment);
    //         let kitty_id = burn_kitty(nft);
