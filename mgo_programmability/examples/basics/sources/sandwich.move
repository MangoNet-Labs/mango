...
module basics::sandwich {
    ...
    public fun collect_profits(_cap: &GroceryOwnerCapability, grocery: &mut Grocery, ctx: &mut TxContext): Coin<MGO> {
    ...
    fun test_make_sandwich() {
        ...
        test_scenario::next_tx(scenario, the_guy);
        {
            ...
            let ham = sandwich::buy_ham(
                grocery,
                coin::mint_for_testing<MGO>(10, ctx),
                ctx
            );

            let bread = sandwich::buy_bread(
                grocery,
                coin::mint_for_testing<MGO>(2, ctx),
                ctx
            );
