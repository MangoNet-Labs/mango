...
module capy::capy_winter {
    ...
    struct CapyPost has key { id: UID, balance: Balance<MGO> }
    ...
    entry fun buy_gift(post: &mut CapyPost, type: u8, payment: vector<Coin<MGO>>, ctx: &mut TxContext) {
    ...
    entry fun buy_premium(
        post: &mut CapyPost, ticket: PremiumTicket, payment: vector<Coin<MGO>>, ctx: &mut TxContext
    ) {
    ...
    fun merge_and_split(
        coins: vector<Coin<MGO>>, amount: u64, ctx: &mut TxContext
    ): (Coin<MGO>, Coin<MGO>) {
    ...
    fun get_link_url(id: &UID, type: u8): Url {
        ...
        vec::append(&mut res, mgo::hex::encode(object::uid_to_bytes(id)));
