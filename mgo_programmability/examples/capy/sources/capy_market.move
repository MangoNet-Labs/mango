...
module capy::capy_market {
    ...
    entry fun take_profits<T: key + store>(
        ...
        let profit = dof::remove<address, Coin<MGO>>(&mut market.id, sender);
    ...
    public fun purchase<T: key + store>(
        ...
        if (dof::exists_(&market.id, owner)) {
            coin::join(dof::borrow_mut<address, Coin<MGO>>(&mut market.id, owner), paid)
        } else {
    ...
    /// Use `&mut Coin<MGO>` to purchase `T` from marketplace.
