...
module capy::capy_item {
    ...
    /// Buy an Item from the `ItemStore`. Pay `Coin<MGO>` and
    /// receive a `CapyItem`.
    public entry fun buy_and_take(
        s: &mut ItemStore, name: vector<u8>, payment: Coin<MGO>, ctx: &mut TxContext
    ) {
        let listing_mut = dof::borrow_mut<vector<u8>, ListedItem>(&mut s.id, name);
    ...
    public entry fun buy_mut(
        s: &mut ItemStore, name: vector<u8>, payment: &mut Coin<MGO>, ctx: &mut TxContext
    ) {
    ...
    public entry fun buy_mul_coin(
        s: &mut ItemStore, name: vector<u8>, coins: vector<Coin<MGO>>, ctx: &mut TxContext
    ) {
