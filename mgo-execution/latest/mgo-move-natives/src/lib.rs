...
use mgo_types::{MOVE_STDLIB_ADDRESS, MGO_FRAMEWORK_ADDRESS, MGO_SYSTEM_ADDRESS};
...
pub fn all_natives(silent: bool) -> NativeFunctionTable {
    let mgo_framework_natives: &[(&str, &str, NativeFunction)] = &[
    ...
    let mgo_framework_natives_iter =
        mgo_framework_natives
            .iter()
            .cloned()
    ...
    let mgo_system_natives: &[(&str, &str, NativeFunction)] = &[(
        "validator",
    ...
    mgo_system_natives
        ...
        .chain(mgo_framework_natives_iter)
        ...
        _ => {
            return Err(
                PartialVMError::new(StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR)
                    .with_message("Mgo verifier guarantees this is a struct".to_string()),
            )
        }
    };
