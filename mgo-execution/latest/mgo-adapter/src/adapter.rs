...
mod checked {
    ...
    use mgo_verifier::{check_for_verifier_timeout, default_verifier_config};
    ...
    use mgo_types::{
        ...
        error::{ExecutionErrorKind, MgoError},
        metrics::LimitsMetrics,
        storage::ChildObjectResolver,
    };
    use mgo_verifier::verifier::mgo_verify_module_metered_check_timeout_only;

    pub fn new_move_vm(
        natives: NativeFunctionTable,
        protocol_config: &ProtocolConfig,
        _enable_profiler: Option<PathBuf>,
    ) -> Result<MoveVM, MgoError> {
        ...
        .map_err(|_| MgoError::ExecutionInvariantViolation)
    }
            ...
            if let Err(e) = verify_module_with_config_metered(verifier_config, module, meter) {
                ...
                if check_for_verifier_timeout(&e.major_status()) {
                    ...
                    metrics
                        ...
                        .inc();
                    return Err(MgoError::ModuleVerificationFailure {
                        error: format!("Verification timedout: {}", e),
                    });
                };
            } else if let Err(err) = mgo_verify_module_metered_check_timeout_only(
                module,
                &BTreeMap::new(),
                meter,
                verifier_config,
