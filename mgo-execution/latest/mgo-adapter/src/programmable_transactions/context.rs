...
mod checked {
    ...
    use mgo_move_natives::object_runtime::{
        self, get_all_uids, max_event_error, LoadedRuntimeObject, ObjectRuntime, RuntimeResults,
    };
    use mgo_protocol_config::ProtocolConfig;
    use mgo_types::execution::ExecutionResults;
    use mgo_types::storage::PackageObject;
    use mgo_types::{
        balance::Balance,
        base_types::{MoveObjectType, ObjectID, MgoAddress, TxContext},
        coin::Coin,
    ...
    pub struct ExecutionContext<'vm, 'state, 'a> {
        ...
        additional_transfers: Vec<(/* new owner */ MgoAddress, ObjectValue)>,
    ...
    impl<'vm, 'state, 'a> ExecutionContext<'vm, 'state, 'a> {
        ...
        where
            'a: 'state,
        {
            let mut linkage_view = LinkageView::new(Box::new(state_view.as_mgo_resolver()));
        ...
        pub fn convert_type_argument_error(&self, idx: usize, error: VMError) -> ExecutionError {
            use move_core_types::vm_status::StatusCode;
            use mgo_types::execution_status::TypeArgumentError;
        ...
        pub(crate) fn execute_function_bypass_visibility(
            ...
            let mut data_store = MgoDataStore::new(&self.linkage_view, &self.new_packages);
    ...
    pub fn load_type_from_struct(
        ...
        let data_store = MgoDataStore::new(linkage_view, new_packages);
    ...
    pub(crate) struct MgoDataStore<'state, 'a> {
        linkage_view: &'a LinkageView<'state>,
        new_packages: &'a [MovePackage],
    }

    impl<'state, 'a> MgoDataStore<'state, 'a> {
        pub(crate) fn new(
            linkage_view: &'a LinkageView<'state>,
            new_packages: &'a [MovePackage],
        ) -> Self {
    ...
    impl<'state, 'a> DataStore for MgoDataStore<'state, 'a> {
