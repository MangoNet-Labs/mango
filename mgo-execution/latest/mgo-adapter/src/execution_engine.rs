...
#[mgo_macros::with_checked_arithmetic]
mod checked {

    use move_binary_format::CompiledModule;
    use move_vm_runtime::move_vm::MoveVM;
    use std::{collections::HashSet, sync::Arc};
    use mgo_types::balance::{
        BALANCE_CREATE_REWARDS_FUNCTION_NAME, BALANCE_DESTROY_REBATES_FUNCTION_NAME,
        BALANCE_MODULE_NAME,
    };
    use mgo_types::execution_mode::{self, ExecutionMode};
    use mgo_types::gas_coin::GAS;
    use mgo_types::messages_checkpoint::CheckpointTimestamp;
    use mgo_types::metrics::LimitsMetrics;
    use mgo_types::object::OBJECT_START_VERSION;
    use mgo_types::programmable_transaction_builder::ProgrammableTransactionBuilder;
    use mgo_types::randomness_state::{
    ...
    use mgo_protocol_config::{check_limit_by_meter, LimitThresholdCrossed, ProtocolConfig};
    use mgo_types::authenticator_state::{
    ...
    use mgo_types::clock::{CLOCK_MODULE_NAME, CONSENSUS_COMMIT_PROLOGUE_FUNCTION_NAME};
    ...
    use mgo_types::error::{ExecutionError, ExecutionErrorKind};
    use mgo_types::execution::is_certificate_denied;
    ...
    #[cfg(msim)]
    use mgo_types::mgo_system_state::advance_epoch_result_injection::maybe_modify_result;
    use mgo_types::mgo_system_state::{AdvanceEpochParams, ADVANCE_EPOCH_SAFE_MODE_FUNCTION_NAME};
    ...
    use mgo_types::transaction::{CheckedInputObjects, RandomnessStateUpdate};
    use mgo_types::{
        base_types::{ObjectRef, MgoAddress, TransactionDigest, TxContext},
        object::{Object, ObjectInner},
        mgo_system_state::{ADVANCE_EPOCH_FUNCTION_NAME, MGO_SYSTEM_MODULE_NAME},
        MGO_AUTHENTICATOR_STATE_OBJECT_ID, MGO_FRAMEWORK_ADDRESS, MGO_FRAMEWORK_PACKAGE_ID,
    ...
    pub fn execute_transaction_to_effects<Mode: ExecutionMode>(
        ...
        let status = if let Err(error) = &execution_result {
            ...
            match error.kind() {
                ...
                K::MgoMoveVerificationError | K::VMVerificationOrDeserializationError => {
                    #[skip_checked_arithmetic]
                    tracing::debug!(
                        kind = ?error.kind(),
                        tx_digest = ?transaction_digest,
    ...
    fn execute_transaction<Mode: ExecutionMode>(
        ...
        // information provided to check_mgo_conserved, because we mint rewards, and burn
    ...
    fn run_conservation_checks<Mode: ExecutionMode>(
        ...
        let mut result: std::result::Result<(), mgo_types::error::ExecutionError> = Ok(());
        if !is_genesis_tx && !Mode::skip_conservation_checks() {
            // ensure that this transaction did not create or destroy MGO, try to recover if the check fails
            let conservation_result = {
                temporary_store
                    .check_mgo_conserved(simple_conservation_checks, cost_summary)
                    .and_then(|()| {
                        if enable_expensive_checks {
                            // ensure that this transaction did not create or destroy MGO, try to recover if the check fails
                            let mut layout_resolver =
                                TypeLayoutResolver::new(move_vm, Box::new(&*temporary_store));
                            temporary_store.check_mgo_conserved_expensive(
                                cost_summary,
                                advance_epoch_gas_summary,
                                &mut layout_resolver,
                            )
            ...
            if let Err(conservation_err) = conservation_result {
                ...
                if let Err(recovery_err) = {
                    ...
                    // we will create or destroy MGO otherwise
                    panic!(
                        "MGO conservation fail in tx block {}: {}\nGas status is {}\nTx was ",
                        tx_ctx.digest(),
                        recovery_err,
                        gas_charger.summary()
                    )
        ...
        } // else, we're in the genesis transaction which mints the MGO supply, and hence does not satisfy MGO conservation, or
            ...
            TransactionKind::Genesis(GenesisTransaction { objects }) => {
                ...
                for genesis_object in objects {
                    match genesis_object {
                        mgo_types::transaction::GenesisObject::RawObject { data, owner } => {
    ...
    pub fn construct_advance_epoch_pt(
        ...
        let storage_rebates = builder.programmable_move_call(
            MGO_SYSTEM_PACKAGE_ID,
            MGO_SYSTEM_MODULE_NAME.to_owned(),
