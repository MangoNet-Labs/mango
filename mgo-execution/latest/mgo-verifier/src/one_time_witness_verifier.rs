...
pub fn verify_module(
    ...
    // In Mgo's framework code there is an exception to the one-time witness type rule - we have a
    // MGO type in the mgo module but it is instantiated outside of the module initializer (in fact,
    // the module has no initializer). The reason for it is that the MGO coin is only instantiated
    ...
    if ModuleId::new(MGO_FRAMEWORK_ADDRESS, ident_str!("mgo").to_owned()) == module.self_id() {
        return Ok(());
    }
