...
use mgo_types::{error::ExecutionError, MGO_FRAMEWORK_ADDRESS};
...
/// All transfer functions (the functions in `mgo::transfer`) are "private" in that they are
...
pub fn verify_module(
    ...
    if *module.address() == MGO_FRAMEWORK_ADDRESS
        && module.name() == IdentStr::new(TEST_SCENARIO_MODULE_NAME).unwrap()
    {
        // exclude test_module which is a test-only module in the Mgo framework which "emulates"
        // transactional execution and needs to allow test code to bypass private generics
        return Ok(());
    }
    let view = &BinaryIndexedView::Module(module);
    // do not need to check the mgo::transfer module itself
...
fn verify_function(
    ...
    for instr in &code.code {
        if let Bytecode::CallGeneric(finst_idx) = instr {
            ...
            if ident == (MGO_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
                verify_private_transfer(view, fhandle, type_arguments, allow_receiving_object_id)?
            } else if ident == (MGO_FRAMEWORK_ADDRESS, EVENT_MODULE) {
...
fn verify_private_transfer(
    ...
    if addr_module(view, self_handle) == (MGO_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
        return Ok(());
    }
    ...
    if !is_defined_in_current_module(view, type_arg) {
        return Err(format!(
            "Invalid call to '{mgo}::transfer::{f}' on an object of type '{t}'. \
            ...
            instead, i.e. '{mgo}::transfer::public_{f}'",
            mgo = MGO_FRAMEWORK_ADDRESS,
            f = fident,
