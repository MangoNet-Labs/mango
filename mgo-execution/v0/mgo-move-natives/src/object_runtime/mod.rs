...
use mgo_types::{
    base_types::{MoveObjectType, ObjectID, SequenceNumber, MgoAddress},
    ...
    MGO_CLOCK_OBJECT_ID, MGO_SYSTEM_STATE_OBJECT_ID,
};
...
pub(crate) struct TestInventories {
    ...
    pub(crate) address_inventories: BTreeMap<MgoAddress, BTreeMap<Type, Set<ObjectID>>>,
...
impl<'a> ObjectRuntime<'a> {
    ...
    pub fn transfer(
        ...
        //   child object
        let is_framework_obj = [MGO_SYSTEM_STATE_OBJECT_ID, MGO_CLOCK_OBJECT_ID].contains(&id);
            ...
            .map(|(id, (owner, type_, value))| {
                let write_kind =
                    if input_objects.contains_key(&id) || loaded_child_objects.contains_key(&id) {
                        ...
                        WriteKind::Mutate
                    } else if id == MGO_SYSTEM_STATE_OBJECT_ID || new_ids.contains_key(&id) {
                        // MGO_SYSTEM_STATE_OBJECT_ID is only transferred during genesis
