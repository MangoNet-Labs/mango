...
//! Objects whose struct type has key ability represent Mgo objects.
//! They have unique IDs that should never be reused. This verifier makes
//! sure that the id field of Mgo objects never get leaked.
//! Unpack is the only bytecode that could extract the id field out of
//! a Mgo object. From there, we track the flow of the value and make
...
use mgo_types::{
    ...
    mgo_system_state::MGO_SYSTEM_MODULE_NAME,
    MGO_FRAMEWORK_ADDRESS, MGO_SYSTEM_ADDRESS,
};
...
const MGO_SYSTEM_CREATE: FunctionIdent = (
    &MGO_SYSTEM_ADDRESS,
    MGO_SYSTEM_MODULE_NAME,
    ident_str!("create"),
);
const MGO_CLOCK_CREATE: FunctionIdent = (
    &MGO_FRAMEWORK_ADDRESS,
    CLOCK_MODULE_NAME,
    ident_str!("create"),
);
...
#[cfg(msim)]
const MGO_AUTHENTICATOR_STATE_CREATE: FunctionIdent = (
    &MGO_FRAMEWORK_ADDRESS,
    AUTHENTICATOR_STATE_MODULE_NAME,
    ident_str!("create"),
);
#[cfg(msim)]
const MGO_RANDOMNESS_STATE_CREATE: FunctionIdent = (
    &MGO_FRAMEWORK_ADDRESS,
    RANDOMNESS_MODULE_NAME,
    ident_str!("create"),
);
#[cfg(msim)]
const MGO_DENY_LIST_OBJECT_CREATE: FunctionIdent = (
    &MGO_FRAMEWORK_ADDRESS,
    COIN_MODULE_NAME,
...
#[cfg(not(msim))]
const FUNCTIONS_TO_SKIP: &[FunctionIdent] = &[MGO_SYSTEM_CREATE, MGO_CLOCK_CREATE];

#[cfg(msim)]
const FUNCTIONS_TO_SKIP: &[FunctionIdent] = &[
    MGO_SYSTEM_CREATE,
    MGO_CLOCK_CREATE,
    MGO_AUTHENTICATOR_STATE_CREATE,
    MGO_RANDOMNESS_STATE_CREATE,
    MGO_DENY_LIST_OBJECT_CREATE,
];
    ...
    if handle.abilities.has_key() && last_value != AbstractValue::Fresh {
        ...
        let msg = format!(
            "Invalid object creation in {cur_package}::{cur_module}::{cur_function}. \
                ...
                The UID must come directly from mgo::{}::{}. \
                Or for tests, it can come from mgo::{}::{}",
