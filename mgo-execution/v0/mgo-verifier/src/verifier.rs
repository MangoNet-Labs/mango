...
pub fn mgo_verify_module_metered_check_timeout_only(
    ...
    if let Err(error) = mgo_verify_module_metered(config, module, fn_info_map, meter) {
        if matches!(
            error.kind(),
...
pub fn mgo_verify_module_unmetered(
    ...
    mgo_verify_module_metered(config, module, fn_info_map, &mut DummyMeter).map_err(|err| {
