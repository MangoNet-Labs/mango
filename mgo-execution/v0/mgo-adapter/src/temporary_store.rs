...
use mgo_types::execution::{DynamicallyLoadedObjectMetadata, ExecutionResults, SharedInput};
use mgo_types::execution_status::ExecutionStatus;
use mgo_types::inner_temporary_store::InnerTemporaryStore;
use mgo_types::storage::{BackingStore, DeleteKindWithOldVersion, PackageObject};
use mgo_types::mgo_system_state::{get_mgo_system_state_wrapper, AdvanceEpochParams};
use mgo_types::type_resolver::LayoutResolver;
use mgo_types::{
    base_types::{
        ObjectDigest, ObjectID, ObjectRef, SequenceNumber, MgoAddress, TransactionDigest,
        VersionDigest,
    },
...
impl<'backing> TemporaryStore<'backing> {
    ...
    ///
    /// 3. all MGO in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// amount of MGO. We need these information for conservation check.
    pub fn check_mgo_conserved(
        ...
        for (id, input, output) in self.get_modified_objects() {
            if let Some((version, storage_rebate)) = input {
                ...
                if do_expensive_checks {
                    total_input_mgo += self.get_input_mgo(&id, version, layout_resolver)?;
                }
            }
        ...
        if do_expensive_checks {
            ...
            if total_input_mgo != total_output_mgo {
                ...
                format!("MGO conservation failed: input={}, output={}, this transaction either mints or burns MGO",
                total_input_mgo,
                total_output_mgo))
            );
            }
        ...
        // all MGO in storage rebate fields of input objects should flow either to the transaction storage rebate, or the non-refundable
        // storage rebate pool
        if total_input_rebate != gas_summary.storage_rebate + gas_summary.non_refundable_storage_fee
        {
            ...
            /*return Err(ExecutionError::invariant_violation(
                format!("MGO conservation failed--{} MGO in storage rebate field of input objects, {} MGO in tx storage rebate or tx non-refundable storage rebate",
                total_input_rebate,
                gas_summary.non_refundable_storage_fee))
            );*/
        }

        // all MGO charged for storage should flow into the storage rebate field of some output object
        if gas_summary.storage_cost != total_output_rebate {
            // TODO: re-enable once we fix the edge case with OOG, gas smashing, and storage rebate
            /*return Err(ExecutionError::invariant_violation(
                format!("MGO conservation failed--{} MGO charged for storage, {} MGO in storage rebate field of output objects",
...
impl<'backing> Storage for TemporaryStore<'backing> {
    ...
    fn record_execution_results(&mut self, results: ExecutionResults) {
        let ExecutionResults::V1(results) = results else {
            panic!("ExecutionResults::V1 expected in mgo-execution v0");
        };
