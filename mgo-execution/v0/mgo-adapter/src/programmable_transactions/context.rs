...
mod checked {
    ...
    use mgo_types::{
        balance::Balance,
        base_types::{MoveObjectType, ObjectID, SequenceNumber, MgoAddress, TxContext},
        coin::Coin,
    ...
    use mgo_types::{execution_mode::ExecutionMode, execution_status::CommandArgumentError};
    ...
    impl<'vm, 'state, 'a> ExecutionContext<'vm, 'state, 'a> {
        pub fn new(
            ...
            // TODO remove this
            let linkage = LinkageView::new(Box::new(state_view.as_mgo_resolver()), init_linkage);
            ...
            // exist. Plus, Mgo Move does not use these changes or events
        ...
        pub fn finish<Mode: ExecutionMode>(self) -> Result<ExecutionResults, ExecutionError> {
            ...
            // Mgo Move programs should never touch global state, so resources should be empty
            assert_invariant!(
                change_set.resources().next().is_none(),
                "Change set must be empty"
            );
            // Mgo Move no longer uses Move's internal event system
