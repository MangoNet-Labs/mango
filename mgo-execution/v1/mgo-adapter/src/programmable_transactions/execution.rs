...
mod checked {
    ...
    use mgo_move_natives::object_runtime::ObjectRuntime;
    use mgo_protocol_config::ProtocolConfig;
    use mgo_types::storage::{get_package_objects, PackageObject};
    use mgo_types::{
        base_types::{
            MoveObjectType, ObjectID, MgoAddress, TxContext, TxContextKind, RESOLVED_ASCII_STR,
    ...
    fn publish_and_verify_modules(
        ...
        // TODO(https://github.com/MangoNetworkOs/Mango/issues/69): avoid this redundant serialization by exposing VM API that allows us to run the linker directly on `Vec<CompiledModule>`
        ...
        // run the Mgo verifier
        for module in modules {
            // Run Mgo bytecode verifier, which runs some additional checks that assume the Move
            // bytecode verifier has passed.
            mgo_verifier::verifier::mgo_verify_module_unmetered(module, &BTreeMap::new())?;
        }
    ...
    fn check_visibility_and_signature<Mode: ExecutionMode>(
        ...
        let data_store = MgoDataStore::new(&context.linkage_view, &no_new_packages);
        let module = context
            .vm
            .get_runtime()
    ...
    fn check_private_generics(
        ...
        if module_ident == (&MGO_FRAMEWORK_ADDRESS, EVENT_MODULE) {
            return Err(ExecutionError::new_with_source(
                ExecutionErrorKind::NonEntryFunctionInvoked,
                format!("Cannot directly call functions in mgo::{}", EVENT_MODULE),
            ));
        }

        if module_ident == (&MGO_FRAMEWORK_ADDRESS, TRANSFER_MODULE)
            && PRIVATE_TRANSFER_FUNCTIONS.contains(&function)
        {
            let msg = format!(
                "Cannot directly call mgo::{m}::{f}. \
                Use the public variant instead, mgo::{m}::public_{f}",
                m = TRANSFER_MODULE,
                f = function
            );
            return Err(ExecutionError::new_with_source(
        ...
        match value {
            ...
            Value::Receiving(_, _, assigned_type) => {
                ...
                let Some(s) = context.vm.get_runtime().get_struct_type(*sidx) else {
                    invariant_violation!("mgo::transfer::Receiving struct not found in session")
                };
    ...
    pub fn is_tx_context(
        ...
        let is_tx_context_type = module_addr == &MGO_FRAMEWORK_ADDRESS
        ...
        fn deserialize<D: serde::de::Deserializer<'d>>(
            ...
            match self {
                ...
                PrimitiveArgumentLayout::Address => {
                    MgoAddress::deserialize(deserializer)?;
                    Ok(())
                }
            }
        }
