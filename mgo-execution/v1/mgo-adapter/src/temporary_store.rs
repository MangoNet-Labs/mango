...
use mgo_types::base_types::VersionDigest;
...
use mgo_types::execution::{
    DynamicallyLoadedObjectMetadata, ExecutionResults, ExecutionResultsV2, SharedInput,
};
use mgo_types::execution_status::ExecutionStatus;
use mgo_types::inner_temporary_store::InnerTemporaryStore;
use mgo_types::storage::{BackingStore, PackageObject};
use mgo_types::mgo_system_state::{get_mgo_system_state_wrapper, AdvanceEpochParams};
use mgo_types::type_resolver::LayoutResolver;
use mgo_types::{
    base_types::{ObjectID, ObjectRef, SequenceNumber, MgoAddress, TransactionDigest},
    effects::EffectsObjectChange,
    error::{ExecutionError, MgoError, MgoResult},
    fp_bail,
    gas::GasCostSummary,
    object::Owner,
    object::{Data, Object},
...
use mgo_types::{is_system_package, MGO_SYSTEM_STATE_OBJECT_ID};
...
impl<'backing> TemporaryStore<'backing> {
    ...
    fn into_effects_v1(
        ...
        let updated_gas_object_info = if let Some(coin_id) = gas_charger.gas_coin() {
            ...
            (
                ...
                Owner::AddressOwner(MgoAddress::default()),
            )
        };
    ...
    pub fn conserve_unmetered_storage_rebate(&mut self, unmetered_storage_rebate: u64) {
        ...
        let mut system_state_wrapper = self
            .read_object(&MGO_SYSTEM_STATE_OBJECT_ID)
        ...
        // If not, we would be dropping MGO on the floor by overriding it.
...
impl<'backing> TemporaryStore<'backing> {
    ...
    fn get_objects_to_authenticate(
    ...
    ) -> MgoResult<(Vec<ObjectID>, HashSet<ObjectID>)> {
        ...
        for id in &self.execution_results.modified_objects {
            ...
            match &old_obj.owner {
                ...
                Owner::Immutable => {
                    assert!(is_epoch_change, "Immutable objects cannot be written, except for Mgo Framework/Move stdlib upgrades at epoch change boundaries");
...
impl<'backing> TemporaryStore<'backing> {
    ...
    /// `MgoGasStatus` `storage_rebate` and `storage_gas_units` track the transaction
...
impl<'backing> TemporaryStore<'backing> {
    pub fn advance_epoch_safe_mode(
        ...
        let wrapper = get_mgo_system_state_wrapper(self.store.as_object_store())
...
impl<'backing> TemporaryStore<'backing> {
    fn get_input_mgo(
        ...
        if let Some(obj) = self.input_objects.get(id) {
            ...
            obj.get_total_mgo(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input MGO in MGO conservation checking for input with \
                         type {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
            ...
            let Ok(Some(obj)) = self.store.get_object_by_key(id, expected_version) else {
                invariant_violation!(
                    "Failed looking up dynamic field {id} in MGO conservation checking"
                );
            };
            obj.get_total_mgo(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input MGO in MGO conservation checking for type \
                         {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
    ...
    /// Check that this transaction neither creates nor destroys MGO. This should hold for all txes
    ...
    ///
    /// 1. all MGO in storage rebate fields of input objects should flow either to the transaction
    ///    storage rebate, or the transaction non-refundable storage rebate
    /// 2. all MGO charged for storage should flow into the storage rebate field of some output
    ///    object
    ///
    ...
    pub fn check_mgo_conserved(
        ...
        // total amount of MGO in storage rebate of input objects
        let mut total_input_rebate = 0;
        // total amount of MGO in storage rebate of output objects
        ...
        if gas_summary.storage_cost == 0 {
                    ...
                    + gas_summary.non_refundable_storage_fee
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "MGO conservation failed -- no storage charges in gas summary \
            ...
            // all MGO in storage rebate fields of input objects should flow either to
            ...
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "MGO conservation failed -- {} MGO in storage rebate field of input objects, \
                        {} MGO in tx storage rebate or tx non-refundable storage rebate",
                    total_input_rebate, gas_summary.non_refundable_storage_fee,
                )));
            }

            // all MGO charged for storage should flow into the storage rebate field
            // of some output object
            if gas_summary.storage_cost != total_output_rebate {
                return Err(ExecutionError::invariant_violation(format!(
                    "MGO conservation failed -- {} MGO charged for storage, \
                        {} MGO in storage rebate field of output objects",
    ...
    /// Check that this transaction neither creates nor destroys MGO.
    /// This more expensive check will check a third invariant on top of the 2 performed
    /// by `check_mgo_conserved` above:
    ///
    /// * all MGO in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// advance epoch transaction would mint `epoch_fees` amount of MGO, and burn `epoch_rebates`
    /// amount of MGO. We need these information for this check.
    pub fn check_mgo_conserved_expensive(
        &self,
        gas_summary: &GasCostSummary,
        advance_epoch_gas_summary: Option<(u64, u64)>,
        layout_resolver: &mut impl LayoutResolver,
    ) -> Result<(), ExecutionError> {
        // total amount of MGO in input objects, including both coins and storage rebates
        let mut total_input_mgo = 0;
        // total amount of MGO in output objects, including both coins and storage rebates
        let mut total_output_mgo = 0;
        for (id, input, output) in self.get_modified_objects() {
            if let Some(input) = input {
                total_input_mgo += self.get_input_mgo(&id, input.version, layout_resolver)?;
            }
            if let Some(object) = output {
                total_output_mgo += object.get_total_mgo(layout_resolver).map_err(|e| {
                    make_invariant_violation!(
                        "Failed looking up output MGO in MGO conservation checking for \
                         mutated type {:?}: {e:#?}",
                        object.struct_tag(),
                    )
                })?;
        ...
        total_output_mgo += gas_summary.computation_cost + gas_summary.non_refundable_storage_fee;
        if let Some((epoch_fees, epoch_rebates)) = advance_epoch_gas_summary {
            total_input_mgo += epoch_fees;
            total_output_mgo += epoch_rebates;
        }
        if total_input_mgo != total_output_mgo {
            return Err(ExecutionError::invariant_violation(format!(
                "MGO conservation failed: input={}, output={}, \
                    this transaction either mints or burns MGO",
                total_input_mgo, total_output_mgo,
            )));
        }
        Ok(())
    }
...
impl<'backing> Storage for TemporaryStore<'backing> {
    ...
    fn record_execution_results(&mut self, results: ExecutionResults) {
        let ExecutionResults::V2(results) = results else {
            panic!("ExecutionResults::V2 expected in mgo-execution v1 and above");
        };
    ...
    fn get_resource(
        ...
        let object = match self.read_object(&ObjectID::from(*address)) {
            ...
            None => match self.read_object(&ObjectID::from(*address)) {
                ...
                Some(x) => {
                    if !x.is_immutable() {
                        fp_bail!(MgoError::ExecutionInvariantViolation);
                    }
                    x
                }
            },
