...
use mgo_types::execution_status::{ExecutionFailureStatus, MoveLocation, MoveLocationOpt};

pub(crate) fn convert_vm_error<S: MoveResolver<Err = MgoError>>(
    error: VMError,
    vm: &MoveVM,
    state_view: &S,
) -> ExecutionError {
    let kind = match (error.major_status(), error.sub_status(), error.location()) {
        ...
        (StatusCode::ABORTED, _, Location::Script) => {
            debug_assert!(false, "Scripts are not used in Mgo");
