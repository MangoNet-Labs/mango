...
pub struct LinkageView<'state> {
    ...
    resolver: Box<dyn MgoResolver + 'state>,
...
impl<'state> LinkageView<'state> {
    pub fn new(resolver: Box<dyn MgoResolver + 'state>) -> Self {
        Self {
            resolver,
            linkage_info: None,
    ...
    pub(crate) fn relocate(&self, module_id: &ModuleId) -> Result<ModuleId, MgoError> {
